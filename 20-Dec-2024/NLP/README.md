...
**Title (ZH)**: LongBench v2：关于现实长上下文多任务的更深入理解和推理的探索 
...
**Abstract (ZH)**: 本文介绍了LongBench v2，这是一个基准测试，旨在评估大语言模型（LLMs）处理需要深刻理解和跨多种实际任务进行推理的长效问题的能力。LongBench v2 包含了503个具有挑战性的多项选择题，背景信息范围从8,000到2,000,000字，并涵盖了六大主要任务类别：单文档问答、多文档问答、长效上下文学习、长对话历史理解、代码仓库理解以及长结构化数据理解。为了确保广泛性和实用性，我们从近100名具有不同专业背景的高度受教育个体中收集了数据。我们采用了自动和人工双重审查流程，以确保高质量和难度，结果在15分钟的时间限制下，人工专家的准确率仅为53.7%。我们的评估显示，表现最好的模型在直接回答问题时，仅能达到50.1%的准确率。相比之下，o1-preview模型，其包含了更长的推理过程，实现了57.7%的准确率，超越了人工基线4%。这些结果突显了增强推理能力和扩展推理时计算量的重要性，以应对LongBench v2中的长效问题挑战。该项目可在以下链接访问：[该链接]。 
...
多选题数据集（如大规模多任务语言理解MMLU）广泛用于评估大语言模型（LLMs）的常识、理解和问题解决能力。然而，这些基准的开源性质以及LLMs训练数据来源的广泛性不可避免地导致了基准污染，从而产生了不可靠的评估结果。为了解决这一问题，我们提出了一种无污染且更具挑战性的多选题基准叫MMLU-CF。该基准通过避免无意的数据泄漏和恶意数据泄漏，重新评估了LLMs对世界知识的理解能力。为了避免无意的数据泄漏，我们从更广泛的领域中获取数据，并设计了三条去污染规则。为了防止恶意数据泄漏，我们将基准划分为具有相似难度和主题分布的验证集和测试集。测试集保持内部封闭，以确保结果的可靠性和透明性；验证集则公开可用，以促进透明度并便于独立验证。我们的评估结果显示，强大的GPT-4o在测试集上的五次提示得分为73.4%，零次提示得分为71.9%，这表明了我们方法在建立更为严格且无污染的评估标准方面的有效性。GitHub仓库地址为：[此链接]，数据集地址为：[此链接]。 
...
**Abstract (ZH)**: 自然语言处理与生成系统近年来显示出在补充和简化专业事实核查人员的昂贵且耗时工作方面的潜力。在本文中，我们根据检索增强生成（RAG）范式，改进了几种当前最先进的自动化事实核查流水线中的若干限制。我们的目标是在更现实的场景下对标基于RAG的方法在生成判决——即简短讨论陈述真实性的文本——的效果，评估其在风格复杂且异构但可靠的知识库上的表现。我们的研究结果显示了一幅复杂的图景：例如，基于大语言模型的检索器在检索方面胜过其他检索技术，尽管它们在处理异构知识库时仍有困难；大型模型在判决的准确性上表现出色，而小型模型在上下文一致性方面表现更好。人类评估认为零样本和单样本方法在信息量方面更受欢迎，微调模型在情感对齐方面表现更佳。 
...
**Abstract (ZH)**: 我们将介绍LlamaFusion，这是一种为预训练的纯文本大语言模型（LLMs）赋予多模态生成能力的框架，使其能够在任意序列中理解和生成文本和图像。LlamaFusion 利用现有的 Llama-3 权重进行自回归文本处理，同时引入额外的并行变压器模块进行图像处理和扩散。在训练过程中，每种模态的数据会被路由到其专门的模块：特定模态的前馈层、查询-键-值投影和规范化层独立处理每种模态，而共享的自注意力层则允许文本和图像特征之间的交互。通过冻结文本特定的模块并仅训练图像特定的模块，LlamaFusion 保留了纯文本LLMs的语言能力，同时增强了强大的视觉理解和生成能力。与从零开始预训练多模态生成模型的方法相比，我们的实验表明，LlamaFusion 使用50%的FLOPs提高了20%的图像理解和3.6%的图像生成能力，同时还保持了Llama-3的语言能力。此外，我们还展示了该框架可以适应现有的具备多模态生成能力的视觉-语言模型。总体而言，该框架不仅利用了现有的文本仅模态LLMs的计算投资，还使语言和视觉能力的并行开发成为可能，为高效的多模态模型开发指明了前景良好的方向。 

---
# Language Models as Continuous Self-Evolving Data Engineers 

**Title (ZH)**: 语言模型作为连续自我演化的数据工程师 
...
**Abstract (ZH)**: 大型语言模型（LLMs）已经在各种任务上展示了出色的能力，但其进一步的发展受限于高质量训练数据的缺乏。此外，传统的训练方法过于依赖由专家标注的数据，从而限制了LLMs的性能。为了解决这一问题，我们提出了一种新的范式，使LLMs能够通过自主生成、清理、审核和标注带有偏好信息的数据来进行自我训练，这一范式称为LANCE。我们的方法表明，LLMs可以作为持续自我进化数据工程师，显著减少了训练后数据构建过程的时间和成本。通过对Qwen2的不同变体进行迭代微调，我们验证了LANCE在各种任务上的有效性，显示其可以持续提升模型性能并保持高质量数据的生成。在八个基准维度上，LANCE使Qwen2-7B的平均得分提高了3.36，Qwen2-7B-Instruct提高了2.70。这种具有自主数据构建的训练范式不仅减少了对人类专家或外部模型的依赖，还确保了数据符合人类的价值观和偏好，为此类未来超智能系统的发展铺平了道路，这些系统可以超越人类的能力。 
...
**Abstract (ZH)**: 近年来，大规模语言模型（LLM）在语言理解和生成能力方面取得了显著进步。然而，由于LLM对计算和存储资源的需求较高，将其部署在资源受限的边缘设备上仍面临挑战。为解决这一问题，我们提出了一个新的LLM模型剪枝方法，称为结构感知自适应剪枝（SAAP），旨在显著降低计算和内存成本，同时保持模型性能。我们首先定义了一个自适应重要性融合度量，通过考虑其同方差不确定性来评估LLM中所有耦合结构的重要性。然后，我们对所有模块的重要性进行排序，以确定应被剪枝的具体层次，以满足特定性能要求。此外，我们开发了一种新的分组微调策略，以提高LLM的推断效率。最后，我们在两个常见任务即零样本分类和文本生成上对多种LLM进行了SAAP方法的评估。实验结果表明，我们的SAAP方法优于多个最先进的基线方法，在LLaMA-7B、Vicuna-7B和LLaMA-13B上分别实现了2.17%、2.37%和2.39%的准确率提升。此外，SAAP还将token生成速度提高了5%，展示了其在资源受限情景下的实际优势。 

---
# Outcome-Refining Process Supervision for Code Generation 

**Title (ZH)**: 代码生成的成果细化过程监督 
...
**Abstract (ZH)**: 大型语言模型在代码生成任务中展现了卓越的能力，但在需要深度算法推理的复杂编程任务上常常表现不佳。虽然通过学习奖励模型进行过程监督在引导推理步骤方面显示出潜力，但这种方法需要昂贵的训练数据，并且在评估可靠性方面存在问题。我们提出了一种名为“结果细化过程监督”的新颖范式，将其视为需要监督的过程本身。我们的框架利用具体的执行信号来确定推理步骤的监督范围，同时使用树状结构探索来并行维护多种解决方案轨迹。实验结果表明，我们的方法能使更小的模型在竞争编程任务中达到高成功率和性能指标，且不需要训练策略奖励模型（PRMs）就能实现更可靠的验证。我们在5个模型和3个数据集上进行了实验，结果表明，我们的方法在正确性和效率方面分别平均提高了26.9%和42.2%。这些结果表明，为解决复杂编程任务提供有结构的推理空间和具体的验证信号至关重要。我们已开源所有代码和数据，链接如下：this https URL 
...
**Abstract (ZH)**: 在本报告中，我们介绍了Qwen2.5，这是一个综合性的大规模语言模型（LLM）系列，旨在满足多样化的需求。与之前的版本相比，Qwen2.5 在预训练和后训练阶段均得到了显著改进。在预训练方面，我们将高质量的预训练数据集从之前的7万亿token扩展到了18万亿token。这为常识、专家知识和推理能力提供了坚实的基础。在后训练方面，我们采用了复杂的监督微调，涉及超过100万个样本，并实施了多阶段强化学习。后训练技术增强了人类偏好，特别是在长文本生成、结构化数据分析和指令执行方面表现出显著的改进。为了有效应对多样化的应用场景，我们提供了多种规格的Qwen2.5 LLM系列。开放权重版本包括基础模型和指令调优模型，同时也提供了量化版本。此外，对于托管解决方案，目前的 proprietary 模型包括两种专家混合（Mixture-of-Experts, MoE）变体：Qwen2.5-Turbo 和 Qwen2.5-Plus，它们均可通过阿里云模型工作室获取。Qwen2.5 在广泛的语言理解、推理、数学、编程、人类偏好对齐等基准测试中表现出顶级性能。具体来说，开放权重旗舰版 Qwen2.5-72B-Instruct 在多个开放和 proprietary 模型中表现出色，并且其性能与大约是其5倍规模的最先进的开放权重模型 Llama-3-405B-Instruct 相竞争。同时，Qwen2.5-Turbo 和 Qwen2.5-Plus 提供了较高的成本效益，并分别与 GPT-4o-mini 和 GPT-4o 的性能相当。此外，作为基础，Qwen2.5 模型在训练专门的模型，如 Qwen2.5-Math、Qwen2.5-Coder、QwQ 和多模态模型等方面发挥了关键作用。 

---
# Review-Then-Refine: A Dynamic Framework for Multi-Hop Question Answering with Temporal Adaptability 

**Title (ZH)**: 基于审查-精炼的动态框架：具有时间适应性的多跳问答方法 
...
**Abstract (ZH)**: 基于检索增强生成（RAG）框架在多跳问答（QA）任务中逐渐显示出其潜力，因为它使大规模语言模型（LLMs）能够结合外部知识并缓解其固有的知识不足。尽管取得了一定的进步，现有的RAG框架通常遵循“先检索后阅读”的范式，但在处理包含时间信息的多跳QA任务时，往往难以检索和合成准确的时间相关信息。为解决这一挑战，本文提出了一种名为“先审核后精炼”的新型框架，旨在增强LLM在包含时间信息的多跳QA场景中的表现。我们的方法首先进入一个审核阶段，在该阶段中，分解后的子查询会被动态重写以包含时间信息，从而使其能够进行后续的自适应检索和推理过程。此外，我们还实现了自适应检索机制，以减少不必要的检索，从而降低语义错误的可能性。在随后的精炼阶段，LLM会综合从每个子查询中检索到的信息以及其内部知识来形成一个连贯的答案。在多个数据集上的广泛实验结果表明了我们所提出框架的有效性，并突显了其在显著提高LLM的多跳问答能力方面的潜力。 
...
**Abstract (ZH)**: 在本文中，我们引入了AceMath，这是一个卓越于解决复杂数学问题的一系列前沿数学模型，同时配备了高效的奖励模型，能够评估生成的解决方案并可靠地识别正确的答案。为了开发这些指令调优的数学模型，我们提出了一种监督微调（SFT）过程，首先在通用领域中实现竞争性的性能，然后针对数学领域进行目标化的微调，使用精心选择的提示和合成生成的响应。由此产生的模型AceMath-72B-Instruct在多个方面显著优于Qwen2.5-Math-72B-Instruct、GPT-4o和Claude-3.5 Sonnet。为了开发数学专门化的奖励模型，我们首先构建了AceMath-RewardBench，这是一个全面且稳健的基准，用于评估数学奖励模型在多种类型和难度的问题上的表现。然后，我们提供了一种系统的方法来构建我们的数学奖励模型。由此产生的模型AceMath-72B-RM在最新的奖励模型中始终表现出色。进一步地，当结合使用AceMath-72B-Instruct和AceMath-72B-RM时，我们在数学推理基准中的平均rm@8得分最高。我们将在以下链接发布模型权重、训练数据和评估基准：[this https URL] 

---
# ConfliBERT: A Language Model for Political Conflict 

**Title (ZH)**: ConflictBERT：一种用于政治冲突的语言模型 
...
**Abstract (ZH)**: 冲突研究学者已经采用基于规则的方法从新闻报道和文本中提取有关政治暴力的信息。近年来，自然语言处理技术的发展超越了僵化的基于规则的方法。我们回顾了我们最近开发的ConfliBERT语言模型（Hu等人，2022年），该模型适用于处理与政治和暴力相关的文本。该模型可以用于从关于政治冲突的文本中提取行为者和行为分类。经过微调后，结果显示，在其相关领域内，ConfliBERT在准确率、精确率和召回率方面优于其他大型语言模型（LLM），如Google的Gemma 2（9B）、Meta的Llama 3.1（7B）和阿里巴巴的Qwen 2.5（14B）。此外，它比这些更为通用的LLM快数百倍。这些结果通过来自BBC、re3d和全球恐怖主义数据库（GTD）的文本进行了说明。 

---
# LLMs Lost in Translation: M-ALERT uncovers Cross-Linguistic Safety Gaps 

**Title (ZH)**: LLMs 在语言转换中的迷失：M-ALERT 揭示跨语言安全差距 
...
**Abstract (ZH)**: 确保多语言大型语言模型（LLMs）的安全是保证安全访问和语言多样性的重要途径。为此，我们引入了M-ALERT，这是一个多语言基准，评估了五种语言（英语、法语、德语、意大利语和西班牙语）中LLMs的安全性。M-ALERT 包含每种语言15,000个高质量的提示，总计75,000个，遵循详细的ALERT分类体系。我们在10个最先进的LLMs上的广泛实验强调了语言特定安全性分析的重要性，揭示了模型在不同语言和类别中的安全表现存在显著不一致性。例如，Llama3.2在意大利语的犯罪税收类别中显示出高风险，但在其他语言中保持安全。类似的差异在所有模型中都可以观察到。相反，某些类别（如毒品大麻和传播犯罪言论）在不同模型和语言中始终会触发不安全的响应。这些发现强调了在LLMs中实施稳健的多语言安全实践的重要性，以确保在多样的用户社区中安全和负责任的使用。 

---
# Chain-of-MetaWriting: Linguistic and Textual Analysis of How Small Language Models Write Young Students Texts 

**Title (ZH)**: 元写作链：小型语言模型如何为中小学生生成文本的语言学和文本分析 
...
**Abstract (ZH)**: 大型语言模型（LLMs）已应用于生成针对不同写作任务的文本：报告、论文和讲故事。然而，语言模型没有关于文本写作过程的元表示，也没有类似于年轻人学习者沟通学习需求的内在学习需求。本文介绍了多语言小型语言模型（SLMs）写作的细粒度语言和文本分析。通过我们的方法Chain-of-MetaWriting，SLMs可以模仿人类写作过程中的某些步骤，如规划和评估。我们主要关注了对法国小学生来说是短篇故事写作任务，以及对本科生来说是论文写作任务。研究结果表明，SLMs在帮助年轻人处理涉及学校暴力等敏感话题时遇到困难，有时使用的词汇过于复杂，不适合目标受众。特别是，输出在文本连贯性和一致性方面与人类生成的文本存在显著差异，特别是在时间连接词、主题过渡和参照方面。 

---
# Knowledge Injection via Prompt Distillation 

**Title (ZH)**: 通过提示精炼实现知识注入 
...
**Abstract (ZH)**: 在许多实际应用场景中，大型语言模型（LLMs）需要融合其预训练数据中尚未包含的新知识。实现这一目标的主要方法包括微调（fine-tuning）和检索增强生成（RAG）。尽管RAG已成为知识注入的标准方法，微调尚未取得与其相当的成功。在本文中，我们提出了一种新的微调技术，用于学习新知识，并证明该技术可以达到与RAG相当的性能。所提出的方法基于一种自我蒸馏方法，称之为提示蒸馏。首先，我们生成关于新知识的问题-答案对。然后，我们使用问题-答案对微调一个学生模型，使其模仿接收了新知识提示的教师模型的输出分布。学生模型与教师模型结构相同，但配备了LoRA适配器。这种训练程序有助于将教师模型提示中的新知识传递给学生模型的权重中。 

---
# Understanding the Dark Side of LLMs' Intrinsic Self-Correction 

**Title (ZH)**: 理解大语言模型内在自我修正的暗面 
...
**Abstract (ZH)**: 内在自我校正是通过反馈提示来改进大型语言模型（LLM）的响应，仅依赖于它们本身的能力。然而，最近的研究表明，当没有 oracle 标签作为反馈提示时，LLM 的内在自我校正会失效。在本文中，我们旨在解释不同任务中 LLM 的内在自我校正，尤其是对于那些失败案例。通过包括一个简单的任务和三个具有前沿技术（SOTA）的 LLM（如 ChatGPT 家族的 o1、4o、3.5-turbo 和 Llama 家族的 2-7B、3-8B 和 3.1-8B），我们设计了三种解释方法来揭示 LLM 内在自我校正的阴暗面。我们发现内在自我校正可以（1）导致 LLM 在中间和最终答案上摇摆不定，并对简单的事实性问题产生提示偏好；（2）在复杂任务中引入类似人类的认知偏差。基于我们的发现，我们还提供了两种简单而有效的缓解策略：问题重复和少量样本的监督微调。我们已在以下网址开源了我们的工作：[此处替换为URL] 

---
# RobustFT: Robust Supervised Fine-tuning for Large Language Models under Noisy Response 

**Title (ZH)**: RobustFT：在噪声响应下大型语言模型的稳健监督微调 
...
**Abstract (ZH)**: 监督微调（SFT）在适应大型语言模型（LLMs）到特定领域或任务中发挥着关键作用。然而，如实验证明的那样，在实际应用中收集的数据不可避免地包含噪声，这对模型在下游任务中的性能构成了重大挑战。因此，迫切需要一个鲁棒的SFT框架来增强模型在下游任务中的能力。为应对这一挑战，我们提出了一种鲁棒SFT框架（RobustFT），该框架在下游任务数据上进行噪声检测和重标记。在噪声识别方面，我们的方法采用多专家协同系统，并结合推理增强模型以实现更出色的噪声检测。在去噪阶段，我们采用了上下文增强策略，该策略结合了最相关的和最自信的知识，并通过仔细评估生成可靠的注释。此外，我们引入了一个基于响应熵的有效数据选择机制，确保只有高质量样本被保留用于微调。在五个数据集上对多个LLMs进行的大量实验表明，RobustFT 在噪声场景中的性能非常出色。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在整合检索增强生成（RAG）后，仍然容易生成虚假信息。平行上下文扩展（PCE）是一条致力于有效地整合未排序的平行上下文的研究路线，但在适应RAG场景时仍然会面临生成虚假信息的问题。本文提出了一种名为DePaC（Dehallucinating Parallel Context Extension）的方法，通过上下文感知的负训练和信息校准的聚合来缓解虚假信息问题。DePaC 设计旨在缓解两种类型的任务内虚假信息：事实捏造（即，LLMs 提出未被上下文支持的断言）和事实遗漏（即，LLMs 未能呈现可以通过上下文验证的断言）。具体地，（1）对于事实捏造，我们应用上下文感知的负训练，通过对LLMs 进行负监督微调，从而明确地引导LLMs 在上下文与问题无关时拒绝作答；（2）对于事实遗漏，我们提出了一种信息校准聚合方法，优先选择那些从上下文获取更多信息增量的上下文窗口。实验结果表明，DePaC 显著缓解了这两种虚假信息类型的问题，并在这些任务上持续实现了更好的性能。 

---
# Why language models collapse when trained on recursively generated text 

**Title (ZH)**: 当使用递归生成的文本进行训练时，语言模型为何会失效 
...
**Abstract (ZH)**: 语言模型（LMs）已经在互联网上广泛用于生成文本。生成的文本通常会被收集到下一代LMs的训练语料库中。先前的工作通过实验发现，当LMs在递归生成的文本上进行训练时会发生崩溃。本文从两个方面为现有的知识做出了贡献。我们给出了一种理论上的证明，解释了LM崩溃的现象。我们的证明揭示了LM崩溃的原因，并证明了所有的自回归LMs都会不可避免地发生崩溃。我们还提出了一项新的发现：当LMs在递归生成的文本上进行训练时，它们的性能逐渐下降，直到最终不如随机初始化的LM。经过训练的LMs会产生大量的重复文本，并且在各种自然语言任务中表现出色程度不佳。上述证明和新发现深化了我们对LM崩溃的理解，并提供了有价值的见解，这可能激发新的训练技术以减轻这种威胁。 
...
**Abstract (ZH)**: 近年来，机器学习领域的最新进展，尤其是大型语言模型（LLMs）如BERT和GPT，提供了丰富的上下文嵌入，从而提高文本表示的效果。然而，当前的文档聚类方法往往忽略了命名实体（NEs）之间更深层的关系以及LLM嵌入的潜在价值。本文提出了一种新的方法，将命名实体识别（NER）和LLM嵌入整合到基于图的框架中进行文档聚类。该方法构建了一个图，其中节点表示文档，边的权重由命名实体的相似性决定，并使用图卷积网络（GCN）进行优化。这确保了对语义上相关文档的有效分组。实验结果表明，与基于共现的传统方法相比，我们的方法在聚类文档（尤其是命名实体丰富的文档）方面表现出更优越的表现。 

---
# Think&Cite: Improving Attributed Text Generation with Self-Guided Tree Search and Progress Reward Modeling 

**Title (ZH)**: 思考与引用：通过自我引导树搜索和进步奖励建模改进带属性的文本生成 
...
**Abstract (ZH)**: 尽管大语言模型（LLMs）具备出色的性能，但它们很容易产生幻觉并生成事实错误的信息。这一挑战促生了属性化文本生成的努力，即促使LLMs生成具有支持证据的内容。在本文中，我们提出了一种新的框架，称为Think&Cite，并将属性化文本生成问题表述为一个结合了搜索过程的多步推理问题。具体而言，我们提出了自我引导蒙特卡洛树搜索（SG-MCTS），该方法利用LLMs的自我反思能力，反思MCTS中的中间状态以指导树的扩展过程。为了提供可靠且全面的反馈，我们引入了进步奖励模型（Progress Reward Models），从生成进步和属性化进步两个方面衡量从根节点到当前状态的搜索过程进展。我们在三个数据集上进行了广泛的实验，结果显示我们的方法显著优于基线方法。 

---
# DS$^2$-ABSA: Dual-Stream Data Synthesis with Label Refinement for Few-Shot Aspect-Based Sentiment Analysis 

**Title (ZH)**: DS$^2$-ABSA：基于标签精炼的双流数据合成Few-Shot方面情感分析 
...
**Abstract (ZH)**: 近年来开发的大型语言模型（LLMs）为低资源场景下的数据稀缺性问题提供了 promising 的新途径。在少样本方面情感分析（ABSA）中，以往的研究探索了数据扩增技术，促使LLMs通过修改现有数据来生成新样本。然而，这些方法未能生成足够的多样性数据，从而影响了其效果。此外，一些研究通过使用特定指令和少量选定示例作为提示，利用上下文学习来进行ABSA。尽管这些方法颇具前景，但LLMs往往生成的标签会偏离任务要求。为了克服这些限制，我们提出了一种针对少样本ABSA的双重数据合成框架——DS$^2$-ABSA。该框架利用LLMs从两个互补的角度进行数据合成：基于关键点和基于实例驱动，以在低资源环境下有效生成具有多样性和高质量的ABSA样本。此外，还集成了一个标签细化模块以改进合成标签。广泛实验表明，DS$^2$-ABSA相较于以往的少样本ABSA解决方案以及其他的LLM驱动的数据生成方法，表现出显著的优越性。 
...
**Abstract (ZH)**: 接收器加权关键值（RWKV）模型提供了一种新型的Transformer架构替代方案，结合了递归和基于注意力系统的优点。与传统的高度依赖自我注意力的Transformer不同，RWKV能够以较低的计算需求高效捕捉长距离依赖关系。通过利用递归框架，RWKV解决了Transformer在处理长序列任务时的一些计算效率问题。RWKV最近因其在多个领域的稳健性能吸引了广泛关注。尽管其受欢迎程度正在增长，但尚未对其进行全面系统的回顾。本文旨在填补这一空白，作为第一篇全面回顾RWKV架构、核心原则及其各领域应用（如自然语言生成、自然语言理解和计算机视觉）的文章。我们评估了RWKV与传统Transformer模型之间的比较，突显了其在高效管理长序列和降低计算成本方面的能力。此外，我们探讨了RWKV所面临的挑战，并提出了未来研究和进步的潜在方向。我们始终维护相关的开源材料于此网址：this https URL。 

---
# Mapping and Influencing the Political Ideology of Large Language Models using Synthetic Personas 

**Title (ZH)**: 使用合成人格映射并影响大规模语言模型的政治意识形态 
...
**Abstract (ZH)**: 对大型语言模型（LLMs）中的政治偏见的分析主要将这些系统视为具有固定观点的单一实体。虽然存在多种测量偏见的方法，但基于人设的提示对LLMs政治倾向的影响尚未被探索。本研究利用PersonaHub（生成的人设描述集合），通过政治极点测试（PCT）来映射基于人设的提示的LLMs的政治分布。然后，我们考察是否可以通过明确的意识形态提示将这些初始政治极点分布操纵为完全对立的政治倾向：右翼独裁主义和左翼自由主义。实验结果表明，生成的人设主要集中在左翼自由主义象限内，并且在使用明确的意识形态描述符进行提示时，模型显示出不同程度的响应性。尽管所有模型都显著向右翼独裁主义方向偏移，但对左翼自由主义方向的偏移较为有限，这表明模型对意识形态操纵的反应存在不对称性，这可能反映了模型训练中固有的偏见。 

---
# DynamicKV: Task-Aware Adaptive KV Cache Compression for Long Context LLMs 

**Title (ZH)**: DynamicKV：面向任务的自适应KV缓存压缩技术用于长上下文语言模型 
...
**Abstract (ZH)**: 高效管理键值对（KV）缓存对于LLM执行长上下文任务（如RAG和总结）至关重要。现有的KV缓存压缩方法强制遵循固定的模式，忽略了任务特定的特性，导致关键信息的保留不足。然而，我们观察到不同任务中各层有不同的激活模式，强调了需要根据每个任务的独特需求进行适应性策略的需求。基于这一观察，我们提出了DynamicKV方法，通过动态优化token的保留来适应特定任务，在每层调整保留的token数量。DynamicKV设立了全局和每层的最大KV缓存预算，并在推理过程中为当前层暂时保留最大预算，并定期更新所有先前层的KV缓存大小。我们的方法在LongBench上仅保留了全量KV缓存大小的1.7%，但达到了约85%的全量KV缓存性能。值得注意的是，即使在极端压缩（90%）的情况下，DynamicKV在Needle-in-a-Haystack测试中仍比最先进的（SOTA）方法在Mistral-7B-Instruct-v0.2上的表现高出11%。该代码将公开发布。 
...
**Abstract (ZH)**: 多步多模态推理任务对多模态大型语言模型（MLLMs）提出了重大挑战，如何有效提升其在这种场景下的表现依然是一个未解之谜。本文提出了一种名为AR-MCTS的通用框架，通过主动检索（AR）和蒙特卡洛树搜索（MCTS）逐步增强MLLMs的推理能力。我们的方法从构建统一的检索模块开始，该模块可以从混合模态检索语料库中获取解决复杂推理问题的关键支持见解。为了弥合自动多模态推理验证的缺口，我们结合使用了MCTS算法和主动检索机制，这使得可以自动生成逐步注释。这种策略动态检索每个推理步骤的关键见解，超越了传统的束搜索采样方法，以提高推理空间的多样性和可靠性。此外，我们引入了一种流程奖励模型，该模型逐步调整以支持多模态推理任务的自动验证。在三个复杂的多模态推理基准测试中的实验结果证实了AR-MCTS框架在提升各种多模态模型性能方面的有效性。进一步的分析表明，AR-MCTS能够优化采样多样性和准确性，从而实现可靠的多模态推理。 

---
# Mention Attention for Pronoun Translation 

**Title (ZH)**: 名词性代词注意力机制在代词翻译中的应用 
...
**Abstract (ZH)**: 大多数代词是指示性语言要素，机器需要解决它们所指代的具体对象，且不同语言在代词使用上存在差异。因此，处理这些差异并翻译代词是机器翻译中的一个挑战。提及（mentions）是代词的候选对象，与代词相比，它们与一般词汇具有更密切的关系。我们认为提取额外的提及特征有助于代词翻译。因此，在解码器中引入一个额外的提及注意模块，用于特别关注源语言提及而忽略非提及词。我们的提及注意模块不仅从源语言提及中提取特征，还考虑目标语言上下文，这有利于代词翻译。此外，我们还引入了两个提及分类器来训练模型识别提及，其输出指导提及注意模块。我们在WMT17 英德翻译任务上进行了实验，并使用BLEU、APT和对比评价指标来评估我们的模型在一般翻译和代词翻译上的表现。我们提出的模型在APT和BLEU评分上优于基础Transformer模型，这证实了我们的假设：通过特别关注源语言提及可以改善代词翻译，并且表明我们引入的新增模块没有对一般翻译质量产生负面影响。 

---
# ResoFilter: Rine-grained Synthetic Data Filtering for Large Language Models through Data-Parameter Resonance Analysis 

**Title (ZH)**: ResoFilter：通过数据分析-参数共振分析进行大规模语言模型的细粒度合成数据过滤 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）在各个领域都展现出了显著的效果，利用GPT进行合成数据生成的数据增强方法已变得非常普遍。然而，增强数据的质量和实用性仍存在疑问，当前的方法缺乏明确的数据特性评估指标。为了解决这些挑战，我们提出了一种名为ResoFilter的新方法，该方法整合了模型、数据和任务来精炼数据集。ResoFilter利用微调过程来获取数据-参数特征，通过模型权重来表示数据特性，从而提高可解释性。我们的实验表明，在数学任务中，ResoFilter仅使用一半的数据就能达到与全面微调相媲美的结果，并且在不同的模型和领域中表现出强大的泛化能力。该方法为构建合成数据集和评估高质量数据提供了有价值的见解，有望为提高数据增强技术和LLMs训练数据质量提供一种有前景的解决方案。为了确保结果的可复制性，我们将在论文被接受后发布我们的代码和数据。 

---
# Disentangling Reasoning Tokens and Boilerplate Tokens For Language Model Fine-tuning 

**Title (ZH)**: 分离推理令牌和模板令牌以进行语言模型 fine-tuning 
...
**Abstract (ZH)**: 使用代理任务数据集来增强大型语言模型（LLMs）的代理能力时，当前的方法通常会平等对待每个样本中的所有标记。然而，我们认为在样本中扮演不同角色的标记，如推理标记与模板标记（例如，用于控制输出格式的标记）之间在重要性和学习复杂性上存在显著差异，需要对其进行分离并分别处理。为了解决这个问题，我们提出了一种新的可重排序感知鉴别器（SHAD）以实现自适应标记区分。SHAD通过利用跨样本输入-输出组合在重新排序后观察到的可预测性差异来对标记进行分类：模板标记由于在样本中重复出现，维持了可预测性，而推理标记则不具备这种特性。利用SHAD，我们提出了思路突出的微调（RFT）方法，在微调过程中自适应地强调推理标记，相较于传统的监督微调（SFT），表现出显著的性能提升。 

---
# ALKAFI-LLAMA3: Fine-Tuning LLMs for Precise Legal Understanding in Palestine 

**Title (ZH)**: ALKAFI-LLAMA3：针对巴勒斯坦精确法律理解的大型语言模型微调 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在多个领域展示了显著的潜力，但在法律领域的应用，尤其是低资源环境下，依然有限。本研究针对巴勒斯坦法律领域面临的挑战进行探讨，其中政治不稳定、法制框架碎片化以及有限的人工智能资源阻碍了有效的机器学习应用。我们基于量化的Llama-3.2-1B-Instruct模型进行微调，并使用从巴勒斯坦法律文本中生成的合成数据集进行训练。通过使用小规模模型和策略性生成的问题-答案对，我们实现了一种低成本且本地可持续的解决方案，能够提供准确且具有上下文相关性的法律指导。实验结果表明，在各类查询类型上表现出色，包括是/否问题、叙述性解释以及复杂的法律区分，同时指出改进的领域，例如处理基于计算的问题和结构化列表格式化。本研究为开发适应资源有限环境的基于人工智能的法律辅助工具提供了途径。 

---
# PsyDraw: A Multi-Agent Multimodal System for Mental Health Screening in Left-Behind Children 

**Title (ZH)**: PsyDraw：用于留守儿童心理健康筛查的多模态多智能体系统 
...
**Abstract (ZH)**: 留守儿童（LBCs）在中国的数量超过6600万，由于父母为了工作而外出务工，他们面临着严重的心理健康挑战。早期筛查和识别风险较高的留守儿童至关重要，但由于心理健康专业人士严重短缺，尤其是在农村地区，这一过程极具挑战性。尽管“房屋-树木-人物”（HTP）测试能够提高儿童的参与率，但对其结果需要专业解读的要求限制了其在资源匮乏地区的应用。为应对这一挑战，我们提出了一种基于多模态大型语言模型的多智能体系统PsyDraw，该系统协助心理健康专业人士分析HTP绘画作品。系统通过两个阶段运行：全面特征分析和专业报告生成，采用专门的代理进行特征提取和心理解读。通过对290名小学生提交的HTP绘画作品的评估，结果显示71.03%的分析与专业评估实现了高水平一致性，26.21%为中等一致性，2.41%为低一致性。系统识别出需要专业关注的案例占31.03%，证明了其作为初步筛查工具的有效性。目前该系统已在试点学校部署，证明了其在资源有限地区支持心理健康专业人士方面的潜力，同时保持了高水平的心理评估专业标准。 

---
# Query pipeline optimization for cancer patient question answering systems 

**Title (ZH)**: 癌症患者查询系统中的查询管道优化 
...
**Abstract (ZH)**: 利用检索增强生成（RAG）技术，通过使用查询管道检索相关外部信息并基于检索的知识来缓解大型语言模型（LLMs）的幻觉现象。然而，癌症患者问答系统（CPQA）的查询管道优化需要针对特定领域分别优化多个组件。我们提出了一种新颖的三方面RAG查询管道优化方法，利用如PubMed和PubMed Central等公共生物医学数据库。我们的优化措施包括：（1）文档检索，通过比较NCBI资源并引入混合语义实时文档检索技术（HSRDR）进行对比分析；（2）段落检索，确定最佳密集检索器与重排序器配对方案；（3）语义表示，引入语义增强重叠分割技术（SEOS）以提高上下文理解能力。在为癌症相关查询定制开发的数据集上，我们的优化RAG方法提高了Claude-3-haiku回答准确性5.24%，比基于推理链的提示提高了约3%，且比朴素的RAG设置提高了约3%。本研究强调了特定领域的查询优化对实现RAG潜力的重要性，并提供了一个稳健的框架来构建更准确可靠的CPQA系统，推动了基于RAG的生物医学系统的发展。 

---
# On Verbalized Confidence Scores for LLMs 

**Title (ZH)**: 关于口头表达的置信度评分对大型语言模型的影响 
...
**Abstract (ZH)**: 大型语言模型（LLMs）的兴起及其与日常生活紧密结合使确保其可信度变得尤为重要。对LLMs进行不确定性量化可以增强人类对其响应的信任度，同时也有助于模型代理基于彼此的不确定性进行更加明智的决策。为了估计响应中的不确定性，通常使用内部词元对数、特定任务的代理模型或多次响应的采样方法。本研究重点关注让LLMs将其不确定性以置信度得分的形式在输出词元中进行口头表达，这是一种适用于提示和模型无关的低开销不确定性量化方法的一种有前景的方法。通过广泛基准测试，我们评估了口头表达的置信度得分在不同数据集、模型和提示方法方面的可靠性。我们的结果表明，这些得分的可靠性强烈依赖于模型的提示方式，但我们发现使用某些提示方法可以提取出校准良好的置信度得分。我们主张，口头表达的置信度得分可以成为未来简单但有效且通用的不确定性量化方法之一。我们的代码可在以下链接访问：[此处提供链接]。 
...
**Abstract (ZH)**: 模型在合成数据上的崩溃表明，迭代训练于自动生成的数据会导致性能逐渐下降。随着人工智能模型的普及，合成数据将从根本上重塑网络数据生态系统。未来的GPT-$\{n\}$模型不可避免地将利用合成数据和人为生成数据的混合进行训练。本文聚焦于两个问题：合成数据对语言模型训练的影响，以及如何合成数据而不导致模型崩溃？我们首先在不同比例的合成数据下预训练语言模型，发现合成数据的比例与模型性能之间存在负相关关系。进一步通过对合成数据进行统计分析，揭示了分布偏移现象和n-克数量子的过度集中。受上述发现的启发，我们提出在人为生成的数据上进行标记编辑以获得半合成数据。作为一种概念验证，我们从理论上证明了标记级别的编辑可以防止模型崩溃，因为测试错误被限制在一个有限的上界内。我们进行了从头开始预训练、连续预训练和监督微调的广泛实验。结果验证了我们理论证明的正确性，即标记级别的编辑可提高数据质量并增强模型性能。 

---
# Each Fake News is Fake in its Own Way: An Attribution Multi-Granularity Benchmark for Multimodal Fake News Detection 

**Title (ZH)**: 每条虚假新闻都有其独特的虚假方式：一种多模态虚假新闻检测的归因多粒度基准 
...
**Abstract (ZH)**: 社交平台在促进信息访问的同时，也充斥着大量虚假信息，导致了一系列负面影响。自动多模态虚假新闻检测是一项值得追求的任务。现有的多模态虚假新闻数据集仅提供真实或虚假的二元标签。然而，真实新闻大同小异，而每条虚假新闻都有自己独特的虚假模式。这些数据集未能反映出各种类型多模态虚假新闻的混合性质。为弥补这一差距，我们构建了一个归属多重粒度多模态虚假新闻检测数据集 \AMG，揭示了内在的虚假模式。此外，我们提出了一种多重粒度线索对齐模型 \our，以实现多模态虚假新闻的检测与归属。实验结果表明，\AMG 是一个具有挑战性的数据集，其归属设置为未来的研究开辟了新的途径。 
...
**Abstract (ZH)**: 先前的研究表明，观察到双方争议的调解员必须能够可靠地区分他们争议的原因是基于对事实认知的不同（因果关系）还是基于价值观的不同（道德观）。在本文中，我们测试OpenAI的大语言模型GPT-3.5和GPT-4能否完成这一任务，并且检查哪一种类型的争议对于这些语言模型来说诊断起来更为困难。我们在Koçak等人（2003）的研究基础上，使用案例设计再现了他们的研究，采用OpenAI的GPT-3.5和GPT-4进行测试。我们发现，这两种大语言模型对因果关系和道德规范区别的语义理解与人类相似，并且能够可靠地区分两者。当被要求诊断对话中的争议来源时，与人类相比，这两种大语言模型倾向于高估因果争议的程度，同时低估道德争议的程度，特别是在道德对齐条件下的具体尺度下表现尤为明显，GPT-4在使用这种依存尺度时尤为突出。相比之下，GPT-3.5在使用依存尺度或远程尺度时的表现不如GPT-4或人类。本研究为使用大语言模型通过诊断争议根源来调解冲突的可能性提供了一个初步测试。 

---
# Analysis and Visualization of Linguistic Structures in Large Language Models: Neural Representations of Verb-Particle Constructions in BERT 

**Title (ZH)**: 大型语言模型中语言结构的分析与可视化：BERT 中动词-粒子构造的神经表示分析与可视化 
...
**Abstract (ZH)**: 本研究探讨了基于Transformer的大规模语言模型（LLMs）中动词-副词组合的内部表示，特别是考察了这些模型在不同神经网络层面上如何捕捉词汇和句法细微差别。采用BERT架构，我们分析了其各层对诸如“agree on”、“come back”和“give up”等各种动词-副词结构的表示效果。我们的方法包括从英国国家语料库（British National Corpus）获取详细的数据集准备，随后进行广泛的模型训练，并通过多维标度（MDS）和广义判别值（GDV）计算等技术进行输出分析。结果表明，BERT的中间层最有效地捕捉句法结构，但不同动词类别的表示准确性存在显著差异。这些发现挑战了对神经网络处理语言元素常规一致性的假设，并表明网络架构与语言表示之间存在复杂的相互作用。我们的研究有助于更好地理解深度学习模型如何理解和处理语言，提供有关当前神经方法在语言分析中潜在能力和限制的见解。本研究不仅推进了计算语言学的知识，还促使进一步研究以优化神经架构，提高语言精度。 

---
# Length Controlled Generation for Black-box LLMs 

**Title (ZH)**: 长度可控生成 for 黑盒大语言模型 
...
**Abstract (ZH)**: 大语言模型（LLMs）在指令执行方面表现出色，但仍在处理生成文本长度方面存在挑战，这对于许多实际应用来说是一项基本要求。现有的长度控制方法通常涉及对LLMs参数的微调，这既不高效也不适用于实际应用。在本文中，我们提出了一种新颖的迭代采样框架，将Metropolis-Hastings算法与重要性采样加速策略相结合，以有效且可靠地控制LLMs生成的文本长度，而不修改其基础参数，从而保持LLMs的原始性能。实验结果表明，我们的框架在Llama3.1上几乎达到了100%的长度控制成功率，适用于长度受限的抽象总结和长度受限的指令执行等任务，同时最小化了额外的计算开销。这为我们方法在更广泛的范围内实现精确长度控制提供了显著潜力，不会牺牲LLMs的灵活性。 

---
# TOMG-Bench: Evaluating LLMs on Text-based Open Molecule Generation 

**Title (ZH)**: TOMG-Bench：评估基于文本的开放分子生成的LLM性能 
...
**Abstract (ZH)**: 在本文中，我们提出了基于文本的开放分子生成基准（TOMG-Bench），这是首个用于评估大规模语言模型（LLMs）开放领域分子生成能力的基准。TOMG-Bench 包括三个主要任务的数据集：分子编辑（MolEdit）、分子优化（MolOpt）和定制分子生成（MolCustom）。每个任务进一步细分为三个子任务，每个子任务包含5000个测试样本。鉴于开放分子生成固有复杂性，我们还开发了一个自动化评估系统，用于衡量生成分子的质量和准确性。我们的全面基准测试显示了当前基于文本指导的分子发现所面临的技术限制和改进潜力。此外，在OpenMolIns 的帮助下，这是一个专门为解决TOMG-Bench 提出的挑战而设计的指令调优数据集，Llama3.1-8B 能够优于所有开源通用LLMs，甚至在TOMG-Bench 上比GPT-3.5-turbo 的表现高出46.5%。我们的代码和数据集可通过此链接获取：https://example.com。 

---
# Learning to Generate Research Idea with Dynamic Control 

**Title (ZH)**: 学习通过动态控制生成研究主题 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）的迅速发展展示了其在加速科学发现方面的潜力，特别是在自动化研究构想过程方面。基于LLM的系统已经在生成假说和研究构想方面展示了潜力。然而，当前的方法主要依赖于基于提示的预训练模型，这限制了它们有效优化生成内容的能力。此外，这些系统还缺乏处理新颖性、可行性和有效性之间复杂相互依赖关系和固有限制的能力，由于这些维度之间的固有权衡，如创新与可行性之间的冲突，使其仍然具有挑战性。为了解决这些局限性，我们首次提出了对LLM进行微调以更好地提出构想，并引入了一种新的框架，该框架采用两阶段方法结合监督微调（SFT）和可控强化学习（RL）。在SFT阶段，模型从研究论文及后续构想的配对中学习基础模式。在RL阶段，通过精细反馈引导的多维度奖励建模对生成的构想进行全面评估和优化，以确保在关键指标上的表现。维度控制器允许动态调整生成策略，而句级解码器则确保推理过程中具有上下文相关的重视。该框架提供了一种平衡的研究构想方法，通过动态权衡新颖性、可行性和有效性之间的权衡来实现高质量的结果。 

---
# How good is GPT at writing political speeches for the White House? 

**Title (ZH)**: GPT在撰写白宫政治演说方面的表现如何？ 
...
**Abstract (ZH)**: 使用大规模语言模型（LLMs），计算机能够根据用户请求生成书面文本。作为一种广泛应用于多种领域的技术，本研究通过将一种名为GPT的LLM生成的演讲与近期美国总统的演讲进行对比，分析了GPT的书面风格。为此，研究对比了里根至拜登时期各任总统的国情咨文（State of the Union，SOTU）与由GPT-3.5和GPT-4生成的版本。与美国总统的演讲相比，GPT更倾向于过度使用“we”这一词形，生成的文本平均而言句子较长，且内容更为简短。此外，GPT更倾向于采用乐观的语气，更频繁地使用政治（如，总统、国会）、象征性（如，自由）和抽象（如，自由）词汇。即使对GPT施加作者的写作风格，生成的演讲仍与目标作者的演讲风格存在显著差异。最后，虽然两个GPT版本各自具有独特的特征，但它们整体上与真实的总统演讲存在显著差异。 

---
# HarmonicEval: Multi-modal, Multi-task, Multi-criteria Automatic Evaluation Using a Vision Language Model 

**Title (ZH)**: 谐波评估：基于视觉语言模型的多模态、多任务、多指标自动化评估 
...
**Abstract (ZH)**: 视觉语言模型（VLMs）在文本和图像理解方面展现了令人印象深刻的能力。然而，现有用于评估VLMs生成文本的指标仅专注于整体质量，这导致了两个局限性：1) 从整体评分中难以识别需要改进的文本哪些方面；2) 指标在预测整体评分时可能会忽略特定的评估标准。为了解决这些问题，我们提出了一种参考自由的评估指标HarmonicEval，该指标通过自底向上的方式聚合各标准分以生成整体评分。此外，我们构建了多任务多标准人工评估（MMHE）数据集，该数据集包含了四类视觉语言任务中的18,000个专家人工判断。实验结果表明，与传统指标相比，HarmonicEval与人工判断的关联度更高，并且为每个标准提供了数值评分。 

---
# KARRIEREWEGE: A Large Scale Career Path Prediction Dataset 

**Title (ZH)**: 职业路径：一个大型规模的职业路径预测数据集 
...
**Abstract (ZH)**: 准确的职业路径预测可以支持众多相关方，包括求职者、招聘人员、人力资源和项目经理。然而，公开可用的数据和职业路径预测工具非常稀缺。本研究引入了KARRIEREWEGE，这是一个全面且公开的数据集，包含超过50万条职业路径，远超之前可用数据集的规模。我们将该数据集与ESCOP分类法相链接，以提供一个有价值的资源用于预测职业轨迹。为了解决简历中常见的自由文本输入问题，我们通过合成岗位名称和描述对数据集进行了增强，形成了KARRIEREWEGE+。这使得可以从非结构化数据中进行准确预测，更贴近实际应用的挑战。我们使用我们的数据集和先前的基准测试了现有的最佳现有模型（State-of-the-Art, SOTA），并观察到在自由文本应用场景中性能和鲁棒性的改进，尤其是由于使用了合成数据。 

---
# Beyond Guilt: Legal Judgment Prediction with Trichotomous Reasoning 

**Title (ZH)**: 超越内疚：基于三元推理的法律判决预测 
...
**Abstract (ZH)**: 在司法实践中，法官应用刑法的三元体系，依次评估行为的构成要件、违法性和有责性，以确定个体的行为是否构成犯罪。尽管当前的法律大型语言模型（LLMs）在判决预测方面显示出令人鼓舞的准确性，但由于缺乏合适的基准数据集，它们缺乏三元推理能力，无法预测无罪判决。因此，每个输入都会自动被分配一个罪名，限制了它们在法律场景中的实用价值。为了解决这一问题，我们引入了LJPIV，这是首个包含无罪判决的法律判决预测基准数据集。遵循三元体系的原则，我们通过基于LLM的数据增强和人工验证扩展了三个广泛使用的法律数据集。我们的实验表明：（1）当前的法律LLMs仍有显著改进空间，即使是最佳模型在LJPIV上的F1得分也低于0.3；（2）我们的策略显著提升了领域内和跨领域判决预测的准确性，特别是对于产生无罪判决的案例。 

---
# Simulation-Free Hierarchical Latent Policy Planning for Proactive Dialogues 

**Title (ZH)**: 无模拟分层潜在策略规划以实现主动对话 
...
**Abstract (ZH)**: 近年来，主动对话领域的最新进展引起了广泛关注，特别是在更复杂的任务目标上（如情感支持和说服）。与传统的目标导向对话不同，主动对话要求更高级的策略规划和灵活性，这需要丰富的场景和全面的策略库来开发此类系统。然而，现有的方法往往依赖于大型语言模型（LLMs）进行用户模拟和在线学习，这导致了与现实情境不符的偏见，并导致效率低下。此外，这些方法依赖于手动定义的、与上下文无关的粗粒度策略，不仅增加了专家成本，还引发了完整性的担忧。在我们的工作中，我们强调了直接从原始的真实对话记录中自动发现策略的潜力。为此，我们提出了一种新的对话策略规划框架——LDPP。该框架完全自动化了从挖掘对话记录中的策略到学习策略规划的全过程。具体来说，我们采用变分自编码器的变体来发现用潜在向量表示的细粒度策略。在自动为数据添加这些潜在策略标签后，我们提出了一个在潜在空间中的离线层次强化学习（RL）算法，以开发有效的策略规划能力。我们的实验结果显示，LDPP 在两个主动对话场景上显著优于现有方法，甚至仅使用一个只有1.8亿参数的LLM时，就超越了ChatGPT。 
...
**Abstract (ZH)**: 随着检索增强生成（RAG）的应用，大规模语言模型（LLM）有望将生成基于检索到的上下文。然而，这一目标受到LLM位置偏见的阻碍，使其难以均匀地关注所有上下文。先前的工作通过合成带有扰动位置的黄金片段，创建一个位置多样化的训练集来解决这一问题。我们进一步扩展了这一想法，提出了一种结合增强和蒸馏的一致性正则化方法。首先，我们通过为每个训练实例增加其位置的扰动来增强每个训练实例，以鼓励在不同排序情况下的一致预测。我们还蒸馏这一对的行为，尽管在某些RAG场景中，检索器给出的顺序对于生成质量至关重要，这可能会导致不利影响。因此，我们提出了CORD（COnsistency and Rank Distillation），以平衡一致性与排名蒸馏。CORD自适应地从插值空间中采样噪声控制的扰动，确保同时保持一致性和尊重排名先验。实证结果表明，这种平衡使CORD在多种RAG基准测试中表现更优。 

---
# CitaLaw: Enhancing LLM with Citations in Legal Domain 

**Title (ZH)**: CitaLaw：在法律领域增强大规模语言模型的引证方法 
...
**Abstract (ZH)**: 在本文中，我们提出了CitaLaw，这是首个用于评估大规模语言模型（LLM）生成合法合规响应并适当引用案例能力的基准测试。CitaLaw 特设了面向普通公众和专业人士的多种多样的法律问题，并与全面的法律文献和判例案例集合作为参考库。该框架使基于LLM的系统能够从参考库中检索支持性引文，并将这些引文与响应中相应的句子对齐。此外，我们还引入了以三段论为基础的评估方法，用于评估检索到的参考文献与LLM生成响应之间的法律一致性，以及这些响应与用户问题的一致性。对2个开放领域和7个法律特定的大规模语言模型进行了广泛的实验，结果显示整合法律参考文献显著提升了响应质量。此外，我们提出的基于三段论的评估方法与人类判断表现出高度一致性。 
...
**Abstract (ZH)**: 在生物医学研究等领域，不断生成大量的研究文献，因此对大规模文本语料库进行探索性检索是至关重要的。本文提出了一种名为ClusterTalk的框架（演示视频和源代码可在以下链接获取：https://thisgetUrl.com），该框架利用多维探索性检索进行语料库探索。我们的系统将文档聚类与分类搜索相结合，允许用户通过交互方式细化其探索，并提出语料库和文档级别的查询。与传统的单维搜索方法（如关键词搜索或聚类）相比，该系统通过鼓励用户与语料库进行更深入的交互，提高了信息的可发现性。本文基于四年内四百多万的PubMed摘要，展示了ClusterTalk框架的功能。 

---
# Multi-Level Optimal Transport for Universal Cross-Tokenizer Knowledge Distillation on Language Models 

**Title (ZH)**: 多级最优传输在语言模型跨令牌器通用知识精炼中的应用 
...
**Abstract (ZH)**: 知识蒸馏（KD）已成为压缩大型语言模型（LLMs）的一种流行技术。现有的KD方法受到教师和学生模型之间必须具有相同分词器（即词汇表）的限制，这限制了它们在处理不同架构家族的LLMs时的灵活性。本文中，我们提出了多层最优运输（MultiLevelOT）方法，这是一种提升通用跨分词器知识蒸馏的新颖方法。我们的方法通过使用多种成本矩阵，同时在标记和序列级别对教师和学生的logit分布进行对齐，从而消除了在维度或标记级别上对应的需求。在标记级别上，MultiLevelOT通过同时优化序列中的所有标记以增强鲁棒性，整合了全局和局部信息。在序列级别上，我们利用Sinkhorn距离高效捕捉logit的复杂分布结构，它通过为散度度量近似Wasserstein距离来实现这一目标。在诸如抽取式QA、生成式QA和摘要等任务上的广泛实验证实，MultiLevelOT在各种设置中均优于最先进的跨分词器KD方法。我们的方法在不同架构家族、模型架构和参数规模的学生和教师模型中都表现出很高的鲁棒性。 
...
**Abstract (ZH)**: 检索增强生成（RAG）的发展减轻了大型语言模型（LLMs）生成内容中过时和幻觉问题，但仍暴露了许多局限性。当通用型LLM用作RAG生成器时，它通常会面临响应信息量不足、响应稳健性和引文质量差的问题。以往为应对这些局限性所采取的措施，要么是通过额外步骤生成响应，要么是通过监督微调（SFT）优化生成器，但这些方法仍未充分符合RAG的要求。因此，如何在保持其端到端LLM形式的同时，从多个偏好角度优化RAG生成器仍是一项挑战。为弥合这一差距，我们提出了一种名为多视角偏好对齐的检索增强生成方法（PA-RAG），该方法旨在全面优化RAG系统生成器，以符合RAG要求。具体而言，我们通过从不同提示文档质量情境下生成的具有不同质量的响应中进行采样，构建高质量的指令微调数据和多视角偏好数据。随后，我们使用监督微调（SFT）和直接偏好优化（DPO）来优化生成器。在三个LLM的四个问答数据集上进行的广泛实验表明，PA-RAG能够显著提高RAG生成器的性能。我们的代码和数据集可在此处获得：[链接]。 

---
# Do Large Language Models Defend Inferentialist Semantics?: On the Logical Expressivism and Anti-Representationalism of LLMs 

**Title (ZH)**: 大型语言模型是否捍卫解释主义语义学？关于大型语言模型的逻辑表达主义与反再现论分析 
...
**Abstract (ZH)**: 语言哲学在历史上一直是通过人类中心主义的视角发展的，但由于大型语言模型（LLMs）如ChatGPT（OpenAI）、Claude（Anthropic）的出现，现在被迫转向后人类中心主义。这些LLMs被认为具有类似于人类的语言能力。传统上，LLMs通过分布语义学被解释为其基础语义学基础。然而，最近的研究正在探索超越分布语义学的替代基础语义学。本文提议罗伯特·布拉朗德（Robert Brandom）的推理主义语义学作为LLMs的基础语义学，特别是聚焦于后人类中心主义趋势中的语言表征主义问题。本文表明，推理主义语义学中的反表征主义、逻辑表达主义以及准共有性在解释LLMs的特征和行为方面是有用的。此外，本文还为LLMs提出了共识真理理论。本文认为，LLMs的特性挑战了语言哲学中的一些主流假设，如语义外部主义和共相性。我们相信本文的观点将促使对反表征主义的语言观点进行重新评估，这可能为语言哲学带来新的发展。 
...
**Abstract (ZH)**: 为什么我们构建本地大型语言模型（LLMs）？本地LLMs应该从目标语言中学习什么？哪些能力可以从中其他语言中转移到目标语言中？特定语言的缩放定律是否存在？为了探索这些问题，我们对35个日语、英语和多语言LLMs在19个日语和英语评估基准上的表现进行了评估，将日语作为本地语言。采用观察性方法，我们分析了基准分数的相关性，并通过对分数进行主成分分析（PCA）来提取本地LLMs的能力因素。我们发现，在英语文本上的训练可以提高日语学术科目（JMMLU）的成绩。此外，增强解决日语代码生成、算术推理、常识和阅读理解任务的能力并不需要专门在日语文本上进行训练。相比之下，在日语文本上的训练可以提升关于日语文本知识的问题回答任务和日英翻译任务的表现，这表明解决这两类任务的能力可以被视为LLMs的“日语能力”。此外，我们确认了这些日语能力与日语文本的计算预算呈正相关。 
...
**Abstract (ZH)**: 随着大型语言模型（LLMs）越来越多地被用作代理，它们在交互环境中的集成和工具使用引入了超出模型本身相关安全问题的新挑战。然而，缺乏全面的代理安全评估基准，严重阻碍了有效的评估和进一步改进。在本文中，我们介绍了Agent-SafetyBench，这是一个全面的基准，旨在评估LLM代理的安全性。Agent-SafetyBench包含了349个交互环境和2000个测试案例，评估了8种安全风险类别，并涵盖了在不安全交互中常见的10种常见失败模式。通过对16个流行的LLM代理进行评估，我们揭示了一个令人担忧的结果：没有一个代理的安全评价值超过60%。这突显了LLM代理中存在的重要安全挑战，并强调了其显著的改进需求。通过定量分析，我们识别出了关键的失败模式，并总结了当前LLM代理中两种基本的安全检测：缺乏稳健性和缺乏风险意识。此外，我们的研究结果表明，仅仅依赖于防御提示是不足以解决这些安全问题的，强调了需要更先进和稳健的策略的重要性。我们在此处发布了Agent-SafetyBench（\url{this https URL}），以促进代理安全性评估和改进的进一步研究和创新。 

---
# From Human Annotation to LLMs: SILICON Annotation Workflow for Management Research 

**Title (ZH)**: 从人力标注到大型语言模型：SILICON标注工作流在管理研究中的应用 
...
**Abstract (ZH)**: 无结构文本数据的注释与分析是管理研究的基础，通常依赖于通过众包平台的人类标注员。虽然大规模语言模型（LLMs）承诺提供一种成本有效且高效的人类标注替代方案，但缺乏一种系统的工作流来评估LLMs何时适合使用，以及如何以可重复的方式进行基于LLM的文本注释。本文通过引入“SILICON”（系统性语言模型推断用于信息分类与标注）工作流来解决这一方法论上的空白。该工作流将现有的人类标注原则与系统化的提示优化和模型选择相结合，解决了诸如制定稳健的注释指南、建立高质量的人类基准、优化提示以及确保在不同LLMs上的可重复性等挑战。我们通过七个案例研究验证了SILICON工作流，这些案例研究涵盖了常见的管理研究任务，包括商务提案评估、对话意图和断点分析、评论属性检测。研究结果强调了验证注释指南一致性的必要性、专家开发的人类基准优于众包基准的重要性、提示优化的迭代性质以及对多种LLM进行测试的必要性。值得注意的是，我们提出了一种基于回归的方法来实证比较不同提示和模型的LLM输出。我们的工作流为基于LLM的注释建立了可重复的过程，以保持科学严谨性。我们为研究人员提供了实用的指导，帮助他们有效地利用生成式AI工具的演进，并保持透明性和可重复性。 

---
# ORBIT: Cost-Effective Dataset Curation for Large Language Model Domain Adaptation with an Astronomy Case Study 

**Title (ZH)**: ORBIT：面向天文领域的大语言模型适应性数据集维护成本效益方法 
...
**Abstract (ZH)**: 近年来，语言模型的研究进展突显了高质量领域特定训练数据的重要性，尤其是在需要专门知识的任务中。通用模型虽然功能强大，但因缺乏特定领域的信息，往往无法满足专家级别的任务需求。领域适应训练可以增强这些模型，但这需要大量高质量的领域特定数据。为了解决这个问题，我们提出了ORBIT，一种低成本的方法，从嘈杂的网络来源中收集大量的高质量领域特定数据，以训练专家级大型语言模型。以天文学为主要案例，我们将1.3万亿词的FineWeb-Edu数据集精炼为一个专注于天文学的高质量、100亿词子集。通过在天文学子集上微调\textsc{LLaMA-3-8B}，我们在MMLU天文学基准测试上的性能从69%提升到76%，并在天文学特定基准测试AstroBench上取得了最佳成绩。此外，我们的模型（Orbit-LLaMA）表现优于\textsc{LLaMA-3-8B-base}，在1000个天文学特定问题中，有73%的情况下GPT-4o评价更偏好它。我们还通过将其应用于法律和医学领域，验证了ORBIT的泛化能力，与未经筛选的基本方法相比，数据质量有了显著提升。我们开源了ORBIT方法，包括整理后的数据集、代码库以及生成的模型，地址为\href{this https URL}{this https URL}。 

---
# All-in-One Tuning and Structural Pruning for Domain-Specific LLMs 

**Title (ZH)**: 面向特定领域的大语言模型的一体化调优与结构剪枝 
...
**Abstract (ZH)**: 现有的针对特定领域应用的大型语言模型（LLMs）的剪枝技术通常遵循两个阶段的过程：首先对预训练的通用LLM进行剪枝，然后在特定领域对其进行微调。然而，来自预训练权重的剪枝决策在微调过程中保持不变，即使权重已被更新。因此，这种剪枝决策与更新后的权重的结合可能是次优的，可能导致性能显著下降。为了解决这些限制，我们提出了一种名为ATP（All-in-One Tuning and Structural Pruning）的新方法，这是一种统一的一阶段结构剪枝与微调方法，通过可训练的剪枝决策生成器在微调过程中动态识别当前的最佳子结构。此外，由于特定领域应用的可用数据有限，低秩适应（LoRA）常被用作微调LLM的方法。在ATP中，我们引入了LoRA感知的前向传播和稀疏正则化，以确保在ATP过程中根据学习到的剪枝决策所对应的有效子结构可以直接被移除。实验结果表明，ATP在法律和医疗领域任务上的表现优于现有的两阶段剪枝方法。具体而言，当剪枝LLaMA2-7B和LLaMA3-8B模型40%的参数时，ATP分别恢复了原始密集模型高达88%和91%的性能。 

---
# ECG-Byte: A Tokenizer for End-to-End Generative Electrocardiogram Language Modeling 

**Title (ZH)**: ECG-Byte：一种用于心电图语言建模的端到端生成型分词器 
...
**Abstract (ZH)**: 大语言模型（LLMs）在文本之外的领域中展现了令人remarkable的适应性，特别是在心电图（ECGs）领域。更具体地说，越来越多的研究致力于从多通道ECGs及其对应的文本提示中生成文本的任务。现有的方法通常涉及使用半监督学习（SSL）目标对特定于ECG的编码器进行预训练，并使用预训练编码器输出的特征对大语言模型进行微调以实现自然语言生成（NLG）。然而，这些方法受限于1）两阶段训练的低效率，以及2）编码器生成特征的可解释性挑战。为了解决这些限制，我们提出了ECG-Byte，这是一种针对ECGs的自回归语言建模的字节对编码（BPE）分词流水线。此方法将ECG信号压缩并编码为tokens，通过直接结合ECG和文本tokens实现端到端的LLM训练，且由于ECG tokens可以直接映射回原始信号，因此更具可解释性。利用ECG-Byte，我们仅在所需数据量的48%和所需时间的一半内，在NLG任务中达到了具有竞争力的性能。 

---
# Memorization Over Reasoning? Exposing and Mitigating Verbatim Memorization in Large Language Models' Character Understanding Evaluation 

**Title (ZH)**: 过度记忆而非推理？揭示并减轻大型语言模型在角色理解评估中逐字记忆的问题 
...
**Abstract (ZH)**: 近年来，大规模语言模型（LLMs）在角色理解任务中展现了 impressive 的表现，例如分析虚构角色的角色、性格和关系。然而，LLMs 所依赖的大量预训练语料库引发了担忧，它们可能更多地依赖于记忆流行的作品而非真正理解和推理。在本文中，我们认为在角色理解任务中，应以“主旨记忆”——即捕捉关键含义——为主要机制，而不是“一字不差的记忆”——即字符串的精确匹配。我们提出了一种简单而有效的方法，可以在角色理解评估中缓解机械记忆现象，同时保留必要的隐含线索以实现理解和推理。我们的方法将对流行虚构作品的机械记忆驱动性能从 96% 的准确率降低到 72%，并且在各种角色理解任务中准确率下降高达 18%。这些发现强调了现有基准数据污染的问题，这些基准往往测量的是记忆而非真正的角色理解。 
...
变换器在自然语言处理（NLP）和信息检索（IR）中占据主导地位，但它们在推理效率方面的不足以及在处理更长上下文时的挑战引起了对替代模型架构的兴趣。在这类模型中，状态空间模型（SSMs）如Mamba显示出有希望的优势，特别是在推理方面的时间复杂度达到了O(1)。尽管具有这一潜力，SSMs在文本重排任务中的有效性——这一任务需要精细的查询-文档交互和对长上下文的理解——仍未得到充分探索。

本研究通过比较基于SSM的架构（具体包括Mamba-1和Mamba-2）和基于变换器的模型在不同规模、架构和预训练目标下的表现，重点关注文本重排任务中的性能和效率。研究发现：（1）Mamba架构在文本排名性能方面达到了可比于相似大小的变换器模型的竞争力；（2）它们在训练和推理效率方面比具有闪存注意力机制的变换器模型略逊一筹；（3）Mamba-2在性能和效率上均优于Mamba-1。这些结果强调了状态空间模型作为变换器替代模型的潜在价值，并指出了未来信息检索应用中的改进方向。 

---
# A Survey on LLM Inference-Time Self-Improvement 

**Title (ZH)**: LLM推理时自我改进的研究综述 
...
**Abstract (ZH)**: 提高推理性能通过在测试时增加计算量的技术最近受到了关注。在本综述中，我们从三个不同的视角调查了LLM在推理时自我改进的现状：独立自我改进，侧重于通过解码或采样方法进行增强；上下文感知自我改进，利用额外的上下文或数据存储；模型辅助自我改进，通过模型合作实现改进。我们提供了对近期相关研究的全面回顾，贡献了一种深入的分类法，并讨论了挑战和限制，为未来的研究提供了见解。 
...
在翻译时，保持了原文的结构和学术探讨的性质，将“peer-reviewing”译为“同行评审”更为具体和准确，同时将疑问句式保留，符合中文的表达习惯。 
...
**Abstract (ZH)**: peer-reviewing在识别重要论文方面的有效性如何？我们将这个问题视为一个预测任务。我们能否基于会议/期刊和“早期效应”（即发表后不久的引用量）来预测哪些论文将在未来被大量引用？结果显示，“早期效应”比会议/期刊更能预测论文的未来引用量。最后，我们提出了应对扩展挑战的建设性建议：（a）处理过多的提交论文数量问题；（b）解决合格审稿人不足的问题。 

---
# Semantic Role Labeling of NomBank Partitives 

**Title (ZH)**: NomBank 部分名词短语的语义角色标注 
...
**Abstract (ZH)**: 本文探讨了针对英语部分名词（如5%/REL of the price/ARG1; The price/ARG1 rose 5 percent/REL）的语义角色标注问题，并使用了诺姆语料库（NomBank annotated corpus）进行标注。文中描述了几种使用传统机器学习和基于变压器的方法，以及集成方法的系统。我们的最高得分离散系统分别使用宾夕法尼亚树库（Penn Treebank）的“黄金”解析和伯克利神经解析器得到了91.74%和91.12%的F1分数。本研究还包括了系统开发中的课堂和实验设置。 

---
# The Role of Handling Attributive Nouns in Improving Chinese-To-English Machine Translation 

**Title (ZH)**: 《形容词性名词短语在提升中文到英文机器翻译中的作用》 
...
**Abstract (ZH)**: 将具有重大语法差异的语言进行互译会带来挑战，这不仅对人类译者构成挑战，同时也对机器翻译系统构成挑战。本研究特别针对汉语中的限制性名词所提出的翻译挑战，这些名词在英语翻译中经常导致歧义。我们通过手动插入被省略的“的”字（X），在宾州中文语篇树库的新闻文章标题中开发了一个专门的数据集，用于微调Hugging Face的汉语到英语翻译模型，特别是提高对这种关键功能词的处理能力。这种专注于特定问题的方法不仅补充了之前研究中提出的广泛策略，还通过特别解决汉语到英语翻译中的常见错误类型，提供了实际的改进。 

---
# Multi-OphthaLingua: A Multilingual Benchmark for Assessing and Debiasing LLM Ophthalmological QA in LMICs 

**Title (ZH)**: 多语眼科语言：用于评估和消除低收入和中等收入国家（LMICs）中LLM眼科问答偏见的多语言基准 
...
**Abstract (ZH)**: 当前的眼科学临床工作流程受制于过度转诊、长时间等待以及复杂多样的医疗记录。大型语言模型（LLMs）为自动化各种程序提供了有希望的解决方案，例如分诊、初步检查如视力评估和报告总结。然而，LLMs 在自然语言问答任务中的表现因不同语言而异，可能会加剧低收入和中等收入国家（LMICs）的医疗保健不平等现象。本研究介绍了首个跨语言人工策划问题的眼科问答基准，使得可以通过直接跨语言对比进行评价。我们在7种不同语言的6个流行LLMs上的评估显示，不同语言之间存在显著的偏差，突显了在LMICs中临床应用LLMs的风险。现有的去偏方法，如翻译链式思维或检索增强生成（RAG），单独使用时很难弥合这一性能差距，往往无法在所有语言中提高性能，并且缺乏对医疗领域的特定性。为了解决这一问题，我们提出了一种新颖的推理时间去偏方法——CLARA（跨语言反思性代理系统），该方法结合了检索增强生成和自我验证技术。我们的方法不仅提升了所有语言的性能，还显著缩小了多语言偏差差距，从而促进全球范围内LLMs的公平应用。 

---
# Fake News Detection: Comparative Evaluation of BERT-like Models and Large Language Models with Generative AI-Annotated Data 

**Title (ZH)**: 虚假新闻检测：基于BERT类模型与生成人工智能标注大语言模型的比较性评估 
...
**Abstract (ZH)**: 假新闻对现代社会的公众意见和社会稳定构成了重大威胁。本研究对基于BERT的编码器-only模型和自回归解码器-only大型语言模型（LLMs）在假新闻检测中的表现进行了比较评估。我们通过GPT-4辅助（一种AI标注方法）和经过人类专家验证的方式构建了一个新闻文章数据集，以确保其可靠性。这两种模型均在该数据集上进行了微调。此外，我们还开发了一种在推理过程中采用多数投票的指令微调LLM方法，用于生成标签。我们的分析表明，在分类任务中，基于BERT的模型通常优于LLMs，而LLMs在应对文本扰动方面表现出更好的稳健性。与弱标签（远监督）数据相比，结果表明，在人类监督下的AI标签能够实现更好的分类效果。本研究强调了结合基于AI的标注与人类监督的有效性，并展示了不同类型的机器学习模型在假新闻检测中的性能。 

---
# Tokenisation is NP-Complete 

**Title (ZH)**: 词元化问题是NP完全问题 
...
**Abstract (ZH)**: 在本文中，我们证明了两种变体的标记化问题的NP完全性，这些问题定义为将数据集压缩为最多 $\delta$ 个符号的问题。具体而言，分别是直接找到词汇表（直接标记化）或选择一系列合并操作（自底向上的标记化）的问题。 

---
# Critical-Questions-of-Thought: Steering LLM reasoning with Argumentative Querying 

**Title (ZH)**: 批判性问题思考：通过论辩性查询引导大规模语言模型推理 
...
**Abstract (ZH)**: 尽管近期在人工智能领域的突破性进展迅速推进了研究，现有的最先进大语言模型（Large Language Models, LLMs）在进行逻辑和数学推理时仍然面临挑战。研究结果表明，LLMs 仍然主要充当高级的数据模式识别工具，在尝试泛化和解决从未见过或与训练数据样本相差较大的逻辑推理问题时表现不佳。为应对这一令人关注的问题，本文借鉴论证理论文献中的“关键问题”概念，特别是丘林（Toulmin）的论证模型。我们表明，利用这些关键问题能够提高LLMs的推理能力。通过探究模型推理过程中的逻辑思路，LLMs可以评估是否存在逻辑错误，并在提供最终回复前纠正这些错误。这一理念源自有效论证程序的金标准：结论只有在其由接受的前提所蕴含时才是有效的。或者，换句话说，在现实世界的近似环境中，如果结论未被反驳，即使信息不完整且存在某种预设逻辑，结论也是有效的。这种方法成功地引导模型输出通过推理管道，从而在基准测试及其链式思维（Chain-of-Thought, CoT）实现方面表现出更好的性能。为此，本文在MT-Bench的推理和数学任务上，对多种LLMs的提出方法进行了全面评估。 

---
# Prompt-A-Video: Prompt Your Video Diffusion Model via Preference-Aligned LLM 

**Title (ZH)**: 当然，以下是翻译成中文后的版本，符合学术规范：

Prompt-A-Video: 通过偏好对齐的大型语言模型优化视频扩散模型

这里的“优化”一词可以根据具体上下文灵活调整为“引导”等更为准确的词汇。 
...
**Abstract (ZH)**: 文本到视频模型通过优化高质量的文本-视频对取得了显著进展，其中文本提示在决定输出视频质量方面起着关键作用。然而，要实现预期的输出通常需要多次修订和迭代推理来完善用户提供的提示。当前用于文本到视频扩散模型的自动提示优化方法面临模态不一致、成本差异和模型无意识等挑战。为了解决这些问题，我们提出了一个基于大型语言模型（LLM）的提示适配框架，名为“Prompt-A-Video”，该框架能够高效地生成以视频为中心、无需人工、且与用户偏好一致的提示，以适应特定的视频扩散模型。我们的方法涉及到一个精心设计的两阶段优化和对齐系统。首先，我们采用奖励引导的提示进化管道自动创建最优提示池，并利用这些提示进行大型语言模型的监督微调（SFT）。然后，使用多维度奖励生成监督微调模型的数据，并采用直接偏好优化（DPO）算法进一步促进偏好对齐。通过广泛的实验和对比分析，我们验证了Prompt-A-Video在不同生成模型中的有效性，突显了其在推动视频生成边界方面的潜力。 

---
# Associative memory inspires improvements for in-context learning using a novel attention residual stream architecture 

**Title (ZH)**: 关联记忆启发了一种新型注意残差流架构在情景学习中的改进 
...
**Abstract (ZH)**: 大型语言模型（LLMs）展示了利用输入序列中的信息以适当的方式响应训练过程中未见过的数据的强大能力。这种能力被称为上下文内学习（ICL）。人类和非人类动物也表现出类似的能力，但它们的神经架构与LLMs有显著差异。尽管如此，LLMs中一个关键组件——注意机制——与现代关联记忆模型相似，这些模型在计算神经科学社区中有广泛应用，并受到生物记忆系统的启发。利用这一联系，我们提出了一个能够执行ICL的关联记忆模型。我们以此为灵感，设计了一种新的残差流架构，允许信息直接流经注意头之间。我们在两层Transformer中训练此架构，并展示了与未做此修改相比，其ICL能力表现得更加迅速。然后，我们在具有800万个参数的小型语言模型中应用此架构，重点关注注意头的值，实验结果也表明在较大且更自然的规模下，ICL性能得到了提高。 

---
# A Cross-Domain Study of the Use of Persuasion Techniques in Online Disinformation 

**Title (ZH)**: 跨域研究在线虚假信息中劝说技术的运用 
...
**Abstract (ZH)**: 无论是在哪个领域或使用哪种语言，虚假信息的目标都是误导或操控公众意见，通常通过运用先进的说服技术来实现。关于如何在虚假信息中武器化说服技术的定性与定量研究多局限于特定主题（例如，COVID-19），跨领域的研究相对较少，导致对这些策略的全面理解不足。本研究采用最新的人机分类方法，对16种说服技术在虚假信息叙事中的作用进行了大规模、多领域的分析，揭示了不同说服技术在不同虚假信息领域中的不均衡应用。此外，我们还进行了一项详尽的案例研究，关注气候变化领域的虚假信息，展示了语言、心理和文化因素如何塑造说服策略以适应独特的主题背景。 

---
# Till the Layers Collapse: Compressing a Deep Neural Network through the Lenses of Batch Normalization Layers 

**Title (ZH)**: 直到层塌缩：通过批量归一化层的视角压缩深度神经网络 
...
**Abstract (ZH)**: 当今，深度神经网络被广泛应用于各种复杂任务中，因其通用性而成为现代技术的强大工具。然而，深度神经网络往往参数过多，使用这些大型模型会消耗大量计算资源。本文引入了一种名为 \textbf{T}ill the \textbf{L}ayers \textbf{C}ollapse (TLC) 的方法，通过批量归一化层的视角压缩深度神经网络。通过减少这些网络的深度，我们的方法降低了深度神经网络的计算需求和总体延迟。我们将在图像分类和自然语言处理（NLP）任务中流行的模型，如Swin-T、MobileNet-V2和RoBERTa上验证这种方法。 

---
# Large Language Models and Code Security: A Systematic Literature Review 

**Title (ZH)**: 大规模语言模型与代码安全：一项系统文献综述 
...
**Abstract (ZH)**: 大型语言模型（LLMs）已经在自动化各种编程任务方面展现出强大的工具潜力，包括与安全相关的工作，如检测和修复漏洞。尽管它们具有令人振奋的性能，但当被要求生成或修改现有代码时，这些模型可能会引入未知的漏洞。在分析代码时，它们可能错过明显的漏洞或发出不存在的漏洞信号。在这篇系统文献综述（SLR）中，我们旨在调查使用LLMs进行各种编码相关任务的安全益处和潜在缺点。首先，我们重点关注当LLMs用于生成代码时可能会引入的漏洞类型。其次，我们分析LLMs在任何给定代码中检测和修复漏洞的能力，并考察首选的提示策略如何影响它们在这两项任务中的表现。最后，我们深入探讨数据投毒攻击如何影响LLMs在上述任务中的性能。 
...
**Abstract (ZH)**: 多模态视频到文本模型在生成视频内容的简短描述方面取得了显著进展。然而，在生成包含视频和音频信息的丰富长篇文章方面仍存在不足。本文介绍了名为M2S的框架，旨在通过结合音频、视频和字符识别来生成长篇文本。M2S包括用于生成和理解视频长文本描述的模块，基于音频分析情感、语速和角色对齐，以及基于视觉的字符识别对齐。通过使用大规模语言模型GPT4o融合多模态信息，M2S在多模态文本生成领域脱颖而出。通过对比实验和人工评估，我们展示了M2S的有效性和准确性。此外，该模型框架具有良好的扩展性和对未来研究的巨大潜力。 

---
# Unveiling Uncertainty: A Deep Dive into Calibration and Performance of Multimodal Large Language Models 

**Title (ZH)**: 揭开不确定性：多模态大型语言模型校准与性能深度探究 
...
**Abstract (ZH)**: 多模态大规模语言模型（MLLMs）结合视觉和文本数据，应用于图像字幕、视觉问答等任务。在医疗保健和自主驾驶等关键领域，合理的不确定性校准至关重要，但也极具挑战性。本文研究了代表性MLLMs在不同场景下的校准情况，包括视觉微调前后和基础大规模语言模型的多模态训练前后。我们发现了它们在性能上的错校准现象，但并未发现这些场景在一致性方面的显著差异。此外，我们还强调了文本和图像之间的不确定性差异及其集成如何影响整体不确定性。为了更好地理解MLLMs的错校准以及它们评估不确定性的能力，我们构建了名为IDK（我不知道）的数据集，这对于评估它们处理未知情况的能力至关重要。我们的发现表明，MLLMs倾向于给出答案而不是承认不确定性，但通过适当的提示调整，这种自我评估能力有所提高。最后，为了校准MLLMs并提高模型的可靠性，我们提出了温度规整和迭代提示优化等技术。我们的研究结果为多模态应用中有效和负责任地部署MLLMs提供了见解。相关代码和IDK数据集：\href{此处替换为实际链接}{这里替换为实际链接}。 
...
**Abstract (ZH)**: 视觉信息提取(VIE)在半结构化文档的理解中发挥着关键作用，已经开发出多种预训练模型以提升性能。然而，这些大多数研究是单语的（通常为英文）。由于英语文本和非英文文本之间预训练语料库的数量和质量严重失衡，很少有研究能够扩展到非英文场景。本文通过系统实验表明，视觉和布局模态在不同语言的图像之间具有不变性。如果从文档图像中去除语言偏见，基于视觉布局的模型可以在跨语言应用中取得杰出的泛化效果。因此，我们提出了一种简单但有效的多语言预训练范式LDP（语言解耦预训练）以更好地利用单语言预训练数据。我们提出的模型LDM（语言解耦模型）首先在与语言无关的数据上进行预训练，然后由扩散模型解耦语言知识，并针对下游语言进行微调。大量的实验结果表明，LDM在所有最新的多语言预训练模型中表现最佳，并且在下游单语言/英文基准测试中也保持着竞争力。 

---
# Sliding Windows Are Not the End: Exploring Full Ranking with Long-Context Large Language Models 

**Title (ZH)**: 滑动窗口并非终点：探索长上下文大型语言模型的全排序能力 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）在列表式段落排序任务中展现了令人兴奋的表现。由于输入长度有限，现有方法通常采用滑动窗口策略。虽然这种策略有效，但在效率上却存在问题，因为它涉及重复和串行处理，通常会多次重新评估相关段落，从而导致冗余的API成本，这些成本与推理令牌的数量成正比。长上下文LLMs的开发使所有段落能够在单次推理中进行完整排序，从而避免了冗余的API成本。在本文中，我们对长上下文LLMs在排序任务中的效率和有效性进行了全面研究。实验结果令人惊讶地发现，在监督微调设置中，使用长上下文LLMs进行完整排序可以实现显著的性能提升和效率改进。此外，我们还发现基于现有方法微调完整排序模型的两个局限性：(1) 滑动窗口策略无法生成完整的排序列表作为训练标签，(2) 语言模型损失不能在标签中强调排名靠前的段落ID。为解决这些问题，我们提出了新的完整列表式标签构建方法以及一种新的基于重要性的学习目标。实验结果表明，我们的方法在基线方法上具有优越的表现。我们的代码可在 \url{此链接} 获取。 
...
**Abstract (ZH)**: 我们研究了使大规模语言模型（LLMs）与人类偏好数据对齐的问题。对比偏好优化在通过优化与策略相关的隐式奖励来对齐LLMs与可用偏好数据方面展现出了有前景的结果。然而，对比目标主要关注两个响应的隐式奖励的相对值，而忽视了它们的实际值，导致对人类偏好的对齐效果不佳。为解决这一局限，我们提出了校准直接偏好优化（Calibrated Direct Preference Optimization, Cal-DPO），这是一种简单而有效的算法。我们表明，通过对隐式奖励进行校准，以确保学习到的隐式奖励与真实奖励在规模上可比较，可以仅通过这种方式实现显著的对齐改进。我们展示了Cal-DPO相较于现有方法的理论优势。实验结果表明，Cal-DPO在多种标准基准上的表现显著优于现成方法。 

---
# GraphEQA: Using 3D Semantic Scene Graphs for Real-time Embodied Question Answering 

**Title (ZH)**: GraphEQA：使用3D语义场景图进行实时嵌入式问题回答 
...
**Abstract (ZH)**: 在具身问答（EQA）中，代理必须探索并发展对未见过环境的语义理解，以便自信地回答情境化问题。由于获取有用的语义表示、在线更新这些表示以及利用先验世界知识进行高效探索和规划的困难，这一问题在机器人学中仍具有挑战性。为了解决这些限制，我们提出了一种名为GraphEQA的新方法，该方法利用实时的3D度量语义场景图（3DSGs）和任务相关的图像作为多模态记忆，将视觉-语言模型（VLMs）接地，以在未见过的环境中执行EQA任务。我们采用了一种分层规划方法，利用3DSGs的分层特性进行结构化规划和语义引导探索。通过在HM-EQA数据集上的模拟实验以及在家庭和办公室环境中的实际世界实验，我们证明我们的方法在完成EQA任务的成功率和规划步骤方面优于关键基线方法。 

---
# MegaPairs: Massive Data Synthesis For Universal Multimodal Retrieval 

**Title (ZH)**: MegaPairs：大规模数据合成以实现通用多模态检索 
...
**Abstract (ZH)**: 尽管多模态检索的需求快速增长，但该领域的进展仍受限于缺乏训练数据。本文介绍了一种创新的数据合成方法MegaPairs，该方法结合了视觉语言模型（VLMs）和开放式领域图像，并生成了大量的合成数据集。我们的实证分析表明，MegaPairs生成了高质量的数据，使多模态检索模型在与现有数据集相比数据量增加70倍的基础上的基线模型显著超越。此外，由于MegaPairs仅依赖于通用图像库和开源的VLMs，它可以轻易扩展，从而实现检索性能的持续提升。目前，我们已生成了超过2600万训练实例，并利用这些数据训练了多个不同规模的模型。这些新模型在4个流行组合图像检索（CIR）基准测试中实现了最先进的零样本性能，并在MMEB提供的36个数据集中取得了最高总体性能。此外，这些模型在下游微调后也显示出显著的性能提升。我们生成的数据集、训练好的模型以及数据合成管道将公开发布，以促进该领域未来的发展。 

---
# Are Longer Prompts Always Better? Prompt Selection in Large Language Models for Recommendation Systems 

**Title (ZH)**: 更长的提示是否总是更好的选择？大型语言模型在推荐系统中的提示选择研究 
...
**Abstract (ZH)**: 在基于大规模语言模型（LLM）的推荐系统（LLM-RS）中，通过利用LLM的通用知识准确预测用户偏好是可能的，而无需大量的训练数据。通过将推荐任务转换为称为提示的自然语言输入，LLM-RS能够有效地解决由于数据稀缺而难以解决但对诸如冷启动和跨域问题等应用程序至关重要的问题。然而，在实际应用中，选择与任务和数据匹配的提示至关重要。尽管在LLM-RS中提出了许多提示，而这些提示在推荐准确性上对目标用户的表示有显著影响，但仍没有明确的指导方针来选择特定的提示。

在本文中，我们对先前研究中的提示进行了分类和分析，以建立实用的提示选择准则。通过使用90个提示和五个实际数据集进行450次实验，我们探讨了提示与推荐准确性之间的关系以及数据集特征。我们发现没有一种提示能够始终优于其他提示，因此根据数据集特征选择提示至关重要。在此基础上，我们提出了一个基于最少验证数据实现更高准确性的提示选择方法。由于增加提示的数量以进行探索会增加成本，我们还引入了一种成本效益高的策略，使用高性能和成本效益高的LLM，这显著降低了探索成本并保持了高预测准确性。我们的工作为提示选择提供了宝贵的见解，推动了准确高效的LLM-RS的发展。 

---
# In-Group Love, Out-Group Hate: A Framework to Measure Affective Polarization via Contentious Online Discussions 

**Title (ZH)**: 自我群体的关爱，他者群体的憎恶：一种通过争议性在线讨论衡量情感极化的框架 
...
**Abstract (ZH)**: 情绪极化是指意识形态群体间的感情鸿沟，表现为对内群体的热爱和对外群体的憎恨，在美国日益加剧，导致了疫情期间如戴口罩和封锁等争议性问题的激化。尽管情绪极化对社会产生了深远影响，但现有的意见转变模型未能考虑到情绪动态，也没有提供有效的方法来实时且稳健地量化情绪极化程度。本文提出了一种离散选择模型来捕捉情绪极化社会网络中的决策过程，并提出了一种统计推断方法，从社交媒体数据中估计关键参数——对内群体的热爱和对外群体的憎恨。通过针对COVID-19疫情期间在线讨论的实证验证，我们证明了该方法能够准确捕捉实际世界中的情绪极化动态，并解释了在戴口罩和封锁态度方面政党差距的快速出现。该框架可用于跟踪争议性问题上的情绪极化，具有广泛影响，可促进数字空间中的建设性在线对话。 

---
# ResQ: Mixed-Precision Quantization of Large Language Models with Low-Rank Residuals 

**Title (ZH)**: ResQ：具有低秩残留的大型语言模型混合精度量化 
...
**Abstract (ZH)**: 在推断时降低大型语言模型（LLMs）的计算成本具有重要意义。对所有权重、激活和键值（KV）缓存张量进行4比特量化，同时不显著降低其泛化能力是极具挑战性的，这是由于激活值中的极端异常值引起的量化误差。为了解决这一问题，我们提出了一种名为ResQ的后训练量化方法，进一步推动了最先进的技术水平。通过主成分分析（PCA），它识别出一个低秩子空间（实际上约为隐藏维度的1/8），在这个子空间中激活值的方差最高，并保持该子空间内的系数在高精度下，例如8比特，而将其余部分量化为4比特。在每个子空间内应用不变的随机旋转，以进一步抑制异常值。我们证明这是一种可以证明是最佳的混合精度量化方案，能够最小化误差。在Llama模型家族上，我们展示了ResQ在多种基准测试中优于最近的统一精度和混合精度的后训练量化方法，相对于SpinQuant方法在Wikitext上的困惑度降低了33%，并在16比特基准之上实现了2.4倍的速度提升。相关代码可在以下链接获取：[提供链接] 

---
# Towards AI-$45^{\circ}$ Law: A Roadmap to Trustworthy AGI 

**Title (ZH)**: 向AI-45°定律迈进：一条通往可信任的AGI的道路 
...
**Abstract (ZH)**: 确保通用人工智能（AGI）可靠地避免有害行为是一个关键性的挑战，特别是在具有高度自主性的系统或关键安全领域。尽管提出了多种安全保证方案和极端风险警告，但关于在安全性和能力之间取得平衡的全面指导原则依然缺乏。在本文中，我们提出了“AI-45°定律”作为一条指导性原则，以平衡可信AGI的发展路径，并介绍了“可信AGI因果梯”作为一项实用框架。该框架基于Judea Pearl的“因果梯”思想，为当前的AI能力和安全性研究提供了一套系统分类和层级结构。该因果梯包括三个核心层次：近似对齐层、干预层和反思层。这些层次解决了AGI和当代AI系统中安全性和可信性方面的主要挑战。在此基础上，我们定义了五级可信AGI：感知可信性、推理可信性、决策可信性、自主可信性和协作可信性。这些层次代表了可信AGI的独特且递进的各个方面。最后，我们提出了若干潜在的治理措施，以支持可信AGI的发展。请注意，本文中通常认为“可信性”是一种广泛意义上的安全性，二者之间没有明确区分。但在某些情境下，安全性指的是系统行为的正确性保证，而可信性则是用户对系统决策的信心。在这种情况下，根据上下文的不同，可能会使用不同的术语或两者兼用。 

---
# Whisper-GPT: A Hybrid Representation Audio Large Language Model 

**Title (ZH)**: 声闻-GPT：一种混合表示音频大型语言模型 
...
**Abstract (ZH)**: 我们提出WHISPER-GPT：一种结合连续音频表示和离散声学token的生成型大型语言模型（LLM），使我们能够在单一架构中同时处理连续的音频表示和离散的token。生成型音频、语音和音乐模型在利用来自神经压缩算法（如ENCODEC）的离散音频token方面取得了巨大的进展。然而，这种方法的主要缺点之一是处理上下文长度，对于高保真度的生成架构来说，如果需要考虑所有不同频率的音频内容来预测下一个token，则上下文长度会急剧增加。通过结合连续的音频表示（如频谱图）和离散的声学token，我们兼顾了两者的优点：在单个token中包含了特定时刻所需的所有音频信息，同时允许LLM预测未来的token，从而充分利用离散token空间带来的采样及其他好处。我们展示了我们的架构在预测下一个token时相比基于token的语音和音乐LLM，如何提高了困惑度和负对数似然度评分。 
