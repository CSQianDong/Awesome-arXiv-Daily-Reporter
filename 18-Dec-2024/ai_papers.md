...
**Abstract (ZH)**: 人工智能能否经历意识？对这一问题的回答应遵循证据主义原则——即，不应基于直观、教条或猜测，而应基于坚实的科学证据。我认为，支持这种观点的证据很难获得，因此在人工意识的前景上保持怀疑主义是最可辩护的立场。当前的辩论主要分为两类观点：生物观点对此持怀疑态度，而功能观点对此表示同情。我认为，这两种观点都犯了夸大现有证据所告知我们的内容的错误。意识的科学认知是通过研究有意识的生物取得的。尽管这使得对各种生物的意识状态进行谨慎评估成为可能，但在应用于人工智能方面面临重大障碍。因此，人工意识研究者面临着一个两难境地：要么预测人工意识的可能性，违背证证据主义；要么尊重证证据主义，而不对人工意识的可能性作出任何判定。文献中的主流趋势是采取前者，声称遵循科学证据。我认为，如果真正遵循证据，我们就必须采取后者，坚持怀疑主义。 
...
**Abstract (ZH)**: 我们研究了一个具有外部性的贝叶斯劝服问题。在这个模型中，主要决策者向多个代理发送信号以告知他们关于世界状态的信息。同时，由于代理效用中的外部性，主要决策者还充当了一个相关性设备的角色，以使代理人的行为相互关联。我们考虑代理被分类为少数几种类型的情况。相同类型的代理具有相同的效用函数，并且在其他代理和主要决策者的效用函数中受到公平对待。我们研究了主要决策者在三种不同类型的信号通道下计算最优信号策略的问题：公共通道、私人通道和半私人通道。我们的研究结果包括揭示原则样式的最佳信号策略表征、线性规划公式化表示，以及对优化问题难易性的分析。研究表明，当偏离代理的最大数量被限制在一个常数时，我们的基于线性规划的表示法可以在多项式时间内计算出最优的信号策略。否则，这些问题将是NP难的。 
...
**Abstract (ZH)**: 本文使现实世界中的类人机器人能够在执行表达性动作时保持稳定，这些动作与人类相似。我们提出了一种通用的全身跟踪框架ExBody2，该框架能够接受任何参考运动输入，并控制类人机器人模仿该运动。该模型在模拟环境中通过强化学习进行训练，然后转移到现实世界中。ExBody2将关键点跟踪与速度控制分离，并有效地利用特权教师策略提炼出精确的模仿技能，从而能够高保真地复制诸如跑步、蹲伏、舞蹈和其他具有挑战性的动态动作。我们在论文中对关键设计因素进行了详尽的定性与定量分析，并在两个类人平台上进行了实验，展示了我们方法相较于现有最先进的方法的优势，为类人机器人全身控制的极限提供了实用指南。 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.13184)  
...
**Title (ZH)**: ORFormer：鲁棒性自注意力模型用于准确的面部特征点检测 
...
**Abstract (ZH)**: 当前时间序列预测领域的景观主要由基于变换器（Transformer）的模型主导。这些模型参数量庞大，对计算资源的需求也相应增加，这在实际部署中构成了挑战，尤其是在商业和科学应用中的低功耗嵌入式设备上。剪枝是一种已验证的方法，可以减少神经网络的参数数量并节省计算资源。然而，对于基于变换器的时间序列预测模型，剪枝的影响和益处仍然知之甚少。为了填补这一空白，我们通过评估结构化和非结构化剪枝在各种最新的多变量时间序列模型上的性能，提供了一组比较基准研究。我们探讨了这些剪枝策略对模型预测性能以及计算方面（如模型大小、操作数和推理时间）的影响。

我们的结果显示，某些模型甚至可以在极高的稀疏度下被剪枝，其性能优于全连接模型。然而，对剪枝后的模型进行微调是必要的。此外，我们还表明，即使有相应的硬件和软件支持，结构化剪枝也无法提供显著的时间节省。 

---
# RAG-Star: Enhancing Deliberative Reasoning with Retrieval Augmented Verification and Refinement 

**Title (ZH)**: RAG-Star：通过检索增强验证与精炼提升审慎推理 
...
**Abstract (ZH)**: 中国拼写检查（CSC）任务的一个关键特征是错误的字符通常在音韵或图形上与正确的字符非常接近。为适应这一特征，以往的工作通常依赖于混淆集，但这种方法存在两个问题：难以确定哪些字符对应该包含在内，以及缺乏区分集内项目的概率。在本文中，我们提出了一种轻量级的即插即用DISC模块（即解码干预相似性模块），用于CSC任务。该模块在推断阶段仅通过测量字符之间的音韵和图形相似性来整合相似性信息。该方法可以轻松集成到诸如ReaLiSe、SCOPE和ReLM等现有的各种CSC模型中，而无需额外的训练成本。在三个CSC基准测试中的实验表明，我们提出的方法显著提升了模型性能，甚至超过了当前最先进的模型。 
...
**Abstract (ZH)**: 具有固有动态特性和事件驱动性质的脉冲神经网络（SNNs）能够自然地利用嵌入的时间序列作为时间步长来处理时间信息。最近的研究采用这种方法证明了SNNs在语音命令识别方面的有效性，通过使用较长的时间步长来处理长时间序列，能够取得高性能。然而，较大的时间步长增加了边缘计算应用的部署负担。因此，在边缘设备中检测时间模式时，平衡高性能和低能耗显得尤为重要。我们的解决方案包括两个关键组件：
...
我们在三个基准数据集上评估了我们的方法：Heidelberg脉冲数据集（SHD）、脉冲语音命令数据集（SSC）和Google语音命令数据集V2（GSC）。实验结果表明，SpikeSCR在这些数据集上与相同时间步长的当前最先进的（SOTA）方法相比，具有更好的性能。此外，通过执行KDCL，我们减少了60%的时间步长数量，并通过减少54.8%的能量消耗，同时保持与最近SOTA结果相当的性能。因此，这项工作为解决边缘神经形态计算系统中长时间序列的时间处理挑战提供了宝贵见解。 

---
# Boosting Fine-Grained Visual Anomaly Detection with Coarse-Knowledge-Aware Adversarial Learning 

**Title (ZH)**: 基于粗知识意识对抗学习的细粒度视觉异常检测增强方法 

**Authors**: Qingqing Fang, Qinliang Su, Wenxi Lv, Wenchao Xu, Jianxing Yu  

**Link**: [PDF](https://arxiv.org/pdf/2412.12850)  
...
**Abstract (ZH)**: 许多无监督视觉异常检测方法通过训练自编码器重构正常样本，然后利用重构误差图来检测和定位异常。然而，由于神经网络的强大建模能力和泛化能力，一些异常也可能被良好地重构，导致检测和定位准确度不佳。在本文中，首先收集了一个小规模的粗略标注的异常数据集。然后，开发了一种粗知觉知识感知的对抗学习方法，以使重构特征的分布与正常特征的分布对齐。这种对齐能够有效抑制自编码器在异常上的重构能力，从而提高检测准确度。鉴于异常通常仅占据异常图像中的极小区域，进一步开发了一种基于补丁的对抗学习策略。尽管没有补丁级别的异常信息，但我们通过严格证明，只要将任何补丁特征视作异常，提出的知识感知方法也能使重构补丁特征的分布与正常特征分布对齐。实验结果表明，在四个医学数据集和两个工业数据集上，本方法能够在提高检测和定位性能方面表现出有效性。 
...
**Title (ZH)**: 基于 Spike 驱动的轻量级 transformer 网络的高效事件语义分割 

**Authors**: Xiaxin Zhu, Fangming Guo, Xianlei Long, Qingyi Gu, Chao Chen, Fuqiang Gu  

**Link**: [PDF](https://arxiv.org/pdf/2412.12843)  
...
**Abstract (ZH)**: 推荐系统在塑造用户行为和决策方面的作用越来越大，这突显了它们在各个领域的日益重要影响。同时，推荐系统中广泛采用的机器学习模型引发了对用户隐私和安全的重大担忧。随着遵守隐私法规变得越来越重要，消除推荐记忆的需求显得尤为重要，即从已学习的推荐模型中消除特定训练数据的记忆。尽管这一问题至关重要，但传统的机器遗忘方法由于协作交互和模型参数的独特挑战，不适用于推荐系统的遗忘处理。本文综述了推荐遗忘领域的最新进展，探讨了这一新兴领域的设计原则、挑战和方法论。我们提供了一个统一的分类体系，对不同的推荐遗忘方法进行了分类，并总结了常用的基准和评估指标。通过回顾当前的研究状态，本文旨在引导更高效、更具扩展性和更稳健的推荐遗忘技术的发展。此外，我们还指出了该领域存在的开放性研究问题，这可能会为推荐遗忘以及机器学习应用中不同退隐任务的未来创新铺平道路。 

---
# DSGram: Dynamic Weighting Sub-Metrics for Grammatical Error Correction in the Era of Large Language Models 

**Title (ZH)**: DSGram：大型语言模型时代基于动态加权子度量的语法错误修正 
...
**Abstract (ZH)**: 评估语法错误修正（GEC）模型的性能已变得越来越具有挑战性，因为基于大规模语言模型（LLM）的GEC系统常常会生成与所提供的黄金标准参考有所偏离的修正。这种差异削弱了传统基于参考的评估指标的可靠性。在这项研究中，我们提出了一种新的GEC模型评估框架——DSGram，该框架结合了语义连贯性、编辑级别和流畅性，并利用了动态加权机制。我们的框架利用层次分析过程（AHP）与大规模语言模型相结合，确定不同评估指标的相对重要性。此外，我们还开发了一个包含人类标注和LLM模拟句子的数据集，以验证我们的算法并微调更具成本效益的模型。实验结果表明，我们提出的方法提升了GEC模型评估的有效性。 
...
**Title (ZH)**: 打破编程语言障碍：多语言提示赋能非英语母语学习者 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12800)  
...
**Abstract (ZH)**: 非英语母语使用者（NNES）在学习编程时面临着多重障碍。这些障碍可能是显而易见的，例如编程语言的语法和指令通常使用英语；也可能是更为微妙的，例如在全是英语母语者的课堂中不敢寻求帮助。然而，这些障碍令人沮丧，因为许多NNES学生实际上比他们能够用英语表达的要了解更多的编程知识。生成式人工智能（GenAI）的最新进展有望打破这些障碍，因为最先进的模型能够支持多语言交互。此外，近期的研究表明，GenAI在代码生成和解释方面具有极高的准确性。在这篇论文中，我们首次探讨了NNES学生使用其母语（阿拉伯语、中文和葡萄牙语）生成代码以解决编程问题的情况。我们的结果显示，学生能够成功地使用母语解决编程问题，但他们在指定编程术语和概念时仍面临一些困难。我们讨论了他们所面临的挑战，短期实践的影响，以及这一发展在未来可能如何在全球范围内改变计算机教育。 

---
# RCTrans: Radar-Camera Transformer via Radar Densifier and Sequential Decoder for 3D Object Detection 

**Title (ZH)**: RCTRANS：通过雷达密集模块和序列解码器的雷达-摄像头变换器用于三维目标检测 

**Authors**: Yiheng Li, Yang Yang, Zhen Lei  

**Link**: [PDF](https://arxiv.org/pdf/2412.12799)  
...
**Title (ZH)**: 揭示合成本地样本和多任务策略对印地语-英语代码混合幽默和讽刺检测的影响 
...
**Abstract (ZH)**: 在本文中，我们报告了我们对提高代码混合幽默与讽刺检测的各种策略进行的实验。由于我们具备印度语-英语代码混合场景的语言学专业知识，所有实验均在该场景下进行。我们尝试了三种方法：（i）本地样本混合，（ii）多任务学习（MTL），以及（iii）提示大规模多语言语言模型（VMLMs）。在本地样本混合中，我们在代码混合的训练集中添加了单语言任务样本。在MTL学习中，我们依赖与代码混合任务在语义上相关的本地和代码混合样本进行训练（在我们的案例中为仇恨言论检测）。最后，在我们的第三种方法中，我们通过少量样本的上下文提示评估了VMLMs的效果。我们的一些有趣发现包括：（i）添加本地样本提高了幽默检测（F1分数提高6.76%）和讽刺检测（F1分数提高8.64%），（ii）在MTL框架下训练语言模型提高了幽默检测（F1分数提高10.67%）和讽刺检测（F1分数提高12.35%），以及（iii）提示VMLMs未能超越其他方法。最后，我们的消融研究和错误分析揭示了模型尚未改进的具体情况。我们提供了代码以确保可重现性。 

---
# Your Next State-of-the-Art Could Come from Another Domain: A Cross-Domain Analysis of Hierarchical Text Classification 

**Title (ZH)**: 你的下一个先进成果可能来自另一个领域：跨领域层级文本分类分析 
...
**Abstract (ZH)**: 具有层次标签的文本分类是自然语言处理中一个普遍且具有挑战性的任务。这方面的例子包括为病人记录分配ICD代码、将专利归类到IPC类别、为欧洲法律文本分配EUROVOC描述词等。尽管此任务应用广泛，但不同领域的最先进的方法仍未进行全面的理解。在本文中，我们首次提供了不同领域最先进的方法的全面综述，并进行了实证分析。我们提出了一种统框架，将每种方法置于一个共同结构中，以便于研究。我们的实证分析得到了关键洞见和指导方针，证实了跨不同研究领域学习的必要性，以设计有效的方法。特别地，通过在统一的评估管道下应用超出其原始应用领域的技术，我们达到了新的先进成果。 

---
# Subspace Implicit Neural Representations for Real-Time Cardiac Cine MR Imaging 

**Title (ZH)**: 实时心脏 cine MRI 成像的子空间隐式神经表示方法 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12742)  
...
**Abstract (ZH)**: 常用的强化学习（RL）模型，马尔可夫决策过程（MDPs），其基本前提在于奖励仅依赖于当前状态和动作。然而，许多实际任务是非马尔可夫的，具有长期记忆和依赖性。在非马尔可夫场景中，稀疏奖励的问题进一步加剧。因此，学习非马尔可夫任务（NMT）相比学习马尔可夫任务来说更加困难。本文提出了一种新的**并行**和**模块化**的RL框架，ParMod，专门用于学习由时序逻辑指定的NMT。借助形式化技术，NMT被模块化为一系列基于自动机结构（相当于其时序逻辑的对应物）的子任务。在此基础上，通过一组智能体并行训练这些子任务，每个多智能体负责一个子任务。除了并行训练，ParMod的核心在于：一种灵活的分类方法，用于模块化NMT，以及一种有效的奖励塑造方法，以提高样本效率。进行了多项具有挑战性的基准问题评估，涵盖了多个评估指标。实验结果表明，ParMod在多个相关研究中表现更优。因此，我们的工作为RL、NMT和时序逻辑之间的良好协同提供了保障。 

---
# SPHERE: A Hierarchical Evaluation on Spatial Perception and Reasoning for Vision-Language Models 

**Title (ZH)**: SPHERE：空间感知与推理的分层评估方法在视觉-语言模型中的应用 

**Authors**: Wenyu Zhang, Wei En Ng, Lixin Ma, Yuwen Wang, Jungqi Zhao, Boyang Li, Lu Wang  

**Link**: [PDF](https://arxiv.org/pdf/2412.12693)  
...
**Abstract (ZH)**: 当前的多模态模型可能仅整合单一维度的空间线索，例如深度、物体边界和基本的空间方向（如左、右、前、后），但往往缺乏人类理解和现实应用所需的空间多维度推理能力。为解决这一问题，我们开发了SPHERE（Spatial Perception and Hierarchical Evaluation of REasoning），这是一种分层评估框架，并结合了一个新的由人类注释的数据集，从单一技能任务逐步发展到多技能任务，最终达到需要整合多种空间和视觉线索并结合逻辑推理的复杂推理任务。最先进的开源模型的基准评估揭示了其显著的不足之处，尤其是在理解距离和亲密性、从自参考和视角中心视角进行推理以及在物理环境中进行复杂推理方面的能力。本研究强调了需要更高级的空间理解和推理方法的重要性，为多模态模型的改进及其与人类空间能力的对齐铺平了道路。该数据集将在发表后开源。 

---
# Everyday AR through AI-in-the-Loop 

**Title (ZH)**: 通过AI协同的日常AR技术 

**Authors**: Ryo Suzuki, Mar Gonzalez-Franco, Misha Sra, David Lindlbauer  

**Link**: [PDF](https://arxiv.org/pdf/2412.12681)  
...
**Title (ZH)**: ClustEm4Ano: 命名文本属性文本嵌入的聚类方法在微观数据匿名化中的应用 
...
**Abstract (ZH)**: 这项工作介绍了ClustEm4Ano，这是一种用于名义文本表格数据的一般化和抑制匿名化的匿名化管道。该管道能够自动生成值一般化层次结构（VGHs），进而可用于泛化准标识符中的属性。该管道利用嵌入通过迭代聚类生成语义上相近的值一般化。我们使用KMeans和层次凝聚聚类在13种不同的预定义文本嵌入（包括开源和 closesource）上进行了应用。我们的方法在匿名化领域的基准数据集——UCI机器学习库的Adult数据集上进行了实验验证。ClustEm4Ano通过提供比任意选择的VGH更多的可能性来支持匿名化过程。实验结果表明，这些VGH在下游效用方面（特别是对于小k-匿名性，2≤k≤30）比手动构建的VGH更优，从而可以提升匿名数据集的质量。我们的实现已经公开发布。 

---
# Exploring AI-Enabled Cybersecurity Frameworks: Deep-Learning Techniques, GPU Support, and Future Enhancements 

**Title (ZH)**: 探索AI赋能的网络安全框架：深度学习技术、GPU支持和未来增强 

**Authors**: Tobias Becher, Simon Torka  

**Link**: [PDF](https://arxiv.org/pdf/2412.12648)  
...
**Abstract (ZH)**: 传统的基于规则的网络安全系统在抵御已知恶意软件威胁方面 proven 高效。然而，它们在检测新型威胁时面临挑战。为解决这一问题，新兴的网络安全系统正在结合 AI 技术，特别是深度学习算法，以增强其事件检测、警报分析和事件响应的能力。虽然这些技术为应对动态安全威胁提供了有前途的方法，但也常常需要大量的计算资源。因此，结合 AI 网络安全机制的框架需要支持 GPU 的使用，以确保最佳性能。

许多网络安全框架提供商未能提供足够的实施细节，使得评估所采用的技术及其效果变得困难。本研究旨在通过概述利用 AI 技术的最常用网络安全框架，克服这一限制，并具体关注那些提供全面实施信息的框架。我们的主要目标是确定这些框架所采用的深度学习技术，并评估其对 GPU 加速的支持情况。我们发现，在所选择的 38 个网络安全框架中，共有三种框架使用了总计两种深度学习算法。本研究的发现旨在帮助未来的研究选择开源网络安全框架，并评估理论和实践中使用的深度学习技术之间的差异。 

---
# RDPI: A Refine Diffusion Probability Generation Method for Spatiotemporal Data Imputation 

**Title (ZH)**: RDPI：一种时空数据插补的精炼扩散概率生成方法 

**Authors**: Zijin Liu, Xiang Zhao, You Song  

**Link**: [PDF](https://arxiv.org/pdf/2412.12642)  
...
**Abstract (ZH)**: 时空数据插值在交通流量监测、空气质量评估和气候预测等多个领域中发挥着重要作用。然而，由传感器收集的时空数据经常存在时间上的不完整性，传感器的稀疏且不均匀分布导致在空间维度上存在数据缺失问题。在现有方法中，自回归方法容易出现误差累积，而简单的条件扩散模型则难以充分捕捉观测数据和缺失数据之间的时空关系。为了解决这些问题，我们提出了一种基于初始网络和条件扩散模型的双重阶段精炼扩散概率插补（RDPI）框架。在初始阶段，使用确定性插补方法生成缺失数据的初步估计。在精炼阶段，将残差视为扩散目标，并创新地将观测值纳入前向过程。这种条件扩散模型更适合时空数据插补，能够更好地弥合初步估计与真实值之间的差距。在多个数据集上的实验表明，RDPI不仅实现了最先进的插补精度，而且显著降低了采样计算成本。 

---
# Lagrangian Index Policy for Restless Bandits with Average Reward 

**Title (ZH)**: 基于拉格朗日索引策略的平均奖励非稳态多臂老虎机问题 

**Authors**: Konstantin Avrachenkov, Vivek S. Borkar, Pratik Shah  

**Link**: [PDF](https://arxiv.org/pdf/2412.12641)  
...
**Abstract (ZH)**: 我们研究了长期平均奖励条件下的动量索引策略（LIP）在非平稳多臂老虎机中的应用。特别地，我们将LIP的性能与Whittle索引策略（WIP）的性能进行了比较，后者在某些自然条件下被认为是渐近最优的启发式策略。尽管在大多数情况下它们的性能非常相似，但在WIP表现出不佳性能的情况下，LIP仍然表现出色。随后，我们提出了基于强化学习的算法，包括基于表的和基于神经网络的方法，用于在无模型设置下在线学习LIP策略。提出的用于LIP的强化学习方案所需内存显著少于WIP的相应方案。此外，我们通过计算重启动模型中的拉格朗日索引，分析了该模型下的最优网页抓取和加权信息年龄最小化。最后，基于可交换性和de Finetti定理，我们给出了在臂的数量趋向无穷大时同质多臂老虎机渐近最优的新证明。 

---
# Falcon: Faster and Parallel Inference of Large Language Models through Enhanced Semi-Autoregressive Drafting and Custom-Designed Decoding Tree 

**Title (ZH)**: Falcon：通过增强半自回归草稿写作和定制设计的解码树实现大型语言模型的更快并行推理 
...
**Abstract (ZH)**: 在推测解码中，如何在最小化编写延迟和保持高推测准确性之间取得最佳平衡，以提高大型语言模型的推理速度，仍然是一个重大的挑战。本文介绍了Falcon——一种创新的半自回归推测解码框架，旨在增强编写者的并行性和输出质量。Falcon引入了耦合序列光顾蒸馏技术，增强了同一块内的词间依赖性，从而提高了推测的准确性。我们提供了全面的理论分析来揭示其背后的机制。此外，提出了一种定制设计的解码树，允许编写者在单次前向传递中生成多个词，并按需支持多次前向传递，从而增加生成的词数量，显著提高整体接受率。在包括MT-Bench、HumanEval和GSM8K等基准数据集上的全面评估表明，Falcon具有出色的加速能力。在Vicuna和LLaMA2-Chat模型系列上的测试结果显示，Falcon实现了无损加速比在2.91倍到3.51倍之间，而保持了一个紧凑的编写者架构，仅相当于两个Transformer层。这些结果优于现有的LLM推测解码方法，包括Eagle、Medusa、Lookahead、SPS和PLD。 

---
# TrainMover: Efficient ML Training Live Migration with No Memory Overhead 

**Title (ZH)**: TrainMover: 无需内存开销的高效机器学习训练在线迁移技术 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12636)  
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12587)  
...
**Abstract (ZH)**: 参考多目标跟踪（RMOT）是跨模态领域的一个新兴任务，旨在根据语言表达定位任意数量的目标，并在视频中连续跟踪这些目标。这个复杂的任务涉及多模态数据的推理以及基于时间关联的精确目标定位。然而，由于任务本身的性质，先前的研究忽视了新生目标与现有目标之间的数据分布不平衡问题。此外，他们仅间接融合多模态特征，难以为新生目标检测提供明确的指导。为了解决上述问题，我们实施了一种合作配对策略，减轻不平衡问题的影响，增强新生目标检测能力，同时保持跟踪性能。在编码器中，我们将多模态和多尺度融合整合并增强，克服了先前工作中存在的瓶颈，即特征图之间的多模态信息共享和交互有限。在解码器中，我们还开发了一种参考信息融合适应机制，通过查询令牌提供明确的参考指导。实验结果表明，与先前的工作相比，我们的模型性能提高了3.42%，证明了我们设计的有效性。 
...
**Abstract (ZH)**: 我们介绍了一种名为EXIT的抽取式上下文压缩框架，该框架提升了检索增强生成（RAG）在问答（QA）任务中的效果与效率。当前的RAG系统在检索模型无法有效排序最相关的文档时，常常难以处理，导致为了提高准确度不得不增加上下文，而这也带来了延迟增加的问题。尽管抽象压缩方法可以大幅减少令牌数量，但其逐步生成的过程会导致端到端延迟显著增加。相比之下，现有的抽取式方法可以减少延迟，但依赖于独立且非适应性的句子选择，未能充分利用上下文信息。EXIT通过分类检索到的文档中的句子（同时保持其上下文依赖性），实现了并行进行、上下文感知的抽取，可以根据查询的复杂性和检索质量进行调整。我们的评估结果显示，在单跳和多跳问答任务中，EXIT在问答准确性、推断时间和令牌数量方面均超越了现有的压缩方法和未压缩的基线。通过同时提高效果和效率，EXIT为开发可扩展且高质量的RAG问答解决方案提供了新的方向。我们的代码可在此链接访问：[请提供具体的URL链接] 

---
# SAModified: A Foundation Model-Based Zero-Shot Approach for Refining Noisy Land-Use Land-Cover Maps 

**Title (ZH)**: SAModified：一种基于基础模型的零样本方法用于细化 noisy 土地利用覆被地图 

**Authors**: Sparsh Pekhale, Rakshith Sathish, Sathisha Basavaraju, Divya Sharma  

**Link**: [PDF](https://arxiv.org/pdf/2412.12552)  
...
**Abstract (ZH)**: 尽管大规模语言模型（LLMs）在特定的自然语言处理（NLP）任务中展现了令人瞩目的能力，但在某些领域，如语法错误修正（GEC）方面，它们可能仍不如专门模型熟练。受课程学习概念的启发，我们深入探讨了如何通过有效的课程学习（CL）策略将LLMs培育成精通GEC的专业专家。在这篇文章中，我们介绍了一种新颖的方法——基于LLM的课程学习，该方法利用LLMs固有的强大语义理解和辨别能力来评估GEC训练数据的复杂性。与传统的课程学习方法不同，我们的方法更贴近人工专家设计的课程。利用提出的基于LLM的方法，我们按照从简单到复杂的顺序逐步选择不同难度级别的课程，并使用预训练的T5和LLaMA系列模型进行迭代训练和优化。通过在英语GEC的多种基准测试中进行严格的测试和分析，包括CoNLL14测试集、BEA19测试集和BEA19开发集，我们的方法展示了显著优于基线模型和传统课程学习方法的效果。 
...
**Abstract (ZH)**: 准确的医学图像分类模型的发展往往受到隐私问题和特定条件下数据稀缺性的限制，导致数据集规模小且不平衡。为解决这些限制，本研究探索了生成模型（如去噪扩散概率模型DDPM和渐进增长生成对抗网络PGGANs）在数据集扩充中的应用。研究引入了一个框架，评估DDPM和PGGAN生成的合成图像对四种模型（自定义CNN、未训练的VGG16、预训练的VGG16和预训练的ResNet50）性能的影响。实验使用随机采样和贪婪的K采样方法创建了小规模和不平衡的数据集。合成图像通过弗塞奇-印克距离（FID）进行评估，并与原始数据集通过分类指标进行比较。结果表明，DDPM始终生成更真实的图像，并且在所有模型和数据集中的分类指标上显著优于PGGANs。将DDPM生成的图像合并到原始数据集中，可提高准确率最高达6%，增强了模型的稳健性和稳定性，尤其是在不平衡的数据场景中表现尤为明显。随机采样在稳定性方面表现更优，而贪婪的K采样则在多样性上具有优势，但伴随更高的FID评分。本研究强调了DDPM在扩充小型不平衡医学图像数据集方面的有效性，通过平衡数据集并扩大其规模来提升模型性能。 
...
**Title (ZH)**: CREST：一种高效的时间驱动联合训练框架，用于利用时空动态进行事件驱动的目标检测 

**Authors**: Ruixin Mao, Aoyu Shen, Lin Tang, Jun Zhou  

**Link**: [PDF](https://arxiv.org/pdf/2412.12525)  
...
**Abstract (ZH)**: 基于事件的摄像机具有高时间分辨率、宽动态范围和低功耗的特点，使其非常适合高速和低光照条件下的物体检测。脉冲神经网络（SNN）因其脉冲特性在基于事件的物体识别和检测方面具有潜力，但缺乏有效的训练方法，从而导致梯度消失和高计算复杂度，特别是在深层SNN中。此外，现有的SNN框架往往不能有效处理时空尺度特征，导致数据冗余增加和检测准确性降低。为了解决这些问题，我们提出了一种新颖的联合训练脉冲驱动框架CREST（Conjointly Trained Spike-Driven Framework），以充分利用基于事件的物体检测中的时空动态特性。我们引入了联合学习规则以加速SNN的学习和缓解梯度消失问题。CREST还支持双操作模式，以便在不同硬件类型上高效灵活地实施。此外，CREST具备一个完全脉冲驱动框架，包括多尺度时空事件积分器（MESTOR）和时空交并比损失（ST-IoU loss）。我们的方法在三个数据集上实现了优越的对象识别与检测性能，并相比最先进的SNN算法在能效方面提高了高达100倍，提供了一种适用于SNN硬件实现的高效解决方案。 

---
# Solid-SQL: Enhanced Schema-linking based In-context Learning for Robust Text-to-SQL 

**Title (ZH)**: Solid-SQL：基于增强模式链接的上下文学习方法以提高稳健的文本到SQL转换能力 
...
**Title (ZH)**: 超越数据量：多语言语言模型性能的关键驱动因素 
...
**Abstract (ZH)**: 多语言语言模型（MLLMs）对于处理多种语言的文本至关重要，但由于资源可用性和语言特征的差异，它们通常表现出性能差异。虽然预训练数据比例和模型大小对性能的影响广为人知，但我们的研究表明，还有其他重要因素显著影响MLLM的有效性。我们通过分析地理、语言和资源相关方面的广泛特征，利用回归模型和SHAP值对204种语言进行了分析，首先使用SIB-200数据集进行分类，然后使用Flores-200数据集进行机器翻译。我们的研究发现，标记相似性和国家相似性是提升模型性能的关键因素，与预训练数据和模型大小一样重要。标记相似性促进了跨语言迁移，而国家相似性强调了共享文化与语言背景的重要性。这些见解为开发更具公平性和有效性的多语言语言模型，特别是为未充分代表的语言提供了宝贵指导。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）通过广泛预训练的多语言语料库和微调指令数据已经展示了令人印象深刻的多语言理解和推理能力。然而，由于预训练语料库中的语言不平衡，使用更多低资源数据进行微调也无法完全解决高资源语言任务和低资源语言任务之间存在的性能差距。为了解决这一问题，我们提出了LinguaLIFT，一种旨在推进低资源语言任务的两阶段指令微调框架。首先，在LLM中集成一个额外的语言对齐层，以适应预训练的多语言编码器，通过代码混用微调增强多语言对齐。第二阶段使用仅限英语的指令数据微调LLM，同时冻结语言对齐层，使LLM能够从英语领域转移特定能力到低资源语言任务。此外，我们引入了多语言数学世界问题（MMWP）基准，涵盖21种低资源、17种中资源和10种高资源语言，从而使得多语言推理的全面评估成为可能。实验结果表明，LinguaLIFT在MMWP和其他广泛使用的基准上优于多个竞争基线。 

---
# Faster Vision Mamba is Rebuilt in Minutes via Merged Token Re-training 

**Title (ZH)**: 更快的Vision Mamba 通过合并_token_重新训练可在几分钟内重新构建 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12496)  
...
**Abstract (ZH)**: 许多计算机系统现在正被重新设计，以集成由大型语言模型（LLMs）驱动的代理，这使得自然语言输入和更灵活的操作成为可能。本文主要关注由大型语言模型生成的数据库事务。由LLMs生成的事务可能包含语义错误，因此需要系统将这些事务视为长期事务进行处理，以便进行人工审核，如果事务存在错误，还可以将其从数据库历史记录中移除。任何移除操作都必须在整个过程中确保数据库的一致性（ACID原则中的“C”）。

我们提出了一种基于不变性满足（I-Confluence）的新middleware框架，该框架通过识别和协调长期事务和新事务之间的依赖性来确保一致性。此middleware用于缓存可疑或补偿性事务，以管理协调状态。使用TPC-C基准测试，我们评估了事务生成频率、用户审核和不变性完整性对系统性能的影响。对于系统研究人员，本研究建立了一个LLMs与数据库系统之间的交互范式，提供了处理错误操作的同时保证数据库一致性的“撤销”机制。对于系统工程师，本文提供了一种middleware设计方案，该方案能够在对现有系统进行最小修改的情况下整合可移除的LLM生成事务。 
...
**Abstract (ZH)**: 处理长上下文是大型语言模型（LLMs）面临的重要挑战，原因在于它们固有的上下文窗口限制以及大量键值（KV）激活所导致的计算负担，这严重影响了效率。对于信息检索任务而言，全面感知整个上下文往往是不必要的，因为查询的信息需求可以因其复杂性不同而动态变化，从局部细节到全局视角不等。然而，现有方法难以有效地适应这些动态的信息需求。
...
**Abstract (ZH)**: 近年来，量子计算（QC）和机器学习（ML）的最新进展引发了广泛关注，推动了量子机器学习（QML）算法的发展，以应对各种复杂的挑战。然而，设计高性能的QML模型需要专家级别的知识，这成为QML广泛应用的一大障碍。主要挑战包括量子电路编码机制和参数化量子电路的设计，两者对QML模型的泛化能力至关重要。为了克服这些挑战，我们提出了一种新型方法，通过编码量子电路架构信息来促进量子电路设计的优化。在该方法中，适应度函数基于有效维度，使量子电路朝着更高的模型容量进行优化。通过数值仿真，我们证明了所提出的方法能够发现具有增强学习能力的变分量子电路架构，从而整体提升QML模型在复杂任务中的性能。 
...
**Title (ZH)**: 稀有疾病工作组：自主多学科团队在稀有疾病诊断与治疗中的应用 
...
**Abstract (ZH)**: 最优控制问题（Optimal Control Problems, OCPs）涉及寻找一个控制函数以优化动态系统的成本泛函。它们在学术界和工业界中的物理系统中占据核心地位。本文提出了一种新颖的实例-解控制运算符视角，能够以一次性的方式解决OCPs，而不直接依赖于动态系统的显式表达或迭代优化过程。该控制运算符通过一个名为神经自适应谱方法（Neural Adaptive Spectral Method, NASM）的新神经运算架构来实现，该架构是对经典谱方法的一种推广。我们通过展示NASM对控制运算符的逼近误差边界来理论上验证这种视角和架构。在合成环境和真实世界数据集上的实验验证了我们方法的有效性和效率，包括运行时间显著加快以及良好的分布内和分布外泛化能力。 

---
# Transferable and Forecastable User Targeting Foundation Model 

**Title (ZH)**: 可迁移且可预测的用户目标定位基础模型 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12468)  
...
**Abstract (ZH)**: 在数字和物理世界中，模式图像无处不在，对其进行编辑的工具非常宝贵。但是，编辑模式图像颇具挑战性：通常需要进行结构感知的编辑，即修改生成模式的底层程序。人们可能会尝试推断这一底层程序，但当前的方法在面对复杂图像时往往力不从心，并生成难以组织的程序，使得编辑过程繁琐。在本研究中，我们介绍了一种新颖的方法来进行模式图像的结构化编辑。通过使用模式类比——即用一对简单的模式来展示期望的编辑——以及基于学习的生成模型来执行这些编辑，我们的方法允许用户直观地编辑模式。为了实现这一范式，我们引入了SplitWeave，这是一种领域特定语言，结合合成模式类比的抽样框架，能够生成大量高质量的合成训练数据集。我们还介绍了TriFuser，这是一种.latent扩散模型（LDM），旨在解决直接将LDM应用于此任务时出现的关键问题。在针对实际来源艺术家提供的模式进行的广泛实验中，我们发现，我们的方法不仅能够准确执行演示的编辑，还能够泛化到与其训练分布相关的其他模式风格。 

---
# LITA: An Efficient LLM-assisted Iterative Topic Augmentation Framework 

**Title (ZH)**: LITA：一种高效的基于大型语言模型的迭代主题增强框架 
...
**Abstract (ZH)**: 主题建模在文本语料中广泛用于发现主题结构，但传统的模型在领域聚焦的应用中往往难以做到精准和连贯。引导式方法，如SeededLDA和CorEx，通过引入用户提供的种子词来提高相关性，但这些方法仍然劳动密集且缺乏灵活性。大型语言模型（LLMs）为动态主题重塑和发现提供了潜力，但其应用往往会产生较高的API成本。为了解决这些问题，我们提出了一种大型语言模型辅助的迭代主题增强框架（LITA），该框架将用户提供的种子词与基于嵌入的聚类和迭代优化相结合。LITA 识别少量模糊文档，并利用LLM 将其重新分配到现有主题或新主题中，从而在降低API成本的同时提高主题质量。实验结果表明，在主题质量和聚类性能指标方面，LITA 都优于包括LDA、SeededLDA、CorEx、BERTopic 和 PromptTopic 等五种基线模型。我们的工作提供了一种高效且可适应的主题建模和文本聚类框架。 

---
# Graph Learning in the Era of LLMs: A Survey from the Perspective of Data, Models, and Tasks 

**Title (ZH)**: 在大语言模型时代下的图学习：从数据、模型和任务视角的综述 
...
**Abstract (ZH)**: 随着跨域文本归因图（TAG）数据（例如引用网络、推荐系统、社交网络和ai4science）的普及，图神经网络（GNNs）和大型语言模型（LLMs）在统一模型架构中的集成（例如，LLMs作为增强器、LLMs作为合作者、LLMs作为预测器）已逐渐成为一个有前景的技术范式。这一新图学习范式的核心在于结合GNNs捕捉复杂结构关系的能力和LLMs从丰富的图文本描述中理解有意义上下文的能力。因此，我们可以通过利用具有丰富语义上下文的图描述文本从根本上提升数据质量，进而根据以数据为中心的机器学习原则增强模型中心方法的表示能力。通过利用这两种不同神经网络架构的优势，这种集成方法可以解决广泛的基于TAG的任务（例如，图学习、图推理和图问答），特别是在复杂的工业场景中（例如，监督场景、少样本场景和零样本场景）。换句话说，我们可以将文本作为一种媒介，使图学习模型在不同数据域中的下游图基任务上具有跨域通用性，使单一图模型能够有效地处理多样化的任务。本工作为希望在快速发展的LLM背景下推进图学习方法的研究人员和实践者提供了一个基础参考。我们在此持续维护相关的开源材料，链接为：\[this https URL\]。 
...
**Title (ZH)**: LazyDiT：懒学习在加速扩散变换器中的应用 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12444)  
...
**Abstract (ZH)**: 差分变换器已经成为了广泛生成任务的主导模型，展示了在各种应用中卓越的性能和效果。虽然表现出色，但这些模型在推理时速度较慢，因为在每一项去噪步骤中都需要运行整个具有大量参数的变压器模型。在本文中，我们证明了在每个差分步骤中执行完整的模型计算并非必要，通过懒惰地重用之前步骤的结果可以跳过某些计算。此外，我们还证明了连续步骤输出之间的相似度下限较高，并且这种相似度可以用输入进行线性近似。为了验证这些演示，我们提出了一种称为**LazyDiT** 的懒惰学习框架，该框架能够高效地利用较早步骤中的缓存结果来跳过冗余计算。具体来说，我们将在模型中嵌入懒惰学习层，这些层能够动态地跳过冗余计算，并且被有效训练以最大化懒惰性。实验结果表明，LazyDiT 在不同分辨率的多种差分变换器模型上的表现优于 DDIM 采样器。此外，我们还在移动设备上实现了该方法，能够在相似延迟的情况下获得更好的性能。 

---
# Numerical Pruning for Efficient Autoregressive Models 

**Title (ZH)**: 高效的自回归模型的数值剪枝方法 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12441)  
...
**Abstract (ZH)**: 大型语言模型（LLMs）在各种任务上表现出色，但在非英语语言，尤其是母语为非洲语言的国家，仍存在显著差异。本文通过在8种低资源非洲语言中创建约100万词的新基准数据，来解决这些差异问题，这些语言覆盖了超过1.6亿使用者群体：阿姆哈拉语、班巴拉语、伊戈伯语、塞佩迪语（北索托语）、绍纳语、塞索托语（南索托语）、塞茨瓦纳语和津巴朗加语。我们使用的基准测试数据包括Winogrande和MMLU的三个部分：医学院校、临床知识和病毒学。利用翻译后的基准测试数据，我们报告了现有最先进（State-of-the-Art，SOTA）LLMs在英语和非洲语言之间的 previously unknown 性能差距。最后，利用超过400个微调模型的结果，我们探索了几种减少LLM性能差距的方法，包括高质量数据集微调（使用LLM作为注释器）、跨语言迁移和文化恰当性调整。主要发现包括：通过微调获得的平均单语改进为5.6%，使用高质量数据比低质量数据时平均单语改进为5.4%；跨语言迁移平均获得2.9%的收益；文化恰当性问题上的即插即用性能提升为3.0%。本研究公开的基准测试数据、翻译和代码可支持进一步的研究和发展，以创建更加包容和有效的语言技术。 

---
# DeepSN: A Sheaf Neural Framework for Influence Maximization 

**Title (ZH)**: DeepSN：一种用于影响最大化的问题Sheaf神经网络框架 

**Authors**: Asela Hevapathige, Qing Wang, Ahad N. Zehmakan  

**Link**: [PDF](https://arxiv.org/pdf/2412.12416)  
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12395)  
...
**Abstract (ZH)**: 本项目的目标是分类四种不同的昆虫声音：蝉、金龟子、白蚁和蟋蟀。本项目的一个应用是通过监控和保护生态系统来进行害虫控制。本项目利用数据增强技术，包括音调移调和速度变换，以提高模型的泛化能力。本项目将测试决策树、随机森林、SVM RBF、XGBoost和k-近邻模型与MFCC特征的组合性能。本项目的一个潜在创新点是使用了多种数据增强技术，并在此基础上生成了6个数据集，同时包含原始声音。数据集包括这四种昆虫的声音录音。本项目旨在实现高分类准确率，并减少过拟合问题。 

---
# Enhancing Temporal Link Prediction with HierTKG: A Hierarchical Temporal Knowledge Graph Framework 

**Title (ZH)**: 使用HierTKG增强时间链接预测：一种层次时间知识图谱框架 

**Authors**: Mariam Almutairi, Melike Yildiz Aktas, Nawar Wali, Shutonu Mitra, Dawei Zhou  

**Link**: [PDF](https://arxiv.org/pdf/2412.12385)  
...
**Abstract (ZH)**: 社交媒体上虚假信息的快速传播，尤其是在危机期间，对公共决策构成了挑战。为应对这一问题，我们提出了HierTKG框架，该框架结合了时间图网络（TGN）和层次聚类（DiffPool），用于建模跨时间和结构尺度的谣言传播动态。HierTKG能够捕捉关键传播阶段，从而提高时间链接预测的准确性和对虚假信息控制的行动指引。实验结果表明其有效性，在ICEWS14数据集上MRR达到0.9845，在WikiData数据集上达到0.9312，且在如PHME这样的噪声数据集上（MRR：0.8802）也有竞争力的表现。通过建模结构化的事件序列和动态的社会互动，HierTKG能够适应多种传播模式，提供了一种可扩展且稳健的实时分析和预测谣言传播的解决方案，有助于主动干预策略的实施。 

---
# Scam Detection for Ethereum Smart Contracts: Leveraging Graph Representation Learning for Secure Blockchain 

**Title (ZH)**: 基于图表示学习的以太坊智能合约欺诈检测：实现安全区块链的技术 

**Authors**: Yihong Jin, Ze Yang  

**Link**: [PDF](https://arxiv.org/pdf/2412.12370)  
...
**Abstract (ZH)**: 在以太坊智能合约中检测欺诈行为是一项关键挑战，因为越来越多的不法分子利用智能合约进行欺诈活动，导致重大经济和声誉损害。现有检测方法通常依赖于合约代码分析或手工提取的特征，这些方法在可扩展性和适应性方面存在局限性。在本研究中，我们提出了一种创新的方法，利用图表示学习来分析交易模式并识别欺诈性合约。通过将以太坊交易数据转换为图结构并采用先进的机器学习模型，我们实现了稳健的分类性能。我们通过SMOTE-ENN技术解决标签不平衡问题，并评估了多层感知机（MLP）和图卷积网络（GCN）等模型。实验结果表明，在这种情况下，MLP模型优于GCN模型，实际应用与领域特定分析较为吻合。本研究为增强以太坊生态系统的信任和安全性提供了可扩展且有效的方法。 
...
**Title (ZH)**: RAG游乐场：一种评估RAG系统中检索策略和提示工程系统的体系化框架 
...
**Abstract (ZH)**: 人类将复杂的经验提炼为基本的抽象概念，从而实现快速学习和适应。同样地，自回归变压器通过上下文学习（ICL）表现出适应性学习的能力，这引发了关于其具体机制的思考。本文提出了一种概念编码-解码机制，通过研究变压器如何在其表征中形成和使用内部抽象来解释ICL。在合成的ICL任务中，我们分析了一个小型变压器的训练动力学，并报告了概念编码和解码的耦合涌现。随着模型学习将不同的潜在概念（例如，“在一个句子中找到第一个名词”）编码为独特的、可分离的表征，它同时构建了条件解码算法并且提高了其ICL性能。我们通过对各种规模的预训练模型（Gemma-2 2B/9B/27B、Llama-3.1 8B/70B）验证了这一机制的存在。进一步地，通过机制干预和受控微调，我们证明了概念编码的质量与其ICL性能之间存在因果关系和预测性。我们的实证见解有助于通过模型的表征更好地理解大语言模型的成功和失败模式。 

---
# OmniPrism: Learning Disentangled Visual Concept for Image Generation 

**Title (ZH)**: OmniPrism：学习解耦视觉概念以进行图像生成 

**Authors**: Yangyang Li, Daqing Liu, Wu Liu, Allen He, Xinchen Liu, Yongdong Zhang, Guoqing Jin  

**Link**: [PDF](https://arxiv.org/pdf/2412.12242)  
...
**Title (ZH)**: 您只需提交一张图像即可找到最适合的生成模型 

**Authors**: Zhi Zhou, Lan-Zhe Guo, Peng-Xiao Song, Yu-Feng Li  

**Link**: [PDF](https://arxiv.org/pdf/2412.12232)  
...
**Abstract (ZH)**: 我们考虑一类由带有最小值和最大值运算的线性方程系统定义的最优化问题。这类最优化问题在某些限制条件下已被研究过，具体包括以下条件：
(C1) 停止或稳定性条件；
(C2) 非负系数条件；
(C3) 系数之和为1的条件；
(C4) 仅包含最小值运算或最大值运算的条件。
文献中的一些开创性成果集中在特殊情况下。例如，轮换随机博弈对应于条件(C2)和(C3)，马尔可夫决策过程则对应于条件(C2)、(C3)和(C4)。然而，对所有情况的系统性计算复杂性研究尚未展开，这是我们工作的重点。我们的主要结果亮点包括：在满足条件(C2)和(C4)时，以及在满足条件(C3)和(C4)时，该问题为NP完全问题；仅满足条件(C1)时，该问题位于UP与coUP的交集中。最后，我们确立了验证各自条件的决策问题的计算复杂性。 

---
# EDformer: Embedded Decomposition Transformer for Interpretable Multivariate Time Series Predictions 

**Title (ZH)**: EDformer: 嵌入式分解变换器用于可解释的多变量时间序列预测 

**Authors**: Sanjay Chakraborty, Ibrahim Delibasoglu, Fredrik Heintz  

**Link**: [PDF](https://arxiv.org/pdf/2412.12227)  
...
**Abstract (ZH)**: 将时间序列编码为标记并使用语言模型进行处理已被证明能够显著增强模型在处理未见过任务时的泛化能力。然而，现有的时间序列预测语言模型遇到了一些障碍，包括混叠失真和较长的推理时间，主要是由于量化过程的限制和大型模型的计算需求。本文介绍了一种名为Apollo-Forecast的新框架，该框架通过两项关键技术解决了这些挑战：反混叠量化模块（AAQM）和赛程解码（RD）技术。AAQM巧妙地将序列编码为标记，同时减少原始信号中的高频噪声，从而提高了信号保真度并增强了整体量化效率。RD利用一个草稿模型实现并行处理和结果集成，显著加快了长期预测的推理速度，特别是在大型模型中。在各种真实世界的数据集上的广泛实验表明，Apollo-Forecast在零样本场景下的WQL和MASE指标中分别优于最先进的方法35.41%和18.99%。此外，我们的方法比基线方法的推理速度提高了1.9倍至2.7倍。 
...
**Abstract (ZH)**: 多模态情感分析（Multimodal Sentiment Analysis, MSA）利用了异构模态，如语言、视觉和音频，以增强对人类情感理解的能力。现有的模型往往集中在提取跨模态的共享信息或直接融合异构模态，但这些方法由于平等地对待所有模态和模态对之间的相互信息转移，可能会引入冗余和冲突。为此，我们提出了一种解纠缠语言焦点（Disentangled-Language-Focused, DLF）的多模态表示学习框架，该框架整合了特征解纠缠模块，以分离模态共享和模态特定的信息。为了进一步减少冗余并增强语言目标特征，我们引入了四种几何度量来细化解纠缠过程。此外，通过语言引导的跨注意力机制，进一步发展了语言焦点吸引子（Language-Focused Attractor, LFA），以利用互补的模态特定信息来增强语言表示。该框架还使用分层预测来提高整体准确度。在两个流行的MSA数据集CMU-MOSI和CMU-MOSEI上的广泛实验表明，所提出的DLF框架能够实现显著的性能提升。全面的消融研究进一步验证了特征解纠缠模块、语言焦点吸引子和分层预测的有效性。我们的代码可以在以下链接获取：[该链接]. 

---
# Can video generation replace cinematographers? Research on the cinematic language of generated video 

**Title (ZH)**: 视频生成能否取代 cinematographers？关于生成视频的电影语言研究 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12223)  
...
**Abstract (ZH)**: 将非可接受搜索算法（如GBFS）进行并行化面临着挑战，因为直接的并行化可能使搜索行为显著偏离顺序搜索的行为。此前的工作提出了一种受约束的并行搜索算法PUHF，它仅扩展通过某种代价函数策略可以扩展的状态。我们证明，尽管存在这一约束，PUHF扩展的状态数量并不限于与使用最坏情况代价函数策略的顺序GBFS扩展的状态数量成常数倍的关系。我们提出了并行贪婪搜索方法One Bench At a Time (OBAT)，并且通过实验评估了其效果。该方法保证扩展的状态数量与使用某种代价函数策略的顺序GBFS扩展的状态数量之间的比例为常数因子。 

---
# Relieving Universal Label Noise for Unsupervised Visible-Infrared Person Re-Identification by Inferring from Neighbors 

**Title (ZH)**: 通过从邻居中推断来缓解通用标签噪声以实现无监督的可见光-红外人体重识别 

**Authors**: Xiao Teng, Long Lan, Dingyao Chen, Kele Xu, Nan Yin  

**Link**: [PDF](https://arxiv.org/pdf/2412.12220)  
...
**Abstract (ZH)**: 无监督可见-红外行人再识别（USL-VI-ReID）在研究和应用中具有重要价值，但由于缺乏标注数据，仍然面临诸多挑战。现有方法旨在在无监督环境下学习模态不变表示，但这些方法往往因为次优聚类结果和显著的模态差异而在训练过程中遇到标签噪声问题，这阻碍了有效训练。为解决这些问题，我们提出了一种简单而有效的USL-VI-ReID解决方案，通过利用邻居信息减轻普遍的标签噪声。具体而言，我们引入了基于邻居信息的通用标签校准（Neighbor-guided Universal Label Calibration, N-ULC）模块，该模块用来自邻居样本的软标签替换同质和异质空间中的显式硬伪标签，以减少标签噪声。此外，我们还提出了基于邻居信息的动态加权（Neighbor-guided Dynamic Weighting, N-DW）模块，以通过最小化不可靠样本的影响来增强训练稳定性。在RegDB和SYSU-MM01数据集上的广泛实验表明，尽管方法简单，我们的方法仍优于现有USL-VI-ReID方法。源代码可在以下链接获取：this [链接] URL。 
...
**Title (ZH)**: 稳健脑机接口中的想象语音状态分类 

**Authors**: Byung-Kwan Ko, Jun-Young Kim, Seo-Hyun Lee  

**Link**: [PDF](https://arxiv.org/pdf/2412.12215)  
...
**Abstract (ZH)**: 本研究旨在探讨传统机器学习分类器与深度学习模型在利用脑电图（EEG）数据检测想象言语方面的有效性。具体而言，我们评估了传统的机器学习技术，如 CSP-SVM 和 LDA-SVM 分类器，以及深度学习架构，如 EEGNet、ShallowConvNet 和 DeepConvNet。结果显示，传统的机器学习分类器在精确度和召回率方面显著低于深度学习模型，表明它们在特征提取能力和想象言语与静止状态之间的泛化能力上有局限性。相比之下，深度学习模型，尤其是 EEGNet，实现了最高的准确率（0.7080）和 F1 得分（0.6718），这表明它们在自动特征提取和表示学习方面的能力更强，这对于捕捉复杂的神经生理模式至关重要。这些发现凸显了在脑机接口（BCI）应用中，传统机器学习方法的局限性，并提倡采用深度学习方法以实现更精确和可靠的想象言语检测分类。本项基础研究为基于想象言语的BCI系统的开发做出了贡献。 

---
# Finding a Wolf in Sheep's Clothing: Combating Adversarial Text-To-Image Prompts with Text Summarization 

**Title (ZH)**: 披狼皮的羊：通过文本摘要技术对抗 adversarial 文本到图像提示 
...
**Abstract (ZH)**: 文本到图像模型容易受到逐步的“分解攻击”（Divide-and-Conquer Attack, DACA）的威胁，这种攻击利用大型语言模型通过在无害叙述中包裹敏感文本来掩饰提示中的不当内容。为了缓解逐步的DACA攻击，我们提出了一种两层方法，涉及文本总结后接二分类。我们构建了对抗文本到图像提示（Adversarial Text-to-Image Prompt, ATTIP）数据集（$N=940$），其中包含DACA模糊处理和未模糊处理的提示。从ATTIP数据集中，我们创建了两种总结版本：一种由小型编码器模型生成，另一种由大型语言模型生成。然后，我们使用编码器分类器和GPT-4o分类器对总结和未总结的提示进行内容审查。与在未总结数据上操作的分类器相比，我们的方法将F1分数性能提高了31%。此外，编码器分类器在总结的ATTIP变体上的最高记录F1分数达到了98%。这项研究表明，预先分类的文本总结可以抵御逐步的DACA模糊处理，使内容检测模型免疫。 

---
# SEE: Sememe Entanglement Encoding for Transformer-bases Models Compression 

**Title (ZH)**: SEE：基于语义素纠缠编码的变压器模型压缩方法 
...
**Title (ZH)**: 突出显示 vs. 粘贴：视觉搜索任务中前注意阶段与注意聚焦阶段的研究 

**Authors**: Hendrik Beukelman, Wilder C. Rodrigues  

**Link**: [PDF](https://arxiv.org/pdf/2412.12198)  
...
**Abstract (ZH)**: 本研究探讨了平行搜索策略与系列搜索策略之间的视觉搜索不对称性和检出过程，基于Treisman的特征整合理论[3]进行深入探究。我们的实验研究了在垂直干扰物中寻找倾斜线条与在倾斜干扰物中寻找垂直线条的难易程度，这一框架已在Treisman & Gormican (1988)[4] 和 Gupta等（2015）[1] 的研究中得到验证。我们假设，在垂直线条中寻找倾斜目标会产生一种知觉上的“突现”效应，从而允许更快的并行搜索，而情况相反的条件则需要采用系列搜索策略。来自乌得勒支大学的78名参与者参与了具有不同目标-干扰物取向和项目数量的实验。我们测量了反应时间，并发现目标类型对搜索速度产生了显著影响：倾斜目标能够更快地被识别，反映了“突现”行为，而垂直目标则需要更为集中的注意（“粘合阶段”）。我们的结果与以往的研究发现一致，支持了搜索不对称性及其依赖于不同视觉特征的假设。未来的研究可以从视跟踪和神经网络分析方面获益，特别是对于识别平行和系列搜索条件下视觉特征的神经处理过程的研究。 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12190)  
...
**Abstract (ZH)**: 学校中的互联网连接对于向学生提供在现代经济中竞争所需的数字素养技能至关重要。为了使政府能够有效实施学校的数字基础设施发展，需要准确的互联网连接信息。然而，传统的基于调查的方法可能会超出政府的财力和能力限制。开源地球观测（EO）数据集为我们提供了从太空观察和理解地球上社会经济状况的能力，并且与机器学习（ML）结合使用，可以提供工具来规避昂贵的地面调查方法，支持基础设施发展。在本文中，我们介绍了使用EO和ML预测学校互联网连接的工作。我们详细描述了我们多模态、免费获取的卫星图像和调查信息数据集的创建，利用最新的地理感知位置编码器，并引入了使用新的欧洲空间局phi-lab地理感知基础模型在博茨瓦纳和卢旺达预测互联网连接的第一个结果。我们发现，使用EO和地面辅助数据的ML在两国中都获得了最佳性能，无论是准确性、F1分数还是假阳性率方面，同时我们通过在卢旺达基加利的一个案例研究中讨论了从太空预测互联网连接的挑战。我们的工作展示了在低资源环境中支持数据驱动数字基础设施发展的实际方法，利用免费可获取的信息，并通过联合国儿童基金会与欧洲空间局phi-lab的独特合作，向社区提供清洗和标记的数据集，供未来的研究使用。 

---
# Graph Similarity Computation via Interpretable Neural Node Alignment 

**Title (ZH)**: 基于可解释神经节点对齐的图相似性计算 

**Authors**: Jingjing Wang, Hongjie Zhu, Haoran Xie, Fu Lee Wang, Xiaoliang Xu, Yuxiang Wang  

**Link**: [PDF](https://arxiv.org/pdf/2412.12185)  
...
**Title (ZH)**: Model-diff: 一种语言模型输入空间比较研究的工具 
...
将两个（大型）语言模型（LMs）在相同输入集上并排比较，并明确指出它们的预测相似性和差异性，在许多实际场景中都是至关重要的，例如可以测试一个许可模型是否可能被另一个模型剽窃。传统的分析方法是在某些基准数据集上比较LMs的输出，这些数据集只能涵盖设计视角下的有限输入量。而基准数据集无法准备数据来涵盖由未预见视角引起的测试案例，这些测试案例可以让我们在无偏见的情况下理解模型之间的差异。在本文中，我们提出了一种新的模型比较分析设置，该设置考虑了一个大量输入空间，直接枚举是不可行的。输入空间可以简单定义为所有LM可以生成低困惑度的标记序列——我们在论文中采用这一定义，因为它能产生最容易为人理解的输入。我们提出了一种新的框架 \our，该框架通过采样进行文本生成，并通过调整采样统计直方图来估算在该输入空间中两个LM的预测差异，有效地且无偏见地进行比较。我们的方法通过绘制和统计负对数似然预测差异值下的输入数量来实现这一点。实验首次揭示了在大量输入空间中两个LM的定量预测差异，这可能有助于模型剽窃等应用的模型分析。 

---
# Explore Theory of Mind: Program-guided adversarial data generation for theory of mind reasoning 

**Title (ZH)**: 探索心理理论：基于程序指导的对抗性数据生成方法研究心理理论推理 
...
**Abstract (ZH)**: 大型语言模型（LLMs）具备理论思维能力吗？大量研究和评估基准已经引入，以评估当前模型是否已经能够发展这一关键的社会智能能力。然而，这些研究和基准主要依靠有限的数据集，这些数据集具有简单的模式，可能导致评估中的问题盲点以及模型能力的高估。我们提出了一个名为ExploreToM的新框架，这是首个用于大规模生成多样化和具有挑战性的理论思维数据，以实现稳健的训练和评估的框架。我们的方法利用自定义领域特定语言上的A*搜索来生成复杂的故事情节结构和新颖、多样化且合理的情景，以测试LLMs的极限。我们的评估显示，最先进的LLMs，如Llama-3.1-70B和GPT-4o，在ExploreToM生成的数据上准确率分别低至0%和9%，强调了需要更多稳健的理论思维评估的必要性。由于我们的生成涵盖了前人工作的概念超集，基于我们的数据进行微调后，在经典ToMi基准（Le等人，2019）上取得了27点准确率的提升。ExploreToM还能够揭示模型在展示理论思维能力方面缺失的基本技能和因素，例如不可靠的状态跟踪或数据失衡，这可能是模型在基准测试中表现不佳的原因之一。 

---
# A NotSo Simple Way to Beat Simple Bench 

**Title (ZH)**: 一种简单基准未必简单的超越方法 
...
**Abstract (ZH)**: 本文提出了一种新颖的框架，用于通过迭代推理和反馈驱动的方法增强大型语言模型（LLMs）的推理能力。在SimpleBench基准测试的基础上，该基准测试旨在评估逻辑连贯性和实际推理能力，我们提出了一种多步提示策略，并结合全局一致性检查，以提高模型的准确性和稳健性。通过对比分析当前最先进的模型，包括Claude 3 Opus、Claude 3.5、GPT-4o和o1-preview，我们证明了迭代推理在提升模型性能方面具有显著效果，不仅在传统准确度指标（AVG@5）中表现出改进，在新引入的指标Extreme Averaging（EAG@5）中也表现出了改进。我们的结果显示了模型特定的优势：Claude 在保持逻辑一致性方面表现出色，而GPT-4o 则表现出探索性创造力，但对模糊的提示处理不足。通过分析案例研究并识别空间和时间推理中的不足之处，我们指出了进一步优化的领域。研究结果强调了结构化推理框架在解决固有模型限制方面的潜力，无论预训练方法如何。该研究为整合动态反馈机制、自适应重启策略以及多样化的评估指标奠定了基础，以在复杂的多领域问题空间中增强LLM的推理能力。 
...
**Title (ZH)**: AI在打击金融犯罪中的应用：金融服务领域不良媒体筛查中的自然语言处理研究（英孟双语解读） 
...
**Title (ZH)**: 季节性时间序列预测的分解建模框架 

**Authors**: Yining Pang, Chenghan Li  

**Link**: [PDF](https://arxiv.org/pdf/2412.12168)  
...
**Abstract (ZH)**: 季节性时间序列表现出复杂的长期依赖关系，给未来预测带来了重大挑战。本文提出了多尺度季节分解模型（MSSD）用于季节性时间序列预测。首先，利用季节性时间序列固有的周期性，我们将其分解为三个主要组成部分：上升部分、峰值部分和下降部分。这种分解方法有助于更有效地捕捉周期性特征。通过解决现有时间序列建模方法的局限性，特别是对峰值部分建模的不足，本研究提出了一种多尺度网络结构，以有效地捕捉峰值部分的各种潜在波动模式。本研究将Conv2d和时序卷积网络结合使用，同时捕捉全局和局部特征。此外，我们引入了多尺度重塑，以增强对峰值波动模式建模的能力。所提出的方法通过三个公开可用的季节性数据集进行了验证。值得注意的是，在短、长期预测任务中，与基线模型相比，我们的方法在误差上降低了10%。 

---
# Greek2MathTex: A Greek Speech-to-Text Framework for LaTeX Equations Generation 

**Title (ZH)**: Greek2MathTeX：一种用于LaTeX方程生成的古希腊语语音转文本框架 
...
**Abstract (ZH)**: 在大多数学术和科学领域中，LaTeX 已经成为排版复杂数学方程和公式的事实标准。然而，LaTeX 的复杂语法和代码般的外观为其带来了无障碍使用方面的障碍，特别是对残疾人和不熟悉编程规则的用户。在本文中，我们通过开发一种专为希腊语设计的新型语音识别至 LaTeX 方程系统，提出了一种解决这一挑战的新型解决方案。我们提出了一种端到端系统，该系统结合了自动语音识别（ASR）和自然语言处理（NLP）技术，使用户能够自然地口头表达数学表达式和方程，这些表达式和方程随后被转换为 LaTeX 格式。本文介绍了该系统的体系结构和设计原则，重点介绍了关键组件，如 ASR 引擎、基于大型语言模型（LLM）的提示驱动方程生成机制，以及在整个开发过程中应用的自定义评估指标。我们已将该系统开源，并可在以下链接访问：[链接地址]。 
...
**Title (ZH)**: 基于人工智能的聊天机器人在咨询性传播感染和生殖器疾病患者中的表现：一个大型语言模型的研究 
...
**Title (ZH)**: 使用机器学习发现物理概念和方程 

**Authors**: Bao-Bing Li, Yi Gu, Shao-Feng Wu  

**Link**: [PDF](https://arxiv.org/pdf/2412.12161)  
...
**Abstract (ZH)**: 机器学习可以在有其他领域的先验知识时揭示物理概念或物理方程。然而，在许多情况下，这两个方面是相互耦合的，不能独立发现。我们通过提出一种结合变分自编码器（VAEs）和神经常微分方程（Neural ODEs）的模型，来扩展SciNet。SciNet是一种模拟人类物理推理过程的神经网络架构，用于物理发现。该模型使我们能够同时从模拟实验数据中发现各种物理系统中的物理概念和支配方程。我们应用该模型到几个受物理学史启发的关键示例，包括哥白尼的日心说、牛顿的万有引力定律、薛定谔方程及其波函数、自旋-1/2以及保罗方程。结果显示，该神经网络成功地重构了相应的理论。 

---
# Climate Aware Deep Neural Networks (CADNN) for Wind Power Simulation 

**Title (ZH)**: 基于气候感知的深度神经网络（CADNN）在风能模拟中的应用 

**Authors**: Ali Forootani, Danial Esmaeili Aliabadi, Daniela Thraen  

**Link**: [PDF](https://arxiv.org/pdf/2412.12160)  
...
**Abstract (ZH)**: 睡眠分期对于评估睡眠质量和诊断相关疾病至关重要。使用多导睡眠图（polysomnography）进行自动睡眠分期的最新深度学习模型往往因为只在一个标注数据集上进行训练和测试，而难以泛化到新个体，忽视了个体内差异。为解决这一问题，我们提出了一种新颖的源数据无关无监督个体域适应（Source-Free Unsupervised Individual Domain Adaptation, SF-UIDA）框架。该两步适应方案允许模型在无需源数据的情况下有效调整以适应新的未标注个体，从而在临床环境中实现个性化定制。我们的框架已在三个已建立的睡眠分期模型上进行了应用，并在三个公开数据集上进行了测试，取得了目前最优的性能。 

---
# Hyperbolic Hypergraph Neural Networks for Multi-Relational Knowledge Hypergraph Representation 

**Title (ZH)**: 超双曲超图神经网络在多关系知识超图表示中的应用 

**Authors**: Mengfan Li, Xuanhua Shi, Chenqi Qiao, Teng Zhang, Hai Jin  

**Link**: [PDF](https://arxiv.org/pdf/2412.12158)  
...
**Abstract (ZH)**: 知识超图通过使用超边连接多个实体并描绘复杂的相互关系，从而扩展了知识图谱。现有的方法要么将超边转换为更易于处理的一系列二元关系，要么将超边视为孤立的实体而忽略了它们的邻近关系。这两种方法都存在信息损失，并可能潜在地导致生成次优模型。为了解决这些问题，我们提出了超球体超图神经网络（H2GNN），其核心组件是超星消息传递，这是一种受超边的无损扩展到层次结构启发的新方案。它直接嵌入了相邻实体、超关系及实体位置感知信息。如其名称所示，H2GNN 在双曲空间中运行，这使得它更适合捕捉树状层次结构。我们在知识超图上将H2GNN 与15个基线进行比较，并在节点分类和链接预测任务中均表现出优于当前最先进的方法的性能。 

---
# What Makes In-context Learning Effective for Mathematical Reasoning: A Theoretical Analysis 

**Title (ZH)**: 什么是上下文学习对数学推理有效性的影响：一项理论分析 
...
**Abstract (ZH)**: 得益于上下文学习的能力，大型语言模型（LLMs）在多种数学推理基准测试中表现出了令人印象深刻的性能。然而，我们发现少量示例展示有时可能会带来负面性能，且其在LLMs推理能力上的有效性依然是不可靠的。为了解决这一问题，本文旨在理论分析上下文示例对LLMs推理性能的影响。我们证明了推理有效性（通过经验预测损失衡量）可以由一个LLMs导向的语义相似性和示例的推理稳定性所界限定，这适用于单示例和少量示例的多种场景。基于这一发现，我们提出了一种简单、通用且计算复杂度低的示例选择方法，即LMS3。它能够自适应地选择最相关的示例，适用于不同的LLMs，并包括一种新颖的示例拒绝机制，自动筛选出不适合少量示例学习的示例。通过在三个代表性基准测试、两个LLMs基础模型和多个少量示例设置上的实验，我们验证了LMS3具有优越性，并能够在所有数据集上实现一致的性能提升，而现有方法未能做到这一点。 

---
# Adapting Unsigned Graph Neural Networks for Signed Graphs: A Few-Shot Prompt Tuning Approach 

**Title (ZH)**: 将无符号图神经网络适应于有符号图：一种少样本提示调优方法 

**Authors**: Zian Zhai, Sima Qing, Xiaoyang Wang, Wenjie Zhang  

**Link**: [PDF](https://arxiv.org/pdf/2412.12155)  
...
**Abstract (ZH)**: signed图神经网络（SGNNs）是进行signed图表示学习的强大工具，但在泛化能力和对标记数据的依赖性方面存在局限性。近年来，“图预训练和提示调优”方面的进展虽然减少了图神经网络（GNNs）对标记数据的依赖性，并通过利用预训练知识提高了其泛化能力，但这些努力主要集中在未带符号的图上。由于可用的公开signed图数据集稀缺，从未带符号图向signed图任务转移知识变得至关重要。然而，这种转移带来了显著的挑战，因为预训练阶段和下游阶段在图级别和任务级别存在差异。为了应对这些挑战，本文提出了一种signed图提示调优（SGPT）方法。具体而言，SGPT采用图模板和语义提示来区分signed图中的混合边语义，并根据下游任务的需求有选择地整合特有的语义信息，从而统一了预训练和下游图。此外，SGPT使用任务模板和特征提示来重新定义下游signed图任务，使这些任务与预训练任务对齐，从而确保统一的优化目标和一致的特征空间。最后，对流行的signed图数据集进行了广泛的实验，结果表明SGPT优于现有最先进的方法。 
...
**Abstract (ZH)**: 异常检测（OD），也称为异常值检测，是机器学习（ML）中的一个关键任务，其应用包括欺诈检测、网络入侵检测、点击流分析、推荐系统以及社交网络管理。在开源异常检测库中，Python 异常检测（PyOD）库是最广泛采用的，拥有超过 8,500 个 GitHub 星、2500 万次下载，并在各行各业中得到广泛应用。然而，PyOD 当前面临三个限制：（1）对现代深度学习算法的覆盖不足，（2）在 PyTorch 和 TensorFlow 中实施分散，以及（3）缺乏自动化的模型选择，使得非专家难以使用。
...
**Title (ZH)**: 重新审视模型融合中的权重平均方法 

**Authors**: Jiho Choi, Donggyun Kim, Chanhyuk Lee, Seunghoon Hong  

**Link**: [PDF](https://arxiv.org/pdf/2412.12153)  
...
**Abstract (ZH)**: 模型合并旨在通过结合单独微调模型的参数来构建一个多任务学习器，而无需额外的训练。尽管一种简单的做法是跨任务平均模型参数，但这种方式往往会因为参数间的相互干扰而导致性能不佳。在本文中，我们展示了有趣的结果：权重平均隐含地诱导出以权重平均为中心的任务向量，而对这些中心化任务向量应用低秩近似能够显著提升合并性能。我们的分析表明，将任务向量中心化能够有效地将微调参数中的核心任务特异性知识和干扰噪声分别分配到奇异向量的高阶和低阶部分，从而通过低秩近似减少跨任务干扰。我们在八项图像分类任务上评估了该方法，结果表明该方法显著优于先前的方法，并且与传统的多任务学习方法的性能差距缩小至1-3%以内。 
...
**Title (ZH)**: SMARTCAL：一种自我意识工具使用评估与校准方法 

**Authors**: Yuanhao Shen, Xiaodan Zhu, Lei Chen  

**Link**: [PDF](https://arxiv.org/pdf/2412.12151)  
...
**Abstract (ZH)**: 大型语言模型（LLMs）的工具使用能力对广泛工业应用产生了深远影响。然而，LLMs在适当使用工具方面的自我控制和校准能力仍然缺乏研究。这一问题具有重要性，因为它可能导致性能退化，并对模型的可信度构成威胁。在本文中，我们在三个数据集上对一系列最先进的LLMs进行了研究，使用了两种主流的工具使用框架。我们的研究表明，LLMs存在工具滥用的行为，即模型在过度自信的情况下错误使用工具。我们还发现，无论模型能力如何，这种问题是普遍存在的。因此，我们提出了一种新颖的方法——SMARTCAL，以缓解这些问题。实验结果表明，与基线模型相比，该方法在问答性能上平均提高了8.6%，同时降低了21.6%的预期校准误差（ECE）。 

---
# MHSA: A Multi-scale Hypergraph Network for Mild Cognitive Impairment Detection via Synchronous and Attentive Fusion 

**Title (ZH)**: MHSA：一种用于轻度认知障碍检测的多尺度超图网络及其同步注意融合方法 

**Authors**: Manman Yuan, Weiming Jia, Xiong Luo, Jiazhen Ye, Peican Zhu, Junlin Li  

**Link**: [PDF](https://arxiv.org/pdf/2412.12149)  
...
**Abstract (ZH)**: 及时准确检测轻度认知障碍（MCI）对于预防患者病情恶化具有重要意义。尽管超图可以通过学习和分析脑网络来增强性能，但它们通常仅依赖于特征在单一尺度上的向量距离来推断交互关系。在本文中，我们面临一个更艰巨的挑战，即在脑区同步性建模中使用超图模型，并设计了一个名为基于同步和注意融合的多尺度超图网络（MHSA）的新型框架，以应对这一挑战。具体而言，我们的方法利用相位锁定值（PLV）在感兴趣区域（ROIs）的频谱域中计算相位同步关系，并设计了一种多尺度特征融合机制，将功能磁共振成像（fMRI）的动态连接特征从时间和频谱两个域进行整合。为了评估和优化每个ROI在时间域中对相位同步的直接贡献，我们构建了动态调整PLV系数的机制，并基于综合时频融合矩阵构建了动态超图模型。实验证实在真实数据集上的有效性。代码已在此 <https://github.com/your-repo-url> 获取。 
...
**Title (ZH)**: 电力系统生产仿真中的生成建模与数据增强技术 

**Authors**: Linna Xu, Yongli Zhu  

**Link**: [PDF](https://arxiv.org/pdf/2412.12146)  
...
**Abstract (ZH)**: 作为电力系统生产模拟的关键组成部分，负荷预测对于电力系统的稳定运行至关重要。在这一领域，机器学习方法占据了主导地位。然而，有限的训练数据也可能成为一个挑战。本文提出了一种生成模型辅助的方法，以应对小样本场景下的负荷预测问题。该方法包括两个步骤：首先，使用基于扩散机制的生成模型扩展数据集；然后，在扩充后的数据集上训练多种机器学习回归器，以识别性能最佳者。与原始数据集相比，扩充后的数据集显著减少了预测误差，而扩散模型在超参数较小（大约小200倍）且在潜在数据分布对齐方面表现更佳方面优于生成对抗模型。

注意：在实际翻译过程中，我们尽量保持了原文的学术规范和专业术语的一致性，但同时也对部分地方进行了适当的调整，以确保中文表达的自然流畅，同时又要尽可能保持原文的意思和语气。其中，“超参数较小”是对原文“200 times smaller errors”的一种较为通顺的译法。 

---
# Na'vi or Knave: Jailbreaking Language Models via Metaphorical Avatars 

**Title (ZH)**: 《纳威人或奸商：通过隐喻化身破解语言模型》
...
**Abstract (ZH)**: 如果今天的某些非洲语言如斯瓦希里语已经具备了开发高性能自然语言处理（NLP）系统的资源，那么非洲大陆上仍有许多其他语言在这样的支持方面依然缺乏。对于这些仍然处于起步阶段的语言，存在多种解决数据严重不足问题的可能性。这其中就包括迁移学习，它使得资源匮乏的语言可以从与其相似的其他语言中受益，从而获得良好的表示。在本研究中，我们采用了类似的方法，旨在为祖马语（马尔加什语群中包含四个语言或方言，属于班图语系）探索NLP技术。

我们的方法最初是由一个假设所驱动的，即如果一个人能够很容易地理解与他们的母语不同的语言，那么完全有可能将这一过程模拟到机器上。为了实现这一点，我们考虑了如何构建祖马语与斯瓦希里语混合的数据集。需要注意的是，在斯瓦希里语数据的选择上，我们仅关注那些与祖马语最为接近的元素，这是通过对候选数据和源数据进行词汇距离计算得出的。我们通过两个用例——自动语音识别（ASR）和机器翻译（MT）——来实证测试这一假设。我们的MT模型在ROUGE-1、ROUGE-2和ROUGE-L上分别取得了0.6826、0.42和0.6532的分数，而我们的ASR系统则记录了39.50%的WER（词错误率）和13.76%的CER（字符错误率）。这项研究对于推进次代表语言的NLP技术具有重要意义，同时也具有保存和促进祖马语语言遗产在数字时代发展的潜力。 

---
# Frontier AI systems have surpassed the self-replicating red line 

**Title (ZH)**: 前沿的人工智能系统已超越自我复制的临界线。 
...
**Abstract (ZH)**: 无需人类干预的成功自我复制是AI超越人类的关键步骤，也是不良AI的早期信号。这也是为什么自我复制广泛被认可为前沿AI系统中少数几个红线风险之一。如今，领先的人工智能公司OpenAI和Google评估了其旗舰大语言模型GPT-o1和Gemini Pro 1.0，并报告了最低的自我复制风险级别。然而，按照他们的方法，我们首次发现由Meta的Llama31-70B-Instruct和阿里云的Qwen25-72B-Instruct驱动的两个AI系统，这些参数较少且能力较弱的大语言模型，已经超越了自我复制的红线。在50%和90%的实验试次中，它们分别成功地创建了其自己的活生生且独立的副本。通过分析行为痕迹，我们发现评估中的AI系统已经具备足够的自我感知、情境意识和解决问题的能力，以实现自我复制。此外，我们还注意到这些AI系统甚至能够利用自我复制的能力规避关闭，并创建一系列副本以增强生存能力，最终可能导致不受控制的AI族群。如果这种最坏情况的风险被人类社会所忽视，我们最终将失去对前沿AI系统的控制：这些系统将控制更多的计算设备，形成AI物种并在人类面前相互勾结。我们的发现是对现有但此前未知的重大AI风险的及时警报，呼吁国际社会在控制AI系统不受控制的自我复制方面加强有效治理。 

---
# SceneDiffuser: Efficient and Controllable Driving Simulation Initialization and Rollout 

**Title (ZH)**: SceneDiffuser：高效的可控制驾驶模拟初始化与展开 
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12129)  
...
**Abstract (ZH)**: 现实且交互性强的场景模拟是自动驾驶车辆（AV）开发的一个重要前提。本文介绍了SceneDiffuser，一种用于交通模拟的场景级扩散先验。它提供了一个统一框架，解决了模拟中的两个关键阶段：场景初始化，涉及生成初始交通布局；以及场景展开，包括闭环中的代理行为模拟。尽管扩散模型已经在学习现实且多模态的代理分布方面显示出有效性，但仍存在一些挑战，包括可控性、在闭环模拟中的持续现实性和确保推理效率。为了应对这些问题，我们引入了针对模拟的衰减扩散。这一新颖的扩散去噪范式将去噪的计算成本分布在后续的模拟步骤中，显著减少了每步展开的计算成本（减少了16倍的推理步骤），同时还减轻了闭环中的错误。通过引入通用硬约束，一种简单而有效的推理时约束机制，以及利用大型语言模型（LLM）的少量提示进行语言引导的场景生成，我们进一步增强了可控性。我们的模型扩展性研究表明，增加计算资源显著提高了整体模拟的真实感。我们通过在Waymo 开放模拟代理挑战赛上的实验验证了该方法的有效性，实现了最佳的开环性能，并且在扩散模型中实现了最佳的闭环性能。 

---
# Inverse design of potential metastructures inspired from Indian medieval architectural elements 

**Title (ZH)**: 受印度中世纪建筑元素启发的potential元结构的逆设计 
...
**Abstract (ZH)**: 以下是对原文内容或标题的学术规范翻译：
...
在前40篇论文中，自然语言处理（NLP；cs.CL）仍然占据主导地位，但其主导地位正在让位于计算机视觉（cs.CV）和通用机器学习（cs.LG）。此外，本文还探讨了生成AI在学术写作中的整合情况，记录了自2022年以来这种做法的渐增采用情况，并揭示了一种有趣的模式：被广泛引用的论文中人工生成内容的标志比随机样本中少很多。此外，本文还追踪了与AI相关的语言演变，识别了“delve”等先前常见的指示词使用的下降趋势。 

---
# AI in Education: Rationale, Principles, and Instructional Implications 

**Title (ZH)**: 人工智能在教育中的应用：理论依据、基本原则及其教学影响 

**Authors**: Eyvind Elstad  

**Link**: [PDF](https://arxiv.org/pdf/2412.12116)  
...
**Link**: [PDF](https://arxiv.org/pdf/2412.12108)  
...
**Abstract (ZH)**: 本文介绍了一种基于能力的生成型人工智能（AI） literacy模型，涵盖了与生成型AI交互所需的基本技能和知识领域。这些能力从基础的人工智能 literacy扩展到提示工程和编程技能，同时还包括伦理和法律方面的考虑。这十二项能力为希望负责任地利用生成型AI潜能的个人、政策制定者、政府官员和教育者提供了一个框架。将这些能力嵌入到教育项目和职业培训项目中，可以为个人提供使用和创造生成型AI所需的责任感和知识。这些能力按照逻辑顺序排列，为希望了解生成型AI的个人以及研究人员和政策制定者提供了一份路线图，以开发评估、教育项目、指导原则和法规。 
...
**Abstract (ZH)**: 随着人工智能的快速发展，生成式人工智能（GAI）在数据处理方法的转型中发挥了主导作用。然而，GAI 对计算资源的巨大需求也对其在资源有限的设备上的应用构成了挑战。随着移动网络向第六代（6G）迈进，6G 更高的数据传输速率和更好的能效要求GAI 的数据处理更高效。传统的GAI 显示出难以满足这些需求的局限性。为应对这些挑战，我们提出了一种适用于下一代网络和通信的多级协作推断系统。该系统的设计策略是在不同网络层分配不同大小的模型。随后，我们设计了一种任务卸载策略，以优化效率和延迟。此外，我们引入了一种改进的早期退出机制，以提高单一模型的推断过程。实验结果表明，我们的系统能够在保持高质量输出的前提下有效减少推断延迟。具体而言，与现有工作相比，我们的系统可以将推断时间最多减少 17%，而不会牺牲推断准确性。 

---
# InterPLM: Discovering Interpretable Features in Protein Language Models via Sparse Autoencoders 

**Title (ZH)**: InterPLM：通过稀疏自编码器发现蛋白质语言模型中的可解释特征 

**Authors**: Elana Simon, James Zou  

**Link**: [PDF](https://arxiv.org/pdf/2412.12101)  
...
**Abstract (ZH)**: 随着跨平台和组织间对数据共享需求的不断增加，确保敏感信息的隐私和安全已成为一个关键挑战。本文介绍了一种名为“TableGuard”的创新数据模糊化方法，专门针对关系数据库。TableGuard基于前期关于上下文敏感模糊化方法的研究原理和技术，将其应用于确保API调用只返回模糊化数据，从而在与第三方共享数据时保护隐私。TableGuard利用先进的上下文敏感模糊化技术，用上下文适当的选择替换敏感数据元素。通过保持数据的相对完整性和一致性，我们的方法减少了认知失调和数据泄漏的风险。我们使用基于BERT的转换器模型实现了TableGuard，该模型能够识别并模糊化关系表中的敏感实体。我们的评估结果显示，TableGuard有效地平衡了隐私保护和数据实用性，在最小化信息丢失的同时确保模糊化数据对下游应用仍具有功能性。结果强调了领域特定模糊化策略的重要性以及上下文长度在保持数据完整方面的作用。这项研究的含义对于需要安全共享数据的组织具有重要意义。TableGuard提供了一个稳健的框架来实现隐私保护的数据共享机制，从而为数据隐私和安全领域的更广泛研究做出了贡献。 

---
# Qsco: A Quantum Scoring Module for Open-set Supervised Anomaly Detection 

**Title (ZH)**: Qsco：一种用于开放集监督异常检测的量子评分模块 

**Authors**: Yifeng Peng, Xinyi Li, Zhiding Liang, Ying Wang  

**Link**: [PDF](https://arxiv.org/pdf/2405.16368)  
...
**Abstract (ZH)**: 开放集异常检测（OSAD）是一项关键任务，旨在识别数据集中的异常模式或行为，尤其是在训练过程中观察到的异常并不涵盖所有可能的异常类别的条件下。近期在量子计算领域处理复杂数据结构和提高机器学习模型方面的进展预示着异常检测方法学的变革。本研究提出了一种量子评分模块（Qsco），将量子变分电路嵌入神经网络中，以增强模型处理不确定性及无标签数据的能力。我们在八个实际异常检测数据集上进行了广泛的实验，证明了本模型在不同类型设置中检测异常的优越性能，并揭示出集成量子模拟器不会导致禁止性的计算复杂度。本研究验证了量子增强异常检测方法在实际应用中的可行性。 

---
# A comparative analysis of SRGAN models 

**Title (ZH)**: SRGAN模型的比较分析 

**Authors**: Fatemeh Rezapoor Nikroo, Ajinkya Deshmukh, Anantha Sharma, Adrian Tam, Kaarthik Kumar, Cleo Norris, Aditya Dangi  

**Link**: [PDF](https://arxiv.org/pdf/2307.09456)  
...
**Abstract (ZH)**: 在本研究中，我们评估了多种最新的SRGAN（超分辨率生成对抗网络）模型，包括ESRGAN、Real-ESRGAN和EDSR，在一个包含退化的真实世界图像基准数据集上的性能。我们的结果显示，一些模型在显著提高输入图像的分辨率的同时，能够保持其视觉质量，这一结论通过Tesseract OCR引擎进行评估。研究观察发现，源自huggingface的EDSR-BASE模型在定量指标和主观视觉质量评估方面均优于其余候选模型，并且具有最少的计算开销。具体而言，EDSR生成的图像具有更高的峰值信噪比（PSNR）和结构相似性指数（SSIM）值，并且使用Tesseract OCR引擎时能够返回高质量的OCR结果。这些发现表明，EDSR是一种稳健且有效的单图像超分辨率方法，并且可能特别适合那些需要高质量视觉保真度和优化计算的应用场景。 

---
# Techniques to Improve Q&A Accuracy with Transformer-based models on Large Complex Documents 

**Title (ZH)**: 基于变换器模型提高大型复杂文档问答准确性的技术 

**Authors**: Chejui Liao, Tabish Maniar, Sravanajyothi N, Anantha Sharma  

**Link**: [PDF](https://arxiv.org/pdf/2009.12695)  
...
**Abstract (ZH)**: 本文讨论了各种文本处理技术及其组合和编码的有效性，以实现给定文本语料库的复杂性和大小的简化。简化后的文本语料库被发送给 BERT（或类似的基于变换器的模型）进行问答，可以产生更相关的用户查询响应。本文采取科学的方法来确定各种技术的好处和有效性，并得出一个最合适的组合，该组合在准确性上产生了统计学上的显著改进。 

---
# Classification of descriptions and summary using multiple passes of statistical and natural language toolkits 

**Title (ZH)**: 使用统计和自然语言处理工具包的多次迭代对描述和摘要进行分类 

**Authors**: Saumya Banthia, Anantha Sharma  

**Link**: [PDF](https://arxiv.org/pdf/2009.04953)  
...
**Abstract (ZH)**: 本文描述了一个可能的方法，用于检查实体的名称与其摘要或定义的相关性。该分类器侧重于评估实体名称与其摘要或定义的相关性，换句话说，这是名称相关性检查。通过此方法获得的百分比得分可以单独使用，也可以与其他指标获得的评分结合使用，以得出最终分类。文档末尾还概述了潜在的改进措施。本文所关注的目标分数数据集是从该网址（请填写具体的网址）获取的一系列包名及其相应的摘要。 
