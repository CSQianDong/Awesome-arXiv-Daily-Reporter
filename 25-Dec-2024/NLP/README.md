...
**Abstract (ZH)**: 我们考虑了在几分钟内生成语音的生成建模，这在长格式多媒体生成和音频原生语音助手中是必需的。然而，当前的语音语言模型在生成超过数十秒的可信语音时存在困难，这主要是由于语音令牌的高时间分辨率导致连贯性丧失，或者由于长序列训练或外推的架构问题，还可能是因为推理时的内存成本。基于这些考虑，我们提出了SpeechSSM，这是首个能够在单次解码会话中从长格式口语音频（例如，16分钟的朗读或即兴演讲）学习并抽取语言信息，而无需中间文本转换的语言模型，这得益于在顺序建模方面最近的线性时间方法的进展。此外，为了解决日益严峻的口语语言评估挑战，尤其是在这种新的长格式设置中，我们提出了以下新的方法：基于嵌入的方法和基于大规模语言模型的评估指标；跨越时间和长度的质量度量；以及新的长格式语音处理和生成基准——LibriSpeech-Long。语音样本和数据集可以在以下链接获取：[提供链接] 

---
# Exploring Embedding Priors in Prompt-Tuning for Improved Interpretability and Control 

**Title (ZH)**: 探索嵌入先验知识在提示调优中的应用，以提高可解释性和可控性 
...
**Abstract (ZH)**: 提示调优是一种通过修改提示嵌入来最小化计算开销，从而适应预训练语言模型的新任务的有效方法。在本文中，我们研究了提示调优中经常观察到的嵌入坍塌现象对最终模型性能的重要性。为了解决这一问题，我们设计了先验嵌入，并将其与收敛的软提示调优和深提示调优方法的后验进行了比较。我们的研究结果表明，先验显著影响调整后的嵌入位置，模型可以有效地使用来自激活空间不同部分的嵌入，包括全新的区域。由于最终的提示调优能力有限，我们推测可控的提示调优后验可能是类似链式思考（COT）蒸馏等任务的良好起点。我们的实验还表明，生成的轨迹并不局限于模型的激活空间中。然而，对于较远的任务（例如，NLP和算术），存在明确的激活簇，而不同NLP任务（例如，问答和掩码语言模型）的激活则位于同一簇中。这些观察提出了一个问题，即单个激活簇对于大语言模型泛化能力的重要性。 

---
# Zero-resource Speech Translation and Recognition with LLMs 

**Title (ZH)**: 基于大语言模型的零资源语音翻译与识别 
...
**Abstract (ZH)**: 尽管在语音处理方面取得了近期进展，零资源语音翻译（ST）和自动语音识别（ASR）仍然是具有挑战性的问题。在这项工作中，我们提出利用一个多语言大型语言模型（LLM）对从未见过配对音频-文本数据的语言进行ST和ASR。我们通过使用预训练的多语言语音编码器、多语言LLM以及一个轻量级的适应模块来实现这一点，该模块将音频表示映射到LLM的词元嵌入空间。我们在ST和ASR方面进行了多项实验，以了解如何最佳地训练模型以及哪些数据对未见过语言的性能影响最大。在ST中，我们的最佳模型能够在一个语言集（CoVoST2）上的两种未见过语言中实现BLEU评分超过23；而在ASR中，我们达到了高达28.2%的词错误率（WER）。最后，我们展示了我们系统的性能受限于LLM生成目标语言文本的能力。 

---
# Distilling Fine-grained Sentiment Understanding from Large Language Models 

**Title (ZH)**: 从大型语言模型中提炼细致粒度的情感理解 
...
**Abstract (ZH)**: 细粒度情感分析（Fine-grained Sentiment Analysis, FSA）旨在从大量的意见文本中提取和总结用户意见。近期研究表明，大型语言模型（Large Language Models, LLMs）具有出色的语情感理解能力。然而，直接将LLMs应用于FSA应用程序会导致高昂的推理成本。因此，本文研究了将细粒度情感理解从LLMs提炼到小型语言模型（Small Language Models, SLMs）中的方法。我们促使LLMs审查并解释给定评论的情感，然后利用生成的内容对SLMs进行预训练。此外，我们开发了一个全面的FSA基准，以评估SLMs和LLMs的性能。在该基准上的实验结果显示：（1）提炼显著提升了SLMs在FSA任务中的性能，在$F_1$分数上取得了6.00%的提升，且提炼模型仅需220M参数便能超过Llama-2-7b；（2）提炼赋予SLMs出色的零样本情感分类能力，使它们能够匹配甚至超越其教师模型。这些结果表明，从LLMs提炼是FSA的一个极具前景的方向。我们将公开我们的代码、数据和预训练模型权重，链接为：\url{this https URL}。 

---
# Libra-Leaderboard: Towards Responsible AI through a Balanced Leaderboard of Safety and Capability 

**Title (ZH)**: 《Libra-Leaderboard：通过平衡安全与能力的排行榜促进负责任的人工智能》

这个标题翻译成中文时，保持了原文的学术规范和专业术语，确保了准确性和专业性。 
...
**Abstract (ZH)**: 为了解决这一问题，我们引入了Libra-Leaderboard，这是一个综合框架，旨在通过平衡性能和安全性的评估来对大规模语言模型（LLM）进行排名。Libra-Leaderboard结合了一个动态排行榜和一个互动的LLM竞技场，鼓励对能力与安全性的联合优化。不同于传统的平均性能与安全性的方法，Libra-Leaderboard 使用一种基于最佳得分距离的计算方法来确定综合排名。这种方法激励模型在不同维度上取得平衡，而不仅仅是专注于某一个维度。在首次发布中，Libra-Leaderboard 评估了来自14家领先组织的26个主流LLM，并在最先进的模型中也发现了关键的安全挑战。 

---
# Token-Budget-Aware LLM Reasoning 

**Title (ZH)**: 面向 token 预算的大型语言模型推理 
...
**Abstract (ZH)**: 推理对于大语言模型（LLMs）在广泛的任务中取得优异表现至关重要。虽然像Chain-of-Thought (CoT)推理这样的方法通过将问题分解为中间步骤来提升LLM的表现，但也导致了显著的令牌使用量增加，进而增加了成本。我们发现当前LLMs的推理过程过于冗长，可以通过在提示中包含合理的令牌预算来压缩推理过程，但令牌预算的选择对实际压缩效果起着关键作用。我们因此提出了一种基于令牌预算感知的LLM推理框架，该框架基于推理复杂度动态估计不同问题的令牌预算，并利用估计出的令牌预算指导推理过程。实验表明，我们的方法仅在性能上略有下降的情况下有效地减少了CoT推理中的令牌成本，提供了一种平衡效率和准确性的实用解决方案。相关代码：[这里](this https URL)。 

---
# Harnessing Large Language Models for Knowledge Graph Question Answering via Adaptive Multi-Aspect Retrieval-Augmentation 

**Title (ZH)**: 利用自适应多方面检索增强技术 harness 大型语言模型进行知识图谱问答 
...
**Abstract (ZH)**: 大型语言模型（LLMs）展示了惊人的能力，但在处理复杂知识推理任务时，由于难以避免地出现幻觉和过时的知识，导致产生事实错误的输出。此前的研究尝试通过从大规模知识图谱（KGs）中检索事实知识，来辅助LLMs进行逻辑推理和答案预测，以此减轻上述问题。然而，这种做法往往引入了噪声和无关数据，尤其是面对多方面知识背景的情况下。这可能会误导LLMs的关注点偏离问题和相关信息。在我们的研究中，我们提出了一种自适应多方面增强知识图谱检索框架（Amar）。该方法检索实体、关系和子图的知识，并将每段检索到的文本转换为提示嵌入。Amar框架包含两个关键子组件：1）一个自我对齐模块，它将实体、关系和子图中的共同点进行对齐，以增强检索文本，从而减少噪声干扰；2）一个相关性闸门模块，该模块使用软闸门来学习问题与多方面检索数据之间的相关性得分，以确定哪些信息应该用于增强LLMs的输出，或者直接过滤掉。我们的方法在两个常用的数据集WebQSP和CWQ上取得了最先进的性能，相较于其最佳竞争对手，准确率提高了1.9%，在逻辑形式生成方面相较于直接使用检索文本作为上下文提示的方法，提高了6.6%。这些结果表明Amar在提高LLMs推理能力方面的有效性。 

---
# Think or Remember? Detecting and Directing LLMs Towards Memorization or Generalization 

**Title (ZH)**: 思考还是回忆？检测并引导LLMs向记忆或泛化方向发展 
...
**Abstract (ZH)**: 在本文中，我们探讨了大型语言模型（LLMs）中记忆和泛化的基础机制，受到了人类大脑功能专业化现象的启发。我们的研究通过对专门设计的数据集和实验规模的LLMs进行研究，以更好地理解这两种行为。具体来说，我们首先通过使用设计的数据集对LLMs进行训练，使其能够同时表现出记忆和泛化的特性，然后（a）考察LLMs在记忆和泛化过程中是否表现出神经元层面的空间分化，（b）通过模型内部表示来预测这些行为，并（c）通过推理时的干预来引导这些行为。我们的研究结果表明，LLMs在记忆和泛化方面的神经元层面分化是可观察到的，而且定向干预能够成功地引导模型的行为。 
...
**Abstract (ZH)**: 为了支持可扩展的词汇语义注释、分析和理论化，我们对在句法约束（例如，所需的从句结构）和语义约束（例如，所需的动词意义）下生成事件描述的不同方法进行了全面评估。我们比较了三种不同的方法：(i) 专家手工生成；(ii) 从标注了句法和语义信息的语料库中进行采样；以及(iii) 在句法和语义信息约束下从语言模型（LM）中进行采样。我们从三个方面对生成的事件描述进行了比较：(a) 自然度，(b) 典型性，以及(c) 独特性。我们发现，所有方法都能可靠地生成自然、典型和独特的事件描述，但手工生成的方法仍然能够生成更为自然、典型和独特的事件描述。我们得出结论，考虑到用于注释和分析的方法对此结果中少量降级的事件描述具有鲁棒性，我们考虑的自动化方法所产生的事件描述在下游注释和分析中已达到足够的质量水平。 
...
**Abstract (ZH)**: 同步语音到文本翻译（SimulST）在说话人的语音输出的同时，将源语言语音实时翻译成目标语言文本，以确保低延迟，从而提高用户理解效率。尽管其旨在应用于未受限的语音，但大多数研究仍集中在预先分割的人工语音上，简化了任务并忽视了重要的挑战。这种狭窄的焦点，加上广泛存在的术语不一致性，限制了研究成果在实际应用中的适用性，最终阻碍了该领域的进展。我们对110篇论文的广泛文献综述不仅揭示了当前研究中的关键问题，还为我们的重要贡献奠定了基础。我们1）定义了SimulST系统的步骤和核心组件，提出了一套标准化的术语和分类学；2）对社区趋势进行了全面分析；3）提出了具体的建议和未来方向，以填补现有文献中的空白，从评估框架到系统架构，从而推动该领域向更现实和有效的SimulST解决方案发展。 

---
# Segment-Based Attention Masking for GPTs 

**Title (ZH)**: 基于段落的注意力掩蔽for GPT模型 
...
**Abstract (ZH)**: 现代语言模型（LMs）的成功很大程度上归功于遮蔽因袭注意（masked causal attention），这是生成预训练变压器（GPT）模型的核心。尽管GPT模型能够一次性处理整个用户请求，但在生成过程中，遮蔽因袭注意是逐步应用于所有输入词元的，这种做法在模型处理输入请求并生成内部表示之前，对初始“预填充”阶段施加了不必要的限制。在这种情况下，模型在生成任何输出词元之前就已经开始处理输入请求。在本文中，遮蔽是在预填充阶段基于已知的块结构来进行的，随后在这一阶段之后通过传统的逐词自回归过程进行。例如，在典型的聊天请求中，系统提示被视为一个块，用户请求被视为下一个块。每个块都被视为一个单元来进行遮蔽，这样每个块的第一个词元可以以非因袭的方式访问后续的词元。然后，模型在传统的因袭方式下生成答案。这种段落级方案不增加任何额外的计算开销。将该方案集成到如Llama和Qwen等模型中时，能够实现最先进的性能。 

---
# Is Large Language Model Good at Triple Set Prediction? An Empirical Study 

**Title (ZH)**: 大型语言模型在三元组集合预测方面表现如何？一项 empirical 研究 
...
**Abstract (ZH)**: 知识图谱填充（Knowledge Graph Completion, KGC）任务的核心在于预测和补全知识图谱中缺失的关系或节点。常见的KGC任务大多是在已知一个或两个元素的情况下推断未知的元素。相比之下，三元组集预测（Triple Set Prediction, TSP）任务更贴近现实，目标是在已知三元组的信息基础上预测所有未知的三元组元素。近年来，大规模语言模型（Large Language Models, LLMs）在语言理解方面取得了显著进步，显示了在KGC任务中的巨大潜力。然而，LLM在TSP任务上的潜力尚未得到充分探索。因此，在本文中，我们提出了一种新的框架，以探索LLM在TSP任务中的优势和局限性。具体来说，该框架包含了基于LLM的规则挖掘和基于LLM的三元组集预测。首先，嵌入丰富语义信息的知识图谱关系列表被利用来引导LLM生成规则。这一过程既高效又独立于统计信息，使其更容易挖掘出有效的和现实的规则。对于每个子图，指定的规则与该子图中的相关三元组相结合，以指导LLM预测缺失的三元组。随后，所有子图的预测结果被整合以推导出知识图谱的完整预测三元组集。最后，该方法在相对完整的CFamily数据集上进行了评估。实验结果表明，当LLM需要充分利用大量事实知识来预测缺失的三元组时，会导致显著的幻觉现象，从而导致性能下降。为了进一步探索这一现象的原因，本文通过详细的案例研究提供了全面的分析。 

---
# Unlocking the Potential of Multiple BERT Models for Bangla Question Answering in NCTB Textbooks 

**Title (ZH)**: 利用多個BERT模型解锁孟加拉语（Bangla）问答（Question Answering）在NCTB教科书中的潜力 
...
**Abstract (ZH)**: 在教育环境中评估文本理解能力对于理解学生表现并改善课程有效性至关重要。本研究探讨了最先进的语言模型——RoBERTa Base、Bangla-BERT和BERT Base，在自动评估来自国家课程和课本委员会（NCTB）6-10年级的孟加拉语段落为基础的问答任务中的能力。我们收集了一个包含约3000个孟加拉语段落为基础的问答实例的数据集，并通过不同超参数配置下的F1分数和精确匹配（EM）指标对这些模型进行了评估。研究结果显示，Bangla-BERT在各项指标中表现最为稳定，尤其是在较小的批量大小、包含停用词以及中等学习率的情况下，分别达到了最高的F1（0.75）和EM（0.53）分数。相比之下，RoBERTa Base在某些配置下表现最弱，F1（0.19）和EM（0.27）分数最低。研究结果强调了优化模型性能时微调超参数的重要性，并指出了机器学习模型在教育环境中评估文本理解的潜力。然而，数据集规模、拼写不一致及计算资源限制等局限性也提示了需要进一步研究以增强这些模型的鲁棒性和适用性。本研究为教育机构中自动评估系统的未来发展奠定了基础，提供了关于孟加拉语文本理解背景下模型性能的关键见解。 
...
**Abstract (ZH)**: 检索增强生成系统依赖于有效的文档检索能力。在设计上，传统的稀疏或密集检索器在多跳检索场景中面临挑战。在本文中，我们提出了GeAR，通过两项关键创新提升RAG（检索增强生成）性能：(i) 图扩展，它可以增强任何传统的基检索器，例如BM25，(ii) 一个结合了图扩展的智能体框架。我们的评估结果表明，GeAR在三个多跳问答数据集上展示了优越的检索性能。此外，与其它多步检索系统相比，我们的系统在具有挑战性的MuSiQue数据集上取得了最先进的结果，同时所需令牌数和迭代次数更少，性能提升超过10%。 

---
# Multilingual Mathematical Reasoning: Advancing Open-Source LLMs in Hindi and English 

**Title (ZH)**: 多语言数学推理：推动印地语和英语开源大语言模型的发展 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在语言任务方面表现出色，但在数学推理方面存在不足，尤其是在非英语语言如 Hindi 中。本研究旨在增强较小的、资源高效的开源 LLMs 的数学推理能力，使其能够在 Hindi 和英语中均表现出色。我们使用零样本、少量样本的思维链方法（CoT）和有监督的微调来评估 OpenHathi 7B、LLaMA-2 7B、WizardMath 7B、Mistral 7B、LLeMMa 7B、MAmmoTH 7B、Gemini Pro 和 GPT-4 等模型。我们的方法包括使用分阶段学习的方法，逐步训练模型解决越来越复杂的问题，提出了一个新的分解策略以简化复杂的算术操作，以及结构化的解题设计将解决问题的过程分为多个阶段。实验结果显示了显著的性能提升。WizardMath 7B 在英语数据集上的准确性超过 Gemini 6%，在 Hindi 数据集上的性能与 Gemini 相当。采用双语方法，结合英语和 Hindi 的样本，得到的结果可与单语言模型媲美，展示了在两种语言中学习数学推理的能力。本研究强调了提高开源 LLMs 的数学推理能力的潜力。 

---
# ChaI-TeA: A Benchmark for Evaluating Autocompletion of Interactions with LLM-based Chatbots 

**Title (ZH)**: ChaI-TeA: 一种评估基于大语言模型的聊天机器人交互自动完成基准 
...
**Abstract (ZH)**: 大语言模型（LLM）的发展正逐渐将越来越多的人机交互转向基于LLM的聊天机器人。这些模型的显著能力使得用户能够使用长篇多样化的自然语言文本进行交互，覆盖广泛的主题和风格。这些消息的措辞是一个耗时耗力的过程，需要一种自动补全解决方案来协助用户。我们引入了聊天机器人交互自动补全任务。我们提出了一种名为ChaI-TeA：CHat InTEraction Autocomplete的自动补全评估框架，用于评价LLM基于的聊天机器人交互。该框架包括对该任务的正式定义，以及合适的数据集和评估指标。我们使用该框架对9个模型进行了测试，发现尽管当前现成的模型表现尚可，但在生成建议的排序方面仍有很大的改进空间。我们为从事该任务的实践者提供了见解，并为该领域的研究人员开辟了新的研究方向。我们推出了该框架，以期成为未来研究的基础。 
...
**Abstract (ZH)**: 本研究引入了一种新颖的方法——双向主题匹配（BTM），用于跨语料库主题建模。BTM能够量化和分析不同语料库之间主题的重叠和差异。该方法是一个灵活的框架，可以整合多种主题建模方法，包括BERTopic、Top2Vec和潜在狄利克雷分配（LDA）。BTM采用双模型方法，为每个语料库训练独立的主题模型，并相互应用，以实现全面的跨语料库比较。该方法论有助于识别共享主题和独特的主题，提供对主题关系的细致见解。通过与基于余弦相似度的方法进行验证，显示出BTM的稳健性，具有较强的一致性指标，并且在处理离群主题方面具有明显优势。通过对气候新闻文章的案例研究展示了BTM的应用价值，揭示了关注气候变化和气候行动的语料库之间重要的主题重叠和区别。BTM的灵活性和精准性使其成为多种应用的宝贵工具，从政治话语分析到跨学科研究。通过整合共享主题和独特主题的分析，BTM提供了一个全面的主题关系探索框架，未来有望扩展到多语言和动态数据集中。本研究突显了BTM的方法论贡献及其跨不同领域推进话语分析的能力。 

---
# Towards Global AI Inclusivity: A Large-Scale Multilingual Terminology Dataset 

**Title (ZH)**: 全球AI包容性之路：大规模多语言术语数据集 
...
**Abstract (ZH)**: 机器翻译领域已经取得了显著进展，但在特定领域术语的翻译，尤其是在人工智能领域，仍然存在挑战。我们引入了GIST，这是一个大规模多语种AI术语数据集，包含从2000年至2023年顶级AI会议论文中提取的5000个术语。这些术语被翻译成了阿拉伯语、中文、法语、日语和俄语，采用了结合了大规模语言模型（LLM）进行提取和人类专业知识进行翻译的混合框架。数据集的质量通过众包评估与现有资源进行了基准测试，结果显示其翻译准确性更为优越。GIST通过后翻译润色方法融入翻译工作流程中，这些方法不需要重新训练模型，且LLM提示在一致地提高BLEU和COMET评分方面表现出色。在ACL Anthology平台上进行的网络演示突显了其实际应用，展示了其对非英语使用者的增强可访问性。本研究旨在解决AI术语资源中的关键空白，并促进AI研究领域的全球包容性和合作。 

---
# Extracting triples from dialogues for conversational social agents 

**Title (ZH)**: 从对话中提取三元组以构建对话型社会代理 
...
**Abstract (ZH)**: 对混合智能协作中沟通的明确理解是创建可控和透明代理的关键。本文描述了若干自然语言理解模型，这些模型可以从社会对话中提取显式的符号三元组。三元组提取主要在知识库完成任务中得到了发展和测试，主要使用维基百科文本和数据进行训练和测试。然而，社会对话作为一种交流方式与之不同，对话参与者通过一系列包含陈述、问题和回答的言语序列来交换信息。诸如共指、省略、并列、隐含和显式的否定或确认等现象在对话中比在维基百科文本中更为突出。因此，我们通过发布社会对话中的三元组提取数据集来尝试填补这一差距。此外，我们还创建了五个三元组提取模型，并在我们的评估数据上进行了测试。这些模型在单句测试中，完整三元组的最高精确度为51.14%，三元组元素的最高精确度为69.32%。然而，贯穿多个回合的对话三元组得分要低得多，这表明从真实对话数据中提取知识具有更大的挑战性。 

---
# Multi-Agents Based on Large Language Models for Knowledge-based Visual Question Answering 

**Title (ZH)**: 基于大型语言模型的多智能体系统在知识驱动的视觉问答中的应用 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在基于知识的视觉问答（VQA）任务中取得了显著成果。然而，现有方法仍然存在一些挑战：无法自主使用外部工具，以及无法协同工作。当人们遇到新的问题时，他们通常能够判断是否需要使用外部工具。例如，对于熟悉的问题，人们倾向于给出直接的答案，而对于不熟悉的问题，则会使用搜索引擎等工具。此外，人们还倾向于与其他成员合作讨论以获得更好的答案。受此启发，我们提出了一种多智能体投票框架。我们设计了三种基于LLM的智能体，模拟团队中不同层级的角色，并根据层级分配可用工具。每个智能体提供相应的答案，最后将所有智能体提供的答案进行投票，以获得最终答案。在OK-VQA和A-OKVQA数据集上的实验表明，我们的方法分别比其他基线提升了2.2和1.0的性能。 

---
# M-Ped: Multi-Prompt Ensemble Decoding for Large Language Models 

**Title (ZH)**: M-Ped：面向大规模语言模型的多提示ensemble解码方法 
...
**Abstract (ZH)**: 随着大型语言模型（LLMs）在自然语言处理（NLP）领域的广泛应用，提升其性能已成为研究热点。本文提出了一种新颖的多提示集合解码方法，旨在通过汇集多重提示结果来增强LLMs的生成质量。对于给定的输入 \(X\)，我们以批处理模式提交 \(n\) 种不同的提示变体 \(X\) 给LLMs进行解码，并获得概率分布。在每个词预测中，我们通过平均批处理内 \(n\) 个概率分布来计算集合概率，并使用该汇总概率生成词。这种方法称为内批汇集（Inner-Batch Ensemble）。为实现高效的批处理推断，我们采用左填充策略以保持批处理内 \(n\) 个提示的输入长度一致。通过在机器翻译、代码生成和文本简化等多种NLP任务上的广泛实验，我们展示了该方法在提升LLM性能方面的有效性。结果表明，该方法在BLEU分数、pass@k率和LENS指标上显著优于传统方法。 

---
# GenAI Content Detection Task 2: AI vs. Human -- Academic Essay Authenticity Challenge 

**Title (ZH)**: GenAI内容检测任务2：AI与人类鉴别——学术论文真实性挑战 
...
**Abstract (ZH)**: 本文介绍了2025年COLING大会期间举办的第一次学术论文 authenticity 挑战赛第一版的全面概述。此次挑战赛旨在检测用于学术目的的机器生成论文和人类撰写的论文。该任务定义如下：“给定一篇论文，判断它是机器生成的还是人类撰写的。”挑战赛涵盖两种语言：英语和阿拉伯语。在评估阶段，有25支队伍提交了英语系统的参赛作品，21支队伍提交了阿拉伯语系统的参赛作品，显示出对该任务的高度兴趣。最后，有七支队伍提交了系统描述论文。大多数提交的作品使用了微调后的Transformer模型，有一支队伍使用了大型语言模型（LLMs），如Llama 2和Llama 3。本文阐述了任务的定义，详细描述了数据集的构建过程，并介绍了评估框架。此外，我们还展示了参赛队伍采用的方法的总结。几乎所有提交的系统都超越了基于n-克RYPT的基线模型，表现最佳的系统在两种语言上的F1分数均超过0.98，这表明在检测机器生成文本方面取得了显著进展。 

---
# Investigating Large Language Models for Code Vulnerability Detection: An Experimental Study 

**Title (ZH)**: 探讨大型语言模型在代码漏洞检测中的应用：一项实验研究 
...
**Abstract (ZH)**: 代码漏洞检测（CVD）对于解决和预防系统安全问题至关重要，对于确保软件安全起着至关重要的作用。以往基于学习的漏洞检测方法依赖于微调中等规模的序列模型或从头训练较小的神经网络。最近大预训练语言模型（LLMs）在包括代码理解和生成在内的多种代码智能任务中展现出了显著的能力。然而，LLMs在检测代码漏洞方面的有效性尚未得到充分探索。本研究旨在通过微调LLMs来填补这一空白，涉及四个广泛使用的开源LLMs。我们还实现了五个以前的图基或中等规模序列模型进行比较。实验在五个常用的数据集上进行，包括短样本和长样本的部分。此外，我们还进行了定量实验以研究类别不平衡问题以及模型在不同长度样本上的性能，这些问题在以往的研究中较少被探讨。为了更好地促进社区的发展，我们在此开放了该研究的所有代码和资源，链接如下：[此链接]和[此链接]。 

---
# Robustness-aware Automatic Prompt Optimization 

**Title (ZH)**: 面向鲁棒性的自动提示优化 
...
**Abstract (ZH)**: 大型语言模型（LLMs）的表现取决于提示的质量和输入数据的语义和结构完整性。然而，当前的提示生成方法主要侧重于生成适用于干净输入数据的提示，往往会忽略扰动输入对提示性能的影响。为了解决这一局限，我们提出了一种名为BATprompt（通过对抗训练生成提示）的新方法，该方法旨在应对输入扰动（如输入中的拼写错误）。借鉴对抗训练技术，BATprompt通过两步过程在各种扰动任务中表现出出色的性能：首先进行对抗扰动，然后通过LLM对未扰动输入进行迭代优化。与传统的对抗攻击方法不同，BATprompt不需要依赖真实的梯度或模型参数。相反，它利用LLM的高级推理、语言理解和自我反思能力模拟梯度，从而指导生成对抗性扰动并优化提示性能。我们在实验中在多种数据集上对BATprompt进行了评估，涵盖语言理解和生成任务。结果显示，BATprompt在各种扰动场景下表现出色，提供了更好的鲁棒性和性能。 

---
# An Analysis on Automated Metrics for Evaluating Japanese-English Chat Translation 

**Title (ZH)**: 对评估日英聊天翻译自动化指标的分析 
...
**Abstract (ZH)**: 本文分析了传统基准指标（如BLEU和TER）和基于神经网络的方法（如BERTScore和COMET）在对话翻译任务中对多个NMT模型性能的评分情况，并探讨了这些指标与人工标注分数相比的表现。结果表明，所有指标在决定哪些模型表现更好时显示出一致性，这表明传统基准指标虽然更快且更简单，但仍具有一定的帮助作用。另一方面，当涉及到更好地与人工判断相关的指标时，基于神经网络的方法优于传统方法，其中COMET在对话翻译任务中与人工标注分数的相关性最高。然而，我们发现即使是最优指标在对含有零形代词的日语句子进行英语翻译评分时也遇到困难。 

---
# On the Applicability of Zero-Shot Cross-Lingual Transfer Learning for Sentiment Classification in Distant Language Pairs 

**Title (ZH)**: 关于零样本跨语言转移学习在疏远语言对情感分类中的适用性研究 
...
**Abstract (ZH)**: 本研究探讨了使用XLM-R预训练模型从英语跨语言转移到日语和印尼语的应用性。我们将结果与几种先前的研究工作进行了比较，这些研究工作要么采用类似零样本的方法，要么采用完全监督的方法，以提供使用XLM-R进行零样本跨语言迁移学习能力的概述，并将其与现有模型进行对比。我们的模型在其中一个日语数据集中取得了最佳结果，并且在其他日语和印尼语数据集中取得了可比的结果，而无需使用目标语言进行训练。此外，研究结果表明，可能不需要为每种语言训练一个单独的模型，而是可以训练一个多语言模型并取得令人满意的性能。 

---
# Survey of Pseudonymization, Abstractive Summarization & Spell Checker for Hindi and Marathi 

**Title (ZH)**: 《关于印地语和马拉地语的匿名化、抽象总结及拼写检查的综述》 
...
**Abstract (ZH)**: 印度广阔的语言多样性为技术进步，尤其是在自然语言处理（NLP）领域，带来了独特的挑战和机遇。尽管在广泛使用的语言上已经取得了显著的进步，但印度的区域语言，如马拉地语和印地语，仍被忽视。在印度区域语言的NLP研究仍处于初步阶段，具有重要意义。本文旨在构建一个平台，使用户能够使用包括文本匿名化、抽象文本摘要和拼写检查等在内的各种功能，这些功能涵盖英语、印地语和马拉地语。这些工具的目的是服务于主要使用印度区域语言的企业客户和消费者客户。 

---
# CoAM: Corpus of All-Type Multiword Expressions 

**Title (ZH)**: CoAM: 所有类型的多词表达式语料库 
...
**Abstract (ZH)**: 多词表达（Multiword Expressions，MWEs）指的是由多个单词组成的惯用语序列。MWE识别，即在文本中检测MWEs，可以在机器翻译等下游任务中发挥关键作用。现有的MWE识别数据集在注释上不一致，局限于一种类型的MWE，或者数据量有限。为了提供可靠的全面评估，我们构建了CoAM：所有类型多词表达语料库，这是通过多步过程构建的语料库，旨在提高数据质量，其中包括人工注释、人工审核和自动一致性检查。CoAM中的MWEs根据类型进行标记，例如名词和动词，以支持精细粒度的错误分析。CoAM的注释使用了一个由我们接口生成器创建的新界面收集，该界面允许轻松和灵活地标注任何形式的MWEs，包括不连续的MWEs。通过使用CoAM进行的实验表明，微调的大规模语言模型在MWE识别方面优于当前最先进的方法。此外，使用我们的带有MWE类型标记的数据进行的分析表明，各种方法中动词MWEs比名词MWEs更容易识别。 

---
# Ensuring Consistency for In-Image Translation 

**Title (ZH)**: 确保图像内翻译的一致性 
...
**Abstract (ZH)**: 图像内机器翻译任务涉及将嵌入图像中的文本进行翻译，并以图像格式呈现翻译结果。尽管该任务在电影海报翻译和日常生活场景图像翻译等各种应用场景中具有广泛的应用前景，现有的方法往往忽略了这一过程中的连贯性问题。本文提出了在图像内机器翻译任务中维持两种类型一致性的需求：翻译一致性和图像生成一致性。前者要求在翻译过程中融合图像信息，后者则需要保持文本图像风格与原始图像风格的一致性，确保背景的完整性。为解决这些连贯性要求，本文提出了一种名为HCIIT（High-Consistency In-Image Translation）的新颖两阶段框架。该框架首先使用多模态多语言大型语言模型进行文本-图像翻译，然后使用扩散模型进行图像填充。在第一阶段中，采用链式思维学习提高模型在翻译过程中利用图像信息的能力。随后，经过风格一致性文本-图像生成训练的扩散模型确保了图像中文本风格的一致性，并保留了背景细节。为此，我们为模型训练创建了一个包含400,000个风格一致的伪文本-图像对的数据集。在自创的数据集和真实图像测试集上的实验结果验证了该框架在确保连贯性和生成高质量翻译图像方面的有效性。 

---
# LSAQ: Layer-Specific Adaptive Quantization for Large Language Model Deployment 

**Title (ZH)**: LSAQ：针对大型语言模型部署的分层自适应量化 
...
**Abstract (ZH)**: 随着大规模语言模型（LLMs）在各个领域展现出卓越的性能，将这些模型部署到边缘设备上已成为一种新的趋势。通过减小LLM的尺寸和内存占用的量化技术，可以有效支持在资源受限的边缘设备上进行部署。然而，现有的“一刀切”量化方法往往无法根据特定的硬件特征和使用场景动态调整LLM的内存消耗。为了克服这一限制，我们提出了一种基于层重要性的自适应量化方法LSAQ（Layer-Specific Adaptive Quantization），这是一种用于根据层重要性自适应量化和动态部署LLM的系统。LSAQ通过从每一层的输入和输出中构建top-k词集，并计算这些集的Jaccard系数来评估每一层的重要性。利用这种评估方法，系统根据边缘设备的资源可用性实时调整量化策略，为不同重要性的层分配不同的精度级别。这种方法显著减少了LLM的存储需求，同时保持了模型的性能，从而能够高效地在多样化的硬件平台上进行部署，并适应不同的使用场景。 

---
# Do Language Models Understand the Cognitive Tasks Given to Them? Investigations with the N-Back Paradigm 

**Title (ZH)**: 语言模型是否理解赋予给它们的认知任务？基于N-步回溯范式的探究 
...
**Abstract (ZH)**: 最初为人类设计的认知任务现在越来越多地被用于研究语言模型。虽然应用这些任务通常比较简单，但解释其结果则颇具挑战性。特别是在模型表现不佳时，人们常常不清楚这种表现与测试的认知能力有限有关，还是由于未能理解任务本身。一项近期的研究认为，GPT 3.5在2-back和3-back任务上的表现下降反映了与人类相似的工作记忆容量限制。通过对各种性能水平的开源语言模型在这些任务上的表现进行分析，我们发现这种表现不佳实际上反映了任务理解和任务集维持能力的限制。此外，我们将表现最好的模型推到更高的n值，并尝试不同的提示策略，之后分析模型的注意力机制。我们的更大目标是为改进语言模型认知评估方法的研究提供支持。 

---
# Molly: Making Large Language Model Agents Solve Python Problem More Logically 

**Title (ZH)**: Molly：使大规模语言模型代理更逻辑地解决Python问题 
...
将大型语言模型（LLMs）作为教学助手应用于智能教育，特别是在计算机课程中，已经引起了广泛关注。为了缩小LLMs与计算机编程教育专家之间的差距，现有研究中主流的方法是微调和检索增强生成（RAG）。然而，针对特定任务的微调可能耗资源且会降低模型的泛化能力。RAG 能够在减少 LLMS 的幻觉方面表现出色，但在推理过程中生成无关事实内容可能会对学习者造成重大困惑。为了解决这些问题，我们引入了 Molly 代理，旨在解决学习 Python 编程语言过程中学习者遇到的问题。我们的代理通过基于场景的交互自动解析学习者的问题意图，从而从构建的知识库中精确检索相关文档。在生成阶段，代理会对生成的响应进行反思，确保它们不仅与事实内容一致，还能有效地回答用户的问题。在使用构建的中文 Python QA 数据集进行广泛实验表明，Molly 代理的有效性，这表明其在提供 Python 问题有用回答方面的性能得到了提升。 

---
# Improving Factuality with Explicit Working Memory 

**Title (ZH)**: 提高事实准确性通过显式工作记忆 
...
**Abstract (ZH)**: 大规模语言模型能够生成事实不准确的内容，这一问题被称为幻觉。近期的工作通过迭代提示增加了检索增强生成方法，以改进事实准确性，但这些方法受限于传统的检索增强生成（RAG）设计。为解决这些挑战，我们提出了EWE（显式工作记忆）这一新颖的方法，通过整合一个能够实时接收外部资源反馈的工作记忆，来增强长文本生成的事实准确性。该记忆基于在线事实核查和检索反馈进行刷新，使EWE能够在生成过程中纠正虚假断言，确保更准确和可靠的输出。我们的实验表明，在四个事实寻求的长文本生成数据集上，EWE在VeriScore这一事实性度量指标上比强大基线提高了2至10个绝对点，同时保持了生成内容的有用性。进一步的分析揭示了记忆更新规则的设计、记忆单元的配置以及检索数据存储的质量是影响模型性能的关键因素。 
...
**Abstract (ZH)**: 尽管预训练语言模型（PLMs）的前馈层神经元能够存储事实性知识，但大多数先前的分析仍局限于定性层面，导致知识表示、神经元激活与模型输出之间的量化关系尚未被充分理解。在本研究中，通过使用事实性探针数据集进行神经元级干预，我们首先揭示了神经元激活与输出词概率之间的线性关系。我们将这种线性关系的梯度称为“神经元经验梯度”，并提出了一种名为NeurGrad的高效计算方法，以促进定量神经元分析。其次，我们探讨了PLMs中的神经元经验梯度是否编码了通用任务知识。为此，我们引入了MCEval8k，这是一个覆盖六个文体和22个任务的多选项知识评估基准。我们的实验表明，神经元经验梯度能够有效捕捉知识，而技能神经元则表现出高效性、普适性、包容性和相互依赖性。这些发现通过神经元经验梯度将知识与PLM输出联系起来，揭示了PLMs存储知识的方式。代码和数据集已公开。 

---
# Factuality or Fiction? Benchmarking Modern LLMs on Ambiguous QA with Citations 

**Title (ZH)**: 事实还是虚构？现代大型语言模型在带有引文的模糊问答任务上的基准测试 
...
**Abstract (ZH)**: 现代大型语言模型（LLMs）在复杂和现实任务上的基准测试对于其发展至关重要。在本工作中，我们评估了最先进的LLMs在具有来源引用的模糊情境下问答（QA）任务上的事实准确性和引用性能。利用三个最近发布的数据集——DisentQA-DupliCite、DisentQA-ParaCite 和 AmbigQA-Cite，这些数据集涵盖了多种现实世界的模糊性，我们分析了两个领先的LLMs模型GPT-4o-mini和Claude-3.5的表现。研究结果表明，较大的、较新的模型一致地能够在模糊情境中预测至少一个正确答案，但在处理存在多个有效答案的情况时表现不佳。另外，所有模型在引文生成方面的表现都很差，引文准确性始终为0。然而，引入意识冲突的提示可以带来显著改进，使模型能够更好地处理多个有效答案并提高引文准确性，同时保持其预测正确答案的能力。这些发现突显了开发能够处理模糊性和提供可靠来源引用的LLMs所面临的挑战和机遇。我们的基准测试研究提供了关键见解，并为未来在可信赖和可解释的问答系统方面的改进奠定了基础。 

---
# Aligning AI Research with the Needs of Clinical Coding Workflows: Eight Recommendations Based on US Data Analysis and Critical Review 

**Title (ZH)**: 将AI研究与临床编码工作流程需求对齐：基于美国数据的分析与批判性评审的八项建议 
...
**Abstract (ZH)**: 临床编码对于医疗收费和数据分析至关重要。手工临床编码劳动密集且容易出错，这激发了对整个编码过程自动化的研究。然而，我们的分析基于美国英语电子健康记录，并且使用这些记录进行自动编码研究的结果表明，广泛使用的评估方法与实际临床情境不相符。例如，仅关注最常用的前50个编码是一种简化处理，因为在实践中使用的编码有数千种。本文旨在使基于AI的编码研究更加贴近临床编码的实际挑战。基于我们的分析，我们提出了八项具体的建议，旨在改进当前的评价方法。此外，我们还提出了超越自动编码的新的人工智能方法，建议提供辅助临床编码员工作流程的替代方法。 
...
**Abstract (ZH)**: 神经网络通常被视为黑盒模型，这在理解其内部机制方面尤其是在自然语言处理（NLP）应用中造成了重大挑战。为了解决这一透明度问题，局部可解释的模型无关解释（LIME）等模型解释技术成为了提供这些复杂系统行为洞见的重要工具。本研究利用LIME来解释一个用于文本分类任务的多层感知机（MLP）神经网络。通过分析个体特征对模型预测的贡献，LIME方法增强了可解释性并支持基于知情的决策。尽管LIME在提供局部解释方面非常有效，但它在捕捉全局模式和特征交互方面存在局限性。本研究突出了LIME的优点和不足，并提出了未来工作方向，以实现对神经NLP模型更全面的可解释性。 

---
# Same Company, Same Signal: The Role of Identity in Earnings Call Transcripts 

**Title (ZH)**: 相同公司，相同信号：身份在盈利电话会议记录中的作用 
...
**Abstract (ZH)**: 事件后波动率预测对于投资者至关重要，以往的研究往往依赖财报电话会议纪要，原因在于其丰富的语义被认为对预测波动率有显著贡献。为进一步探讨电话会议纪要如何影响波动率，我们引入了DEC数据集，该数据集通过之前被忽视的“前收盘后市”特征和密集的证券代码覆盖率，实现了准确的波动率计算。与其他基准数据集不同，每个证券代码在DEC中提供了约20次财报记录。利用DEC，我们发现事件后的波动率经历了显著的变化，每个证券代码都展现出独特的波动率分布。为了利用历史事件后的波动率并捕捉特定于每个证券的模式，我们提出了两种无训练的基准模型：事件后波动率（PEV）和同证券事件后波动率（STPEV）。这些基准模型在DEC和现有基准数据集上均超过了基于电话会议纪要的所有模型。此外，我们还证明当前的电话会议纪要表示主要捕捉到的是证券身份信息，而非每份财报的具体财务见解。这一结论通过两个关键观察得以证实：相同证券代码的财报表示显示出显著更高的相似性，而基于电话会议纪要模型的预测与此前的事件后波动率表现出强大的相关性。 

---
# StructTest: Benchmarking LLMs' Reasoning through Compositional Structured Outputs 

**Title (ZH)**: StructTest：通过组合结构化输出评估LLMs的推理能力 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）的快速发展迫切需要稳健、无偏见且可扩展的方法来评估其能力。然而，人工标注的成本难以规模化，基于模型的评估容易受到答案风格偏见的影响，而基于目标答案的标准则容易受到数据污染和作弊的影响。为了解决这些限制，我们提出了一种名为StructTest的新型基准，该基准通过生成组成性指定的结构化输出来评估LLMs的能力，这是一种无偏见、易于实施且难以作弊的衡量标准。该评估通过基于规则的评估器进行确定性执行，可以轻松扩展到新的任务。通过对涵盖多样性任务领域的结构化输出进行测试——包括摘要、代码、HTML和数学——我们展示了StructTest作为通用推理能力代理的有效性，因为生成结构化输出往往需要内部逻辑推理。我们认为，StructTest为客观和稳健的模型评估提供了一个重要的补充方法。 

---
# Correctness is not Faithfulness in RAG Attributions 

**Title (ZH)**: 正确性不等于忠诚性在RAG属性中的体现 
...
**Abstract (ZH)**: 检索相关上下文是减少幻觉并提高答案可靠性的常见方法。明确引用来源文献可以让用户验证生成的答案并增加信任度。以往的研究主要评估引用的正确性——即引用的文献是否支持相应的内容。然而，引用的正确性本身是不够的。为了建立对归属答案的信任，我们必须同时检查引用的正确性和忠实性。在本文中，我们首先拆分引用正确性和忠实性这两个概念，这两个概念在前人的研究中应用不一致。忠实性保证模型对引用文献的依赖是真实可靠的，反映的是实际的参考使用情况，而不是与先验信念表面一致的现象，我们称之为后合理化。我们设计了一个实验，揭示了后合理化问题的普遍性，这会削弱可靠的归属，并可能导致不适当的信任。我们的研究结果表明，当前归属答案中缺乏引用忠实性的情况相当普遍（高达57%的引用），这表明在语言模型中，需要评估正确性和忠实性以实现可靠的归属。 
...
**Abstract (ZH)**: 因果推理能力对于大型语言模型（LLMs）在广泛的应用领域，如教育和医疗保健中至关重要。然而，目前仍然缺乏相应的基准来更好地理解这种能力。现有的LLM基准主要基于对话任务、学术数学测验和编程测验。这些基准在良好的标准化环境中评估LLM，但它们在评估解决实际问题的能力方面存在局限性。在这项工作中，我们提供了一个名为CARL-GT的基准，该基准利用图和表格数据评估大型语言模型的因果推理能力。该基准包含多样化的任务，从因果图推理、知识发现和决策制定方面评估LLM。此外，我们还为这些任务开发了有效的零样本学习提示。在我们的实验中，我们利用该基准评估开源的LLM，并详细比较了LLM在因果推理方面的能力。我们发现，LLM在因果推理方面仍然比较薄弱，尤其是在利用表格数据发现新见解方面。此外，我们通过分析LLM在不同任务上的表现，研究并讨论了不同基准任务之间的关系。实验结果表明，LLM在不同任务上的能力存在差异，而不同类别任务（因果图推理、知识发现和决策制定）之间的表现更具相关性，相较于同一类别的任务而言。 

---
# Path-of-Thoughts: Extracting and Following Paths for Robust Relational Reasoning with Large Language Models 

**Title (ZH)**: 路径思考：提取和跟随路径以增强大型语言模型的稳健关系推理 
...
**Abstract (ZH)**: 大型语言模型（LLMs）具有丰富的语义知识，但在处理复杂推理任务时常常遇到挑战，尤其是在亲属关系或空间推理等关系推理问题方面。本文提出了一种名为Path-of-Thoughts（PoT）的新型框架，旨在通过将任务分解为三个关键阶段——图提取、路径识别和推理——来应对关系推理问题。与之前的 approach 不同，PoT 有效地提取出一个任务无关的图，能够识别问题背景中的关键实体、关系和属性。随后，PoT 在图中识别出与提出的问题相关的推理路径，有助于推断潜在的答案。实验评估结果表明，PoT 在四个基准数据集上表现显著优于最先进的基线方法（最高提升率达到21.3%），并且在无需微调或大量调用LLM的情况下实现了这一成绩。此外，与先前的神经符号方法相比，PoT 通过利用图的组合性质表现出更强的抗LLM错误能力。 

---
# IITR-CIOL@NLU of Devanagari Script Languages 2025: Multilingual Hate Speech Detection and Target Identification in Devanagari-Scripted Languages 

**Title (ZH)**: IITR-CIOL@NLU 2025：印地语系语言中的多语言仇恨言论检测与目标识别 
...
**Abstract (ZH)**: 本文专注于解决印地语系语言（包括印地语、马拉地语、尼泊尔语、薄普尔语和梵语）中的仇恨言论检测和目标识别两个子任务。Subtask B 涉及在线文本中的仇恨言论检测，而 Subtask C 则要求识别仇恨言论的具体目标，如个人、组织或社群。我们提出了一种名为 MultilingualRobertaClass 的模型，该模型建立在预训练的多语言转换器模型 ia-multilingual-transliterated-roberta 之上，专门针对多语言和转写文本环境中的分类任务进行了优化。该模型利用上下文化嵌入来处理语言多样性，并包含一个二分类的分类器头部。在测试集上，我们在 Subtask B 中获得了 88.40% 的准确率，在 Subtask C 中获得了 66.11% 的准确率。 

---
# BenCzechMark : A Czech-centric Multitask and Multimetric Benchmark for Large Language Models with Duel Scoring Mechanism 

**Title (ZH)**: BenCzechMark：一种基于捷克语的多任务和多指标基准测试，具备 Duel 评分机制的大语言模型评价体系 
...
**Abstract (ZH)**: 我们介绍了BenCzechMark（BCM），这是首个专为大规模语言模型设计的全面捷克语基准测试，提供了多种任务、多种任务格式和多种评估指标。其评分系统基于统计显著性理论，并借鉴社会偏好理论对任务进行综合评分。该基准测试包含50项挑战性任务，且具有相应的测试数据集，主要采用捷克本土语言，其中11项为新收集的任务。这些任务涵盖了8个领域，涵盖众多领域，包括历史捷克新闻、学生的文章或语言学习者的作文，以及口语。

此外，我们收集并清理了BUT-Large捷克语语料库，这是目前最大的可供公开使用的捷克语语料库，我们利用其进行（i）污染分析，（ii）首个以捷克为中心的大规模（7B）语言模型的持续预训练，并采用特定于捷克的语言分词。我们将我们的模型作为基线，与公开的多语言模型进行比较。最后，我们发布并维护了一个排行榜，已有44个模型提交，新的模型提交可以在以下网址提交：[此链接地址]。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在广泛的语言相关任务中展示了卓越的能力，包括解决复杂的推理问题。提升LLM性能的一种有效技术是上下文学习，通过纳入解释性的示例来引导模型的回应，从而使模型逐步进行推理。然而，选择适合模型的示例是一个挑战，因为每个数据集需要一套独特的示例来帮助LLM有效学习并在测试集中表现良好。当前的研究通常依赖于基于不确定性或多样性的选择策略来选择需要标注的示例并改进模型学习。然而，这些研究通常采用非适应性方法，一次性选择一组示例。我们认为，这种非适应性策略可能会导致一个在涵盖知识方面高度冗余的示例集，从而降低其总体信息量。为了解决这一局限性，我们提出了一种名为 \textsc{Adaptive-Prompt} 的新方法，该方法通过利用先前选择的示例的模型反馈来进行示例的适应性选择。实验结果表明，\textsc{Adaptive-Prompt} 显著提升了LLM在各种推理任务中的性能。 

---
# Evaluating LLM Reasoning in the Operations Research Domain with ORQA 

**Title (ZH)**: 使用ORQA评估LLM在运筹学领域的推理能力 
...
**Abstract (ZH)**: 在本文中，我们介绍了并应用了运筹学问答（ORQA），这是一个新的基准测试，旨在评估大型语言模型（LLMs）在运筹学（OR）这一专业技术领域的泛化能力。该基准测试评估LLMs在面对多样且复杂的优化问题时，是否能够模拟运筹学专家的知识和推理技能。该数据集由运筹学专家开发，包含需要多步推理来构建其数学模型的实际优化问题。对各种开源LLMs，如LLaMA 3.1、DeepSeek和Mixtral的评估结果显示，它们的表现有限，突显了它们在向专业技术领域泛化方面的能力差距。本研究为LLMs的泛化能力这一持续讨论的领域提供了有价值的贡献，并为未来在此领域的研究提供了宝贵见解。该数据集和评估代码已公开可供访问。 

---
# Joint Knowledge Editing for Information Enrichment and Probability Promotion 

**Title (ZH)**: 联合知识编辑以增强信息丰富度和促进概率性推理 
...
**Abstract (ZH)**: 大型语言模型中存储的知识需要及时更新，以反映现实世界信息的动态性质。为了更新知识，大多数知识编辑方法侧重于低层，因为最近对知识检索过程的研究显示，答案信息主要集中在低层中。然而，这些探针仅揭示了原始答案的关键检索阶段，而编辑的目标是纠正模型对目标答案的预测。这种不一致性表明，现有的探针方法及其相关编辑方法都是有缺陷的。为解决这种不一致性并识别出关键编辑区域，我们提出了一种对比探针方法，并定位了两个关键阶段，即模型在原始答案与目标答案之间的行为差异：低层信息丰富化和高层概率促进。基于这些洞见，我们开发了联合信息丰富化和概率促进的知识编辑方法（JEEP），该方法联合编辑低层和高层，以修改这两个关键检索阶段。考虑到双重修改带来的相互干扰和渐进性遗忘，JEEP 设计时确保对不同区域的更新具有相同的目标并相互补充。我们通过在不同模型上编辑多达数千个事实，包括 GPT-J（6B）和 LLaMA（7B），并对各种编辑目标进行评估。在所有测试场景中，JEEP 都取得了最佳性能，验证了我们探针方法揭示的内容和编辑方法设计的有效性。我们的代码和数据可在此处查看：[链接]。 

---
# Evaluating and Enhancing LLMs for Multi-turn Text-to-SQL with Multiple Question Types 

**Title (ZH)**: 评估和提升用于多轮文本到SQL转换的大型语言模型，以支持多种问题类型 
...
**Abstract (ZH)**: 近年来，大型语言模型（LLMs）的最新进展显著推进了文本到SQL系统的发展。然而，大多数基于LLM的方法往往狭隘地集中于SQL生成，忽视了实际对话查询的复杂性。这种疏忽可能导致对模糊问题的不可靠响应，尤其是那些无法直接用SQL解决的问题。为弥补这一差距，我们提出了一种名为MMSQL的综合测试套件，通过模拟具有多种问题类型和多轮问答交互的现实场景，来评估LLMs的问题分类和SQL生成能力。使用MMSQL，我们评估了多种流行的LLM模型，包括开源和闭源模型，并确定了影响其在这些场景中性能的关键因素。此外，我们引入了一种基于LLM的多代理框架，该框架使用专门的代理来识别问题类型并确定适当的回答策略。我们的实验表明，这种做法显著增强了模型处理对话动态复杂性的能力，有效地应对用户查询的多样性和复杂性。 

---
# Overview of the 2024 ALTA Shared Task: Detect Automatic AI-Generated Sentences for Human-AI Hybrid Articles 

**Title (ZH)**: 2024年度ALTA共享任务综述：检测人工智能生成的句子以用于人机混合文章 
...
**Abstract (ZH)**: 自2010年起，ALTA共享任务每年举行一次。2024年的任务旨在检测混合环境中的人工生成文本，其中文本可能包含人类生成的部分和机器生成的部分。本文介绍了该任务的内容、评估标准以及参与共享任务的系统的性能结果。 

---
# Enhancing Knowledge Distillation for LLMs with Response-Priming Prompting 

**Title (ZH)**: 使用响应引导提示增强大语言模型的知识蒸馏 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）已经在广泛自然语言处理（NLP）任务中展示了卓越的表现。然而，这些模型由于计算需求巨大和资源限制，常常难以部署。知识蒸馏（KD）是一种有效的方法，可以将大模型的性能转移到小型模型中。传统的KD方法主要关注教师模型的直接输出，对知识转移过程中提示的作用关注较少。在本文中，我们提出了一种新颖的响应引导提示策略，应用于知识蒸馏管道中，以增强学生的模型性能。我们的方法通过从量化的大模型中蒸馏知识，微调一个小一些的Llama 3.1 8B Instruct模型。我们应用LoRA优化，并在GSM8K基准上进行评估。实验证明，将推理提示整合到提出的KD管道中，显著提高了学生模型的性能，为在资源受限环境中部署强大模型提供了一种有效途径。我们发现，使用Ground Truth提示的模型在GSM8K基准上的性能相比未使用提示的模型提高了55%。对学生模型的自注意力层进行深入研究表明，更成功的提示模型通常在其注意力头中表现出某些积极行为，这些行为与其更高的准确性有关。我们的实现可以在以下链接中找到：https://这个链接地址。 

---
# Evaluating the Capabilities of Large Language Models for Multi-label Emotion Understanding 

**Title (ZH)**: 评估大型语言模型在多标签情感理解能力上的表现 
...
**Abstract (ZH)**: 大型语言模型（LLMs）展示了令人鼓舞的学习和推理能力。与其它NLP任务相比，多语言和多标签情感评估任务在LLMs中的研究仍然相对不足。在本文中，我们推出了一个名为EthioEmo的数据集，该数据集包含了四门埃塞俄比亚语言——阿姆哈拉语（amh）、奥罗莫语（orm）、索马里语（som）和提格雷尼亚语（tir）的情感分类任务。我们还使用SemEval 2018 Task 1中的额外英语多标签情感分类数据集进行了实验。评估包括仅编码器、编码器-解码器以及仅解码器的语言模型。我们比较了零样本和少样本方法与微调较小语言模型的效果。结果表明，即使对于如英语这类高资源语言，准确的多标签情感分类仍然不足，并且高资源语言和低资源语言之间的性能差距仍然很大。结果还表明，不同语言和模型类型在情感分类任务中的表现存在差异。EthioEmo数据集已公开，旨在进一步提高对语言模型中情感理解以及人们如何通过不同语言表达情感的理解。 

---
# Look Ahead Text Understanding and LLM Stitching 

**Title (ZH)**: 面向未来的文本理解与大规模语言模型集成 
...
**Abstract (ZH)**: 本文以前瞻段落识别（LASI）为例，提出了一个前瞻文本理解问题。此类问题不仅在生成型人工智能和人机交互中可能产生，任何需要理解文本或对话发展趋向的场景下也可能出现。我们使用基于变换器的大型语言模型（LLM）来解决这一问题。研究表明，LASI比经典段落识别（SI）更具挑战性。我们认为，双向上下文信息（如BERT）和单向预测能力（如GPT）都有助于这一任务的完成。我们提出了一种方法，将BERT和GPT结合在一起。实验结果表明，我们的方法在包含噪声的文本（这对于生成型人工智能中的发展文本而言通常是常见的）情况下，优于现有的模型。本文探讨了其他与社交媒体密切相关的重要前瞻文本理解任务，如前瞻情感分类，并指出了通过结合预训练的大型语言模型来利用这些机会的可能性。 

---
# Leveraging Sentiment for Offensive Text Classification 

**Title (ZH)**: 利用情感分析进行攻击性文本分类 
...
**Abstract (ZH)**: 在本文中，我们进行实验以分析情感信息是否能帮助模型更有效地分类具有攻击性的文本。我们在此实验中使用SemEval 2019任务6（OLID）数据集。首先，我们利用预训练的语言模型预测每个实例的情感。之后，我们选择在OLID测试集上表现最佳的模型，并在扩充的OLID数据集上对其进行训练，以分析其性能。实验结果表明，使用情感信息可以提高模型的整体性能。 
...
**Abstract (ZH)**: 尽管像GPT和BERT这样的大型语言模型已经在自然语言处理以及特定领域的应用中展现出了前所未有的技能，却出现了一种未被探索的现象，我们称之为罗塞塔 paradox。罗塞塔 paradox 描述了知识领域间反直觉的表现反转现象。这一悖论揭示了这些大规模语言模型在高度专业化领域中表现出色，但在要求一般性日常生活知识的任务上表现不佳。本文对罗塞塔 paradox 进行了形式化定义，并引入了一个全景分析框架，该框架包括领域特异性指数（Domain Specificity Index, DSI）和表现反转指标（Performance Inversion Metric, PIM），用于一致地量化大规模语言模型在特定领域的行为。

本文采用了这一悖论，并通过广泛实验对不同模型和知识领域的多样性进行了一系列调查，从丰富的技术领域到常识推理。我们的研究发现，罗塞塔 paradox 也许是数据分布的产物，也可能是深层神经网络内在架构和涌现属性的本质特征。我们展示了不同模型架构、规模和训练方法之间的对比分析，揭示了这一悖论特殊表现形式的奥秘，并挑战了现有的评估指标。 

---
# Inductive Linguistic Reasoning with Large Language Models 

**Title (ZH)**: 使用大型语言模型进行归纳语言推理 
...
**Abstract (ZH)**: 评估大型语言模型（LLMs）在语言推理能力方面的表现是理解其在大规模应用中可能暴露出的技术缺陷的重要任务。在这项工作中，我们通过考察极低资源语言下的语言谜题，研究这类模型进行抽象多语言推理的能力。由于这些翻译任务涉及归纳和演绎推理，我们研究是否可以从种子示例自动推导出多样化的辅助演示，通过类比提示实现这一点。我们采用两阶段程序，首先是使用语言模型生成类比示例，然后将其与提供的目标语言示例一起在上下文中应用。我们在modeLing数据集上的结果显示，类比提示能够有效激发模型对语言语法规则相似性的理解和应用，相较于因果推理方法，GPT-4o的性能提高了8.1%，Llama-3.1-405B-Instruct的性能提高了5.9%。这些性能提升归因于自动生成的类比示例以及由较弱的多语言模型生成的类比示例。此外，我们还展示了该方法在 Linguistics Olympiad 竞赛中的其他任务中具有泛化能力，使用GPT-4o在LINGOLY数据集中实现了各种问题类型和难度水平的显著改进。我们还报告了一些关于推动语言推理性能的有趣现象的研究结果，表明这些谜题是评估新推理方法的有效基准。 

---
# How Well Do LLMs Generate Code for Different Application Domains? Benchmark and Evaluation 

**Title (ZH)**: 不同应用领域中大型语言模型生成代码的能力如何？基准测试与评估 
...
**Abstract (ZH)**: 近年来，由代码大规模语言模型（code LLMs）驱动的AI编程助手被集成到各种实际软件开发环境中，显著提高了开发人员的 productivity。然而，现有的代码生成基准主要集中在通用场景上，使得LLMs在特定应用领域的代码生成性能鲜为人知。本文旨在填补这一空白，介绍了一个新的基准——MultiCodeBench。MultiCodeBench包含2,400个编程任务，涵盖了12个流行的软件开发领域和15种编程语言。具体来说，我们进行了深入研究，确定了这12个应用领域。鉴于每个领域可能涉及多个技术框架，且不同框架在编码过程中会带来不同的挑战，我们在每个领域内归类了常用的框架和平台。然后，我们从与这些子领域相关的GitHub仓库中抽取编程问题。为确保任务的质量并缓解数据泄漏问题，我们邀请注释者重新撰写MultiCodeBench中每个任务的docstring。此外，我们构建了一个基于静态分析的依赖解析工具，用于提取每个任务的真实依赖关系，从而进行更深层次的性能分析。通过在MultiCodeBench上对十一个代表性的主流LLM进行广泛的实验，我们揭示了不同应用领域中LLM的代码生成性能，为下游领域开发者在选择LLM时提供了实用见解。此外，我们分析了模型在完成软件应用开发任务时失败的原因，为模型开发者提供了提升特定领域代码生成能力的指导。 
...
**Abstract (ZH)**: 预测是一项难以评估的任务：真正的结果只有在将来才能知道。近期研究表明，LLM预测器正迅速接近人类级别的表现，这引发了这样的问题：我们如何能够即时地评估这些预测器的表现？我们遵循一致性检查框架，通过衡量预测器在不同逻辑相关问题上的预测结果的一致性来评估其性能。我们提出了一个新的、通用的一致性度量标准，该标准基于套利概念：例如，如果一个预测AI非理性地预测两党（民主党与共和党）在2024年美国总统选举中获胜的概率均为60%，那么套利者可以根据预测器的预测进行交易并获利。我们建立了一个自动评估系统，该系统生成一组基础问题，从这些问题中实例化一致性检查，获取预测器的预测结果，并度量这些预测结果的一致性。我们随后构建了一个标准的、符合评分准则的预测基准，并展示了我们的（即时）一致性度量与LLM预测器的未来才知的真实布利尔得分相关联。我们还发布了一个2028年生效的一致性基准，为预测提供了一个长期评估工具。 

---
# Characterizations of Language Generation With Breadth 

**Title (ZH)**: 语言生成的广度特征研究 
...
**Abstract (ZH)**: 我们研究了Kleinberg和Mullainathan [KM24]引入的限制下语言生成问题，该问题建立在Gold [Gol67]和Angluin [Ang79]的经典工作中。[KM24]提出了一种算法，能够生成任意可数语言集合中的字符串，在限制内进行。尽管该算法最终会产生目标语言$K$中的字符串，但它牺牲了广度，即生成$K$中所有字符串的能力。[KM24]中的一个关键开放问题是这种一致性和广度之间的权衡是否不可避免。

最近的研究引入了不同的广度一致生成的概念。Kalavasis、Mehrotra和Velegkas [KVM24]提出了三种定义：精确广度生成、近似广度生成和消歧生成。同时独立地，Charikar和Pabbaraju [CP24a]提出了详尽生成的概念。两篇论文分别探讨了这些广度概念下生成的可能性。

基于[CP24a, KVM24]的工作，我们全面地刻画了这些概念及其自然组合下的语言生成。对于精确广度，我们提供了无条件的下界，去除了[KVM24]中的一个技术条件，并扩展了[CP24a]结果，该结果适用于特定的语言集合。我们证明了精确广度生成由Angluin的识别条件完全刻画。我们进一步引入了Angluin条件的较弱版本，该条件紧密地刻画了近似广度和详尽生成，并证明了它们的等价性。此外，我们证明了消歧生成作为更广泛结果的一个特殊情况也由Angluin条件刻画。最后，我们通过提供无条件的下界加强了[KVM24]的工作，表明Angluin条件刻画了稳定生成器的先前广度概念。这表明了在近似广度下的稳定生成与不稳定生成之间的差异。 

---
# Explainable Multi-Modal Data Exploration in Natural Language via LLM Agent 

**Title (ZH)**: 通过大规模语言模型代理进行可解释的多模态数据探索 
...
**Abstract (ZH)**: 国际企业、组织或医院收集了大量的多模态数据，这些数据存储在数据库、文本文档、图片和视频中。虽然在多模态数据探索和自然语言问题自动转换为数据库查询语言的数据库系统领域最近都取得了一些进展，但将数据库系统与其他非结构化的模态（如自然语言中的图片）结合起来进行查询的研究挑战仍未得到广泛探索。

在本文中，我们提出了一种名为XMODE的系统，该系统能够在自然语言中实现可解释的多模态数据探索。我们的方法基于以下研究贡献：（1）我们的系统受到一个实际应用场景的启发，使用户能够探索多模态信息系统。（2）XMODE利用基于大语言模型（LLM）的代理式AI框架，将自然语言问题分解为诸如文本到SQL生成和图像分析等子任务。（3）在基于关系数据和图像的多模态数据集上的实验结果表明，我们的系统在准确性和各种性能指标（如查询延迟、API成本、规划效率和解释质量）方面均超越了现有的多模态数据探索系统，这得益于LLM更强的推理能力的更有效利用。 

---
# LongDocURL: a Comprehensive Multimodal Long Document Benchmark Integrating Understanding, Reasoning, and Locating 

**Title (ZH)**: 长文档URL：一个全面的多模态长文档基准，集成理解、推理和定位功能 
...
**Abstract (ZH)**: 大规模视觉语言模型（LVLMs）显著提升了文档理解能力，能够处理复杂文档元素、更长的上下文以及更广泛的任务。然而，现有的文档理解基准主要局限于处理少量页面，并未能提供全面的布局元素定位分析。在本文中，我们首先定义了三个主要任务类别：长文档理解、数值推理和跨元素定位，并提出了一种综合基准——LongDocURL，该基准集成了上述三个主要任务，并包含20个基于不同主要任务和答案证据分类的子任务。此外，我们开发了一个半自动构建管道，收集了2,325个高质量的问答对，涵盖了超过33,000页的文档，显著优于现有基准。随后，我们在26种不同的配置下对开源和封闭源模型进行了全面的评估实验，揭示了该领域的关键性能差距。 
...
**Abstract (ZH)**: 代码审查是软件开发中至关重要但要求高的一个方面，引起了对自动化审查评论的浓厚兴趣。传统的评论评价方法，主要基于文本相似度，面临着两个主要挑战：开源项目中人工编写评论的一致可靠性不一致，以及文本相似度与提高代码质量、检测缺陷等目标之间的弱关联性。

本研究通过引入一套新的评估标准，结合先前研究和开发者访谈的结果，对基准评论进行了实证分析。我们随后重新评估现有的评价方法。我们的评价框架DeepCRCEval结合了人类评估者和大型语言模型（LLMs），基于设定的标准对当前技术进行全面重新评估。此外，我们还引入了一种创新且高效的基线模型LLM-Reviewer，利用LLMs的少样本学习能力，进行目标导向的比较。

我们的研究揭示了文本相似度指标的局限性，发现基准评论中只有不到10%的高度适用于自动化。相比之下，DeepCRCEval能够有效地区分高质和低质评论，显示出更可靠的评价机制。将LLM评估者纳入DeepCRCEval显著提高了效率，减少了88.78%的时间和90.32%的成本。此外，LLM-Reviewer展示了在评论生成中聚焦实际任务目标的巨大潜力。 

---
# ICM-Assistant: Instruction-tuning Multimodal Large Language Models for Rule-based Explainable Image Content Moderation 

**Title (ZH)**: ICM-Assistant：基于指令调优的多模态大型语言模型在规则驱动的可解释图像内容审核中的应用 
...
**Abstract (ZH)**: 互联网上的争议性内容大量泛滥，侵犯了各种文化和规范标准以及儿童保护标准。传统的图像内容审核（ICM）模型在生成适用于多种标准的精准审核决策方面存在局限性，而最近引入的多模态大型语言模型（MLLMs），当应用于通用规则导向的ICM时，往往会产生与人类审核员不符的分类和解释结果。为实现灵活、可解释和准确的ICM，我们设计了一个新型的基于规则的数据集生成管道，将简洁的人类定义规则分解，并利用精心设计的多阶段提示来丰富简洁明了的图像标注。我们的ICM-Instruct数据集包括详细的审核解释和审核问答对。基于此，我们在基于规则的ICM框架下创建了我们的ICM-Assistant模型，使其便于在实际应用中使用。我们的ICM-Assistant模型表现出卓越的性能和灵活性。具体而言，它在各种来源上显著优于现有方法，一致地提高了审核分类（平均提升36.8%）和审核解释质量（平均提升26.6%），并且优于现有的MLLMs。代码/数据可在以下链接获取：这个https URL。 

---
# VLABench: A Large-Scale Benchmark for Language-Conditioned Robotics Manipulation with Long-Horizon Reasoning Tasks 

**Title (ZH)**: VLABench：一种用于语言条件化机器人 manipulation 任务的大型基准，包含长期预测推理任务 
...
**Abstract (ZH)**: 通用型体现型智能体旨在理解用户的自然指令或意图，并精确地行动以完成通用任务。近年来，基于基础模型的方法，尤其是视觉-语言-动作模型（VLAs），在解决语言条件下的操作任务（LCM）方面展示了巨大的潜力。然而，现有的基准测试并不能充分满足VLAs及其相关算法的需求。为了在这种语境下更好地定义通用型任务，并推进在VLAs方面的研究，我们提出了VLABench——一个开源的基准测试，用于评估通用LCM任务学习。VLABench提供了100个精心设计的任务类别，每个类别都具有强大的随机化处理，并包含超过2000个物体。与之前的基准测试相比，VLABench在四个方面具有独特优势：1）需要世界知识和常识转移的任务；2）自然语言指令，包含隐含的人类意图，而非模板；3）要求多步推理的长期 horizon 任务；4）同时评估行动策略和语言模型能力。该基准测试评估多个技能，包括对网格与纹理的理解、空间关系、语义指令、物理定律、知识转移与推理等。为了支持下游微调，我们提供了一组高质量的训练数据，这些数据通过结合启发式技能和先验信息的自动化框架收集而来。实验结果显示，当前最先进的预训练VLAs和基于VLM的工作流程都面临一定的挑战。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在建模文本序列中的隐含关系方面取得了显著进展，这为生命科学领域带来了独特的机遇。生命科学领域中拥有来自多种物种的大规模单细胞组学数据集，为训练基础模型提供了坚实的基础。然而，不同物种之间的数据规模差异构成了发展的障碍，阻碍了跨物种遗传数据分析综合模型的构建。在这项研究中，我们提出了一种创新的混合方法，结合了LLMs的通用知识能力和针对单细胞组学数据的领域特定表示模型，以实现跨物种遗传数据分析。我们首先以基因作为表示的基本单元，并利用成熟语言模型（如LLaMA-2）的功能描述来初始化基因表示。通过输入单细胞基因水平表达数据并使用提示，我们有效地基于不同物种和细胞类型中基因差异表达水平构建了细胞表示。在实验中，我们构建了来自人类和小鼠的发育细胞，并针对难以注释的细胞群进行研究。我们通过基本任务（如细胞注释和可视化分析）评估了我们的方法，结果表明与使用LLMs的其他方法相比，我们的方法在准确性和互操作性方面具有显著改进。我们的混合方法增强了单细胞数据的表示，并为跨物种遗传分析的未来研究提供了稳健的框架。 

---
# GeneSUM: Large Language Model-based Gene Summary Extraction 

**Title (ZH)**: GeneSUM：基于大语言模型的基因摘要提取 
...
**Abstract (ZH)**: 生物医学研究领域新兴的主题不断扩展，提供了大量关于基因及其功能的信息。这种知识的快速膨胀为科学研究带来了前所未有的发现机会，同时也给试图跟上最新进展的研究人员带来了严峻挑战。其中一个显著挑战是导航庞大的文献库以提取关键的基因相关信息，这是一个耗时且繁琐的任务。为了提高这一过程的效率，必须解决几个关键挑战：（1）文献量庞大，（2）基因功能复杂，以及（3）自动整合与生成。为此，我们提出了一种双重自动化基因摘要提取器——GeneSUM，它利用了一个大规模语言模型（LLM）。我们的方法首先检索并消除目标基因文献中的冗余信息，然后微调LLM以优化和简化摘要生成过程。我们进行了广泛实验以验证我们所提出框架的有效性。结果表明，LLM显著增强了基因特定信息的整合，从而在持续的研究中提高了决策的效率。 
...
**Abstract (ZH)**: 社交媒体平台上人工生成文本（AIGTs）的比例正在快速增长。然而，AIGTs 的滥用可能会对公众舆论产生深远影响，例如传播虚假信息和操纵叙事。尽管这一点十分重要，但缺乏对社交媒体上AIGTs普遍性的系统性研究。为弥补这一空白，本文旨在量化、监控和分析社交媒体上的AIGTs。首先，我们从Medium、Quora和Reddit等3个主要社交平台收集了一个包含大约240万条帖子的数据集（SM-D）。然后，我们构建了一个多样化的数据集（AIGTBench）来训练和评估AIGT检测器。AIGTBench结合了流行的开源数据集以及由12个LLM生成的来自社交媒体文本的数据集，作为评估主流检测器的标准基准。基于此设置，我们确定了表现最佳的检测器（OSM-Det）。接着，我们将OSM-Det应用于SM-D，以追踪AIGTs随时间的变化趋势，并观察从2022年1月到2024年10月期间不同社交媒体平台上AIGT归属率（AAR）的变化趋势。具体来说，Medium和Quora的AAR显著增长，从1.77%增加到37.03%和从2.06%增加到38.95%。相比之下，Reddit的增长较为缓慢，AAR在同一时期从1.31%增加到2.45%。进一步分析表明，AIGTs在语言模式、话题分布、参与度水平和作者的关注者分布等多个维度上与人类撰写的文本有所不同。我们希望建立在社交媒体上的AIGTs分析和发现能够为该领域的未来研究提供洞见。 

---
# AEIOU: A Unified Defense Framework against NSFW Prompts in Text-to-Image Models 

**Title (ZH)**: AEIOU：面向文本到图像模型的不良内容提示统一防御框架 
...
**Abstract (ZH)**: 随着文本到图像（T2I）模型的不断进步和广泛应用，它们相关安全问题的重要性日益凸显。恶意用户经常利用这些模型通过有害或对抗性提示生成不适合工作环境（NSFW）的图像，这突显了确保模型输出完整性和合规性需要建立牢固防护措施的必要性。当前的内部防护措施往往会降低图像质量，而外部检测方法通常准确性较低且效率低下。

在本文中，我们介绍了一种名为AEIOU的防御框架，它适应性强、高效、可解释、可优化且统一，适用于对抗NSFW提示的T2I模型。AEIOU从模型文本编码器的隐藏状态中提取NSFW特征，并利用这些特征的可分离性来检测NSFW提示。检测过程高效，所需推理时间极少。AEIOU还提供了实时的检测结果解释，并支持通过数据增强技术进行优化。该框架具有通用性，可以适应各种T2I架构。我们的广泛实验表明，AEIOU在性能上显著优于商业和开源的审核工具，在所有数据集上的准确率均超过95%，并提高了至少十倍的效率。它有效地对抗适应性攻击，并在少量样本和支持多标签场景中表现出色。 

---
# Generating Traffic Scenarios via In-Context Learning to Learn Better Motion Planner 

**Title (ZH)**: 通过上下文学习生成交通场景以改进运动规划器 
...
**Abstract (ZH)**: 自主驾驶中的运动规划是其核心技术之一。最先进的运动规划器都是通过详尽标注的数据集进行训练的，这些数据集不仅标注成本高昂，而且难以涵盖罕见的但至关重要的场景。未能考虑到这些问题可能会对运动规划器造成严重风险，并可能导致测试中的事故。一种直观的解决方案是通过编程和执行仿真器（例如CARLA）手动组合这些场景。然而，这种方法会产生大量的人力成本。鉴于此，我们提出了一种低成本的方法来生成多样化的关键交通场景，进而训练更加鲁棒的运动规划器。首先，我们将交通场景表示为脚本，这些脚本随后被仿真器用来生成交通场景。接下来，我们开发了一种方法，该方法接受用户指定的文本描述，使用上下文学习将这些描述翻译成脚本。生成的脚本随后发送给仿真器，从而产生相应的交通场景。由于我们的方法可以生成大量安全关键的交通场景，我们使用它们作为运动规划器的合成训练数据。为了展示生成场景的价值，我们在我们的合成数据、真实世界数据以及两者组合的数据上训练现有的运动规划器。实验结果显示，使用我们数据训练的运动规划器显著优于仅使用真实世界数据进行训练的运动规划器，这证明了我们合成数据的价值及其生成方法的有效性。我们的源代码可在以下链接获取：[这里填写链接]。 
...
**Abstract (ZH)**: 随着基础模型和视觉语言模型的进步，以及有效微调技术的发展，已经开发出了多种通用和专用模型，用于各种视觉任务。尽管这些模型具有高度的灵活性和易用性，但没有一个模型能够满足所有潜在用户可能设想的所有任务和/或应用需求。最近的一些方法，如视觉编程和集成工具有关的多模态大语言模型，旨在通过程序合成来应对复杂视觉任务，但这些方法忽略了用户约束（例如，性能/计算需求），产生难以部署的特例解决方案，并且有时可能需要底层指令，这些指令超出了新手用户的操作能力。为了解决这些局限性，我们引入了MMFactory，这是一种通用框架，包含模型和度量路由组件，类似于一个解决方案搜索引擎，能够跨各种可用模型进行搜索。基于任务描述、少量样本输入输出对（可选地还包括资源和/或性能约束），MMFactory 可以通过实例化和组合其模型库中的视语工具来提出多元的程序解决方案。除了合成这些解决方案外，MMFactory 还提出了一种度量标准并评估性能/资源特性，使用户可以根据其独特的设计约束来选择合适的解决方案。从技术角度看，我们还引入了一种基于多智能体大语言模型对话的委员会解决方案提议者，利用多智能体对话生成可执行、多元化、通用性和鲁棒性的解决方案。实验结果表明，MMFactory 能够通过提供针对用户问题规格量身定制的最先进的解决方案，超越现有方法。项目页面可通过此链接访问：[项目页面链接]。 

---
# Lla-VAP: LSTM Ensemble of Llama and VAP for Turn-Taking Prediction 

**Title (ZH)**: Lla-VAP：Llama和VAP的LSTM集成模型用于轮流预测 
...
**Abstract (ZH)**: 交替发言预测是指预测对话中说话者何时将发言机会让给另一发言者开始说话的任务。本项目在此基础上，通过采用多模态集成方法，结合大型语言模型（LLMs）和语音活动投影（VAP）模型，进一步扩展了现有的交替发言预测策略。通过将LLMs的语言能力与VAP模型的时间精确性相结合，我们旨在提高在有脚本和无脚本对话场景中识别转换点（turn-replacement points，TRPs）的准确性和效率。我们的方法在In-Conversation Corpus (ICC) 和 Coached Conversational Preference Elicitation (CCPE) 数据集上进行评估，突显了现有模型的优势和局限性，并提出了一个潜在更稳健的框架，以增强预测能力。 

---
# Emoji Retrieval from Gibberish or Garbled Social Media Text: A Novel Methodology and A Case Study 

**Title (ZH)**: 从 gibberish 或乱码社交媒体文本中检索Emoji：一种新的方法论及案例研究 
...
**Abstract (ZH)**: 本文阐述了一种广泛应用于社交媒体平台的表情符号的使用情况，但这些表情符号往往在噪声或变形的文字中丢失，给数据解析和机器学习带来了挑战。传统的预处理方法建议移除此类文本，这可能导致表情符号及其上下文意义的丢失。本文提出了一种三步反向工程方法，以从社交媒体帖子中的变形文本中检索表情符号。该方法还识别了在社交媒体数据挖掘过程中产生此类文本的原因。为了评估其有效性，该方法被应用于关于猴痘疫情（Mpox outbreak）的509,248条推文数据集，该数据集曾被约30篇先前的研究工作引用，但未能从变形文本中检索出表情符号。我们的方法从76,914条推文中检索出了157,748个表情符号。通过使用诸如Flesch阅读流畅度、Flesch-Kincaid 年级水平、Coleman-Liau索引、Automated Readability指数、Dale-Chall可读性评分、Text Standard和阅读时间等指标，展示了文本的可读性和连贯性的改善。此外，还分析了这些推文中各个表情符号的频率及其使用模式，并呈现了结果。 

---
# Theoretical Constraints on the Expressive Power of $\mathsf{RoPE}$-based Tensor Attention Transformers 

**Title (ZH)**: 基于RoPE的张量注意力变换器的表达能力理论约束 
...
张量注意机制通过在多种模态间捕获高阶相关性，扩展了传统的注意力机制，解决了基于矩阵的经典注意力机制的局限性。同时，旋转位置嵌入（$\mathsf{RoPE}$）在长上下文场景中展示了在编码位置信息方面的优越性能，显著提升了Transformer模型的表征能力。尽管这些方法在实际应用中取得了显著的成功，但它们的理论限制仍然未被广泛探索。在本研究中，我们分析了张量注意机制及基于$\mathsf{RoPE}$的张量注意机制的电路复杂性，表明在多项式精度、常数深度层以及线性或次线性隐藏维度的假设下，它们无法解决固定成员资格问题或$(A_{F,r})^*$闭包问题（在$\mathsf{TC}^0 \neq \mathsf{NC}^1$的假设下）。这些发现突显了张量注意机制及基于$\mathsf{RoPE}$的张量注意机制Transformer在实验性能和理论限制之间存在的差距，为指导更加理论化的方法设计和扩展Transformer模型提供了洞见。 
...
**Abstract (ZH)**: 尽管大型语言模型（LLMs）在处理和生成文本数据方面展现了显著的能力，但它们的预训练词汇表不适于捕捉时间序列中内在的微妙的时间动态和模式。这些词汇表设计用于表示的是自然语言符号的离散性质，这与时间序列数据的连续数值性质并不一致。为了弥补这一根本性限制，我们提出了 VITRO 方法。该方法借鉴了视觉-语言领域的文本反转优化技术，旨在学习一种针对每个数据集的新型时间序列词汇表，以弥合自然语言离散、语义性质与时间序列数据连续、数值性质之间的差距。我们证明，可学习的时间序列特定伪词嵌入相比现有的通用语言模型词汇表更能表示时间序列数据，而使用 VITRO 方法增强的方法在大多数数据集上实现了长期预测的最先进性能。 

---
# A Multimodal Emotion Recognition System: Integrating Facial Expressions, Body Movement, Speech, and Spoken Language 

**Title (ZH)**: 多模态情感识别系统：融合面部表情、身体动作、语音和口头语言 
...
**Abstract (ZH)**: 传统的心理评估主要依赖于人类的观察和解释，这些方法容易受到主观性、偏见、疲劳和不一致性的影响。为了解决这些问题，本研究提出了一种多模态情绪识别系统，以提供一种标准化、客观和数据驱动的工具，支持如心理学家、精神病学家和临床医生等评估者。该系统结合了面部表情识别、语音识别、口语语言分析和身体动作分析，以捕捉人类评估中经常被忽视的细微情绪线索。通过结合这些模态，系统可以提供更为稳健和全面的情绪状态评估，从而降低误解和过度诊断的风险。初步在模拟现实环境中的测试表明，该系统有可能提供可靠的情感洞察，从而提高诊断准确性。本研究强调了自动化多模态分析在传统心理评估实践中的巨大潜力，以及其在临床和治疗环境中的广泛应用。 

---
# Bridging the Data Provenance Gap Across Text, Speech and Video 

**Title (ZH)**: 跨越文本、语音和视频的数据来源差距bridging text, speech, and video data provenance gaps 
...
**Abstract (ZH)**: 人工智能的进步主要依赖于训练数据的数量和质量。尽管如此，关于广泛使用的数据集属性的实证分析仍然不足，特别是针对文本之外的数据集。在这项工作中，我们对跨模态——流行的文本、语音和视频数据集——进行了迄今为止最大规模且前所未有的纵向审计，涵盖了数据集的详细来源趋势和使用限制，以及其地理位置和语言代表性。我们的手工分析涉及1990年至2024年间近4000个公共数据集，涉及608种语言、798个数据源、659个组织和67个国家。

我们发现，多模态机器学习应用在过去几年中越来越多地转向网页抓取、合成和社交媒体平台，如YouTube，作为其训练集的主要来源，自2019年以来已经超越了所有其他来源。其次，在追溯数据集的来源链时，我们发现虽然只有不到33%的数据集受到严格的版权限制，但被广泛使用的数据集中的源内容超过80%带有非商业使用限制。最后，尽管公共AI训练数据集代表的语言和地理数目在过去几年不断增加，我们的审计结果显示，相对的地理和多语言代表性在2013年以来的覆盖范围并未显著改善。

我们认为，通过此次审计的工作范围，能够对数据来源、限制和西方中心主义趋势进行系统的实证研究，并且对这些问题的透明度是负责任AI进步的关键。作为提高数据集透明度和负责任使用的一份贡献，我们发布了整个多模态审计报告，以便从业者可以追踪文本、语音和视频数据的来源。 

---
# Ensemble Machine Learning Model for Inner Speech Recognition: A Subject-Specific Investigation 

**Title (ZH)**: 针对个体特点的内部语言识别集成机器学习模型研究 
...
**Abstract (ZH)**: 近年来，内言语识别受到了广泛关注，其应用领域包括康复、辅助技术开发和认知评估等。然而，由于语言和言语产生是一个复杂的过程，识别言语成分仍然是一个挑战。先前采取了不同的方法来达成这一目标，但仍有新的方法值得探索。此外，基于主题的分析对于了解内言语产生过程中的大脑动力学至关重要，这可以为神经科学研究带来新的方法。已经在西班牙人群中使用了一个公开可用的数据集——Talking Out Loud Dataset，该数据集使用128通道表面脑电图（EEG）信号来开发机器学习（Machine Learning, ML）技术以分类内言语。研究对象在说四个单词（上、下、右、左）时收集了数据。统计方法被用于检测并去除脑电信号中的运动伪影。从时域、频域和时频域中提取了大量的特征（每个通道191个）。探索了八种特征选择算法，并选择最佳特征选择技术进行后续评估。评估了六种机器学习算法的性能，并提出了一种集成模型。还探索了深度学习（Deep Learning, DL）模型，并将其结果与传统的机器学习方法进行了比较。所提出的集成模型通过堆叠五个最佳逻辑回归模型，在使用表面EEG信号分类四个内言语词时获得了81.13%的整体准确率和81.12%的F1分数。提出的数据框架与提出的经典机器学习模型的集成显示了使用表面EEG信号分类内言语的潜力。 

---
# Large Language Model Safety: A Holistic Survey 

**Title (ZH)**: 大型语言模型安全：一项全面综述 
...
**Abstract (ZH)**: 大型语言模型（LLM）的迅速发展和部署已为人工智能领域带来了一个全新的前沿领域，其具备前所未有的自然语言理解和生成能力。然而，这些模型日益被整合到关键应用中，引发了重大安全 concerns，因此需要彻底评估其潜在风险及其缓解策略。

本文综述了当前LLM安全的总体状况，涵盖四大类别：价值错配、对抗性攻击的鲁棒性、滥用以及自主人工智能风险。除了对这四个方面缓解方法和评估资源的全面回顾外，我们还探讨了四个与LLM安全相关的主题：LLM代理的安全影响、可解释性在增强LLM安全中的作用、一揽子人工智能公司和机构为LLM安全提出的和遵循的技术路线图，以及旨在保障LLM安全的人工智能治理政策，其中包括国际合作建议、政策提案以及未来监管方向的讨论。

我们的研究结果强调了对LLM安全采取主动和多方面方法的必要性，重视技术解决方案、伦理考量以及稳健治理框架的集成。本文旨在为学术研究人员、行业从业人员和政策制定者提供基础资源，阐明安全集成LLM所面临的挑战和机遇。最终，本文旨在促进LLM安全可控的发展，符合利用人工智能推动社会进步和福祉的总体目标。相关论文的列表已在此 [https://example.com] 公开供公众查阅。 
