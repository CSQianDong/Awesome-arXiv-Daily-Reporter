...
**Title (ZH)**: AutoSculpt：一种基于模式的模型自剪枝框架，结合强化学习与图学习 
...
**Abstract (ZH)**: 大型语言模型（LLMs）广泛应用于诸如问答和机器翻译等自然语言处理任务。然而，由于缺乏标注数据以及生化性质的手动标注难度大，LLMs 在分子生成任务中的性能仍然受到限制，尤其是在涉及多性质约束的任务中。本工作中，我们提出了一种两步框架 PEIT（Property Enhanced Instruction Tuning），以改进LLMs在与分子相关的任务表现。在第一步中，我们使用文本描述、SMILES 和生化性质作为多模态输入，预训练一个名为 PEIT-GEN 的模型，通过对齐多模态表示以合成指令数据。在第二步中，我们将现有的开源LLMs 与合成数据进行微调，所得到的 PEIT-LLM 可以处理分子描述、基于文本的分子生成、分子性质预测以及我们新提出的多约束条件的分子生成任务。实验结果表明，我们预训练的 PEIT-GEN 在分子描述任务中优于 MolT5 和 BioT5，展示了文本描述、结构和生化性质之间的模态对齐良好。此外，PEIT-LLM 在多任务分子生成中表现出显著的提升，证明了 PEIT 框架在各种分子任务中的可扩展性。我们在此 https://github.com/... 释放了代码、构建的指令数据和模型检查点。 
...
**Title (ZH)**: 开放科学在人工智能领域的非凡有效性：一项复制研究 
...
**Abstract (ZH)**: 我们研究由Kleinberg和Mullainathan在[KM24]中引入的在极限下生成语言的方法，该方法基于Gold的经典工作[Gol67]和Angluin的经典工作[Ang79]。[KM24]提出了一种算法，可以从任何可数的语言集合中生成字符串。虽然该算法最终会输出目标语言$K$中的字符串，但它牺牲了广度，即生成$K$中所有字符串的能力。[KM24]中的一个关键开问题在于这种一致性和广度之间的权衡是否不可避免。
...
**Title (ZH)**: 自动驾驶车辆中联合自适应OFDM与强化学习设计：利用更新时延 
...
**Abstract (ZH)**: 基于毫米波（mmWave）的正交频分复用（OFDM）技术在高分辨率传感和高速数据传输方面表现出色，是合适的替代方案。为了满足通信和传感需求，许多研究提出了静态配置，其中波的超参数，如每个帧中的符号数和通信时间槽中的帧数已经预先定义。然而，两个事实迫使我们必须重新定义问题：（1）环境通常是动态和不确定的；（2）mmWave严重受无线环境的影响。这一挑战在自主驾驶车辆（AV）系统中表现尤为突出。该系统利用mmWave进行集成传感与通信（ISAC），以管理和适应环境变化。本研究考虑了一个AV网络，其中AV结合队列状态信息（QSI）和信道状态信息（CSI）与强化学习技术来管理通信和传感。这使AV能够实现两个主要目标：与其他AV建立稳定的通信链路，并以高分辨率准确估计周围物体的速度。因此，通信性能的评估基于队列状态、有效数据速率和丢弃的包速率。相比之下，传感效果的评估使用速度分辨率。此外，我们采用自适应OFDM技术进行动态调制，建议一种奖励函数，利用更新的年龄来处理通信缓冲区并提高传感效果。系统通过优势演员-评论家算法（A2C）和近端策略优化（PPO）进行验证。此外，我们将我们的解决方案与现有设计进行比较，并通过计算机仿真展示了其优越性能。 

---
# How "Real" is Your Real-Time Simultaneous Speech-to-Text Translation System? 

**Title (ZH)**: 你的实时同步语音转文字翻译系统有多“真实”？ 
...
**Abstract (ZH)**: 同时进行语音到文本翻译（SimulST）是在演讲者发言的同时将其源语言语音翻译成目标语言文本，以确保低延迟，从而提高用户的理解能力。尽管SimulST旨在处理无限制的语音，但大多数研究都集中在人类预先分段的语音上，简化了任务并忽视了重要的挑战。这种狭窄的研究焦点以及广泛的术语不一致，限制了研究结果在实际应用中的适用性，最终妨碍了该领域的发展。我们对110篇论文的广泛文献综述不仅揭示了当前研究中的关键问题，也为我们的主要贡献奠定了基础。我们1）定义了SimulST系统的步骤及其核心组件，提出了标准化的术语和分类体系；2）对社区趋势进行了深入分析；3）提供了具体的建议和未来方向，以弥合现有文献中的空白，从评估框架到系统架构，推动领域向更现实和有效的SimulST解决方案发展。 
...
**Title (ZH)**: 现有语音数据集在训练用于集体解决问题的机器学习模型方面的适用性综述与讨论 
...
**Abstract (ZH)**: 本报告分析了现有数据集适用于设计新的机器学习模型、决策方法和分析算法以改进协作解决问题的能力，并罗列了未来所需数据集的要求。假设问题解决是在约三至四人的团队中进行的，团队成员之间相互交流。该数据集包含此类团队的语音录制。分析方法基于能够捕捉认知活动、社交活动和情绪状态的指标。报告对大量用于语音理解的研究领域（该领域与协作解决问题有一些相似之处）的数据集进行了分析。 

---
# GeFL: Model-Agnostic Federated Learning with Generative Models 

**Title (ZH)**: GeFL：基于生成模型的无模型依赖联邦学习 
...
**Abstract (ZH)**: 联邦学习（FL）是一种在保持用户隐私的同时进行分布式学习的有前途范式。然而，近期模型的不断增大使得少数用户承担整个模型变得不可行。这导致用户采用基于不同计算能力和网络带宽的异质模型。鉴于FL通常涉及训练单一全局模型，异质模型下的联邦学习应予以关注。在本文中，我们提出了一种名为生成模型辅助联邦学习（GeFL）的方法，结合了一个生成模型，该模型可以汇总异质模型用户之间的全局知识。在各种分类任务实验中，GeFL相比基准方法表现出显著的性能提升，并在隐私和可扩展性方面存在一定的限制。为应对这些挑战，我们引入了一种新的框架GeFL-F。该框架通过特征生成模型辅助目标网络的训练。通过实验证明，GeFL-F能够保持一致的性能提升，并且在隐私保护和对大量客户端的鲁棒性方面表现出色。相关代码在 [1] 中提供。 

---
# SoK: On the Offensive Potential of AI 

**Title (ZH)**: SoK：人工智能的进攻潜力分析 
...
**Abstract (ZH)**: 我们的社会越来越多地受益于人工智能（AI）。不幸的是，越来越多的证据表明，AI 也被用于攻击性目的。已有研究表明，AI 的部署可能导致安全和隐私目标的侵犯。然而，目前没有任何研究能够系统性地描绘出 AI 的攻击潜力。在本文综述论文中，我们旨在为系统性分析攻击性 AI 的异质能力奠定基础。具体而言，我们将做到以下几点：(i) 考虑 AI 对人类和系统的影响，(ii) 整合和提炼学术文献、专家意见、工业领域以及普通公众（这些都是有关攻击性 AI 的有价值信息来源）的知识。

为了使这些多样化的知识来源能够对齐，我们制定了一个反映有关攻击性 AI 的关键技术因素的共同标准。借助这些标准，我们系统性地分析了以下内容：95篇研究论文；38篇信息安全简报（例如，来自 BlackHat 的）；涉及549名具有不同背景和专业知识的个人的用户研究回应；以及12位专家的意见。我们的贡献不仅揭示了 AI 今天可以被攻击性使用的令人担忧的方式（其中一些方式在先前的研究中被忽视），而且还为未来几十年应对这一威胁提供了一个立足点。 
...
**Abstract (ZH)**: 检索增强生成系统依赖于有效的文档检索能力。设计上，传统的稀疏检索器或密集检索器在多跳检索场景中面临诸多挑战。在本文中，我们提出了一种名为GeAR的新方法，通过两项关键创新提高了RAG（Retrieval-Augmented Generation）的性能：(i) 图扩展，可以增强任何传统的基线检索器，如BM25；(ii) 一个融合图扩展的代理框架。我们的评估结果显示，GeAR在三个多跳问答数据集上表现出优越的检索性能。此外，我们的系统在具有挑战性的MuSiQue数据集上达到了最先进的结果，相较其他多步检索系统，GeAR在性能提升超过10%的情况下，所需的词汇量和迭代次数更少。 

---
# Multilingual Mathematical Reasoning: Advancing Open-Source LLMs in Hindi and English 

**Title (ZH)**: 多语言数学推理：推动印地语和英语开源大规模语言模型的发展 
...
**Title (ZH)**: TPAoI：在计算优先网络中确保边缘网络的服务状态新鲜性 
...
**Abstract (ZH)**: 在计算优先网络中，保持网络边缘最新的准确状态信息对于有效访问远程服务至关重要。这一过程通常涉及三个阶段：状态更新、用户访问和用户请求。然而，目前关于状态有效性的研究，如查询时延（Query Age of Information, QAoI），并没有全面涵盖这些阶段。因此，本文提出了一种新的度量指标TPAoI，旨在通过测量服务状态的新鲜度来优化更新决策。边缘环境的随机性，表现为更新、请求和用户访问时间的不可预测的通信延迟，为建模带来了重大挑战。为了应对这一挑战，我们将问题建模为马尔可夫决策过程（MDP），并采用双深Q-网络（Dueling Double Deep Q-Network, D3QN）算法进行优化。详尽的实验表明，提出的TPAoI度量指标有效减小了时延（Age of Information, AoI），确保了动态边缘环境中的及时和可靠服务更新。结果表明，与QAoI指标相比，TPAoI将AoI平均减少了47%，与传统的AoI指标相比，更新频率平均减少了48%，显示出显著的改进。 

---
# RDPM: Solve Diffusion Probabilistic Models via Recurrent Token Prediction 

**Title (ZH)**: RDPM：通过递归Token预测解决 diffusion概率模型 
...
**Abstract (ZH)**: 本文探讨了进化多目标优化（EMO）中的理论问题，并 focuses 于交叉算子在多目标优化中的作用。虽然使用交叉算子的优势尚未得到充分理解，且关于交叉算子的严格运行时分析远远落后于其在实际中的应用，尤其是在多于两个目标的情况下。我们提出了一类多目标问题，并对广泛应用的 NSGA-III 算法进行了理论上的运行时分析，以证明在使用交叉算子时可以实现指数级的加速。特别是，该算法在使用交叉算子时可以在期望多项式时间内发现 Pareto 集，而在不使用交叉算子的情况下，甚至要找到一个 Pareto 最优解都需要指数级的时间。据我们所知，这是第一次在多目标优化中通过严格的运行时分析证明，在多于两个目标的情况下使用交叉算子可以获得指数级的性能差距。 
...
**Title (ZH)**: 揭开欺诈团伙对图神经网络威胁的面纱：基于图神经网络的欺诈检测器的多目标图注入攻击 
...
**Abstract (ZH)**: 近年来的研究表明，超图神经网络（Hypergraph Neural Networks，HGNNs）对对抗攻击极为敏感。现有方法主要关注基于梯度的超图修改攻击，忽视了超图中的节点跨越以及超边的群体身份，导致攻击效果有限且容易被检测出来。本文提出了一个新的框架，即通过向精英超边注入同质节点的对抗攻击（IE-Attack），以应对这些挑战。首先，利用超图中的节点跨越特性，我们提出了一种精英超边采样器来识别要注入的超边。其次，我们利用核密度估计（KDE）提出了一种节点生成器，生成具有超边群体身份的同质节点。最后，通过将同质节点注入精英超边，IE-Attack 提高了攻击性能并增强了攻击的不可察觉性。我们在五个真实数据集上进行了广泛的实验，以验证 IE-Attack 的有效性及其对现有先进方法的优越性。 
...
**Title (ZH)**: 点DeepONet：结合PointNet的深度算子网络，用于非参数化3D几何和载荷条件的非线性分析 
...
**Abstract (ZH)**: 工程领域中的非线性结构分析通常需要大量的有限元仿真，这限制了其在设计优化、不确定性量化和实时控制中的应用。传统的基于深度学习的代理模型，如卷积神经网络（CNNs）、物理信息神经网络（PINNs）和傅里叶神经算子（FNOs），在处理复杂非参数三维（3D）几何结构、方向性变化负载以及无结构网格上的高保真预测时面临挑战。本研究提出了基于操作学习的代理模型——Point-DeepONet，该模型将PointNet集成到DeepONet框架中。Point-DeepONet可以直接处理非参数点云数据，并通过引入符号距离函数（SDF）来结合几何上下文，从而在无需网格参数化或重新训练的情况下准确预测三维位移和von Mises应力场。使用大约5,000个节点（原始200,000节点网格的2.5%）的数据训练后，Point-DeepONet仍然能够在高保真度下预测整个网格：在水平负载情况下，其位移的决定系数可达0.987，von Mises应力的决定系数达0.923。相比之下，进行类似的非线性有限元分析需要大约19.32分钟，而Point-DeepONet可以提供几秒钟内的预测结果，大约快400倍，同时随着数据集大小的增加，该模型保持了出色的可扩展性和准确性。这些成果表明，Point-DeepONet有可能实现快速且高保真的结构分析，从而在复杂工程工作流程中支持更有效的设计探索和明智决策。 

---
# Addressing Spatial-Temporal Data Heterogeneity in Federated Continual Learning via Tail Anchor 

**Title (ZH)**: 通过尾部锚定方法解决联邦持续学习中时空数据异质性问题 
...
**Abstract (ZH)**: 自监督图表示学习（SSGRL）是一种用于减少或避免手动标注的表示学习范式。SSGRL的核心部分是图数据增强。现有方法通常依赖于通过试验和错误识别出的启发式方法，并且其有效性往往仅限于某些应用领域。此外，目前尚不清楚一种启发式方法为什么比另一种更好。近年来的研究还反对一些技术（例如，dropout，它可能会改变分子图的属性或破坏基于图的文档分类任务的相关信号）。

在本研究中，我们提出了一种基于数据的新型SSGRL方法，能够自动从图中编码的信号（即节点的预测特征和拓扑信息）中学习合适的图增强方法。我们提出了两个互补的方法，分别生成可学习的特征增强和拓扑增强。前者学习节点特征的多视图增强，后者学习高阶拓扑视图。此外，这些增强方法是与表示一同学习的。我们的方法具有通用性，既可以应用于同构图，也可以应用于异构图。我们在节点分类（使用九个同构和异构数据集）和图属性预测（使用另外八个数据集）方面进行了大量实验。实验结果表明，所提出的方法与当前最佳的SSGRL基线方法相当或优于这些基线方法，并且在性能上与部分监督方法相似。源代码已匿名化处理，可在以下链接获取：[ANONYMIZED_URL] 

---
# M-Ped: Multi-Prompt Ensemble Decoding for Large Language Models 

**Title (ZH)**: M-Ped：多提示 Ensemble 解码 для 大型语言模型
...
**Abstract (ZH)**: 随着大型语言模型（LLMs）在自然语言处理（NLP）领域的广泛应用，提升其性能已成为研究热点。本文提出了一种新颖的多提示ensemble解码方法，旨在通过利用多个提示生成结果的聚合来增强LLMs的生成质量。对于一个唯一的输入$X$，我们以批处理模式提交$n$种不同格式的$X$提示，通过解码并推导概率分布。对于每个词项预测，我们通过计算批处理内$n$个概率分布的平均值来计算ensemble概率，并利用该聚合概率生成词项。这种方法被称为内批ensemble。为了促进高效的批处理推理，我们采用左填充策略，以保持$n$个提示之间的输入长度一致。通过在机器翻译、代码生成和文本简化等多种NLP任务上的广泛实验，我们展示了该方法在提升LLMs性能方面的有效性。结果显示，该方法在BLEU评分、pass@$k$率和LENS指标方面相较于传统方法有显著的提升。 

---
# Quo Vadis, Anomaly Detection? LLMs and VLMs in the Spotlight 

**Title (ZH)**: 《 anomaly detection 前途何方？来自预训练语言模型和多模态语言模型的关注》
...
**Abstract (ZH)**: 视频异常检测（VAD）通过集成大语言模型（LLMs）和视觉-语言模型（VLMs），在可解释性、时间推理和动态开放世界场景中的泛化能力方面取得了显著进展，解决了关键挑战。本文对2024年的LLM-/VLM-基于的方法进行了深入综述，重点关注四个方面：（i）通过语义洞察和文本解释增强可解释性，使视觉异常更加易于理解；（ii）捕捉复杂的时序关系，以检测和定位视频帧中的动态异常；（iii）实现少样本和零样本检测，减少对大规模标注数据集的依赖；（iv）通过使用语义理解和运动特征解决开放世界和类无差别异常，以实现时空一致性。我们强调这些方法有望重新定义VAD的格局。此外，我们探讨了LLMs和VLMs提供的视觉和文本模态之间的协同作用，突显它们的综合优势，并提出未来方向，以充分利用这些模型在增强视频异常检测方面的潜力。 

---
# Navigating Data Corruption in Machine Learning: Balancing Quality, Quantity, and Imputation Strategies 

**Title (ZH)**: 在机器学习中导航数据损坏问题：平衡质量、数量与插补策略 
...
**Abstract (ZH)**: 数据损坏，包括缺失数据和噪声数据，对现实世界中的机器学习构成了重大挑战。本研究探讨了数据损坏对模型性能的影响，并通过两个实验设置探索了缓解这些影响的策略：自然语言处理任务的监督学习（NLP-SL）和用于交通信号优化的深度强化学习（Signal-RL）。我们分析了数据损坏程度与模型性能之间的关系，评估了数据插补方法的有效性，并评估了扩大数据集以应对数据损坏的实用性。
...
这些研究发现为数据预处理、插补策略和数据收集实践提供了实际指导，有助于在嘈杂环境中构建稳健的机器学习系统。 

---
# DeepCRCEval: Revisiting the Evaluation of Code Review Comment Generation 

**Title (ZH)**: DeepCRCEval: 重新评估代码审查评论生成的效果 
...
**Abstract (ZH)**: 代码审查是软件开发中至关重要但极具挑战性的一个方面，引发了对自动化审查意见的兴趣。传统上，这些审查意见的评估方法主要依赖于文本相似度，面临两大主要挑战：开源项目中人工撰写的审查意见可靠性不一致，以及文本相似度与提高代码质量、检测缺陷等目标之间的弱关联性。
...
研究结果揭示了文本相似度度量的局限性，发现少于10%的基准审查意见适合自动化。相比之下，DeepCRCEval能够有效地区分高质量和低质量的审查意见，证明其是一种更加可靠的评估机制。将LLM评估者纳入DeepCRCEval显著提高了效率，分别降低了88.78%的时间和90.32%的成本。此外，LLM-Reviewer展示了在评论生成中集中于实际目标的显著潜力。 

---
# Towards understanding how attention mechanism works in deep learning 

**Title (ZH)**: Towards Understanding How the Attention Mechanism Functions in Deep Learning 
...
**Abstract (ZH)**: 信用卡欺诈给持卡人和发卡银行造成了相当大的经济损失。现有的方法通常使用基于机器学习的分类器从带有标签的交易记录中检测欺诈行为。然而，由于标注数据的成本高昂，带有标签的数据在数十亿真实交易中所占比例通常较小，这意味着它们未能充分利用大量未标注数据中的自然特征。因此，我们提出了一种半监督图神经网络用于欺诈检测。具体而言，我们利用交易记录构建了一个时序交易图，该图由时序交易（节点）及其相互作用（边）组成。然后，我们通过门控时序注意力网络（GTAN）在节点间传递消息，学习交易表示。进一步地，我们通过交易间的风险传播模型化欺诈模式。我们在一个实际的交易数据集以及两个公开可用的欺诈检测数据集上进行了广泛的实验。实验结果显示，我们提出的方法（即GTAN）在三个欺诈检测数据集上优于其他最先进的基线方法。半监督实验进一步证明，即使使用少量的标注数据，我们的模型也表现出色的欺诈检测性能。 

---
# GenAI Content Detection Task 2: AI vs. Human -- Academic Essay Authenticity Challenge 

**Title (ZH)**: GenAI内容检测任务2：AI与人类之争——学术论文真实性挑战 
...
**Abstract (ZH)**: 本文全面概述了2025年共译世界语言学会议（COLING 2025）期间举办的学术论文真实性挑战赛的第一轮比赛。该挑战赛旨在检测用于学术目的的机器生成论文与人类撰写论文之间的差异。任务定义如下：“给定一篇论文，判断它是由机器生成的还是由人类撰写的。”该挑战包括两种语言：英语和阿拉伯语。在评估阶段，25支队伍提交了英语系统的解决方案，21支队伍提交了阿拉伯语系统的解决方案，反映了对这项任务的极大兴趣。最终，有七支队伍提交了系统描述论文。大多数提交的系统都使用了微调的变压器模型，有一支队伍使用了大型语言模型（LLMs），如Llama 2和Llama 3。本文概述了任务定义，详细描述了数据集的构建过程，并阐述了评估框架。此外，我们还介绍了参与队伍所采用的方法概述。几乎所有的提交系统都超越了基于n-克gram的基本基准，最佳系统在两种语言上的F1分数都超过了0.98，表明在检测机器生成文本方面取得了显著进展。 

---
# Sampling Bag of Views for Open-Vocabulary Object Detection 

**Title (ZH)**: 开放词汇物体检测中的视角采样词袋模型 
...
**Abstract (ZH)**: 现有的开放词汇对象检测（OVD）方法通过将物体区域嵌入与相应的视觉语言模型（VLM）特征对齐，来测试未见过的类别。最近的一项研究表明，VLM 暗含地学习了图像中语义概念的组分结构。这种方法不是使用单个区域嵌入，而是利用区域嵌入的“袋”（bag of region embeddings）作为新的表示形式，以整合组分结构到 OVD 任务中。然而，这种方法往往未能捕捉到每个区域的上下文概念，导致噪声组分结构，从而仅带来微小的性能改进和较低的效率。为了解决这个问题，我们提出了一种基于概念的对齐方法，通过采样更强大且高效的组分结构来改进这一过程。我们的方法将上下文相关“概念”分组到一个“袋”中，并调整“袋”内概念规模以实现更有效的嵌入对齐。结合 Faster R-CNN，我们的方法在开放词汇 COCO 和 LVIS 基准中对新类别的测评中实现了 2.6 个 box AP50 和 0.5 个 mask AP 的改进。此外，我们的方法在浮点运算量（FLOPs）上比之前的研究减少了 80.3%，显著提高了效率。实验结果表明，所提出的方法在 OVD 数据集上优于之前的最先进的模型。 

---
# Robust Semi-Supervised Learning in Open Environments 

**Title (ZH)**: 在开放环境下的稳健半监督学习 
...
**Title (ZH)**: 使用多层感知器和LSTM从语音信号特征检测与预报帕金森病进展 
...
**Abstract (ZH)**: 帕金森病的准确诊断，尤其是早期阶段，是一项具有挑战性的任务。机器学习技术的应用有助于提高帕金森病检测的诊断准确性，但仅有少数研究致力于疾病的进展预测。在本研究中，我们使用长短期记忆网络（LSTM）对帕金森病患者的语音信号中的诊断特征进行训练，以预测疾病进展；同时，我们使用多层感知机（MLP）对相同的诊断特征进行训练，以检测疾病。通过使用两种广为人知的特征选择方法（Relief-F和序列前向选择）选出的诊断特征应用于LSTM和MLP，结果显示，这些特征能够准确预测疾病进展至第二阶段和第三阶段，以及疾病的存在情况。 
...
**Abstract (ZH)**: 弗雷歇回归将线性回归扩展到度量空间中的复杂响应建模，使其特别适用于多标签回归，其中每个实例可以有多个相关标签。然而，该框架内的变量选择仍然研究不足。本文提出了一种新的变量选择方法，该方法采用隐式正则化而不是传统的显式正则化方法，后者可能会引入偏差。我们的方法可以有效地捕捉预测因子与响应之间的非线性交互作用，同时促进模型稀疏性。我们提供了理论结果来证明选择一致性，并通过数值示例展示了该方法的性能。 

---
# An Automatic Graph Construction Framework based on Large Language Models for Recommendation 

**Title (ZH)**: 基于大型语言模型的自动图构建框架用于推荐系统 
...
**Abstract (ZH)**: 区分空间关系是人类认知的基本部分，这需要对跨实例的细腻感知。尽管像MME、MMBench和SEED这样的基准已经综合评估了各种能力，包括视觉空间推理（VSR），但专门针对视觉定位推理的高质量和数量充足的评估与优化数据集仍然不足，尤其是针对视觉大型语言模型（VLLMs）。为解决这一问题，我们首先使用VSR数据集对当前的VLLMs进行了诊断，并提出了一套统一的测试集。我们发现当前的VLLMs存在对语言指令过度敏感而对视觉位置信息敏感度不足的矛盾。通过从数据调整和模型结构两个方面扩展原始基准，我们缓解了这一现象。根据我们所知，这是首次使用扩散模型有控制地扩展空间定位图像数据，并将原始视觉编码器（CLIP）与其他3个强大的视觉编码器（SigLIP、SAM和DINO）相结合。经过对数据和模型扩展的组合实验后，我们得到了一个适用于不同指令的VLLM VSR专家（VSRE），并且能够更准确地区分视觉位置信息的差异。VSRE在VSR测试集上的准确性提高了超过27%。它在VSR数据集及其相关子集和其他评估基准中的位置推理方面表现优异。我们开源了扩展后的模型及数据和附录，链接为 \url{this https URL} 并希望这将加速VLLM在VSR学习方面的进展。 
...
**Abstract (ZH)**: 本文分析了传统基准指标（如 BLEU 和 TER）和基于神经网络的方法（如 BERTScore 和 COMET）在聊天翻译中对几种神经机器翻译（NMT）模型性能的评分情况，并探讨了这些指标与人工标注评分的性能对比。结果表明，在对聊天翻译的 NMT 模型进行排名时，所有指标在决定哪些模型表现更好上似乎是一致的，这表明传统基准指标，尽管更快且更简单，仍然具有一定的帮助作用。另一方面，在与人类判断的关联性方面，基于神经网络的指标优于传统指标，COMET 在聊天翻译的评分中与人工标注评分的关联性最高。然而，我们发现，即使是最优秀的指标，在评分包含零人称代词的英语翻译时也变得困难重重。 

---
# On the Applicability of Zero-Shot Cross-Lingual Transfer Learning for Sentiment Classification in Distant Language Pairs 

**Title (ZH)**: 关于零样本跨语言迁移学习在遥远语言对情感分类中的适用性研究 
...
**Abstract (ZH)**: 本研究探讨了使用XLM-R预训练模型将英语知识跨语言迁移至日语和印度尼西亚语的适用性。我们将结果与几种先前研究进行了比较，这些研究要么采用了类似零样本的方法，要么采用了完全监督的方法，从而提供了一份关于使用XLM-R在零样本迁移学习方面与其他现有模型能力对比的概览。我们的模型在日语数据集中取得了最佳结果，在其他日语和印度尼西亚语数据集中也取得了可比拟的结果，而无需使用目标语言进行训练。此外，结果表明，有可能训练一个多语言模型，而不是为每种语言训练一个独立的模型，从而获得令人满意的结果。 
...
**Title (ZH)**: 增强在线连续学习的插件式状态空间模型和类条件离散混合方法 
...
**Title (ZH)**: KunServe：以参数为中心的内存管理下的弹性高效大语言模型服务 
...
**Abstract (ZH)**: 大型语言模型（LLM）服务的状态特性在负载突变或长时间生成请求（如链式推理）时，可能会迅速耗尽宝贵的GPU内存，从而由于请求队列造成的延迟突增。然而，最先进的基于KV缓存的方法通过丢弃、迁移或交换KV缓存来应对负载突增，这在正在进行的请求和服务中的新请求性能之间存在关键权衡，因而仍然存在显著的问题。本文提出了一个关键观察，即模型参数与请求无关，并且在多个GPU上进行了复制，因此提出了一种参数中心的方法，通过选择性地丢弃复制的参数来保留宝贵的内存以供新请求使用。然而，LLM需要将KV缓存绑定在模型参数中，因此丢弃参数会导致巨大的计算浪费或长时间的网络延迟，从而影响所有正在进行的请求。基于注意力操作与其他操作可以解耦这一观察，本文进一步提出了一种新颖的远程注意力机制，通过流水线并行，以从远程GPU上的参数借用额外的内存来服务即将到来的请求。本文还解决了包括活跃的KV缓存与不完整参数交换、生成满足内存需求与协同执行开销平衡的计划以及在过载解除后无缝恢复参数等其他挑战。评估结果显示，KUNSERVE在过载条件下将请求的尾部TTFT减少了最多27.3倍，相较于最先进的方法。 

---
# Survey of Pseudonymization, Abstractive Summarization & Spell Checker for Hindi and Marathi 

**Title (ZH)**: 《关于印地语和马拉地语的匿名化、抽象总结与拼写检查的综述》 
...
**Abstract (ZH)**: 印度庞大的语言多样性为技术进步，特别是在自然语言处理（NLP）领域，带来了独特的挑战和机遇。尽管在广泛使用的语言上已经取得了显著进展，但印度的区域语言，如马尔加希拉语（Marathi）和印地语（Hindi），仍然未得到充分的服务。对于印度区域语言的NLP研究仍处于初期阶段，具有极大的重要性。本文旨在构建一个平台，使用户能够使用诸如文本匿名化、抽象文本总结和拼写检查等特性，支持英语、印地语和马尔加希拉语。这些工具的主要目标是为企业和以印度区域语言为主导的消费者客户群体提供服务。 

---
# Smooth-Foley: Creating Continuous Sound for Video-to-Audio Generation Under Semantic Guidance 

**Title (ZH)**: Smooth-Foley: 在语义引导下生成连续音频的视频_to_音频转化方法 
...
大型语言模型（LLMs）在建模文本序列中的隐藏关系方面取得了显著的进步，这为生命科学领域带来了独特的机会。在这一领域，来自多种物种的单细胞组学数据集为训练基础模型奠定了基础。然而，不同物种间数据规模的差异阻碍了跨物种遗传数据分析全面模型的开发。本研究提出了一种创新的混合方法，将LLMs的一般知识能力与单细胞组学数据领域特定表示模型相结合，用于解释遗传数据。我们从基因作为基本表示单元的角度出发，通过使用功能描述对基因进行初始化，并结合成熟的语言模型，如LaMMA-2的优势。通过输入单细胞水平的基因表达数据和提示信息，我们能够基于不同物种和细胞类型中基因差异表达水平来建模细胞表示。在实验中，我们构建了来自人类和小鼠的发育细胞，并特别针对难以注释的细胞类型。我们通过诸如细胞注释和可视化分析等基本任务，评估了我们的方法。结果显示，与使用LLM的其他方法相比，我们的方法在准确性和互操作性方面表现出明显的改进。我们的混合方法增强了单细胞数据的表示，为跨物种遗传分析未来研究提供了稳健的框架。 

---
# GeneSUM: Large Language Model-based Gene Summary Extraction 

**Title (ZH)**: GeneSUM：基于大规模语言模型的基因总结提取 
...
**Abstract (ZH)**: 生物医学研究中的新兴主题不断扩展，提供了大量关于基因及其功能的信息。这种知识的快速增长为科学发现提供了前所未有的机会，但同时也给研究人员带来了巨大的挑战，他们需要及时跟进最新的进展。其中一项重大挑战是导航庞大的文献库以提取与基因相关的重要信息，这是一项耗时且繁琐的工作。为了提高这一过程的效率，必须解决几个关键挑战：(1) 文献的庞大数量，(2) 基因功能的复杂性，以及(3) 自动化整合与生成。为此，我们提出了GeneSUM，这是一种利用大规模语言模型（LLM）的两阶段自动基因摘要提取器。我们的方法检索并消除目标基因文献的冗余，然后对LLM进行微调，以改进和简化摘要过程。我们进行了广泛的实验来验证我们提出的框架的有效性。结果表明，大规模语言模型显著增强了基因特异性信息的整合，从而在正在进行的研究中实现了更高效的决策。 

---
# EvalMuse-40K: A Reliable and Fine-Grained Benchmark with Comprehensive Human Annotations for Text-to-Image Generation Model Evaluation 

**Title (ZH)**: EvalMuse-40K：一种可靠的细粒度基准，包含全面的人类标注，用于文本到图像生成模型评估 
...
**Abstract (ZH)**: 近年来，文本到图像（T2I）生成模型取得了显著进展。相应地，涌现出许多自动化度量方法来评估生成模型的图像-文本对准能力。然而，这些自动化度量方法的性能比较受到现有小规模数据集的限制。此外，这些数据集不具备从细粒度层面评估自动化度量方法的能力。本研究中，我们贡献了一个名为EvalMuse-40K的基准数据集，该数据集包含40,000个图像-文本对，并附有详细的人工标注，用于图像-文本对准相关任务。在构建过程中，我们采用多种策略，如平衡提示采样和数据重新注释，以确保基准数据集的多样性和可靠性，从而全面评估图像-文本对准度量方法的有效性。同时，我们引入了两种新的方法来评估T2I模型的图像-文本对准能力：FGA-BLIP2方法涉及对视觉语言模型进行端到端微调，以生成细粒度的图像-文本对准评分；PN-VQA方法则在视觉问答模型中采用新颖的正负样本问答方式，实施零样本细粒度评估。这两种方法在图像-文本对准评估中均表现出色。我们还使用这些方法对当前AIGC模型进行了排名，结果可作为未来研究的参考资料，促进T2I生成技术的发展。我们将提供数据和代码的公开访问。 

---
# Text-Aware Adapter for Few-Shot Keyword Spotting 

**Title (ZH)**: 文本意识适配器在少样本关键词识别中的应用 
...
**Abstract (ZH)**: 近年来，柔性关键词识别（Keyword Spotting，KWS）技术在采用文本注册的情况下取得了进展，使得用户能够在注册时无需朗读关键词便实现个性化设置。然而，目标关键词的表现仍有提升空间。本文提出了一种新的少量样本迁移学习方法，称为文本感知适配器（Text-Aware Adapter，TA-adapter），旨在利用有限的语音样本增强预训练的柔性KWS模型，以特定关键词为目标。为了适应声学编码器，我们利用联合预训练的文本编码器生成关键词的文本嵌入，该嵌入作为代表向量。通过仅微调网络的一部分，同时保持核心组件权重不变，TA-适配器在少量样本KWS中表现出极高的效率，并能够无缝切换回原始预训练模型。在我们的实验中，TA-适配器在Google Speech Commands V2数据集中35种不同的关键词上展示了显著的性能提升，仅增加了0.14%的总参数量。 

---
# Do Language Models Understand the Cognitive Tasks Given to Them? Investigations with the N-Back Paradigm 

**Title (ZH)**: 语言模型是否理解分配给它们的认知任务？使用N--back范式进行探究 
...
**Abstract (ZH)**: 最初为人类设计的认知任务现在越来越多地用于研究语言模型。虽然将这些任务应用于语言模型通常是直接的，但解释其结果却颇具挑战性。尤其当模型表现不佳时，通常不清楚这是因为被测试的认知能力有限，还是因为未能理解任务本身。最近一项研究认为，GPT 3.5在2-back和3-back任务中的表现下降反映了类似人类的工作记忆容量限制。通过对一系列不同性能水平的开源语言模型在这些任务中的表现进行分析，我们认为表现不佳实际上反映了任务理解能力和任务集维持能力的限制。此外，我们将表现最佳的模型推至更高的n值，并尝试了不同的提示策略，最后分析了模型的注意力机制。我们的更大目标是为持续进行的语言模型认知评估方法改进的对话做出贡献。 

---
# SongGLM: Lyric-to-Melody Generation with 2D Alignment Encoding and Multi-Task Pre-Training 

**Title (ZH)**: SongGLM：基于2D对齐编码和多任务预训练的歌词到旋律生成 
...
**Abstract (ZH)**: 歌词到旋律生成旨在根据给定的歌词自动生成旋律，需要捕捉歌词和旋律之间复杂的细微联系。然而，以往的工作通常存在两个主要挑战：1）歌词-旋律对齐建模，通常简化为一个字/词对应一个音符的对齐，而其他方法则面临对齐精度较低的问题；2）歌词-旋律和声建模，通常过于依赖中间步骤或严格规则，限制了模型的能力和生成多样性。在本文中，我们提出了一种名为SongGLM的歌词到旋律生成系统，该系统利用2D对齐编码和基于通用语言模型（GLM）的多任务预训练，以确保歌词和旋律之间的对齐和和声一致性。具体而言，1）我们引入了一种统一的歌词和旋律符号表示形式，并采用词级和短语级（2D）对齐编码来捕捉歌词-旋律对齐；2）我们设计了一种多任务预训练框架，包含层次空白填充目标（n-gram、短语和长跨度），并将歌词-旋律关系纳入和声化的n-gram的提取中，以确保歌词-旋律的和声一致性。我们还构建了一个包含超过20万首英文歌曲配对数据集用于预训练和微调。客观和主观测试结果表明，SongGLM在对齐和和声方面表现出显著改进，超过了所有之前的基线方法。 
...
**Title (ZH)**: 基于多站信息的注意力机制框架：用于地震早期预警 
...
**Abstract (ZH)**: 海洋预报对于科学研究和社会福祉都至关重要。目前，最准确的预报系统是全球海洋预报系统（GOFSs），它们将海洋状态变量（OSVs）表示为离散网格，并使用数值方法求解描述海洋状态变量转变的偏微分方程（PDEs）。然而，GOFSs的计算成本较高，容易累积误差。近年来，基于大型人工智能（AI）的模型显著提高了预报速度和准确性。不幸的是，构建一个涵盖跨时空尺度和海洋-大气耦合预报的大型AI海洋预报系统仍然是一个显著的挑战。在这里，我们介绍了一种跨时空尺度和海洋-大气耦合的海洋预报系统——LangYa。结果显示，LangYa的时间嵌入模块使其能够预测从1天到7天的不同预报时效。海洋-大气耦合模块有效地模拟了海洋与大气之间的相互作用。海洋自注意力模块在训练期间提高了网络稳定性并加速了收敛，而自适应热跃层损失函数则提高了热跃层预报的准确性。与现有的基于数值和AI的海洋预报系统相比，LangYa使用了1996年至今27年的GLORYS12（全球海洋再分析与模拟版本12）全球海洋数据进行训练，并实现了更为可靠的确定性OSVs预报结果。LangYa预报系统为全球海洋研究人员提供了一种强大的软件工具，用于准确的海洋预报，开启了海洋科学的新范式。 

---
# BRIDGE: Bundle Recommendation via Instruction-Driven Generation 

**Title (ZH)**: BRIDGE：基于指令驱动生成的捆绑推荐方法 
...
**Abstract (ZH)**: 小目标检测旨在定位和分类图像中的小目标。随着大规模视力-语言预训练的进展，微调预训练的目标检测模型已成为一种有前途的方法。然而，微调大型模型在计算和内存上都代价高昂。为了解决这个问题，本文引入了多点位置插入（MPI）微调，这是一种用于小目标检测的参数高效微调（PEFT）方法。具体而言，MPI 将多个位置嵌入整合到一个冻结的预训练模型中，通过提供精确的位置信息给潜在特征，使得模型能够高效地检测小目标。通过实验，我们在 SODA-D 数据集上证明了所提出方法的有效性。MPI 在参数量显著减少的情况下，与传统的 PEFT 方法（如 CoOp 和 VPT）表现出相似的性能。 

---
# Generating Traffic Scenarios via In-Context Learning to Learn Better Motion Planner 

**Title (ZH)**: 通过上下文学习生成交通场景以获得更好的运动规划器 
...
**Abstract (ZH)**: 自主规划是自动驾驶的关键组成部分。最先进的自主规划算法是在精心收集的数据集上训练的，这些数据集不仅标注成本高昂，而且无法全面覆盖罕见但至关重要的场景。未能考虑到这些场景会对自主规划算法构成重大风险，并可能导致测试过程中出现事故。一种直观的解决方案是通过编程和执行模拟器（例如CARLA）手动组合这些场景。然而，这种方法会带来巨大的人力成本。为应对这一挑战，我们提出了一种低成本的方法，以生成多样化的关键交通场景，从而训练更加稳健的自主规划算法。首先，我们将交通场景表示为脚本，这些脚本能用于模拟器生成交通场景。接下来，我们开发了一种方法，该方法接受用户指定的文字描述，一种大型语言模型（LLM）通过上下文学习将其转化为脚本。生成的脚本随后传递给模拟器，以产生相应的交通场景。由于我们的方法能够生成大量安全关键的交通场景，我们将这些场景用作自主规划算法的合成训练数据。为了展示生成场景的价值，我们在合成数据、真实世界数据以及其中的组合上训练现有的自主规划算法。我们的实验结果显示，使用我们数据训练的自主规划算法在性能上显著优于仅基于真实世界数据训练的算法，这表明我们合成数据的有用性和我们数据生成方法的有效性。我们的源代码可从以下链接获得：this https URL。 

---
# Prompt Tuning for Item Cold-start Recommendation 

**Title (ZH)**: 商品冷启动推荐的提示调优 
...
**Abstract (ZH)**: 在第二语言习得中，预测模型有助于教育者采用多样化的教学策略，引起了广泛的研究关注。然而，尽管模型准确性得到了广泛研究，但模型公平性却未能受到足够的关注。模型公平性确保对各群体的公正对待，防止基于性别、种族或经济背景等因素的无意偏见。一个公平的模型应产生公正的结果，而不系统性地对任何群体造成不利影响。
...
本研究强调了在预测模型中解决公平性问题的重要性，以确保平台和区域之间的公平教育策略。 
...
**Title (ZH)**: 来自 gibberish 或乱码社交媒体文本的 Emoji 提取：一种新型方法和案例研究 
...
**Abstract (ZH)**: 社交媒体平台中广泛使用表情符号，但由于噪音或混乱的文本，这些表情符号常常丢失，这对数据分析和机器学习提出了挑战。传统的预处理方法建议移除这样的文本，这可能会导致表情符号及其语境意义的丢失。本文提出了一种三步逆向工程方法，用于从社交媒体帖子中的混乱文本中提取表情符号。该方法还分析了社交媒体数据挖掘过程中生成此类文本的原因。为评估其实效性，该方法应用于关于猴痘疫情的509,248条推文数据集，该数据集被引用了约30篇先前的研究工作，这些研究工作未能从混乱文本中提取表情符号。我们的方法从76,914条推文中提取了157,748个表情符号。通过Flesch阅读易懂性、Flesch-Kincaid阅读等级、Coleman-Liau指数、自动可读性指数、达利-查尔可读性评分、文本标准和阅读时间等指标，证明了文本可读性和连贯性的改进。此外，还分析了这些推文中个别表情符号的频率及其使用模式，并展示了分析结果。 
...
**Title (ZH)**: 将人工智能研究与临床编码工作流程需求对齐：基于美国数据分析与批判性审查的八条建议 
...
**Abstract (ZH)**: 临床编码对于医疗收费和数据分析至关重要。手工临床编码劳动密集且易出错，因此推动了向该过程完全自动化的研究。然而，基于对美国英语电子健康记录和使用这些记录进行自动化编码研究的分析，我们发现广泛使用的评估方法并未与实际临床情境对齐。例如，专注于最常见的前50个代码的评估过于简化，因为在实践中存在数千个不同的代码。本文旨在将AI编码研究更紧密地与临床编码的实际挑战相联系。基于我们的分析，我们提出了八项具体建议，旨在改进当前的评估方法。此外，我们还提出了超越自动化编码的新型AI方法，建议替代方法以协助编码人员在其工作中。 

---
# Theoretical Constraints on the Expressive Power of $\mathsf{RoPE}$-based Tensor Attention Transformers 

**Title (ZH)**: 基于RoPE的张量注意变换器的表达能力的理论约束 
...
**Abstract (ZH)**: 准确预测行人的轨迹在自动驾驶或服务机器人等领域具有重要意义。深度生成模型在这项任务中取得了顶级的表现，前提是拥有足够的标注轨迹进行训练。为此，大量的合成生成且已标注的轨迹存在（例如，由电子游戏生成）。然而，这些轨迹并不旨在真实地模拟行人的运动，并且在训练预测模型方面效果不佳。我们提出了一种方法和一种架构，在训练时对合成轨迹进行增广，并采用了对抗的方法。我们展示了，在对现有最先进的生成模型进行评估时，训练时对轨迹进行增广能够显著提升其在真实世界轨迹上的表现。 

---
# Explainability in Neural Networks for Natural Language Processing Tasks 

**Title (ZH)**: 自然语言处理任务中神经网络的可解释性 
...
**Title (ZH)**: 实时电力市场中 congestion 管理与利润最大化的一体化学习与优化 
...
**Abstract (ZH)**: 广播仍然是大众信息传播的一个普遍媒介，AM/FM台的听众人数超过了基于智能手机的社交媒体和现场电视。越来越多地，广播内容也通过网络流式传输并在互联网上访问。我们提出了一种名为WavePulse的框架，用于实时记录、文档化和分析广播内容。虽然我们的框架具有普遍适用性，但我们展示了WavePulse在与一组政治科学家团队合作项目中的有效性，该团队关注2024年总统选举。我们使用WavePulse监控了396个新闻广播台的现场流式传输，共处理了接近50万小时的音频流。这些流被转换为带时间戳的分辩文本，并分析以跟踪涉及国家级和州级关键政治科学问题的动态信息流动。我们的分析揭示了地方问题与国家趋势之间的互动，提供了有关信息流动的见解。我们的结果表明，WavePulse能够有效捕捉和分析来自网络的广播现场流内容。代码和数据集可以在 \url{该链接} 获取。 
...
**Title (ZH)**: ICPR 2024 韧性字符分类的领域适应与泛化竞赛 (DAGECC) 
...
**Abstract (ZH)**: 本文是ICPR 2024会议框架内组织的DAGECC（域适应和泛化在字符分类任务中的应用）竞赛的配套论文。在此论文中，我们介绍了我们为该社区提出的任务的总体背景，介绍了为竞赛准备的数据集，并提供了结果总结，包括前三名获奖作品的描述。竞赛主要围绕域适应和泛化展开，我们的核心目标是通过提供高质量、轻量级且适用于实际场景的数据集，激发兴趣并促进相关领域的研究进展，支持新型想法的快速原型设计和验证。 
...
**Abstract (ZH)**: 时空神经网络（TNNs），一种特殊的脉冲神经网络，借鉴了新皮层利用脉冲时机进行信息处理的方式。近期的研究提出了一种微架构框架和定制的宏级工具集，以设计高度能效的应用特定TNN。这些近期的研究依赖于手动硬件设计，这是一个劳动密集型且耗时的过程。此外，目前没有开源的功能模拟框架可用于TNN。本文介绍了TNNGen，这是一种从PyTorch软件模型自动设计TNN至后布局网表的开创性努力。TNNGen包括一种新颖的PyTorch功能模拟器（用于TNN建模和应用探索）以及基于Python的硬件生成器（用于PyTorch到RTL和RTL到布局的转换）。我们模拟了跨多种感官模态的时间序列信号聚类的七个代表性TNN设计，并评估了它们的后布局硬件复杂性和设计运行时间，以证明TNNGen的有效性。我们还强调了TNNGen能够准确预测硅片性能指标的能力，而无需运行硬件工艺流程。 
...
**Abstract (ZH)**: 人工智能（AI）在推动移动网络系统方面起着关键作用，通过实现智能功能和自动化。从4G到5G的过渡对AI在构建以垂直行业为主导的网络方面产生了重大影响。在此背景下，3GPP已规定并引入了网络核心中的网络数据管理功能（NWDAF）实体，通过基于AI算法提供见解来改进网络编排。本文提出了一种不断演化的NWDAF框架，以呈现进一步增强核心网络AI能力所需的接口。此外，我们还确定了一组实现分布式e-NWDAF的研究方向。 

---
# tuGEMM: Area-Power-Efficient Temporal Unary GEMM Architecture for Low-Precision Edge AI 

**Title (ZH)**: tuGEMM：面向低精度边缘AI的面积-功耗高效时间一元GEMM架构 
...
**Abstract (ZH)**: 以下是对原文的翻译，同时确保符合学术规范：
...
**Title (ZH)**: 乳腺肿块识别中转移预训练深度卷积神经网络的分析 
...
**Abstract (ZH)**: 基于预训练卷积神经网络（CNN）的乳腺癌检测引起了其他传统计算机系统广泛的兴趣。在过去的几年中，CNN技术已经成为在乳腺X线摄影图像中发现癌症最有前途的方法之一。在本文中，我们研究了在预训练CNN中冻结层对乳腺癌检测的影响，通过将乳腺X线摄影图像分类为良性或恶性来实现这一目标。基于冻结卷积层块的数量，我们探讨了不同VGG19的几种情形。本研究中共有六种情形。本研究的主要优点有两点：一是提高了模型检测乳腺癌的能力；二是通过冻结某些层从而使VGG19的训练时间减少。为了评估这些情形的表现，我们利用了1693张良性与恶性乳腺癌的微生物学图像。根据报告的结果，从冻结VGG19的第一个块中获得了最佳识别率，其敏感性为95.64%，而完整训练整个VGG19的结果为94.48%。 

---
# ArchComplete: Autoregressive 3D Architectural Design Generation with Hierarchical Diffusion-Based Upsampling 

**Title (ZH)**: ArchComplete: 基于分层扩散上采样的自回归三维建筑设计生成 
...
**Abstract (ZH)**: $\textit{ArchComplete}$ 是一个两阶段的密集体素基三维生成流水线，旨在应对建筑设计中复杂的几何形状和拓扑结构，辅助早期设计过程中的构想和几何细节化。在第一阶段，设计了一个 $\textit{3D 轴量化 VQGAN}$ 模型，其组成通过自回归变压器建模，用于生成粗略的模型。随后，在第二阶段，定义了一组三维条件去噪扩散概率模型组成的层次体素上采样网络，以补充粗略形状的精细几何细节。第一阶段利用包含新颖二维半感知损失的数据集进行训练，该损失能捕捉不同抽象层次的输入复杂性，而第二阶段则在随机裁剪的局部体素块上进行训练，所需计算资源和内存显著减少。对于推理，流水线首先自回归生成分辨率为 $64^3$ 的房屋模型，然后逐步细化至 $256^3$ 的分辨率，体素大小可小至 $18\text{cm}$。$\textit{ArchComplete}$ 支持多种交互模式，可以解决多种任务，包括插值、变体生成、无条件合成，以及两种条件合成任务：形状完成和规划图完成，还有几何细节化。实验结果表明，在现有指标上取得了显著的改进。 

---
# Adaptive Signal Analysis for Automated Subsurface Defect Detection Using Impact Echo in Concrete Slabs 

**Title (ZH)**: 使用混凝土板冲击回波进行自动地下缺陷检测的自适应信号分析 
...
**Abstract (ZH)**: 我们提出了BenCzechMark（BCM），这是首个为大型语言模型设计的综合性捷克语基准测试，提供了各种任务、多种任务格式和多种评估指标。其评分系统基于统计显著性理论，并借鉴社会偏好理论，在任务之间进行聚合。我们的基准测试包括50项具有挑战性的任务，其中包括相应的测试数据集，主要为捷克原文，另有11个新收集的数据集。这些任务涵盖了8个类别，涵盖了不同的领域，包括历史捷克新闻、学生或语言学习者的散文，以及口语。

此外，我们收集并清洗了BUT-Large捷克语语料库，这是目前最大的公开可用的洁净捷克语语料库，我们利用它进行以下工作：(i) 污染分析；(ii) 第一个以捷克为中心的7B语言模型的持续预训练，其中包含捷克特定的分词。我们将我们的模型作为基线，用于与已有的多语言模型进行比较。最后，我们发布并维护了一个排行榜，已有44个模型提交，新的模型提交可以通过以下链接进行提交：[相应链接]。 
...
**Title (ZH)**: 一种在长期群体推荐中平衡便利性和营养性的新方法：多模态食谱的多元推理及其在BEACON中的实现 
...
**Abstract (ZH)**: 无论是健康人群还是有健康状况的人，都会面临选择一日三餐（早餐、午餐和晚餐）的问题，这些餐饭由开胃菜、主菜、配菜、甜点和饮料等多种食品组合而成。这个选择通常涉及营养选择（例如，盐分和糖分水平，营养成分）与便利性（例如，成本和可获取性、菜系类型、食材来源）之间的权衡。我们提出了一种基于数据的餐食推荐解决方案，该方案考虑了可定制的餐食配置和时间范围。该解决方案在满足用户偏好时，也会考虑到食物成分和烹饪过程。我们的贡献包括引入了良好的度量标准，一种将文本食谱转换为最近引入的多模态丰富食谱表示（R3格式）的方法，以及通过上下文多臂赌注学习方法取得初步积极成果，还有基于使用启发的BEACON系统原型。 

---
# The Power of Adaptation: Boosting In-Context Learning through Adaptive Prompting 

**Title (ZH)**: 适应性的力量：通过自适应提示增强上下文学习 
...
**Abstract (ZH)**: 我们将重点放在一个用于解决降级操作符已知且为线性的逆问题的神经网络架构上。该架构通过从目标函数的最小化衍生出的正向-反向算法“展开”而成，该目标函数结合了数据保真项、Tikhonov类型正则化项以及一个潜在的非光滑凸惩罚项。我们从理论角度分析了该逆方法对输入扰动的鲁棒性。确保鲁棒性符合逆问题理论的原则，因为它确保了逆方法的连续性和对小噪声的鲁棒性，这一点至关重要，因为已知深度神经网络对对抗性扰动极为敏感。我们工作的关键新颖之处在于，我们考察了提出网络对其偏差（逆问题中观测数据的表征）扰动的鲁棒性。此外，我们还提供了分析中推导出的数值Lipschitz界示例。 
...
**Abstract (ZH)**: 机器学习在科学研究中的广泛应用在根本上引发了模型不透明性和科学理解之间的紧张关系。虽然有人倡导内在可解释的模型，我们提出了计算可解释性（Computational Interpretabilism, CI）作为科学人工智能后验可解释性的哲学框架。类比人类专业知识，其中后验理性化与可靠表现并存，CI 认为当模型在经验验证的约束下进行结构化解读时，科学知识得以产生。通过中介的理解和有边界的真知识，我们展示了后验方法如何在不需要完全机械透明性的前提下获得 epistemically（证认识论上）正当的洞察，从而解决模型复杂性和科学理解之间的紧张关系。 
...
**Abstract (ZH)**: 大型语言模型中存储的知识需要及时更新以反映现实世界信息的动态性质。为了更新知识，大多数知识编辑方法集中于低层，因为最近对知识检索过程的探查表明，答案信息主要集中在低层。然而，这些探查仅揭示了原始答案的关键检索阶段，而编辑的目标则是纠正模型对目标答案的预测。这种不一致性表明，现有的探查方法及其相关编辑方法均有不足。为了缓解这种不一致性并识别关键编辑区域，我们提出了基于对比的方法，并定位了模型行为在原始答案和目标答案之间发生分歧的两个关键阶段：低层信息丰富化和高层概率促进。基于这些见解，我们开发了联合信息丰富化和概率促进的知识编辑方法（JEEP），旨在同时编辑低层和高层，以修改这两个关键检索阶段。考虑到双重修改导致的相互干扰和遗忘增加，JEEP 设计为确保不同区域的更新共享相同的目标并相互补充。我们通过在不同模型（如 GPT-J（6B）和 LLaMA（7B））上编辑数千个事实，并应对各种编辑目标（如添加事实和反事实知识），严格评估了 JEEP 的性能。在所有测试场景中，JEEP 都取得了最佳表现，验证了我们探查方法揭示的有效性和编辑方法设计的有效性。我们的代码和数据可在以下链接获取：[此处插入链接]。 

---
# Evaluating and Enhancing LLMs for Multi-turn Text-to-SQL with Multiple Question Types 

**Title (ZH)**: 评估和提升多轮文本到SQL转换的LLM性能，支持多种问题类型 
...
**Abstract (ZH)**: 近年来，大型语言模型（LLMs）的发展显著提升了文本到SQL（text-to-SQL）系统的性能。然而，大多数基于LLM的方法往往倾向于仅专注于SQL生成，忽视了实际对话查询的复杂性。这种忽略可能导致响应不可靠，特别是对于那些不能直接通过SQL解决的含糊问题。为解决这一问题，我们提出了一种名为MMSQL的全面测试套件，该套件通过模拟包含多种问题类型和多轮Q&A交互的现实场景来评估LLM的问题分类和SQL生成能力。利用MMSQL，我们评估了包括开源和闭源模型在内的流行LLM的性能，并识别了影响它们在这些场景中表现的关键因素。此外，我们还引入了一种基于LLM的多智能体框架，该框架使用专门的智能体来识别问题类型并确定相应的回答策略。实验结果表明，这种方法显著增强了模型在处理对话动态复杂性方面的能力，能够有效应对用户查询的多样性和复杂性。 

---
# Active Geospatial Search for Efficient Tenant Eviction Outreach 

**Title (ZH)**: 高效的租户驱逐宣传活动中的主动地理空间搜索 
...
**Title (ZH)**: 基于主动采样的迁移学习在健康状态和多中心数据中的快速训练与校准：BCI-P300的应用 
...
**Abstract (ZH)**: 机器学习和深度学习的进步提高了脑-计算机接口（BCI）的性能，但由于个体健康状况、硬件差异和文化差异等因素的影响，其广泛应用受到了限制。许多研究往往集中在统一的单一站点实验中，并在统一的环境中进行，这可能导致在具有良好性能的同时，难以适应现实世界的多样性。深度学习模型旨在提高BCI分类准确性，已有研究表明，通过迁移学习可以在其他人的数据上训练的预训练模型来适应个体神经模式，从而有望提高泛化能力和减少过拟合。然而，在处理来自不同设备、受试者、多中心不同国家和健康与患者群体的多样化和不均衡数据集时，仍面临诸多挑战。

在最大异质性的环境中，我们提出了一种基于Poisson Sampling Disk（PDS）的自适应迁移学习方法，称为活跃采样（Active Sampling, AS），这种方法在基于PDS的卷积神经网络中适应性地调整从源数据到目标域的过渡。使用两个国际上可复制的独立数据集进行的实验结果表明，在40%的自适应微调条件下，基于活跃采样的分类准确率平均提高了5.36%，标准差降低了12.22%。这些结果在分类准确率、计算时间和训练效率方面都优于传统的迁移学习方法。前景在于所提出的活跃采样（AS）方法在迁移学习中的应用。 

---
# MANGO: Multimodal Acuity traNsformer for intelliGent ICU Outcomes 

**Title (ZH)**: MANGO：多模态敏锐度变换器以实现智能重症监护病房 outcome预测 
...
**Abstract (ZH)**: 尽管像GPT和BERT这样的大型语言模型已经在自然语言处理和特定领域应用中展现了前所未有的技能，但出现了一种未被探索的现象，我们称之为罗塞塔悖论。罗塞塔悖论描述了知识领域跨域下的反常识性能反转现象。这一悖论揭示了这些大语言模型在高度专业化领域中表现出色，但在需要广泛且日常生活知识的任务中却表现不佳。本文正式定义了罗塞塔悖论，并介绍了一个全景分析框架，其中包括领域特异性指数（Domain Specificity Index, DSI）和性能反转度量（Performance Inversion Metric, PIM），以一致地量化大语言模型在不同领域的表现行为。

我们采用这一悖论，并通过广泛覆盖不同模型和知识领域的实验来开展一系列研究，范围从丰富的技术领域到常识推理。研究结果表明，罗塞塔悖论很可能并不是数据分布的产物，而是深度神经网络内在的结构和涌现性质的表现。我们对不同模型架构、规模和训练方法进行了比较分析，揭示了这一悖论特有的表现方式，并挑战了标准评估指标。 

---
# Inductive Linguistic Reasoning with Large Language Models 

**Title (ZH)**: 大规模语言模型中的归纳语言推理 
...
**Abstract (ZH)**: 我们推导出了一组因果深度神经网络，其架构源于张量（多线性）因子分析框架，该框架有助于正向和反向因果推断。正向因果问题通过由因果胶囊和张量变换组成的神经架构来解决。因果胶囊计算一组不变的因果因子表示，这些表示的交互由张量变换来控制。反向因果问题则通过实现多线性投影算法的神经网络来解决。该架构逆向操作顺序，估计导致结果的原因。作为一种替代激进的瓶颈维度缩减或正则化回归的方法，后者可能会掩盖固有的欠定反问题，我们建议使用分段张量模型来建模数据形成机制的不同方面，其中的多线性投影产生多个候选解决方案。我们的正向和反向问题可以通过浅层架构解决，但为了实现计算上的可扩展性，我们通过利用块代数推导出一组深层神经网络。交错的核层次结构导致了双重非线性张量因子模型。由张量因子分析推导出的因果神经网络在数据方面是无偏的，但以面部图像作为示例进行说明。还描述了序贯、并行和异步并行计算策略。 

---
# MotifGPL: Motif-Enhanced Graph Prototype Learning for Deciphering Urban Social Segregation 

**Title (ZH)**: MotifGPL：特征增强的图原型学习方法用于理解城市社会隔离现象 
...
**Abstract (ZH)**: 城市的社交隔离现象在种族、住房和收入等多个维度上日益多样化和严重化。随着城市空间和社会关系的复杂化，都会区居民面临的社交隔离水平也各不相同。若这些问题不予解决，将可能导致犯罪率上升、社会紧张局势加剧等严重问题。准确量化和分析城市空间结构及居民互动关系对于解决社交隔离问题至关重要。以往的研究主要关注城市社交隔离的表面指标，缺乏对城市结构和流动性的全面分析，这导致未能充分捕捉到社交隔离的复杂性。为了弥补这一不足，我们提出了一种名为 motif-enhanced graph prototype learning (MotifGPL) 的框架，该框架包含三个关键模块：基于原型的图结构提取、motif 分布发现及城市图结构重构。具体而言，我们通过图结构原型学习方法从城市空间图和出发-目的地图中提取关键原型，同时结合兴趣点、街景图像和流量指数等关键城市属性。为了增强解释性，motif 分布发现模块将每个原型与其相似的 motif 匹配，这些 motif 代表了反映局部模式的简单图结构。最后，我们利用 motif 分布结果指导两图的重构。此模型能够详细探索城市空间结构和居民流动性模式，有助于识别和分析影响城市社交隔离的 motif 模式，并指导城市图结构的重构。实验结果表明，MotifGPL 有效地揭示了影响城市社交隔离的关键 motif，并为缓解这一问题提供了稳健的指导。 

---
# Multi-Agent Norm Perception and Induction in Distributed Healthcare 

**Title (ZH)**: 多agent规范感知与诱导在分布式医疗中的应用 
...
**Abstract (ZH)**: 本文提出了一种多智能体规范感知与归纳学习模型，旨在通过动态交互过程将自主代理系统整合到分布式医疗环境中。医疗规范系统及其共享渠道的特性要求多智能体系统采用不同的方法来学习两种类型的规范。在此基础上，该模型使智能体能够同时学习描述性规范，这些规范捕捉集体倾向，以及规范性规范，这些规范规定理想行为。通过参数化的混合概率密度模型和实践增强的马尔可夫博弈，多智能体系统在动态交互中感知描述性规范并捕捉涌现的规范性规范。我们使用了从2016年到2020年神经医学中心的数据集进行了实验。 
...
**Abstract (ZH)**: 国际企业、组织或医院收集了大量存储在数据库、文本文档、图像和视频中的多模态数据。尽管在多模态数据探索和自动将自然语言问题转换为数据库查询语言的数据库系统领域都取得了一定进展，但将数据库系统与其他未结构化的模态如自然语言中的图像结合进行查询的研究挑战仍然广泛未被探索。

本文提出了一种名为XMODE的系统，该系统能够通过自然语言实现可解释的多模态数据探索。我们的方法基于以下几个研究贡献：（1）我们的系统受到一个实际应用场景的启发，该应用场景使用户能够探索多模态信息系统。（2）XMODE利用基于LLM的代理AI框架将自然语言问题分解为子任务，如文本到SQL生成和图像分析。（3）实验结果表明，该系统在多模态数据集（包括关系数据和图像）上的表现优于最先进的多模态探索系统，不仅在准确性方面，在查询延迟、API费用、规划效率和解释质量等各个性能指标方面也表现更优，这得益于对LLM推理能力的有效利用。 

---
# GUI Testing Arena: A Unified Benchmark for Advancing Autonomous GUI Testing Agent 

**Title (ZH)**: GUI 测试 arena：促进自主 GUI 测试代理发展的统一基准 
...
**Abstract (ZH)**: 如今，GUI代理的研究是人工智能社区的热点话题。然而，当前的研究主要集中在GUI任务自动化上，限制了其在各种GUI场景中的应用范围。本文提出了一种形式化和全面的环境，用于评估整个自动化GUI测试（GTArena）的过程，提供了一个公平、标准化的环境，以一致的方式运行多种多媒体大型语言模型。我们将测试过程分为三个关键子任务：测试意图生成、测试任务执行和GUI缺陷检测，并基于这些任务构建了一个基准数据集，进行全面评估。该评估使用三种数据类型：真实移动应用程序、人工注入缺陷的移动应用程序和合成数据，全面评估其在该特定任务中的能力。此外，我们提出了一种方法，帮助研究人员探索多媒体语言大型模型在特定场景中的性能与其在标准基准测试中的通用能力之间的关联。实验结果表明，即使是最先进的模型也无法在自动化GUI测试的所有子任务中表现良好，突显了当前自主GUI测试能力和其实际应用之间的显著差距。这一差距为GUI代理未来发展的方向提供了指导。我们的代码见此链接：[[提供链接的URL]]。

请注意，链接部分需要具体提供一个URL地址，此处用占位符代替。您可以将其替换为具体的链接地址。 

---
# LongDocURL: a Comprehensive Multimodal Long Document Benchmark Integrating Understanding, Reasoning, and Locating 

**Title (ZH)**: LongDocURL：一个综合性的多模态长文档基准，集成理解、推理和定位功能 
...
**Abstract (ZH)**: 大视觉语言模型（LVLMs）显著提高了文档理解能力，能够处理复杂的文档元素、较长的上下文以及更广泛的任务。然而，现有的文档理解基准仅限于处理少量页面，并未能提供全面的布局元素定位分析。本文首先定义了三个主要任务类别：长文档理解、数值推理和跨元素定位，然后提出了一个综合基准——LongDocURL，该基准整合了上述三个主要任务，并包括基于不同主要任务和答案证据分类的20个子任务。此外，我们开发了一种半自动构建管道，并收集了2,325对高质量的问题-答案配对，涵盖了超过33,000页的文档，显著优于现有基准。随后，我们对26种不同配置的开源和封闭源模型进行了全面评估实验，揭示了该领域的关键性能差距。 

---
# Research on the Proximity Relationships of Psychosomatic Disease Knowledge Graph Modules Extracted by Large Language Models 

**Title (ZH)**: 大型语言模型提取的身心疾病知识图谱模块间逼近关系研究 
...
**Abstract (ZH)**: 随着社会变革的加速，心身疾病的发病率显著增加，成为全球健康问题中的主要挑战。这需要一种创新的知识体系和分析方法来辅助诊断和治疗。为此，我们构建了一个本体模型和实体类型，使用BERT模型和LoRA微调的大规模语言模型（LLM）进行命名实体识别，并构建了一个包含9668个三元组的知识图谱。接下来，通过分析疾病、症状和药物模块之间的网络距离，发现疾病之间的更近距离预测了其临床表现、治疗方法和心理机制的更大相似性；症状之间的更近距离表明它们更有可能同时出现。最后，通过比较proximity d和proximity z分数，结果显示，在诊断关系中，主要诊断关系中的症状-疾病配对具有更强的相关性和更高的参考价值。研究结果揭示了疾病、共发症状以及治疗策略相似性之间的潜在联系，提供了对心身疾病的诊断和治疗的新视角，并为未来的心理健康研究和实践提供了宝贵的信息。 
...
**Abstract (ZH)**: 以下是将该内容翻译成中文的版本，符合学术规范：

物理-信息系统（Cyber-physical Systems, CPS）结合了工程化的计算和物理组件，旨在在动态环境中做出决策并相互作用。确保CPS的安全性至关重要，这需要在各种复杂场景中进行广泛的测试。为了尽可能多地生成测试场景，以往的努力主要集中在使用形式语言描述场景以生成场景。本文提出了一种替代方法：在开源游戏引擎Godot中重建场景。我们开发了一条生产线程，可以从提供的场景图像直接重建测试场景。这些重建的场景可以部署在模拟环境中以评估CPS。这种方法为在真实环境中测试CPS提供了一种具有扩展性和灵活性的解决方案。 

---
# Weak Scaling Capability in Token Space: An Observation from Large Vision Language Model 

**Title (ZH)**: 在Token空间中的弱扩展能力：一项来自大型视觉语言模型的观察 
...
**Abstract (ZH)**: 关于参数数量和训练数据规模的扩展能力已经得到了广泛验证。一个尚未被探索的重要问题是：扩展能力是否也类似于视觉标记的数量？本研究通过探讨视觉标记数量与视觉语言模型性能之间的关系来填补这一空白。我们的理论分析和实证评估表明，模型在长度\(N_l\)上的扩展能力较弱，性能大约为\(S(N_l) \approx (c/N_l)^{\alpha}\)，其中\(c\)和\(\alpha\)是超参数。有趣的是，用户的问题是否包含在输入中对这种扩展行为的影响相对较小。此外，当问题与任务相关时，将用户的问题与视觉标记融合可以提高模型性能。为应对大规模视觉标记带来的计算挑战，我们提出了一种新颖的架构，该架构能够高效地减少标记数量，并将用户问题标识符集成到表示中。我们的研究结果可能为在特定任务约束下开发更高效和有效的视觉语言模型提供见解。 

---
# The Thousand Brains Project: A New Paradigm for Sensorimotor Intelligence 

**Title (ZH)**: 千脑计划：传感器imotor智能的新范式 
...
**Abstract (ZH)**: 过去十年中，人工智能取得了迅速进展，主要得益于深度学习系统规模的扩大。尽管取得了这些进展，要在多样化的现实环境中有效地运作的智能系统创建仍是一个重大挑战。本白皮书概述了千脑计划，这是一个正在进行的研究项目，旨在开发一种源自新皮层运作原理的替代性互补形式的AI。我们介绍了千脑系统的一个早期版本，这是一种感觉运动代理，特别适合快速学习各种任务，并最终实现人类新皮层具备的所有能力。其核心设计思想是使用一个重复的计算单元——学习模块，这些学习模块建模于哺乳动物大脑中的皮层柱。每个学习模块作为一个半独立的单元运行，能够模拟整个对象、通过空间结构化的参考框架表示信息，并且能够估算和执行世界中的运动。学习过程是快速的关联性过程，类似于大脑中的希布式学习，利用关于世界空间结构的归纳偏见，以实现快速和持续的学习。多个学习模块可以通过“皮层消息协议”(CMP) 进行分级和非分级交互，从而创建更抽象的表示，并支持多模态融合。我们概述了设计千脑系统的关键原则，并提供了我们第一个此类系统实例Monty的实现细节。代码可在以下链接找到：[此链接]，更详细的文档可在以下链接找到：[此链接]。 

---
# Pirates of the RAG: Adaptively Attacking LLMs to Leak Knowledge Bases 

**Title (ZH)**: 《基于RAG的海盗：适应性攻击大语言模型以泄露知识库》

注：RAG（Retrieval-Augmented Generation）是一种结合检索和生成的模型架构，该论文标题可能是在形象地将一些攻击行为比喻为“海盗”，旨在探索如何通过适应性攻击方法从大语言模型中泄露知识库中的信息。 
...
**Abstract (ZH)**: 随着检索增强生成（RAG）系统在多种实际服务中的广泛应用，其安全性问题变得尤为严重。RAG系统通过检索机制增强大型语言模型（LLM）的生成能力，该机制依赖于一个私有知识库。如果知识库被意外泄露，可能会导致严重的后果，包括私人和敏感信息的泄露。本文提出了一种黑盒攻击方法，旨在迫使RAG系统泄露其私有知识库，该方法不同于现有方法之处在于它具有适应性和自动化。基于相关性的机制以及攻击者侧开源LLM有助于生成有效的查询以泄露知识库的大部分（隐藏的部分）。广泛的实验表明，所提出的算法在不同RAG流水线和领域中的表现优于非常近期的相关方法，这些方法要么不是完全黑盒，要么不是适应性的，要么不是基于开源模型。我们的研究结果强调了在设计和部署RAG系统时更加 robust的信息安全保障的紧迫需求。 

---
# MinsStudio: A Streamlined Package for Minecraft AI Agent Development 

**Title (ZH)**: MinsStudio：MCaiAgent开发的 streamlined 包装器 
...
**Abstract (ZH)**: 《我的世界》（Minecraft）已经成为用于测试嵌入式智能和序贯决策研究的重要平台，然而，开发和验证新型代理仍然受到重大工程挑战的限制。本文介绍了MineStudio，一个开源软件包，旨在简化《我的世界》中的嵌入式策略开发。MineStudio 是第一个全面整合七个关键工程组件的工具：模拟器、数据、模型、离线预训练、在线微调、推理和基准测试，从而让用户能够集中精力在算法创新上。我们提供了用户友好的API设计，并附有全面的文档和教程。完整的代码库可以在以下链接公开访问：这个https URL。 

---
# Improving Multi-Step Reasoning Abilities of Large Language Models with Direct Advantage Policy Optimization 

**Title (ZH)**: 通过直接优势策略优化提高大型语言模型的多步推理能力 
...
**Abstract (ZH)**: 强化学习（RL）在提升大规模语言模型（LLMs）推理能力方面的作用变得越来越重要。尽管RL在许多场景中取得了成功，但在改善LLMs推理能力方面仍面临许多挑战。一个挑战是稀疏奖励，这使得RL的优化变得困难，并需要大量的数据样本。另一个挑战源自于RL固有的不稳定性，尤其是使用演员-评论家（Actor-Critic, AC）方法来推导最优策略时，这通常会导致训练过程不稳定。为了解决这些问题，我们引入了直接优势策略优化（DAPO），这是一种新颖的离线RL算法。不同于仅依赖于最终结果奖励的标准对齐方法（如DPO），DAPO 使用一个评论家函数来预测每一步的推理准确性，从而生成密集信号以优化生成策略。此外，在DAPO中，演员和评论家组件是独立训练的，避免了标准AC算法（如PPO）中常见的共同训练不稳定性。我们利用数学和代码查询数据集训练DAPO，并在多个基准测试中评估其性能。结果显示，DAPO能够有效地增强SFT模型和RL模型的数学和代码能力，证明了DAPO的有效性。 

---
# Annotating References to Mythological Entities in French Literature 

**Title (ZH)**: 法语文学中神话实体引用的标注研究 
...
**Abstract (ZH)**: 在本文中，我们探讨了大型语言模型（LLMs）在标注现代和当代法国文学中对罗马和希腊神话实体引用的相关性。我们提出了一套标注方案，并展示了近期的LLMs可以直接应用于遵循该方案的标注工作，尽管偶尔会出现显著的分析错误。此外，我们证明了LLMs（特别是ChatGPT）能够为文学作者使用神话引用提供解释性的洞见。然而，我们还发现，当LLMs作为信息检索引擎使用时，它们在准确识别小说中的相关段落方面存在困难，经常产生幻觉并生成虚假示例，这一问题引发了显著的伦理关切。尽管如此，当谨慎使用时，LLMs仍然是在大规模手动方法难以全面完成的任务中进行高精度标注的重要工具。 

---
# VISION: A Modular AI Assistant for Natural Human-Instrument Interaction at Scientific User Facilities 

**Title (ZH)**: VISION：一种模块化的AI辅助工具，用于科学用户设施中的自然人-仪器交互 
...
**Abstract (ZH)**: 科学用户设施，如同步辐射光束线，配备了广泛的硬件和软件工具，需要一套代码库来实现人机交互。这通常需要开发人员参与，以建立用户/研究人员与复杂仪器之间的连接。生成式人工智能的出现提供了一个机会，可以弥合这一知识差距，实现无缝通信和高效的实验流程。在此，我们通过组装多个具有特定任务认知功能的AI模块，为虚拟科学伴侣（VISION）构建了一个模块化架构，这些模块为大型语言模型（LLMs）提供技术支持。借助VISION，我们在光束工作站上进行了基于LLM的操作，实现了低延迟，并展示了首个通过语音控制的X射线散射实验。该模块化和可扩展的架构便于新仪器和技术的快速适应。基于自然语言的科学研究实验开发是通往未来科学范式变革的关键一步，未来科学外脑——科学家认知的合成扩展——可能彻底改变科学研究和发现的方式。 
...
**Abstract (ZH)**: 社交媒体平台正越来越多地出现AI生成文本（AIGT）。然而，AIGT的滥用可能对公众舆论产生深远影响，例如传播虚假信息和操控叙事。尽管其重要性不言而喻，但针对社交媒体上AIGT的系统性研究仍然缺乏。为填补这一空白，本文旨在量化、监控和分析社交媒体上的AIGT。我们首先从Medium、Quora和Reddit这3个主要社交媒体平台收集了一个包含约240万条帖子的数据集（SM-D）。然后，我们构建了一个多样化的数据集（AIGTBench），用于训练和评估AIGT检测器。AIGTBench结合了流行的开源数据集和由社交媒体文本生成的来自12个LLM的AIGT数据集，作为主流检测器的基准。通过这一设置，我们确定了表现最佳的检测器（OSM-Det）。随后，我们将OSM-Det应用于SM-D，以追踪随着时间变化的AIGT趋势，并观察从2022年1月到2024年10月间不同社交媒体平台上的AI归属率（AAR）变化趋势。具体而言，Medium和Quora的AAR表现出显著增长，从1.77%升至37.03%，从2.06%升至38.95%。相比之下，Reddit的增长速度较慢，AAR在同一期间从1.31%升至2.45%。进一步的分析表明，AIGT在多个维度上与人类撰写的文本存在差异，包括语言模式、话题分布、互动水平以及作者的粉丝分布。我们希望本文对社交媒体中AIGT的分析和发现能够为该领域的未来研究提供重要启示。 

---
# Exact Acceleration of Subgraph Graph Neural Networks by Eliminating Computation Redundancy 

**Title (ZH)**: 通过消除计算冗余加快子图图神经网络的精确加速 
...
**Abstract (ZH)**: 图神经网络（GNNs）已成为处理图任务的一种广泛使用的框架。许多近期的研究提出了在每个图的众多子图上应用图卷积方法的概念，即子图图神经网络（subgraph GNNs），以增强GNNs区分子图的能力。为了最大程度地提高可表达性，子图GNNs经常需要每个子图的大小与原始图相同。尽管这些方法在性能上表现出色，但它们仍面临挑战，因为子图的数量庞大且规模较大，这导致了训练数据量的激增，从而产生了存储和计算效率问题。针对这一问题，本文引入了一种名为Ego-Nets-Fit-All（ENFA）的模型，该模型统一以较小的ego-net作为子图，从而提高了存储和计算效率，同时确保输出与原始子图GNN相同，甚至无论将整个图视为子图也是如此。关键在于识别并消除子图之间的冗余计算。例如，节点$v_i$可能出现在多个子图中，但距离这些子图的中心点较远（子图之间的不对称部分）。因此，可以在原始图中对每个子图的前几轮消息传递进行一次计算，而不是在每个子图中重复计算。这种策略使我们的ENFA能够以精确的方式加速子图GNN，而不像之前的一些采样方法那样常常会损失性能。通过对多种数据集进行广泛实验发现，与传统的子图GNN相比，ENFA可以减少存储空间29.0%至84.5%，并提高训练效率高达1.66倍。 

---
# AutoDroid-V2: Boosting SLM-based GUI Agents via Code Generation 

**Title (ZH)**: AutoDroid-V2：通过代码生成增强基于SLM的GUI代理 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）为移动UI代理带来了令人兴奋的新进展，这是一个长期的研究领域，旨在通过移动UI交互完成任意自然语言任务。然而，现有的UI代理通常需要高性能的强大模型进行高度推理，这些模型难以在最终用户的设备上本地部署，这引发了关于用户隐私和集中式服务成本的巨大担忧。一种减少所需模型大小的方法是定制一个小型的专业模型，并使用高质量的训练数据，例如大规模的人类演示多样化的应用程序和任务，而这样的数据集极其难以获得。受到近期小型语言模型（SLMs）惊人的编码能力的启发，我们提出将UI任务自动化问题转化为代码生成问题，该问题可以通过设备上的SLM有效地解决，并通过设备上的代码解释器高效执行。不同于可以通过大量公共数据集进行广泛预训练的普通编码任务，生成UI自动化代码具有挑战性，因为目标应用程序具有多样性、复杂性和变异性。因此，我们采用文档为中心的方法，自动为每个应用程序建立细粒度的API文档，并基于此文档生成多样化的任务样本。通过使用合成文档和任务样本引导代理，它学习生成能够完成未知任务的精确和高效的脚本。基于与当前最先进的移动UI代理的详细比较，我们的方法在显著提高移动任务自动化方面表现出更高的成功率和更低的延迟/令牌消耗。代码也将开源。 
...
**Abstract (ZH)**: 表格数据占比超过企业数据资产的80%，在各个领域都至关重要。随着对隐私保护和数据共享限制的关注不断增加，生成高质量的合成表格数据变得尤为重要。近期的研究表明，大型语言模型（LLMs）可以通过利用语义信息并克服热编码带来的高维数据挑战，有效地生成逼真的表格数据。然而，当前的方法并未充分利用表格中丰富的信息。为解决这一问题，我们基于提示增强引入了AI生成表格（AIGT）方法，这是一种利用元数据信息（如表格描述和表格模式）作为提示来生成极高质量合成数据的创新方法。为克服LLMs的令牌限制约束，我们提出了长令牌分割算法，使得AIGT能够模拟任意规模的表格。在20个公开数据集以及支付宝风控系统中的两个真实行业数据集上，AIGT实现了14个数据集的最佳性能。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）因其在各个领域的卓越能力而备受关注，然而其庞大的参数规模也给实际部署带来了挑战。结构化剪枝是一种有效的方法，可以平衡模型性能与效率，但在计算资源受限的情况下，LLMs的性能恢复是一个主要挑战。因此，我们提出了一种基于Optimal Brain Surgeon框架的低成本且快速的结构化剪枝方法，名为SlimGPT。我们提出了批次贪婪剪枝，通过分组的Cholesky分解增强头别剪枝误差估计，并通过动态分组大小提高FFN的剪枝效率，从而在不到一小时内实现接近局部最优的剪枝结果。此外，我们从误差累积的角度探讨了逐层剪枝的局限性，并提出了增量剪枝比，这是一种非均匀剪枝策略，以减少性能下降。在LLaMA基准测试上的实验结果表明，SlimGPT优于其他方法，并实现了最先进的结果。 

---
# Tackling the Dynamicity in a Production LLM Serving System with SOTA Optimizations via Hybrid Prefill/Decode/Verify Scheduling on Efficient Meta-kernels 

**Title (ZH)**: 使用高效元内核上混合预填充/解码/验证调度与SOTA优化应对生产LLM服务系统中的动态性 
...
**Abstract (ZH)**: 为了满足生产级大型语言模型（LLM）服务系统对低延迟和成本效率不断增长的需求，集成先进的优化技术是必要的。然而，结合这些优化技术后，LLM 动态且不可预测的输入输出长度问题进一步加剧了工作负载的变异性问题，使得在AI加速器上保持高效率尤其在基于瓷砖编程模型的DSAs（数据流加速器）上格外困难。为应对这一挑战，我们提出了XY-Serve，这是一种具有广泛适应性的、昇腾原生的一体化生产级LLM服务系统。核心思想是一种抽象机制，通过将计算分解为统一的硬件友好型细粒度元原语来平滑工作负载的变异性。在注意力机制方面，我们提出了一种元内核，该内核采用具有架构感知的瓷砖大小计算基本模式的矩阵乘法-softmax-矩阵乘法。对于GEMM（通用矩阵乘法），我们引入了一种虚拟填充方案，该方案可以适应动态形状变化，同时使用高效的具有多种固定瓷砖大小的GEMM原语。XY-Serve与vLLM 高度兼容。实验结果显示，在昇腾NPUs上，与当前可用的基准相比，端到端吞吐量提高了高达89%。此外，与现有库中的GEMM（平均快14.6%）和注意力（平均快21.5%）内核相比，我们的方法也表现出更好的性能。虽然这项工作专为昇腾设计，但相信该方法也可以轻易地适用于SIMT架构。 

---
# Real-world Deployment and Evaluation of PErioperative AI CHatbot (PEACH) -- a Large Language Model Chatbot for Perioperative Medicine 

**Title (ZH)**: 在实际环境中部署和评估术中AI聊天机器人（PEACH）——一个大型语言模型聊天机器人在围手术期医学中的应用 
...
**Abstract (ZH)**: 大型语言模型（LLMs）正逐渐成为医疗领域中强大的工具，特别是在处理复杂且领域特定的任务方面。本研究描述了PErioperative AI CHatbot (PEACH)系统的发展和评估过程，该系统通过将本地围手术期指南集成到一个安全的LLM框架中，以支持术前临床决策。PEACH系统嵌入了35个机构的围手术期协议，并在安全的Claude 3.5 Sonet LLM框架中通过新加坡政府开发的Pair Chat进行静默部署和测试。准确度、安全性及可用性得到了评估。幻觉和偏离根据潜在危害被分类，用户反馈则通过技术接受模型（TAM）进行评价。在静默部署后，对一种协议进行了更新。

在240次真实的临床迭代中，第一代PEACH达到了97.5%的准确率（78/80），并且整体准确率为96.7%（232/240）。经过更新后的PEACH的准确率提高到了97.9%（235/240），这在0.018的显著性水平上显著高于95%的准确率（p = 0.018，95% CI: 0.952-0.991）。仅观察到一次幻觉和两次偏离。临床医生报告，在95%的情况下，PEACH加速了决策过程，而内部一致性信度在0.772-0.893之间，且观察者间一致性信度在0.610-0.784之间。

PEACH是一个准确且灵活的工具，可以增强围手术期决策的一致性和效率。未来的研究应探索其在不同专科中的可扩展性及其对临床结果的影响。 

---
# AutoSculpt: A Pattern-based Model Auto-pruning Framework Using Reinforcement Learning and Graph Learning 

**Title (ZH)**: AutoSculpt：一种基于模式的模型自动剪枝框架——结合强化学习与图学习 
...
**Abstract (ZH)**: 随着深度神经网络（DNNs）越来越多地部署在边缘设备上，优化这些模型以适应受限的计算资源变得至关重要。现有的自动剪枝方法因DNN模型的多样性、各类算子（例如，滤波器）以及剪枝精细度与模型精度之间的平衡难度而遇到挑战。为了应对这些限制，我们提出了AutoSculpt，这是一种基于模式的自动化剪枝框架，通过图学习和深度强化学习（DRL）来提高效率和准确性。AutoSculpt自动生成并剪枝DNN架构内的可识别规律，从而实现运行时加速。AutoSculpt包含三个关键步骤：（1）将DNNs表示为图，以编码其拓扑结构和参数依赖关系；（2）嵌入高效计算的剪枝模式；（3）利用DRL迭代优化自动剪枝策略，以达到压缩与精度的最佳平衡。实验结果表明，AutoSculpt在各种架构（包括ResNet、MobileNet、VGG和Vision Transformer）上表现出色，剪枝率最高可达90%，并在FLOPs减少方面取得了近18%的改进，超越了所有基线方法。有关代码可访问以下链接：[请在此处提供链接] 

---
# Property Enhanced Instruction Tuning for Multi-task Molecule Generation with Large Language Models 

**Title (ZH)**: 使用大型语言模型的多任务分子生成中增强性质的指令调优 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在各种自然语言处理任务中得到了广泛应用，例如问答和机器翻译。然而，由于缺乏标记数据以及生物化学性质的手动标注难度大，这些模型在分子生成任务中的性能仍然有限，特别是在涉及多属性约束的任务中。在这项工作中，我们提出了一种两步框架PEIT（Property Enhanced Instruction Tuning）来提高LLMs在与分子相关的任务中的性能。在第一步中，我们使用文本描述、SMILES 和生物化学性质作为多模态输入，预训练一个名为PEIT-GEN的模型，通过对齐多模态 Representation 合成指令数据。在第二步中，我们使用合成数据微调现有的开源LLMs，得到的PEIT-LLM能够处理分子描述、基于文本的分子生成、分子性质预测，以及我们新提出的多约束分子生成任务。实验结果显示，我们预训练的PEIT-GEN在分子描述任务上比MolT5和BioT5表现出更好的性能，说明了文本描述、结构和生物化学性质之间的模态对齐良好。此外，PEIT-LLM 在多任务分子生成方面显示出明显的改进，证明了PEIT框架对各种分子任务的适用性。我们在https://github.com/alibaba/Qwen-LLM 上发布了代码、构造的指令数据和模型检查点。 

---
# Understanding Artificial Neural Network's Behavior from Neuron Activation Perspective 

**Title (ZH)**: 从神经元激活视角理解人工神经网络的行为 
...
**Abstract (ZH)**: 本文通过神经元激活动态的视角探讨了深度神经网络（DNN）的复杂行为。我们提出了一种概率框架，可以将模型的神经元激活模式视为一个随机过程，揭示了神经网络缩放定律（如超参数化）以及损失函数随数据集规模增长的幂律衰减等理论洞察。通过对关键数学关系的推导，我们表明激活神经元的数量以 $N\left(1-\left(\frac{bN}{D+bN}\right)^b\right)$ 的形式增加，并且神经元激活应遵循幂律分布。基于这两个数学结果，我们展示了即使在超参数化的情况下，DNN 也能保持泛化能力，并详细解释了在对数坐标下（即数据量线性增长）绘制损失曲线时观察到的相变现象。此外，通过结合上述两种现象以及神经元激活的幂律分布，我们推导出了数据规模增加时神经网络损失函数的幂律衰减。进一步的分析填补了经验观察与理论依据之间的空白，提供了关于参数效率和模型压缩性可测试的预测。这些发现为理解神经网络的缩放提供了基础，并为优化DNN性能提供了新的方向。 

---
# Dynamic Multi-Agent Orchestration and Retrieval for Multi-Source Question-Answer Systems using Large Language Models 

**Title (ZH)**: 使用大规模语言模型实现多源问答系统中的动态多代理编排与检索 
...
**Abstract (ZH)**: 我们提出了一种方法论，该方法论结合了大型语言模型（LLM）检索中的多种先进技术，以支持稳健的多源问答系统的开发。该方法论旨在通过协调多代理编排和动态检索方法，整合来自多样化数据源的信息，包括未结构化文档（如PDF）和结构化数据库。该方法论利用了专门的代理（如SQL代理、检索增强生成（RAG）代理和路由器代理），这些代理可以根据每个查询的性质动态选择最合适的检索策略。为提高准确性和上下文相关性，我们采用实时动态提示工程技术，使其能够根据特定查询的上下文进行适应性调整。该方法论的效果在合同管理领域得到了验证，该领域常常需要无缝地处理未结构化和结构化数据之间的交互。我们的研究成果表明，这种方法可以提高响应的准确性和相关性，提供一个具备灵活性和扩展性的框架，适用于跨不同领域和数据源的问答系统开发。 

---
# Study of the Proper NNUE Dataset 

**Title (ZH)**: NNUE 数据集的适当研究 
...
**Abstract (ZH)**: NNUE（高效可更新神经网络）已经革新了棋类引擎的开发，几乎所有顶级引擎都采用了NNUE模型以保持竞争力。NNUE训练中的一个关键挑战是在像国际象棋这样复杂领域创建高质量的数据集，特别是战术和战略评估对性能至关重要。然而，构建有效数据集的方法仍然缺乏充分的理解和文档记录。本文提出了一种生成和筛选由“寂静”位置组成的高质量数据集的算法，这些位置稳定且不受战术波动的影响。我们的方法提供了数据集生成的明确方法论，并可以在各种评估函数中进行复制和泛化。测试结果表明，这种方法显著提高了引擎性能，验证了其有效性。 
...
**Abstract (ZH)**: 本研究探讨了监视资本主义的运作机制，重点关注网络浏览和搜索过程中个人数据的传输。通过分析网络流量，揭示了各种实体如何追踪和收集数字足迹。研究揭示了用户与网络服务之间交换的具体数据类型，并强调了这些过程中涉及的复杂算法。我们提供了数据收集实践的具体证据，并提出了增强数据保护和透明度的策略。本研究强调了在数字时代建立 robust 数据保护框架和实施伦理数据使用的重要性，以解决隐私问题。 

---
# Contrato360 2.0: A Document and Database-Driven Question-Answer System using Large Language Models and Agents 

**Title (ZH)**: Contrato360 2.0：一种基于文档和数据库的大型语言模型与智能体驱动的问题回答系统 
...
**Abstract (ZH)**: 我们提出了一种问答（Q&A）应用程序，旨在通过结合合同文件（PDF）和从合同管理系统中检索的数据来支持合同管理过程。这些数据通过大型语言模型（LLM）处理，以提供精确和相关的信息。通过使用检索增强生成（RAG）、文本到SQL技术以及动态协调工作流程的代理，这些技术进一步提高了答案的准确性。这些方法消除了重新训练语言模型的需要。此外，我们还采用了提示工程来精细调整响应的重点。我们的研究结果表明，这种多代理协调和多种技术的结合显著提高了答案的相关性和准确性，为未来的信息系统提供了有希望的发展方向。 

---
# Causal Composition Diffusion Model for Closed-loop Traffic Generation 

**Title (ZH)**: 因果组合扩散模型在闭环交通生成中的应用 
...
**Abstract (ZH)**: 仿真在自动驾驶的安全评估中至关重要，特别是在捕捉复杂的互动行为方面。然而，在长尾情况中生成逼真且可控的交通场景仍然是一个重大挑战。现有的生成模型在用户定义的可控性与逼真性之间存在冲突的目标上表现不佳，而在安全关键的背景下这种冲突更加突出。在本研究中，我们引入了因果组成扩散模型（CCDiff），这是一种基于结构的扩散框架，用以应对这些挑战。我们首先将可控且逼真的闭环仿真学习形式化为一个约束优化问题。然后，CCDiff通过自动识别并在扩散过程中注入因果结构来最大化可控性并遵守逼真性，为增强两者提供结构化的指导。通过在基准数据集和闭环仿真器上的严格评估，CCDiff在生成逼真且用户偏好的轨迹方面表现出显著优于现有最先进的方法。我们的结果显示，CCDiff在关键指标（如碰撞率、离路率、FDE和舒适度）上提高了闭环性能，有效提取和利用了因果结构。 

---
# The Unreasonable Effectiveness of Open Science in AI: A Replication Study 

**Title (ZH)**: 开源科学在人工智能中的意想不到的有效性：一项复制研究 
...
**Abstract (ZH)**: 科学领域已经报告了可重复性危机，但这种危机对人工智能研究的影响程度尚未完全了解。因此，我们进行了一项系统性重现研究，包括30篇高引用率的AI研究论文，并尽可能利用原始材料。最终，有8篇论文因需要获取项目过程中实际上难以获得的数据或硬件而被拒收。有6篇论文完全重现成功，另有5篇部分重现成功。总体而言，有50%的论文在某种程度上得到了重现。代码和数据的可用性与可重现性密切相关，86%共享代码和数据的论文得到了完全或部分重现，而仅共享数据的论文中，这一比例仅为33%。数据文档的质量与成功重现相关。文档质量差或描述不准确的数据很可能导致重现失败。令人惊讶的是，代码文档的质量与成功重现无显著相关性。无论代码文档的质量如何，只要代码得到共享，就不会对成功重现造成影响。这项研究强调了开放科学的有效性，并突显了正确记录数据工作的重要性。 

---
# Bridging the Data Provenance Gap Across Text, Speech and Video 

**Title (ZH)**: 跨文本、语音和视频的数据溯源差距桥梁构建 
...
**Abstract (ZH)**: 人工智能的进步主要受到训练数据的规模和质量的推动。尽管如此，对于除文本之外广泛建立的数据集的属性进行实证分析的研究仍然不足。本文首次对跨模态（包括流行的文本、语音和视频数据集）进行全面纵向审计，从数据的详细采集趋势和使用限制，到其地理和语言代表性。我们手动分析了自1990年至2024年间近4000个公共数据集，覆盖了608种语言、798个来源、659个组织和67个国家。研究发现，多模态机器学习应用已主要转向网页抓取、合成和社交媒体平台（如YouTube）作为其训练集，从2019年起，这些来源已超越所有其他来源。其次，在追溯数据集的来源链中，我们发现尽管只有不到33%的数据集带有严格许可，但广泛使用的文本、语音和视频数据集中超过80%的原始内容都带有非商业限制。最后，与公共AI训练数据中语言和地理覆盖范围的上升趋势相反，我们的审计结果显示相对地理和多语言代表性测量仍未显著改善自2013年以来的覆盖率。我们认为我们的全面审计使我们能够从生态系统层面实证考察数据来源、限制和西方中心主义的趋势，而这些问题的透明度对于负责任的人工智能的发展至关重要。作为增强数据集透明度和负责任使用的一份贡献，我们发布了整个多模态审计，使从业者能够追踪文本、语音和视频数据的来源证明。 

---
# Decentralized Intelligence in GameFi: Embodied AI Agents and the Convergence of DeFi and Virtual Ecosystems 

**Title (ZH)**: 游戏金融中的去中心化智能：具身AI代理及其与去中心化金融和虚拟生态系统融合的研究 
...
**Abstract (ZH)**: 在快速发展的GameFi领域，即游戏与去中心化金融（DeFi）的融合，增强玩家参与度和经济互动的需求尤为迫切。我们的GameFi生态系统旨在通过将先进的具身人工智能代理整合到GameFi平台上，从根本上改变这一领域。这些人工智能代理使用最先进的大型语言模型（LLMs），如GPT-4和Claude AI，能够与玩家进行主动、适应性强且富有上下文的互动。这些代理超越了传统的预设回应机制，成为游戏中故事情节和经济系统的重要参与者，直接影响玩家策略和游戏内经济。我们解决了当前GameFi平台中缺乏沉浸式AI互动和社区参与机制的问题。通过将人工智能代理与区块链技术深度融合，我们建立了一个共识驱动的去中心化GameFi生态系统。这个生态系统赋予创作者通过其贡献获利的能力，并促进玩家和创作者之间的民主化合作。此外，我们将DeFi机制嵌入游戏体验中，增强了经济参与度并为游戏内的金融互动提供了新的机会。我们的方法提升了玩家的沉浸感和留存率，并通过将传统游戏与Web3技术结合，推进了GameFi生态系统的发展。通过整合复杂的AI和DeFi元素，我们促进了更具吸引力、经济上更稳健且更加注重社区合作的游戏环境的形成。该项目代表了GameFi领域的重大进展，提供了可以应用于整个游戏行业的见解和方法论。 

---
# DiTCtrl: Exploring Attention Control in Multi-Modal Diffusion Transformer for Tuning-Free Multi-Prompt Longer Video Generation 

**Title (ZH)**: DiTCtrl：在多模态扩散变换器中探索注意力控制以实现无调参多提示长视频生成 
...
**Abstract (ZH)**: 类似于Sora的视频生成模型在具有多模态扩散变换器（MM-DiT）架构的情况下取得了显著进展。然而，目前的视频生成模型主要侧重于单一提示，难以生成多个连续提示下的连贯场景，无法更好地反映真实世界的动态情景。虽然一些开创性的工作已经探索了多提示视频生成，但它们面临着严格的数据需求、提示跟随能力弱和不自然的过渡等问题。为了解决这些问题，我们首次提出了一种名为DiTCtrl的训练无监督的多提示视频生成方法，适用于MM-DiT架构。我们的核心思路是将多提示视频生成任务视为具有平滑过渡的视频编辑。为了实现这一目标，我们首先分析了MM-DiT的注意力机制，发现其3D全注意力模式类似于UNet-like扩散模型中的跨/自我注意力模块，这使得在多提示视频生成过程中能够通过注意力共享实现掩码引导的精准语义控制。基于我们精心设计的方法，DiTCtrl生成的视频在提供多个连续提示的情况下实现了平滑过渡和一致的对象运动，而无需额外的训练。此外，我们还提出了MPVBench，这是一个专门为多提示视频生成设计的新基准，用于评估多提示生成的性能。广泛的实验结果表明，我们的方法在无需额外训练的情况下实现了业界最佳性能。 

---
# A Paragraph is All It Takes: Rich Robot Behaviors from Interacting, Trusted LLMs 

**Title (ZH)**: 一节文字足矣：来自相互信赖的大型语言模型的丰富机器人行为 
...
**Abstract (ZH)**: 大型语言模型（LLMs）是公共环境知识以及动物和人类行为的紧凑表示。将LLMs应用于机器人技术可能提供一条道路，使得机器人能够在有限甚至无需微调的情况下，在大多数人类任务中表现出色。除了越来越复杂的推理和任务规划，适当设计的LLM网络提供了升级能力的便利性，并允许人类直接观察机器人的思维过程。在这里，我们探讨了使用LLMs控制物理机器人的优势、局限性和特殊性。基本系统由四个通过WebSocket和ROS2消息传递实现的人类语言数据总线进行通信的LLM组成。令人惊讶的是，尽管机器人的数据融合周期仅为1Hz，中央数据总线的运行速度也受到人类大脑的极限，约为40比特/秒，但仍然能够实现丰富多彩的机器人行为和在各种任务中良好的表现。自然语言在LLM之间进行通信使得人类可以直接观察机器人的推理和决策，并且用简单的英语编写的一组规则可以使系统的行为变得容易偏置。这些规则被不可更改地写入以太坊中，这是一个全球性、公开且抗审查的图灵完备计算机。我们建议，通过使用自然语言作为相互作用的人工智能之间的数据总线，并使用不可更改的公共账本来存储行为约束，有可能构建出兼具意外丰富性能、易于升级和持久人类一致性的机器人。 
...
**Abstract (ZH)**: 近年来，越来越多由代码大规模语言模型（LLM）驱动的AI编程助手被整合到各种实际的软件开发环境中，显著提升了开发人员的生产力。然而，现有的代码生成基准主要关注通用场景，使得LLM在特定应用领域的代码生成性能很大程度上仍然是未知的。本文引入了一个新的基准MultiCodeBench来填补这一空白。MultiCodeBench包含2,400个编程任务，涵盖了12种流行的软件开发领域和15种编程语言。具体而言，我们进行了深入研究以确定这12个应用领域。鉴于每个领域可能涉及多种技术框架，且不同的框架在编程过程中会带来不同的挑战，我们在每个领域内归类常用框架和平台。然后，我们从与这些子领域相关的GitHub仓库中抽取编程问题。为了确保任务的质量并缓解数据泄露问题，我们邀请注释者重新编写MultiCodeBench中每个任务的文档注释。此外，我们构建了一个基于静态分析的依赖解析工具，以提取每个任务的真实依赖关系，从而实现更深入的性能分析。通过在MultiCodeBench上进行广泛的实验，我们揭示了不同应用领域的LLM代码生成性能，为下游领域开发人员在选择LLM时提供了实用的见解。此外，我们分析了模型在完成软件应用开发任务时失败的原因，为模型开发人员提供了增强特定领域代码生成能力的指导。 

---
# Token-Budget-Aware LLM Reasoning 

**Title (ZH)**: 面向 tokens 预算的大型语言模型推理 
...
**Abstract (ZH)**: 推理对于大型语言模型（LLM）在广泛任务中取得优异表现至关重要。虽然像链式思考（Chain-of-Thought, CoT）这样的推理方法通过将问题分解为中间步骤来提升LLM的性能，但也带来了显著的token开销，增加了成本。我们发现当前LLM的推理过程过于冗长，通过在提示中包含合理的token预算，可以压缩推理过程，但token预算的选择在实际压缩效果中起着关键作用。我们随后提出了一个token预算感知的LLM推理框架，该框架根据推理复杂度动态估算不同问题的token预算，并利用估算出的token预算引导推理过程。实验结果显示，我们的方法在CoT推理中有效降低了token成本，仅略有性能下降，提供了一种平衡效率与准确性的实用解决方案。代码：点击这里🔗。

（注意：这里的“点击这里🔗”是一个占位符，实际的代码链接需要替换为实际的URL。） 

---
# Advancing Deformable Medical Image Registration with Multi-axis Cross-covariance Attention 

**Title (ZH)**: 基于多轴交叉协方差注意力的可变形医疗图像配准方法研究 
...
**Abstract (ZH)**: 变形图像配准是医学图像分析的基本要求。近年来，基于深度学习的配准方法广泛使用变压器，因为自注意力机制（SA）能够通过自注意力捕捉长距离依赖关系。然而，自注意力机制的高计算和内存负载（随着空间分辨率的平方增长）阻碍了变压器在处理高分辨率图像特征中的细微纹理信息（例如，在全分辨率和半分辨率图像中）方面的能力。这限制了变形配准的应用，因为高分辨率纹理信息对于在细微解剖结构之间找到精确的逐像素对应关系至关重要。交叉协方差注意力（XCA）是一种“转置”的自注意力机制，它在特征通道之间进行操作，其复杂度与空间分辨率呈线性增长，为捕捉高分辨率图像特征间的长距离依赖关系提供了可能性。然而，现有的基于XCA的变压器仅能捕获粗略的全局长距离依赖关系，这不适合主要依赖细粒度局部对应的变形图像配准。在本研究中，我们提出了一种改进现有基于深度学习的配准方法的方法，即嵌入新的XCA机制。为此，我们设计了一种专为变形医学图像配准优化的XCA基础变压器块，名为Multi-Axis XCA（MAXCA）。我们的MAXCA作为一种通用网络块，可以嵌入到各种配准网络架构中。通过多轴设计并行应用区域和扩张XCA，它可以同时捕捉高分辨率图像特征间的全局和局部长距离依赖关系。在两个广泛基准化的跨/内病人配准任务中，使用七个公开医学数据集进行的广泛实验表明，我们的MAXCA块能够实现最先进的配准性能。 
...
**Abstract (ZH)**: 预测是一项难以评估的任务：真实情况只有在未来才能知晓。最近的研究显示，大规模语言模型（LLM）预测器迅速接近人类水平的表现，因此提出了一个问题：我们如何能够瞬时地对标和评估这些预测器？我们基于一致性检查框架，从不同逻辑相关的问题上的一致性来衡量预测器的表现。我们提出了一种基于套利原则的一致性度量方法：例如，如果一个预测AI不切实际地预测民主党与共和党在2024年美国总统选举中获胜的概率均为60%，那么套利者可以利用预测后果进行交易并获利。我们建立了一个自动化评估系统，该系统生成一组基础问题，从这些问题中实例化一致性检查，要求预测器提出预测，并测量预测的一致性。然后，我们构建了一个标准的贝叶斯评分明基准，证明了我们的（即时性）一致性度量与大型语言模型预测器的未来未知的真实布里尔得分呈相关性。我们还发布了一个将于2028年解决的度量基准，为长期预测评估提供了一个工具。 
...
**Abstract (ZH)**: 我们研究了Kleinberg和Mullainathan [KM24]引入的极限语言生成问题，这一问题建立在Gold [Gol67]和Angluin [Ang79]的古典工作之上。[KM24]提出了一种算法，能够从任意可数语言集合中生成字符串。虽然该算法最终会产生目标语言$K$中的字符串，但它牺牲了广度，即无法生成$K$中的所有字符串。[KM24]中的一个关键开放问题是这种一致性与广度之间的权衡是否不可避免。

近期的研究提出了不同的广度一致生成的概念。Kalavasis、Mehrotra和Velegkas [KVM24]引入了三种定义：精确广度生成、近似广度生成和去模糊生成。同时，Charikar和Pabbaraju [CP24a] 提出了全面生成。两组研究都探讨了在这些广度概念下生成的可能性。

在[CP24a, KVM24]的基础上，我们全面描述了这些概念及其自然组合下的语言生成。对于精确广度生成，我们提供了一个无条件的下界，移除了[KVM24]中的一个技术性条件，并扩展了[CP24a]的结果，该结果适用于特定语言集合。我们证明精确广度生成由Angluin识别条件来刻画。在此基础上，我们引入了一个较弱的Angluin条件，该条件能够精确刻画近似广度生成和全面生成，并证明了它们的等价性。此外，我们展示了去模糊生成也可以通过Angluin条件来刻画，作为更广泛结果的一个特例。最后，我们增强了[KVM24]中的结果，给出了稳定生成器的无条件下界，证明了Angluin条件刻画了稳定生成器下的先前广度概念。这一结果表明在近似广度下稳定生成与不稳定生成之间的区别。 

---
# Joint Adaptive OFDM and Reinforcement Learning Design for Autonomous Vehicles: Leveraging Age of Updates 

**Title (ZH)**: 自动驾驶车辆中适应性OFDM与强化学习设计的联合研究：利用更新年龄 
...
**Abstract (ZH)**: 基于毫米波（mmWave）的正交频分复用（OFDM）在高分辨率传感和高速数据传输方面表现出色，是合适的替代方案。为了满足通信和传感需求，许多研究提出了静态配置，其中波的超参数，如每帧符号数和通信时隙中的帧数已经预定义。然而，两个事实迫使我们重新定义该问题：（1）环境通常是动态和不确定的；（2）mmWave严重受无线环境影响。一个典型例子，其中该挑战非常突出的是自动驾驶车辆（AV）。该系统利用基于毫米波的集成传感与通信（ISAC）来管理数据传输和环境的动态变化。在此项工作中，我们考虑一个自动驾驶车辆网络，其中AV利用其队列状态信息（QSI）和信道状态信息（CSI），结合强化学习技术来管理通信与传感。这使AV能够实现两个主要目标：与其他AV建立稳定的通信链接，并以高分辨率准确估计周围物体的速度。因此，通信性能的评估基于队列状态、有效数据传输速率和丢弃包率。相比之下，传感效果的评估使用的是速度分辨率。此外，我们利用自适应OFDM技术进行动态调制，并建议一种基于更新年龄的奖励函数来处理通信缓存并提高传感效果。我们使用优势演员批评（A2C）和近端策略优化（PPO）来验证该系统，并通过计算机仿真与现有设计进行比较，展示了其更优越的性能。 

---
# How "Real" is Your Real-Time Simultaneous Speech-to-Text Translation System? 

**Title (ZH)**: 你的实时同步 speech-to-text 翻译系统有多“真实”？ 
...
**Abstract (ZH)**: 同步口译转文本（SimulST）在说话人说话的同时将源语言口语转化为目标语言文本，以确保低延迟，从而提高用户的理解能力。尽管SimulST最初应用目标是无限制的口语，但大多数研究主要集中在人类预先分段的口语上，简化了任务并忽视了显著的挑战。这种狭窄的研究重点以及广泛存在的术语不一致性限制了研究结果在实际应用中的适用性，最终阻碍了该领域的发展。我们对110篇论文的广泛文献综述不仅揭示了当前研究中的这些关键问题，也为我们的核心贡献奠定了基础。我们1）定义了SimulST系统的工作步骤和核心组件，提出了一套标准化的术语和分类体系；2）对社区趋势进行了全面分析；3）提供了具体的建议和未来发展方向，以弥补现有文献中的差距，从评估框架到系统架构，推动该领域朝着更加现实和有效的SimulST解决方案发展。 

---
# An Overview and Discussion of the Suitability of Existing Speech Datasets to Train Machine Learning Models for Collective Problem Solving 

**Title (ZH)**: 现有语音数据集在训练用于集体问题解决的机器学习模型中的适用性综述与讨论 
...
**Abstract (ZH)**: 本报告评估了现有数据集在设计新的机器学习模型、决策方法和分析算法以提高协作解决问题方面的适用性，同时列出了未来所需数据集的要求。假设问题解决是在由约三到四名成员组成的团队中进行的，这些成员相互交流。数据集由这些团队的对话录音组成。评估方法基于能够捕捉认知、社会和情感活动及情境的指标。报告分析了用于语音理解的一组大型数据集，该领域在某些方面与协作解决问题相似。 

---
# GeFL: Model-Agnostic Federated Learning with Generative Models 

**Title (ZH)**: GeFL：基于生成模型的模型无关 federated 学习 
...
**Abstract (ZH)**: 联邦学习（FL）是一种在保持用户隐私的同时进行分布式学习的有前景范式。然而，近年来模型的越来越大使得少数用户难以承担整个模型的开销。这促使用户根据各自的计算能力和网络带宽采用异质模型。因此，鉴于FL通常涉及训练单一全局模型，异质模型的FL应当予以解决。在本文中，我们提出了生成模型辅助联邦学习（GeFL），其中包括一个生成模型，该生成模型可以聚集异质模型用户之间的全局知识。我们在各种分类任务上的实验表明，GeFL在性能上显著优于基线模型，但在隐私和可扩展性方面存在一定的局限性。为解决这些挑战，我们引入了一个新颖的框架GeFL-F。该框架利用特征生成模型来辅助训练目标网络。我们实证研究表明，GeFL-F在保持一致性能增益的同时，更好地保护了隐私，并对大量客户端更具鲁棒性。代码可以在此处获取 [1]。
...
**Abstract (ZH)**: 随着社会日益依赖人工智能（AI），不法分子越来越多地利用AI进行攻击行为。尽管已有研究揭示了多种AI导致安全和隐私问题的案例，但目前尚无研究能全面描绘出AI的攻击潜能。在这篇综述论文中，我们旨在为系统分析异质性攻击AI能力奠定基础。具体来说，我们将从两个方面进行研究：（i）考虑AI对人类和系统的影响；（ii）整合和提炼学术文献、专家意见、产业环境以及普通公众的相关知识，这些信息来源都是研究攻击性AI的重要资源。

为了使这些多样化的知识来源实现对齐，我们提出了一套共同的评估标准，这些标准反映了与攻击性AI密切相关的关键技术因素。借助这些标准，我们系统地分析了以下内容：95篇研究论文；38份信息安全简报（例如，来自BlackHat的简报）；包含549名背景和专长各异的个体的研究用户反馈；以及12位专家的意见。我们的研究不仅揭示了AI目前可用于攻击的一些方式（其中一些方式可能被先前的研究所忽视），也为未来应对这一威胁提供了基础。 

---
# GeAR: Graph-enhanced Agent for Retrieval-augmented Generation 

**Title (ZH)**: GeAR：图增强代理的检索增强生成方法 
...
**Abstract (ZH)**: 检索增强生成系统依赖于有效的文档检索能力。在设计上，传统的稀疏或密集检索器在多跳检索场景中面临挑战。在本文中，我们提出了GeAR，通过两项关键创新提升了RAG的性能：(i) 图扩展，该技术能够增强任何传统的基线检索器，例如BM25；(ii) 一种结合图扩展的代理框架。我们的评估结果显示，GeAR在三个多跳问答数据集上的检索性能显著优于其他基线。此外，我们的系统在具有挑战性的MuSiQue数据集上取得了最先进的结果，相比其他多步检索系统，GeAR在所需令牌数和迭代次数上均有所减少，性能提升超过10%。 

---
# Multilingual Mathematical Reasoning: Advancing Open-Source LLMs in Hindi and English 

**Title (ZH)**: 多语言数学推理：促进印地语和英语开源大语言模型的发展 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在语言任务中表现出色，但在数学推理方面存在不足，特别是在非英语语言如 Hindi 中。本研究旨在提升小型、资源高效的开源 LLMs 在 Hindi 和英语中的数学推理能力。我们使用零样本、少样本链式思考（CoT）方法和监督微调来评估 OpenHathi 7B、LLaMA-2 7B、WizardMath 7B、Mistral 7B、LLeMMa 7B、MAammoTH 7B、Gemini Pro 和 GPT-4 等模型。我们的方法结合了层次学习，逐步训练模型解决越来越复杂的问题，创新地引入了一种分解策略来简化复杂的算术操作，以及结构化解决方案设计，将解决方案划分为不同阶段。实验结果显示了显著的性能提升。WizardMath 7B 在英语数据集上的准确率超过 Gemini 的 6%，在 Hindi 数据集上的性能与 Gemini相当。采用结合英语和 Hindi 样本的双语方法取得了与单一语言模型相近的结果，证明了在两种语言中学习数学推理的能力。本研究突显了提高开源 LLMs 数学推理能力的潜力。 

---
# A Statistical Framework for Ranking LLM-Based Chatbots 

**Title (ZH)**: 基于大规模语言模型的聊天机器人的排名统计框架 
...
**Abstract (ZH)**: 大型语言模型（LLMs）已经彻底改变了自然语言处理领域，而类似Chatbot Arena这样的框架提供了评估这些模型的先锋平台。通过基于人工判断进行数百万次的成对比较，Chatbot Arena已成为LLM评估的重要基石，提供了丰富的数据集，用于在开放式的对话任务中对模型进行排名。在此基础上，我们提出了一种统计框架，结合关键进展以解决成对比较分析中的特定挑战。首先，我们引入了一个因素关联模型，以增强处理系绑的能力——这是人类判断比较中的一个关键方面，显著提高了模型对观测数据的拟合度。其次，我们将框架扩展以建模竞争对手之间的协方差，从而更深入地了解性能关系，并促进直观地划分性能等级。第三，我们通过引入新型约束条件解决了由参数非唯一性带来的优化挑战，确保了参数估计的稳定性和可解释性。通过严格的评估和广泛的实验，我们的框架在建模成对比较数据方面展示了显著的优势。为了支持再现性和实际应用，我们发布了leaderbot，这是一款开源的Python包，实现了我们的模型和分析。 

---
# TPAoI: Ensuring Fresh Service Status at the Network Edge in Compute-First Networking 

**Title (ZH)**: TPAoI：在计算优先网络中确保边缘网络服务状态的 freshness 
...
**Abstract (ZH)**: 在计算优先网络中，在网络边缘保持最新的准确状态信息对于有效访问远程服务至关重要。这一过程通常涉及三个阶段：状态更新、用户访问和用户请求。然而，现有的关于状态有效性的研究，如查询时的信息年龄（QAoI），未能全面涵盖所有这些阶段。因此，本文提出了一种新的度量标准TPAoI，旨在通过测量服务状态的新鲜度来优化更新决策。边缘环境中固有的随机性，表现为更新、请求和用户访问时间的不可预测通信延迟，给建模带来了显著的挑战。为此，我们将该问题建模为马尔可夫决策过程（MDP），并采用双层双深度Q网络（D3QN）算法进行优化。大量实验表明，提出的TPAoI度量标准有效地减少了信息年龄（AoI），确保在动态边缘环境中实现及时可靠的更新。结果表明，与QAoI度量相比，TPAoI将AoI平均减少了47%，相对传统的AoI度量降低了48%的更新频次，显示出显著的改进效果。 
...
**Abstract (ZH)**: 扩散概率模型（DPMs）已成为高保真图像合成的默认方法，通过在连续的VAE潜在空间上执行扩散过程，这与大型语言模型（LLMs）所采用的文本生成方法有显著差异。在本文中，我们提出了一种新的生成框架——递归扩散概率模型（RDPM），通过递归的标记预测机制来增强扩散过程，从而开创了离散扩散的领域。通过逐步将高斯噪声引入图像的潜在表示，并以递归方式编码为矢量量化标记，RDPM 促使了一种独特的离散值域上的扩散过程。该过程迭代地预测后续时间步的标记代码，将初始的标准高斯噪声逐步转化为源数据分布，损失函数与GPT类模型类似。RDPM 在保持计算速度优势（仅需少量推理步骤）的情况下表现出优异的性能。这个模型不仅利用扩散过程确保高质量生成，还将连续信号转换为一系列高保真的离散标记，从而与文本等其他离散标记保持统一的优化策略。我们期待这项工作将有助于开发多模态生成的统一模型，特别是通过将图像、视频、音频等连续信号域与文本集成。我们将开源代码和模型权重供社区使用。 

---
# ChaI-TeA: A Benchmark for Evaluating Autocompletion of Interactions with LLM-based Chatbots 

**Title (ZH)**: ChaI-TeA: 一种评估基于大语言模型的聊天机器人交互自动完成效果的基准 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）的发展使得越来越多的人机交互转向基于LLM的聊天机器人。这些模型的强大能力使得用户能够使用包含广泛主题和风格的长篇自然语言文本进行交互。撰写这些消息是耗费时间和精力的任务，因此需要一种自动完成方案来辅助用户。我们引入了聊天机器人交互自动完成任务。我们提出了ChaI-TeA：聊天交互自动完成；这是一个用于LLM基于聊天机器人交互的自动完成评估框架。该框架包括任务的正式定义，以及合适的数据集和评估指标。我们使用该框架对任务进行了评估，具体步骤如下：首先正式定义任务并提供合适的数据集和评估指标；然后对9个模型进行了自动完成任务的测试，发现虽然当前的现成模型表现相当不错，但仍然有很大的改进空间，尤其是在生成建议的排名方面。我们为在该任务上工作的实践者提供了见解，并为该领域的研究人员开辟了新的研究方向。我们将发布该框架，以作为未来研究的基础。 

---
# A Many Objective Problem Where Crossover is Provably Indispensable 

**Title (ZH)**: 一个交叉操作被证明必不可少的多目标问题 
...
**Abstract (ZH)**: 本文探讨了进化多目标优化（EMO）中的理论问题，并重点讨论了在多目标优化中交叉操作的作用。交叉操作的优势尚不清楚，对于包含交叉操作的严格运行时分析远远落后于实际应用，特别是在多于两个目标的情况下。我们提出了一类多目标问题，并对该问题上的广泛使用的NSGA-III进行了理论运行时分析，以证明交叉操作可以在运行时提供指数级的加速效果。特别是，在使用交叉操作的情况下，该算法可以预期在多项式时间内找到帕雷托前沿，而无需交叉操作，则需要指数级的时间来找到一个帕雷托最优解。据我们所知，这是首次在多目标优化中进行严格运行时分析，并且展示了在多于两个目标的情况下使用交叉操作时存在指数级性能差距。 

---
# Unveiling the Threat of Fraud Gangs to Graph Neural Networks: Multi-Target Graph Injection Attacks against GNN-Based Fraud Detectors 

**Title (ZH)**: 揭示诈骗团伙对图神经网络的威胁：针对基于图神经网络的诈骗检测器的多目标图注入攻击 
...
**Abstract (ZH)**: 图神经网络（GNNs）已经被证明是一种有效的欺诈检测工具，能够识别欺诈用户并揭露恶意行为。然而，针对基于GNN的欺诈检测器的攻击及其风险的研究相对较少，这使得潜在威胁未能得到充分解决。最近的研究表明，欺诈行为越来越组织成团伙或集团。在本文中，我们设计了攻击场景，其中欺诈团伙的目标是通过共谋掩饰其非法活动，从而使欺诈节点被误分类为正常节点。基于这些场景，我们通过模拟欺诈团伙在三个实际欺诈案例中的攻击，研究了针对基于GNN的欺诈检测器的对抗攻击：虚假评论、假新闻和医疗保险欺诈。我们将这些攻击定义为多目标图注入攻击，并提出了一种基于变压器的多目标一次性图注入攻击模型MonTi。MonTi使用变压器编码器同时生成所有攻击节点的属性和边，相比大多数仅按顺序生成这些元素的现有图注入攻击方法，更有效地捕捉属性和边之间的相互依赖关系。此外，MonTi针对每个攻击节点自适应分配度预算，以探索涉及目标、候选和攻击节点的多样化注入结构，而现有的方法只是在整个攻击节点上固定度预算。实验结果表明，MonTi在五个实际图数据集上优于最先进的图注入攻击方法。 

---
# Hypergraph Attacks via Injecting Homogeneous Nodes into Elite Hyperedges 

**Title (ZH)**: 通过向精英超边注入同质节点进行超图攻击 
...
**Abstract (ZH)**: 最近的研究表明，超图神经网络（HGNNs）对 adversarial（对抗性）攻击较为脆弱。现有的方法主要关注基于梯度的超图修改攻击，忽视了超图中的节点覆盖以及超边的组身份，这导致攻击效果有限且容易被检测到。本文介绍了一种新的框架，即通过向精英超边注入同质节点来对抗超图攻击（IE-Attack），以应对这些挑战。首先，利用超图中的节点覆盖特性，我们提出了一种精英超边采样器以识别出需要注入的超边。其次，我们利用内核密度估计（KDE）提出了一种节点生成器来生成带有超边组身份的同质节点。最后，通过将同质节点注入到精英超边中，IE-Attack 提高了攻击性能并增强了攻击的不可察觉性。我们在五个真实的数据集上进行了广泛的实验，以验证 IE-Attack 的有效性及其相对于现有最佳方法的优势。 

---
# Point-DeepONet: A Deep Operator Network Integrating PointNet for Nonlinear Analysis of Non-Parametric 3D Geometries and Load Conditions 

**Title (ZH)**: 点DeepONet：集成PointNet的深度算子网络，用于非参数3D几何形状和载荷条件的非线性分析 
...
**Abstract (ZH)**: 工程领域的非线性结构分析经常需要进行大量的有限元模拟，这限制了它们在设计优化、不确定性量化和实时控制等方面的应用。传统的基于深度学习的代理模型，如卷积神经网络（CNNs）、物理知情神经网络（PINNs）和傅里叶神经操作符（FNOs），在处理复杂非参数三维（3D）几何形状、方向性变化的载荷以及非结构化网格上的高保真预测时面临挑战。本文提出了基于操作符学习的代理模型Point-DeepONet，该模型将PointNet整合到DeepONet框架中。通过直接处理非参数点云，并结合法向距离函数（SDF）提供几何上下文，Point-DeepONet能够在无需网格参数化或重新训练的情况下，准确预测三维位移和von Mises应力场。仅通过使用大约5,000个节点（原始200,000节点网格的2.5%），Point-DeepONet仍能在高保真度下预测整个网格，在一个水平载荷情况下，位移的决定系数达到了0.987，von Mises应力的决定系数达到了0.923。相比需要大约19.32分钟每个案例的非线性有限元分析，Point-DeepONet可以提供几乎即时的预测，大约快400倍，同时随着数据集规模的增加，保持了良好的扩展性和准确性。这些结果突显了Point-DeepONet在实现快速、高保真结构分析方面的潜力，最终支持更有效的结构设计探索和基于数据的决策，在复杂的工程工作流程中发挥了重要作用。 

---
# Addressing Spatial-Temporal Data Heterogeneity in Federated Continual Learning via Tail Anchor 

**Title (ZH)**: 通过Tail Anchor解决联邦持续学习中的时空数据异质性问题 
...
**Abstract (ZH)**: 联邦持续学习（Federated Continual Learning, FCL）允许每个客户端从任务流中不断地更新其知识，从而增强了联邦学习在实际场景中的应用性。然而，FCL 不仅需要解决客户端之间存在的空间数据异质性，还需要解决任务之间存在的时间数据异质性。本文通过实验证明，这种输入级别上的异质性显著影响了模型的内部参数和输出，导致严重的局部和先前知识的时空灾难性遗忘。为此，我们提出了联邦尾锚（FedTA）方法，通过结合可训练的尾锚与冻结的输出特征，调整其在特征空间中的位置，从而克服参数遗忘和输出遗忘。此外，FedTA 还包括三个新型组件：输入增强，用于提高预训练模型在下游任务中的性能；选择性输入知识融合，用于服务器端异质局部知识的融合；和最佳全局原型选择，用于在特征空间中为每个类别找到最优锚点。广泛的实验表明，FedTA 不仅优于现有的 FCL 方法，还能够有效保持特征的相对位置，不受空间和时间变化的影响。 

---
# Multi-Agents Based on Large Language Models for Knowledge-based Visual Question Answering 

**Title (ZH)**: 基于大型语言模型的多agents知识驱动视觉问题回答 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在基于知识的视觉问答（VQA）任务上取得了令人印象深刻的成果。然而，现有方法仍然面临一些挑战：无法自主使用外部工具，以及无法进行团队协作。人类在遇到新问题时通常能够判断是否需要使用外部工具，例如，他们倾向于对熟悉的问题给出直接答案，而在遇到不熟悉的问题时，则倾向于使用像搜索引擎这样的工具。此外，人类还倾向于与他人合作并讨论，以获得更好的答案。受此启发，我们提出了多智能体投票框架。我们设计了三个基于LLM的智能体来模拟团队中不同级别的角色，并根据这些角色分配可用的工具。每个智能体提供相应的答案，最后通过投票将所有智能体提供的答案整合成最终答案。在OK-VQA和A-OKVQA数据集上的实验结果显示，我们的方法分别比其他基线方法提高了2.2和1.0的性能。 

---
# The Value of AI-Generated Metadata for UGC Platforms: Evidence from a Large-scale Field Experiment 

**Title (ZH)**: AI生成元数据对UGC平台的价值：一项大规模现场实验的证据 
...
**Abstract (ZH)**: 生成AI（AIGC）的内容，例如广告文案、产品描述和社会媒体帖子，在企业实践中变得越来越普遍。然而，生成AI（AIGC）元数据的价值，例如标题，在用户生成内容（UGC）平台上仍不明确。为了解决这一问题，我们在亚洲一个领先的短视频平台上进行了大规模实地实验，为约100万用户提供了AI生成标题的访问权，以供其上传的视频使用。我们的研究发现，提供AI生成的标题显著提升了内容的消费量，增加了有效观看次数1.6%和观看时长0.9%。当内容创作者采用这些标题时，这些增幅分别达到了7.1%和4.1%。这种观众增长效应主要归因于使用这种生成性AI（GAI）工具增加了视频具有标题的可能性，提高了41.4%。这种效应在受到元数据稀疏影响更大的群体中更为明显。机制分析表明，AI生成的元数据在平台推荐系统中提高了用户-视频匹配的准确性。有趣的是，对于内容创作者本来会自行添加标题的视频，采用AI生成的标题反而对其平均观众量有所降低，这意味着AI生成的标题可能不如人工生成的高质量。然而，当内容创作者选择与GAI协同创作，并显著修订AI生成的标题时，视频的表现超过了完全由AI或人工生成标题的对照组，展示了人类与AI协同创作的优势。本研究突显了在UGC平台上，AI生成元数据和人机协同创作对提高用户内容匹配和内容消费的价值。 

---
# FloNa: Floor Plan Guided Embodied Visual Navigation 

**Title (ZH)**: FloNa：基于楼层平面图的体态视觉导航 
...
**Abstract (ZH)**: 人类自然依赖于平面图在不熟悉的环境中导航，因为平面图易于获取、可靠并且提供了丰富的几何指导。然而，现有的视觉导航任务忽视了这种宝贵的知识，导致导航效果有限。为消除这种差距，我们提出了一项新的导航任务：平面图视觉导航（FloNa），这是首次尝试将平面图纳入体感视觉导航。尽管平面图提供了显著的优势，但仍出现了两个主要挑战：(1) 在保证无碰撞导航的同时，处理平面图与实际场景布局之间的空间不一致性，以及(2) 尽管它们具有不同的模态，仍需对观测图像与平面图草图进行对齐。为应对这些挑战，我们提出了一种新颖的扩散政策框架FloDiff，该框架包含一个定位模块，以帮助当前观测与平面图之间的对齐。我们还在iGibson仿真器中收集了117个场景的20,000个导航实例，以支持训练和评估。广泛的实验表明，我们的框架在使用平面图知识的不熟悉场景中具有有效性与高效性。项目网站：[这里](this https URL)。 

---
# Exploring Graph Mamba: A Comprehensive Survey on State-Space Models for Graph Learning 

**Title (ZH)**: 探索Graph Mamba：关于图学习中状态空间模型的全面综述 
...
**Abstract (ZH)**: Graph Mamba，作为一种强大的图嵌入技术，在生物信息学、社交网络和推荐系统等领域已成为关键的基石。本综述是首个致力于Graph Mamba的全面研究，旨在解决其应用、挑战和未来潜力理解中的关键空白。我们首先提供对原版Graph Mamba架构的详细解释，强调其关键组成部分和工作原理。随后，我们探讨了最近提出的各种改进和增强措施，以提高其性能和适用性。为了展示Graph Mamba的多样性和灵活性，我们研究了其在不同领域的应用。我们还对Graph Mamba及其变体进行了比较分析，以揭示它们的独特特性和潜在应用场景。此外，我们确定了Graph Mamba在未来可能的应用领域，并突显其在这些领域中可能引发的数据分析变革。最后，我们讨论了与Graph Mamba相关的当前限制和待解决的研究问题，通过承认这些挑战，我们旨在激发该领域进一步的研究和发展。本综述为新手和有经验的研究者提供了宝贵资源，帮助他们理解和利用Graph Mamba的潜力。 

---
# Mulberry: Empowering MLLM with o1-like Reasoning and Reflection via Collective Monte Carlo Tree Search 

**Title (ZH)**: 桑树：通过集体蒙特卡洛树搜索增强MLLM的o1-like 推理与反思能力 
...
**Abstract (ZH)**: 在本文中，我们致力于开发一种多语言逻辑模型（MLLM），使其能够通过学习生成推理过程中的每一中间步骤来理解和回答问题，直到最终答案。为此，我们提出了一种名为集体蒙特卡洛树搜索（CoMCTS）的新推理学习方法，该方法将集体学习引入“树搜索”，以实现有效的推理路径搜索和学习。CoMCTS的核心思想是利用多个模型的集体知识，通过扩展、模拟、错误定位、反向传播和选择四个迭代操作，协作地假设、搜索并识别通向正确答案的有效推理路径。

使用CoMCTS，我们构建了包含丰富、明确推理节点树的Multberry-260k多模态数据集，每个问题都有一个这样的树。借助Multberry-260k，我们进行了集体的强化学习（SFT），训练出了具备类似于o1逐步推理和反思能力的一系列MLLMs（Multberry模型）。广泛的经验研究表明，我们提出的方法在各个基准测试中表现出优越性。相关代码将会发布在如下链接：此 https 链接。 
...
**Abstract (ZH)**: 自监督图表示学习（SSGRL）是一种减少或避免手动标注的表示学习范式。SSGRL 中的一个关键部分是图数据增强。现有方法通常依赖于通过试错识别出的一些启发式方法，并且这些方法在某些应用场景中才有效。此外，目前尚不清楚为什么一种启发式方法优于另一种。近年来，一些研究反对使用某些技术（例如，会引起分子图性质变化或破坏基于图的文档分类任务中相关信号的 dropout 技术）。

在本研究中，我们提出了一种基于数据的 SSGRL 方法，能够从图中编码的信号（即节点的预测特征和拓扑信息）自动学习合适的图增强。我们提出了两种互补的方法，分别生成可学习的特征增强和拓扑增强。前者学习节点特征的多视图增强，后者学习拓扑的高阶视图。此外，这些增强是与表示一起学习的。我们的方法具有通用性，可以应用于同质图和异质图。我们在节点分类（使用九个同质和异质数据集）和图属性预测（使用八个数据集）中进行了广泛实验。实验结果表明，所提出的方法能够匹配甚至超越当前最佳的 SSGRL 基线，并且与半监督方法的性能相当。匿名源代码可在以下网址获取：[提供网址] 

---
# M-Ped: Multi-Prompt Ensemble Decoding for Large Language Models 

**Title (ZH)**: M-Ped：面向大规模语言模型的多提示ensemble解码 
...
**Abstract (ZH)**: 随着大型语言模型（LLMs）在自然语言处理（NLP）领域的广泛应用，提升其性能成为研究热点。本文提出了一种新颖的多提示批处理解码方法，通过利用多个提示结果的聚合来增强LLMs的生成质量。对于给定的独特输入$X$，我们以批处理模式提交$n$个$X$的提示变体给LLMs进行解码，并推导出概率分布。对于每个词元预测，我们通过计算批处理内的$n$个概率分布的平均值来计算集成概率，并利用该聚合概率生成词元。这种方法被称为内批集成（Inner-Batch Ensemble）。为了支持高效的批处理推理，我们实现了左填充策略，以保持$n$个提示之间输入长度的一致性。通过在机器翻译、代码生成和文本简化等多种NLP任务上的广泛实验，我们展示了该方法在增强LLMs性能方面的有效性。结果显示，与传统方法相比，该方法在BLEU分数、pass@$k$率和LENS指标上取得了显著的改进。 

---
# Quo Vadis, Anomaly Detection? LLMs and VLMs in the Spotlight 

**Title (ZH)**: 《通往何方：异常检测领域的发展方向——大语言模型和多模态大模型的 Spotlight》

注：这里的“Spotlight”一词，根据上下文，可以理解为这两个模型在异常检测领域受到了特别的关注或重视。因此，在翻译时，我们将其意译为“发展方向”的部分，以更好地反映出原英文标题的含义。 
...
**Abstract (ZH)**: 视频异常检测（VAD）通过将大型语言模型（LLMs）和多模态视觉-语言模型（VLMs）结合起来取得了显著的进步，解决了诸如可解释性、时间推理和泛化能力等关键挑战，特别是在动态且开放世界场景中的问题。本文对2024年基于LLM/VLM的最新方法进行了深入综述，重点关注四个关键方面：（i）通过语义洞察和文本解释增强可解释性，使视觉异常更容易理解；（ii）捕捉复杂的时序关系，以检测和定位视频帧中的动态异常；（iii）实现少样本和零样本检测，以减少对大量标注数据的依赖；（iv）通过使用语义理解和运动特征解决开放世界和类无关联的异常，实现时空一致性。我们强调了这些方法重新定义VAD领域的潜力。此外，我们探讨了LLMs和VLMs提供的视觉与文本模态之间的协同作用，突显了它们的综合优势，并提出了未来方向，以充分利用这些方法增强视频异常检测的潜力。 

---
# Navigating Data Corruption in Machine Learning: Balancing Quality, Quantity, and Imputation Strategies 

**Title (ZH)**: 在机器学习中导航数据破坏：平衡质量、数量与填充策略 
...
**Abstract (ZH)**: 数据 corruption，包括缺失和噪声数据，对现实世界的机器学习提出了显著挑战。本研究探讨了数据 corruption 对模型性能的影响，并通过两种实验设置探究了缓解这些影响的策略：自然语言处理任务的监督学习（NLP-SL）和交通信号优化的深度强化学习（Signal-RL）。我们分析了数据 corruption 水平与模型性能之间的关系，评估了数据插补方法的有效性，并评估了扩大数据集以应对数据 corruption 的实用性。
结果显示，在数据 corruption 情况下，模型性能遵循一个递减回报曲线，符合指数函数模型。缺失数据虽然有害，但其危害程度低于噪声数据，后者会导致严重的性能下降和训练不稳定性，尤其是在 Sequential 决策任务如 Signal-RL 中。插补策略存在权衡：它们可以恢复缺失信息，但可能引入噪声。它们的效果取决于插补准确性及 corruption 比例。我们识别了插补优势热图中的不同区域，包括一个“插补优势角落”和一个“插补劣势边缘”，并根据其决策边界将任务分为“噪声敏感”或“噪声不敏感”两类。
此外，我们发现增加数据集大小可以在一定程度上减轻但无法完全克服数据 corruption 的影响。随 corruption 增加，额外数据的边际效用递减。一种经验规则出现：大约 30% 的数据是决定性能的关键部分，而剩下的 70% 对性能影响较小。
这些发现为数据预处理、插补策略和数据采集实践提供了实际指导，有助于开发在噪声环境中更加稳健的机器学习系统。 
...
**Abstract (ZH)**: 代码审查是软件开发中至关重要但要求较高的方面，引发了对自动审查评论的兴趣。传统的评论评估方法，主要基于文本相似性，面临着两大主要挑战：开源项目中人工撰写的评论的一致性可靠性较低，以及文本相似性与提高代码质量、缺陷检测等目标之间的弱相关性。

本研究采用一套新颖的评价标准对基准评论进行实证分析，这些标准由先前的研究和开发者访谈所启发。我们随后以类似的方式重新审视现有的评估方法。我们的评价框架 DeepCRCEval 结合了人工评估者和大规模语言模型（LLMs），基于设定的标准对当前技术进行全面再评估。此外，我们还引入了一种创新且高效的基线方法 LLM-Reviewer，利用 LLM 的少量示例学习能力进行有针对性的比较。

我们的研究突显了文本相似性度量的局限性，发现只有不到10%的基准评论适合自动化。相比之下，DeepCRCEval 能够有效地区分高质量和低质量的评论，证明其是一种更可靠的评估机制。将 LLM 评估者纳入 DeepCRCEval 显著提高了效率，分别将时间和成本减少了88.78%和90.32%。此外，LLM-Reviewer 显示出在评论生成中集中于任务目标的巨大潜力。 

---
# Towards understanding how attention mechanism works in deep learning 

**Title (ZH)**: 深入理解深度学习中注意力机制的工作原理 
...
**Abstract (ZH)**: 注意力机制已在主流神经网络架构中得到广泛应用，如Transformer和图形注意力网络。然而，其内在的工作原理仍然不够清楚。它的本质是什么？它与传统的机器学习算法之间是否存在联系？在本研究中，我们从流形学习、聚类和监督学习的经典度量和向量空间属性出发，检查相似性计算的过程。我们识别了这些方法中相似性计算和信息传播的关键特征，并证明了深度学习中的自我注意力机制遵循相似的原则，但更灵活和适应性强。我们将自我注意力机制分解为一个可学习的伪度量函数和基于相似性计算的信息传播过程。在伪度量是一个度量的变换且某些合理的假设成立的情况下，我们证明自我注意力机制通过连续建模收敛于漂移-扩散过程。此外，我们给出了一个一般伪度量函数下的注意力机制的一阶分析。这项研究通过物理直觉帮助我们理解注意力机制的效果及其原理。最后，我们通过利用度量学习的概念提出了一个改进的注意力机制——度量注意力，以增强学习所需度量的能力。实验结果表明，在训练效率、准确性和鲁棒性方面，度量注意力机制优于自我注意力机制。 
...
**Abstract (ZH)**: 信用卡欺诈对持卡人和发卡银行均造成了显著的经济损失。当代方法采用基于机器学习的分类器从标注的交易记录中检测欺诈行为。但由于标注数据通常只是数十亿实际交易中的极小部分，且标注成本高昂，这意味着现有的方法未能充分利用未标注数据中的许多自然特征。因此，我们提出了一种半监督图神经网络以进行欺诈检测。具体而言，我们利用交易记录构建一个时间动态交易图，该图由时间交易（节点）及其相互作用（边）组成。然后，我们通过门控时间注意网络（GTAN）在节点之间传递消息以学习交易表示。进一步地，我们通过交易间的风险传播来建模欺诈模式。我们在现实世界的交易数据集以及两个公开可用的欺诈检测数据集上进行了广泛的实验。实验结果表明，我们提出的方法即GTAN，在三个欺诈检测数据集上优于其他最先进的基准方法。半监督实验表明，即使仅使用少量标注数据，我们的模型也能展现出出色的欺诈检测性能。 

---
# GenAI Content Detection Task 2: AI vs. Human -- Academic Essay Authenticity Challenge 

**Title (ZH)**: GenAI内容检测任务2：AI与人类的较量——学术论文原创性挑战 
...
**Abstract (ZH)**: 本文全面概述了在COLING 2025期间联合举办的GenAI内容检测共享任务中组织的第一届学术论文真实性挑战赛。该挑战主要关注检测用于学术目的的机器生成论文与人类撰写的论文。任务定义如下：“给定一篇论文，判断它是由机器生成的还是由人类撰写的。”该挑战涉及两种语言：英语和阿拉伯语。在评价阶段，有25支队伍提交了英语系统的模型，21支队伍提交了阿拉伯语系统的模型，反映出人们对这一任务的广泛关注。最终，有七支队伍提交了系统描述论文。大多数提交的系统都使用了微调后的变换器模型，其中一支队伍使用了大型语言模型（LLMs），如Llama 2和Llama 3。本文阐述了任务的定义、数据集的构建过程以及评价框架，并介绍了参赛队伍采用的方法。几乎所有提交的系统都优于基于n-克什米尔的基线，顶级系统在两种语言上的F1分数均超过0.98，表明在检测机器生成文本方面取得了显著进展。 

---
# Sampling Bag of Views for Open-Vocabulary Object Detection 

**Title (ZH)**: 开放词汇物体检测中的视图袋采样 
...
**Abstract (ZH)**: 现有的开放词汇对象检测（OVD）方法通过将对象区域嵌入与相应的视觉语言模型（VLM）特征对齐，来测试未见过的类别。最近的一项研究利用了这样一个想法，即VLMs隐式学习图像中语义概念的组合结构。该方法不再使用单个区域嵌入，而是利用一组区域嵌入作为新的表示形式，将组合结构融入到OVD任务中。然而，这种方法往往无法捕捉到每个区域的上下文概念，导致组合结构具有噪声，从而只能实现边际性能提升和效率降低。为了解决这一问题，我们提出了一种新颖的概念导向对齐方法，采用更有力量且更有效的组合结构。我们的方法将上下文相关性的“概念”归纳为一组，并调整袋子内概念的规模，以实现更有效的嵌入对齐。结合Faster R-CNN，我们的方法在开放词汇COCO和LVIS基准上的新类别上实现了2.6个box AP50和0.5个mask AP的改进。此外，与先前的研究相比，我们的方法在运算量（FLOPs）上减少了80.3%的CLIP计算，显著提升了效率。实验证明，所提出的方法在OVD数据集上性能优于之前的最先进的模型。 

---
# Robust Semi-Supervised Learning in Open Environments 

**Title (ZH)**: 开放环境下的鲁棒半监督学习 
...
**Abstract (ZH)**: 半监督学习（Semi-supervised Learning, SSL）旨在通过利用未标记数据来改善学习性能，尤其是在标签稀缺的情况下。传统的SSL研究通常假设近似环境，其中标记数据和未标记数据之间的重要因素（如标签、特征、分布）是一致的。然而，在更实际的任务中，涉及开放环境，其中标记数据和未标记数据之间的重要因素往往是不一致的。有研究表明，利用不一致的未标记数据会导致严重的性能下降，甚至比简单的监督学习基准还要差。手动验证未标记数据的质量并不理想，因此，在开放环境中研究具有不一致未标记数据的稳健SSL变得十分重要。本文简要介绍了这一研究领域的进展，重点介绍了与标签、特征和数据分布不一致性相关的SSL技术，并提出了评估基准。还讨论了一些开放研究问题，供参考。 

---
# Detection and Forecasting of Parkinson Disease Progression from Speech Signal Features Using MultiLayer Perceptron and LSTM 

**Title (ZH)**: 使用多层感知器和长短期记忆网络从语音信号特征检测与预测帕金森病进展 
...
**Abstract (ZH)**: 帕金森病的准确诊断，尤其是在早期阶段，是一个具有挑战性的任务。机器学习技术的应用有助于提高帕金森病检测的诊断准确性，但仅有少数研究集中在疾病进展的预测上。在这项研究工作中，我们使用Long Short Term Memory (LSTM) 神经网络训练了帕金森病患者的言语信号特征，以预测疾病进展；同时，我们使用多层感知机（Multilayer Perceptron, MLP）训练了相同的诊断特征，以检测疾病。通过使用两种已知的特征选择方法——Relief-F和顺序递增选择法（Sequential Forward Selection），我们选择了诊断特征，并将其应用于LSTM和MLP中。这些特征的使用能够准确预测疾病进展至第二阶段和第三阶段及其存在情况。 
...
**Abstract (ZH)**: Fréchet回归将线性回归扩展到度量空间中，用于建模复杂的响应值，使其尤其适用于多标签回归问题，其中每个实例可以有多个相关的标签。然而，该框架下的变量选择仍处于探索阶段。本文提出了一种新的变量选择方法，该方法采用隐式正则化，而非传统的显式正则化方法，后者可能会引入偏差。我们的方法能够有效捕捉预测变量与响应之间的非线性交互作用，同时促进模型稀疏性。我们提供了理论结果来证明选择一致性，并通过数值示例展示了该方法的性能。 
...
**Abstract (ZH)**: 图神经网络（GNNs）已发展成为从图结构数据进行推荐的先进方法。然而，大多数现有的基于GNN的推荐方法侧重于模型结构和学习策略的优化，基于预定义的图结构，忽视了图构建阶段的重要性。早期的图构建工作通常依赖于特定规则或众包，这两种方法要么过于简单，要么过于耗时。最近的研究开始利用大规模语言模型（LLMs）自动进行图构建，以利用其丰富的开放世界知识和卓越的推理能力。然而，它们通常存在两个缺点：（1）全局视角的不透明性（例如，忽视上下文信息）和（2）构建效率低下。为了解决这些问题，我们引入了AutoGraph，这是一种基于LLMs的推荐自动图构建框架。具体而言，我们首先使用LLMs推断用户偏好和项目知识，这些信息被编码为语义向量。接着，我们使用向量量化从语义向量中提取潜在因子。这些潜在因子随后被整合为额外的节点，连接用户/项目节点，从而生成具有深入全局视角语义的图。我们还设计了基于元路径的消息聚合，以有效地聚合语义和协作信息。该框架是模型无关的，并且能够与不同的骨干模型兼容。针对三个真实世界的数据集进行的广泛实验表明，AutoGraph在与现有基线方法相比时，在效能和效率方面表现出色。我们已在华为广告平台上部署了AutoGraph，并在在线A/B测试中获得了2.69%的RPM提升和7.31%的eCPM提升。目前，AutoGraph已成为主要流量模型，服务于数亿人。 

---
# Expand VSR Benchmark for VLLM to Expertize in Spatial Rules 

**Title (ZH)**: 将VSR基准扩展到VLLM，以专精于空间规则 
...
**Abstract (ZH)**: 辨别空间关系是人类认知的基本部分，需要对跨实例进行精细感知。虽然基准测试如MME、MMBench和SEED已经全面评估了各种能力，包括视觉空间推理（VSR），但对于视觉大型语言模型（VLLMs）在视觉位置推理方面的数量和质量评估与优化数据集仍然不足。为解决这一问题，我们首先使用VSR数据集诊断当前的VLLMs，并提出了一个统一的测试集。我们发现当前的VLLMs表现出对语言指令过于敏感而对视觉位置信息不够敏感的矛盾现象。通过对数据和模型结构进行扩展，我们减轻了这一现象的影响。据我们所知，我们首次利用扩散模型可控地扩展了具有空间定位的数据，并将原始视觉编码器（如CLIP）与其他三种强有力的视觉编码器（SigLIP、SAM和DINO）进行了整合。在对数据和模型进行组合实验后，我们获得了名为VLLM VSR Expert（VSRE）的模型，该模型不仅在不同指令上具有更好的泛化能力，还能够准确地区分视觉位置信息的差异。VSRE在VSR测试集上的准确率提高了超过27%。它在VSR数据集和相关评价基准的子集中的位置推理表现上变得非常强大。我们开源了扩展的数据和模型附录，相关资源可访问 \url{this https URL}，希望这能够加速对VSR学习的VLLM的研究进程。 
...
**Abstract (ZH)**: 多视图聚类（Multi-view Clustering, MVC）的目标是通过整合多视角下的互补信息来提高聚类性能。延迟融合多视图聚类（Late Fusion Multi-View Clustering, LFMVC）通过综合多样的聚类结果为统一共识提供了前景，但当前的LFMVC方法难以处理嘈杂和冗余的分区，并且往往无法捕捉多视角之间的高阶相关性。为了解决这些局限性，我们提出了一种新的理论框架，用于分析多种核$k$-means的泛化误差界，利用局部Rademacher复杂性和主特征值比例进行分析。我们的分析确定了收敛速率为$\mathcal{O}(1/n)$，显着优于现有的$\mathcal{O}(\sqrt{k/n})$速度。基于这一见解，我们在多线性$k$-means框架内提出了低通图滤波策略，以减轻噪声和冗余，进一步优化主特征值比例，从而提高聚类准确性。基准数据集上的实验结果证实，我们的方法在聚类性能和鲁棒性方面优于最先进的方法。相关代码可通过以下链接访问：this https URL。 

---
# VLABench: A Large-Scale Benchmark for Language-Conditioned Robotics Manipulation with Long-Horizon Reasoning Tasks 

**Title (ZH)**: VLABench：一种用于基于语言指导的机器人 manipulation 与长期推理任务的大规模基准测试 
...
**Abstract (ZH)**: 通用型具身代理旨在理解用户的自然指令或意图，并精确执行以完成通用任务。最近，基于基础模型的方法，尤其是视觉-语言-动作模型（VLAs），显示出解决语言条件下的操作任务（LCM）的显著潜力。然而，现有的基准尚不能充分满足VLAs及其相关算法的需求。为了更好地定义此类通用任务的语境，并推动VLAs领域的研究，我们提出了VLABench，这是一个开源基准，用于评估通用LCM任务的学习。VLABench提供了100个精心设计的任务类别，每个类别都有较强的随机性，并包含了2000多个物体。相较之前的基准，VLABench在四个方面脱颖而出：

1. 需要世界知识和常识转移的任务；
2. 自然语言指令中隐含人类意图而非模板的任务；
3. 长期展望的任务，要求多步推理；
4. 评估动作策略和语言模型能力。

该基准评估多项能力，包括网格与纹理理解、空间关系、语义指令、物理法则、知识转移和推理等。为了支持下游微调，我们提供了通过结合启发式技能和先验信息的自动化框架收集的高质量训练数据。实验结果表明，当前最先进的预训练VLAs和基于VLM的工作流程在我们的任务中都面临挑战。 

---
# An Analysis on Automated Metrics for Evaluating Japanese-English Chat Translation 

**Title (ZH)**: 对评估日英聊天翻译的自动化指标进行分析 
...
**Abstract (ZH)**: 本文分析了传统基准指标（如 BLEU 和 TER）和基于神经网络的方法（如 BERTScore 和 COMET）在聊天翻译任务中评价多个神经机器翻译（NMT）模型性能的方式，并比较了这些指标与人工标注评分的相关性。研究结果表明，对于排列聊天翻译中 NMT 模型的性能，所有指标在决定哪个模型表现更好方面都表现出一致性。这暗示了即使传统基准指标使用起来更快、更简便，它们仍然具有一定的实用性。另一方面，当与人类判断的相关性更好时，基于神经网络的指标优于传统指标，其中 COMET 在聊天翻译任务中与人工标注评分的相关性最高。然而，我们还表明，即使是最优指标在评估由日语无标记人称代词引发的英语句子翻译时也存在问题。 
...
**Abstract (ZH)**: 本研究探讨了使用XLM-R预训练模型从英语跨语言转移到日语和印尼语的应用可行性。我们将结果与使用类似零样本方法或完全监督方法的多个先前工作进行了比较，以提供XLM-R在零样本跨语言迁移学习方法方面的能力概述，与现有模型相比。我们的模型在其中一个日语数据集中取得了最佳结果，在其他日语和印尼语数据集中取得了可比的结果，而无需使用目标语言进行训练。此外，结果表明，可以训练一个多语言模型，而不是为每种语言单独训练一个模型，并能取得令人鼓舞的结果。 
...
**Abstract (ZH)**: 文本到图像生成模型在从文本生成图像方面表现出色，但在确保生成图像与提示之间的一致性和连贯性方面存在困难。本文提出了一种名为TextMatch的新框架，该框架利用多模态优化来解决文本到图像(T2I)生成和编辑中的图像-文本不一致问题。TextMatch利用大型语言模型（LLMs）和视觉问答（VQA）模型来评估提示和生成图像之间的语义一致性。通过整合多模态上下文学习和推理链，我们的方法在迭代优化过程中动态改进提示，从而确保生成的图像更好地捕捉用户意图，提高图像的真实性和相关性。广泛实验证明，TextMatch在多个基准测试中显著提高了文本-图像的一致性，为提升文本到图像生成模型的能力建立了可靠框架。我们的代码可通过以下链接获取：this https URL。 

---
# Enhancing Online Continual Learning with Plug-and-Play State Space Model and Class-Conditional Mixture of Discretization 

**Title (ZH)**: 使用即插即用状态空间模型和类条件离散混合模型增强在线连续学习 
...
**Abstract (ZH)**: 在线连续学习（OCL）旨在从仅出现一次的数据流中学习新任务，同时保留之前学习任务的知识。现有的大多数方法依赖于重放机制，主要通过正则化或知识蒸馏增强记忆保留能力。然而，这些方法往往忽视了模型的适应性，限制了其从在线训练数据中逐步学习可泛化和区分性强的特征的能力。为解决这一问题，我们提出了一种插件模块S6MOD，它可以无缝集成到大多数现有方法中，直接提高模型的适应性。具体而言，S6MOD 在骨干网络之后引入了一个额外分支，其中混合离散化选择性地调整选择性状态空间模型中的参数，丰富了选择性扫描模式，使得模型能够适应性地选择当前动态中最敏感的离散化方法。我们还设计了一种基于类别的路由算法，用于动态、基于不确定性调整，并实现了一种对比离散化损失来优化此算法。结合我们的模块与多种模型的广泛实验表明，S6MOD 显著提升了模型的适应性，带来了实质性的性能提升，并达到了最先进的效果。 

---
# Molar: Multimodal LLMs with Collaborative Filtering Alignment for Enhanced Sequential Recommendation 

**Title (ZH)**: Molar：具有合作过滤对齐的多模态LLM增强序列推荐 
...
**Abstract (ZH)**: 过去十年中，序列推荐（SR）系统经历了显著的发展，从传统的协同过滤过渡到深度学习方法，并且更近一步发展到了大型语言模型（LLMs）。虽然LLMs的应用推动了显著的进步，但这些模型本质上缺乏协同过滤信息，主要依赖于文本内容数据，忽视了其他模态的数据，从而未能实现最佳的推荐性能。为了解决这一局限性，我们提出了一个名为Molar的多模态大型语言模型序列推荐框架，该框架将多种内容模态与ID信息结合起来，有效捕捉协同信号。Molar采用MLLM从文本和非文本数据中生成统一的项目表示，促进全面的多模态建模并丰富项目嵌入。此外，它通过后对齐机制引入协同过滤信号，将基于内容和ID的用户表示进行对齐，确保精准个性化和稳健性能。通过无缝结合多模态内容与协同过滤洞察，Molar捕获了用户的兴趣和上下文语义，从而提高了推荐准确性。广泛实验验证了Molar在传统和LLM基础上的基线模型中表现出显著优势，突显了其利用多模态数据和协同信号进行序列推荐任务的能力。Molar的源代码可在以下链接获取：[提供链接]。 

---
# INVESTORBENCH: A Benchmark for Financial Decision-Making Tasks with LLM-based Agent 

**Title (ZH)**: INVESTORBENCH：基于LLM代理的金融决策任务基准 
...
**Abstract (ZH)**: 近年来，大型语言模型（LLM）为基础的代理在金融决策中的潜力得到了广泛关注。尽管取得了这些进展，但该领域的研究目前面临两大主要挑战：（1）缺乏一个全面且适应多种金融任务的LLM代理框架，以及（2）缺乏标准化的基准和一致的数据集来评估代理的表现。为了解决这些问题，我们介绍了InvestorBench，这是首个专门用于评估适用于各种金融决策背景下的LLM代理的基准。InvestorBench通过提供一个全面的任务套件，增强了LLM使能代理的灵活性，该套件适用于不同的金融产品，包括单个股票、加密货币和交易所交易基金（ETFs）。此外，我们使用13种不同的LLM作为骨干模型，评估我们的代理框架在各种市场环境和任务中的推理和决策能力。此外，我们还收集了一个多模态的开源数据集集合，并开发了一个全面的金融决策环境套件。这为在各种场景下评估金融代理的表现提供了一个高度可访问的平台。 

---
# KunServe: Elastic and Efficient Large Language Model Serving with Parameter-centric Memory Management 

**Title (ZH)**: KunServe: 基于参数中心的内存管理弹性高效大型语言模型服务 
...
**Abstract (ZH)**: 大语言模型（LLM）服务的状态特性在负载高峰或长生成请求（如链式推理）情况下，容易耗尽宝贵的GPU内存，导致因队列等待请求而引起的服务延迟突增。然而，最先进的基于KV缓存的方法通过丢弃、迁移或交换KV缓存来应对突发负载，这种做法在正在进行的请求和即将到来的请求之间的性能上面临权衡，因此仍然会对性能造成严重影响。本文提出了一项重要观察，即模型参数与请求无关，并且可以在不同GPU之间复制，因此提出了一种参数为中心的方法，通过有选择地丢弃复本参数，为请求留出宝贵的内存。然而，LLM需要将KV缓存与模型参数绑定存储，因此丢弃参数可能会导致巨大的计算浪费或长时间的网络延迟，从而影响所有正在进行的请求。基于注意力操作可以与其他操作解耦的观察，本文进一步提出了一种新的基于管道并行性的远程注意力机制，通过借用远程GPU上的参数额外内存来服务即将到来的请求。此外，本文还解决了其他几个挑战，包括动态交换不完整参数的KV缓存、生成一个平衡内存需求与协作执行开销的适当计划，以及在突发情况结束后无缝恢复参数。评估结果表明，KUNSERVE在突发情况下将请求的尾部TTFT降低了最多27.3倍，相比于当前最先进的方法。 

---
# Survey of Pseudonymization, Abstractive Summarization & Spell Checker for Hindi and Marathi 

**Title (ZH)**: 关于Hindi和Marathi语言的匿名化、抽象总结及拼写检查的调查研究 
...
**Abstract (ZH)**: 印度 vast 的语言多样性为技术进步，特别是在自然语言处理（NLP）领域，带来了独特的挑战和机遇。尽管在广泛使用的语言的NLP 应用中取得了显著进展，但印度的区域语言，如马哈拉施特拉语和印地语，仍然得不到充分的服务。针对印度区域语言的 NLP 研究仍处于起步阶段，但其重要性不容小觑。本文旨在构建一个平台，使用户能够使用文本匿名化、抽象文本总结和拼写检查等功能，这些功能支持英语、印地语和马哈拉施特拉语。这些工具的目标是服务于主要使用印度区域语言的商业和消费者客户。 

---
# Smooth-Foley: Creating Continuous Sound for Video-to-Audio Generation Under Semantic Guidance 

**Title (ZH)**: Smooth-Foley: 在语义指导下的视频到音频生成中的连续声音创建 
...
**Abstract (ZH)**: 视频到音频（V2A）生成任务由于在产生实景声音方面的实用性而在多媒体领域引起了广泛关注。通过输入语义和时间条件，生成模型能够指示声音事件及其时间发生。最近在合成沉浸式和同步音频方面的研究遇到了挑战，特别是在包含移动视觉元素的视频中。时间条件不够准确，而低分辨率的语义条件进一步加剧了这一问题。为了解决这些问题，我们提出了一种名为Smooth-Foley的V2A生成模型，该模型通过对生成过程中的文本标签进行语义引导，以增强音频的语义和时间对齐。我们训练了两个适配器来利用预训练的文本到音频生成模型。帧适配器整合了高分辨率的帧级视频特征，而时间适配器则整合了从视觉帧相似性和文本标签中获得的时间条件。从文本标签中引入语义指导实现了精确的音视频对齐。我们进行了广泛的定量和定性实验。结果显示，相比现有模型，Smooth-Foley 在连续声音场景和一般场景中都表现更优。在语义指导的帮助下，由Smooth-Foley生成的音频具有更高的质量并更好地遵循物理规律。 
...
**Abstract (ZH)**: 大型语言模型（LLMs）在文本序列中建模隐藏关系方面取得了显著进步，这为生命科学领域提供了独特的机会。生命科学中，来自多种物种的单细胞组学数据集合为训练基础模型奠定了基础。然而，各物种间数据规模的不一致性成为了一个挑战，阻碍了跨物种遗传数据分析的全面模型的开发。在本文中，我们提出了一种创新的混合方法，将LLMs的一般知识能力和针对单细胞组学数据的领域特定表示模型相结合，用于解读单细胞组学数据。我们首先以基因作为基本表示单位。基因表示使用功能性描述进行初始化，利用成熟的LLaMA-2等语言模型的优势。通过输入单细胞基因表达数据并使用提示，我们能够基于不同物种和细胞类型中基因表达水平的差异来建模细胞表示。在实验中，我们从人类和小鼠构建了发育细胞，并特别针对那些难以注释的细胞进行研究。我们通过基本任务如细胞注释和可视化分析来评估我们的方法。结果显示，与使用LLMs的其他方法相比，我们的方法在准确性和互操作性方面表现出显著的改进。我们的混合方法增强了单细胞数据的表示，并为跨物种遗传分析的未来研究提供了坚实框架。 

---
# GeneSUM: Large Language Model-based Gene Summary Extraction 

**Title (ZH)**: GeneSUM：基于大型语言模型的基因总结提取 
...
**Abstract (ZH)**: 生物医学研究中的新兴课题正不断扩展，提供了关于基因及其功能的丰富信息。知识的快速增长为科学研究带来了前所未有的发现机会，同时也给研究人员跟踪最新进展带来了巨大挑战。一个显著的挑战是如何在浩如烟海的文献中导航并提取关键的基因相关信息，这是一个耗时且繁琐的任务。为了提高这一过程的效率，必须解决几个关键挑战：（1）大量文献的存在，（2）基因功能的复杂性，以及（3）自动化整合与生成。为此，我们提出了一种名为GeneSUM的两阶段自动化基因摘要提取器，利用大型语言模型（LLM）。该方法首先检索并消除目标基因文献的冗余，然后对LLM进行微调，以优化和简化摘要过程。我们进行了广泛的实验来验证所提出框架的有效性。结果表明，LLM显著增强了基因特异性信息的整合，从而在正在进行的研究中提高了决策效率。 

---
# EvalMuse-40K: A Reliable and Fine-Grained Benchmark with Comprehensive Human Annotations for Text-to-Image Generation Model Evaluation 

**Title (ZH)**: EvalMuse-40K：一种可靠且精细粒度的基准测试，包含全面的人工标注，用于文本到图像生成模型评估 
...
**Abstract (ZH)**: 近年来，文本到图像（Text-to-Image, T2I）生成模型取得了显著进展。相应地，出现了许多自动评估指标，用于评价生成模型的图像-文本对齐能力。然而，现有评估指标的性能比较受限于小规模数据集。此外，这些数据集缺乏在细粒度层面评估自动评估指标性能的能力。在本研究中，我们贡献了一个名为EvalMuse-40K的基准数据集，收录了40K个带有细粒度人工注释的图像-文本对，适用于图像-文本对齐相关的任务。在数据集构建过程中，我们采用了各种策略，如平衡提示采样和数据重新注释，以确保基准数据集的多样性和可靠性，从而能够全面评估图像-文本对齐指标在T2I模型中的效果。同时，我们引入了两种新的方法来评估T2I模型的图像-文本对齐能力：FGA-BLIP2涉及端到端微调一个视觉-语言模型以产生细粒度的图像-文本对齐分数，PN-VQA则采用一种新颖的正负样本VQA方法在VQA模型中实现零样本的细粒度评估。这两种方法在图像-文本对齐评估中均取得了显著的性能。我们还使用这些方法对当前的AIGC模型进行了排名，结果可为未来的研究提供参考，并促进T2I生成的发展。数据和代码将公开提供。 

---
# Text-Aware Adapter for Few-Shot Keyword Spotting 

**Title (ZH)**: 基于文本的适配器用于少样本关键词识别 
...
**Abstract (ZH)**: 近年来，基于文本注册的柔性关键词识别（Keyword Spotting, KWS）取得了进展，使得用户可以在注册时不发出关键词本身即可个性化设置关键词。然而，目标关键词的性能仍有提升空间。在本文中，我们提出了一种新颖的少样本迁移学习方法，称为文本感知适配器（Text-Aware Adapter, TA-adapter），旨在通过有限的语音样本增强预训练的柔性KWS模型。为了适应声学编码器，我们利用一个联合预训练的文本编码器生成一个文本嵌入，该嵌入作为关键词的代表性向量。仅微调网络的小部分，而保持核心组件的权重不变，TA-适配器证明了其在少样本KWS中的高效性，并确保模型能够平滑地返回到原始预训练模型。在我们的实验中，TA-适配器在Google Speech Commands V2数据集中35个不同关键词上实现了显著的性能提升，而参数总量仅增加了0.14%。 
...
**Abstract (ZH)**: originally设计用于人类的认知任务现在越来越多地用于研究语言模型。虽然应用这些任务通常比较简单，但解释其结果可能会很具挑战性。特别是当模型表现不佳时，常常不清楚这是由于被测试的认知能力的限制，还是对任务本身的理解失败。最近的一项研究认为，GPT 3.5在2-back和3-back任务上的表现下降反映了与人类相似的短期记忆容量限制。通过对多种不同性能水平的语言模型在这类任务上的分析，我们发现表现不佳实际上反映的是任务理解和任务维持方面的限制。此外，我们推动表现最佳的模型进行更高的n值测试，并尝试不同的提示策略，然后分析模型的注意力机制。我们更大的目标是为语言模型的认知评估方法的改进做出贡献，并加入正在进行的讨论中。 
...
**Abstract (ZH)**: 歌词到旋律生成旨在根据给定的歌词自动生成旋律，需要捕捉歌词与旋律之间复杂而微妙的相关性。然而，以往的工作通常面临两个主要挑战：1）歌词-旋律对齐建模，通常简化为一个字/词对应一个音符的对齐，而其他方法则存在对齐精度较低的问题；2）歌词-旋律和声建模，通常依赖于中间步骤或严格规则，限制了模型的能力和生成的多样性。在此基础上，我们提出了SongGLM，这是一种利用二维对齐编码及基于通用语言模型（GLM）的多任务预训练的歌词到旋律生成系统，以确保歌词与旋律之间的对齐和和声。具体来说，1）我们引入了一种统一的符号化歌曲表示，包含字级和短语级（二维）对齐编码，以捕捉歌词与旋律的对齐；2）我们设计了一种包含层级空白填充目标（n-gram、短语和长跨度）的多任务预训练框架，并将歌词与旋律关系融入和谐n-gram的提取中，以确保歌词与旋律的和声。我们还构建了一个包含超过20万首英文歌曲片段的大规模歌词-旋律配对数据集，用于预训练和微调。客观和主观结果表明，SongGLM能够在对齐和和声方面显著提高从歌词生成旋律的效果，并优于所有先前的基线方法。 

---
# EvoPat: A Multi-LLM-based Patents Summarization and Analysis Agent 

**Title (ZH)**: EvoPat：一个基于多大型语言模型的专利总结与分析代理 
...
**Abstract (ZH)**: 科学技术与知识的迅速增长在每年新提交专利数量的指数级增加中得到了反映。虽然这些专利驱动了创新，但也为研究人员和工程师，尤其是新手，带来了巨大挑战。为了避免在庞大而复杂的领域中寻找趋势和突破性的研究工作，研究人员迫切需要高效的工具来总结、评估和情境化专利，揭示其创新贡献和基础科学内容。为了解决这一需求，我们提出了EvoPat，一种基于多个超大规模语言模型（Multi-LLM）的专利助手，旨在通过检索增强生成（RAG）和先进的搜索策略帮助用户分析专利。EvoPat利用多个大型语言模型，各自承担特定角色，如规划、识别创新点和进行比较性评估。系统整合了本地数据库中的数据，包括专利、文献、产品目录和公司档案，以及在线搜索，以提供最新洞见。还实现了自动收集未包含在原始数据库中的信息功能。通过在自然语言处理（NLP）领域的广泛测试，我们表明，EvoPat在专利总结、比较分析和技术评估等任务上优于GPT-4。EvoPat代表了朝着创建能够助力研究人员和工程师高效导航专利领域复杂性的AI工具迈出的重要一步。 

---
# An Attention-based Framework with Multistation Information for Earthquake Early Warnings 

**Title (ZH)**: 基于注意力机制的多站信息融合地震早期预警框架 
...
**Abstract (ZH)**: 地震预警系统在减少地震灾害风险方面发挥着至关重要的作用。先前，主流的建模系统是单站模型。这类模型接收特定站点的数据信号，并预测该地点的P相到波到达时间、震级和强度等参数。各种方法都证明了良好的性能。然而，大多数方法面临着加快警报时间、对远地区域提供预警、以及利用全球信息提升性能等方面的挑战。最近，深度学习在许多领域产生了深远影响，包括地震学。因此，本论文提出了一种基于深度学习的框架，称为SENSE，用于地震预警系统的强度预测任务。为了从区域或国家的角度明确考虑全局信息，SENSE的输入包括给定区域或国家中一组站点的统计信息。SENSE模型设计的目的是学习这些输入站点之间的关系及各站点的地域特定特征。因此，除了通过多站数据提供更可靠的预测外，SENSE还具备为尚未接收到信号的远地区域提供预警的能力。本研究在台湾和日本的数据集上进行了广泛的实验。结果表明，SENSE的性能与现有的其他先进方法相当甚至更优。 

---
# LangYa: Revolutionizing Cross-Spatiotemporal Ocean Forecasting 

**Title (ZH)**: LangYa：重塑跨时空海洋预报技术 
...
**Abstract (ZH)**: 海洋预报对于科学科研和社会福祉至关重要。目前，最准确的预报系统是全球海洋预报系统（GOFSs），它将海洋状态变量（OSVs）表示为离散网格，并使用数值方法求解描述海洋状态变量转换的偏微分方程（PDEs）。然而，GOFSs 的计算成本高昂且容易积累误差。最近，大规模的人工智能（AI）基于模型极大地提升了预报的速度和准确性。不幸的是，建立一个既能进行跨时空和大气-海洋耦合预报的大型AI海洋预报系统，仍然是一个重大挑战。在此，我们介绍了一种跨时空和大气-海洋耦合的海洋预报系统——LangYa。研究表明，LangYa中的时间嵌入模块使其能够进行从1天到7天不同提前量的预报。大气-海洋耦合模块有效地模拟了大气-海洋相互作用。海洋自注意力模块在训练过程中提高了网络的稳定性并加速了收敛，而自适应观测锋面损失函数提高了观测锋面预报的准确性。与现有的数值和AI基于的海洋预报系统相比，LangYa 使用了GLORYS12v12版本中的27年的全球海洋数据进行训练，并取得了更可靠的确定性海洋状态变量（OSVs）预报结果。LangYa 预报系统为全球海洋研究人员提供了强大的软件工具，以实现精确的海洋预报，并为海洋科学开辟了一个新的范式。 
...
**Abstract (ZH)**: 捆绑推荐的目标是向用户推荐一组相互关联的项目。然而，多样化的交互类型和稀疏的交互矩阵往往给之前的方法带来了准确预测用户-捆绑采用的挑战。受远处监督策略和生成范式的启发，我们提出了一种名为BRIDGE的新颖框架，用于捆绑推荐。该框架包含两个主要组成部分，即基于相关性 的物品聚类模块和伪捆绑生成模块。

受远处监督方法的启发，前者旨在生成更多的辅助信息，例如指示性的项目簇，而无需使用外部数据来进行训练。这些信息随后与用户历史交互中的协作信号相结合，以生成伪“理想”捆绑。这种能力使BRIDGE能够探索所有捆绑的方面，而不仅限于现有的真实世界捆绑。它有效填补了用户想象与预定义捆绑之间的差距，从而改善了捆绑推荐的表现。

实验结果在五个基准数据集中验证了我们的模型在排名基方法上的优越性。 

---
# Multi-Point Positional Insertion Tuning for Small Object Detection 

**Title (ZH)**: 面向小对象检测的多点位置插入调谐 
...
**Abstract (ZH)**: 小物体检测旨在定位和分类图像中的小物体。随着大规模视觉-语言预训练的进展，微调预训练的目标检测模型已成为一种有前途的方法。然而，微调大型模型在计算和内存上都非常昂贵。为了解决这一问题，本文引入了多点位置插入（MPI）调优，这是一种参数高效微调（PEFT）方法，适用于小物体检测。具体而言，MPI 将多个位置嵌入整合到冻结的预训练模型中，通过向潜在特征提供精确的位置信息，实现小物体的高效检测。通过实验，我们在 SODA-D 数据集上证明了所提出方法的有效性。MPI 在参数量显著减少的情况下，表现与传统 PEFT 方法（包括 CoOp 和 VPT）相当。 

---
# Generating Traffic Scenarios via In-Context Learning to Learn Better Motion Planner 

**Title (ZH)**: 通过_context中的学习生成交通场景以优化运动规划器 
...
**Abstract (ZH)**: 自主驾驶中的运动规划是其关键组成部分。最新的运动规划器通常是在精心准备的数据集上进行训练，这类数据集不仅标注成本高昂，而且无法充分捕捉到罕见但至关重要的场景。忽视这些场景会给运动规划器带来重大风险，并可能在测试过程中引发事故。一个直观的解决方案是通过编程和执行模拟器（例如CARLA）手动组成这样的场景。然而，这种方法会带来显著的人工成本。为了解决这个问题，我们提出了一种低成本的方法，以生成多种多样的关键交通场景，从而训练出更 robust 的运动规划器。首先，我们将交通场景表示为脚本，然后使用这些脚本由模拟器生成具体的交通场景。接下来，我们开发了一种方法，该方法接受用户指定的文字描述，并利用上下文学习（in-context learning）由大型语言模型（LLM）将其转化为脚本。输出的脚本被发送给模拟器，模拟器据此生成相应的交通场景。鉴于我们的方法可以生成大量的安全关键交通场景，我们使用这些场景作为运动规划器的合成训练数据。为了证明生成场景的价值，我们使用我们的合成数据、真实世界的数据集以及两者的结合来训练现有的运动规划器。实验结果显示，使用我们数据训练的运动规划器在性能上明显优于仅使用真实世界数据训练的运动规划器，这表明我们的合成数据及其生成方法的有效性。我们的源代码可以在以下链接中获得：this https URL。 

---
# Prompt Tuning for Item Cold-start Recommendation 

**Title (ZH)**: 冷启动推荐中的提示调优 
...
**Abstract (ZH)**: 冷启动问题对于在线推荐系统至关重要，因为冷启动阶段的成功决定了物品是否能够转变为受欢迎的物品。提示学习是一种用于自然语言处理（NLP）以解决零样本或少量样本问题的强大力量技术，并已被应用到推荐系统中以应对类似挑战。然而，现有的方法通常依赖于基于内容的属性或文本描述作为提示，我们认为这在冷启动推荐中可能是次优解决方案，原因在于1）与推荐任务之间的语义差距，2）模型偏差，即预热物品为模型提供了大部分的正面反馈，这是冷启动问题的核心，阻碍了在冷启动物品上的推荐质量。我们提出利用高价值的正面反馈，称其为尖峰反馈作为提示信息，以同时解决上述两个问题。实验结果证明，与现有工作中提出的基于内容的描述相比，正面反馈更适合作为提示信息，因为它能够弥合语义差距。此外，我们提出了物品个性化提示网络，将尖峰反馈编码到模型中，以缓解由正面反馈主导问题引起的模型偏差。在四个真实世界数据集上的广泛实验表明，我们的模型在各种商业指标上的性能优于现有方法。此外，PROMO已在一款大型短视频分享平台上成功部署，这是一个拥有数亿用户的商业短视频应用，实验结果在冷启动场景中显示出显著的性能提升。 

---
# COMO: Cross-Mamba Interaction and Offset-Guided Fusion for Multimodal Object Detection 

**Title (ZH)**: COMO：跨Mamba交互与偏移引导融合在多模态目标检测中的应用 
...
**Abstract (ZH)**: 单模态目标检测任务在遇到多变的情景时往往会经历性能下降。相比之下，多模态目标检测任务可以通过整合来自多种模态的数据，提供更加全面的对象特征信息。当前的多模态目标检测方法通常使用包括传统神经网络和基于变换器的模型在内的各种融合技术，实施特征融合策略并实现互补信息。然而，由于多模态图像是由不同的传感器捕获的，它们之间往往存在对齐问题，使得直接匹配变得困难。这种对齐问题妨碍了在不同模态中为同对象建立强大关联的能力。本文提出了一种名为CrOss-Mamba交互与偏移引导融合（COMO）框架的新方法，专用于多模态目标检测任务。COMO框架采用交叉MAMBA技术来构建特征交互方程，实现多模态有序状态计算，这种计算方式既促进了多模态交互融合输出，又减少了计算开销并提高了效率。此外，COMO利用那些较少受对齐问题影响的高层次特征，以促进模态间交互并传递互补信息，解决了由视角和采集时间变化造成的定位偏差问题。进一步地，COMO在交叉MAMBA模块中结合了全局和局部扫描机制，以捕捉具有局部相关性的特征，特别是在遥感图像中。为了保留低级特征，偏移引导融合机制确保了多尺度特征的有效利用，允许构建一个多尺度融合数据立方体，从而增强检测性能。 
...
**Abstract (ZH)**: 随着基础模型和视觉-语言模型的进步以及有效的细调技术，已开发出大量适用于各种视觉任务的通用和专用模型。尽管这些模型具有灵活性和易用性，但没有单一模型能够处理所有预期用户可能想象的任务和/或应用。近年来，通过程序合成来解决复杂视觉任务的方法，如视觉编程和集成工具的多模态大型语言模型（ multimodal LLMs），试图解决这一问题。然而，这些方法往往忽略了用户约束（如性能/计算需求），生成的仅针对测试样例特定的解决方案，部署起来困难，并且有时需要低级指令，这可能超出普通用户的能力。

为解决这些局限性，我们引入了MMFactory，这是一个通用框架，包括模型和指标路由组件，其作用类似于一个解决方案搜索引擎，可以在各种可用模型中进行搜索。根据任务描述以及少量样本的输入输出数据（可选地还包括资源和/或性能约束），MMFactory可以利用其模型库中的视觉-语言工具进行实例化和组合，建议出多样化的程序化解决方案。除了生成这些解决方案外，MMFactory还提出评估指标，并对性能/资源特性进行评估，允许用户选择符合其独特设计约束的解决方案。

从技术层面来看，我们还引入了一种基于委员会的解决方案提案者，利用多智能体大型语言模型（multi-agent LLM）对话生成可执行的、多样化的、通用的和鲁棒的解决方案。实验结果表明，MMFactory通过提供针对用户问题具体要求的最先进的解决方案，优于现有的方法。

项目网页可通过以下链接访问：[这里请插入链接] 
...
**Abstract (ZH)**: 组合 libraries 是探索二元和三元截面在多组分相图中物理性质演化的一个强大方法。尽管自20世纪60年代以来组合 libraries 的合成已经发展起来，并且随着先进实验室自动化技术的应用而加快，但更广泛地应用组合 libraries 仍然依赖于快速可靠的全浓度范围结构和功能测量。扫描探针显微镜（SPM），包括压电响应力显微镜（PFM），为定量、功能相关的 combi-library 读出提供了巨大潜力。在本文中，我们演示了全自动 SPM 的实现，以研究 Sm 掺杂的 BiFeO3 和 ZnxMg1-xO 系统中弛豫铁电体性质的演化。我们还提出了并比较了基于高斯过程的贝叶斯优化模型，强调局部可重复性（有效噪声）是优化实验工作流程中的一个关键因素。当与上游合成控制相结合时，全自动 SPM 在材料合成与表征之间发挥着关键作用。 
...
**Abstract (ZH)**: 尽管预训练语言模型（PLMs）的前馈层神经元可以存储事实知识，但大多数前期分析仍停留在定性层面，使得知识表示、神经元激活与模型输出之间的量化关系尚不清楚。在本研究中，通过使用事实探针数据集进行单个神经元级别的干预，我们首次揭示了神经元激活与输出标记概率之间的线性关系。我们将这种线性关系的梯度称为“神经元经验梯度”，并提出了一种名为NeurGrad的有效计算方法，以促进神经元的定量分析。我们进一步研究PLMs中的神经元经验梯度是否编码通用任务知识，为此，我们引入了MCEval8k，这是一个涵盖六个体裁和22项任务的多选知识评估基准。我们的实验确认，神经元经验梯度能够有效捕捉知识，而技能神经元则表现出效率、通用性、包容性和相互依赖性。这些发现通过神经元经验梯度将知识与PLM的输出联系起来，揭示了PLM如何存储知识。代码和数据集已开源。 

---
# Beyond Gradient Averaging in Parallel Optimization: Improved Robustness through Gradient Agreement Filtering 

**Title (ZH)**: 超越并行优化中的梯度平均：通过梯度一致性过滤提高鲁棒性 
...
**Abstract (ZH)**: 我们引入了梯度一致性过滤（GAF）方法，以改进分布式深度学习优化中的梯度平均化。传统的分布式数据并行随机梯度下降涉及计算各个微批量梯度的平均值得到宏观批量梯度，进而更新模型参数。我们发现，在训练后期阶段，微批量梯度往往彼此正交或负相关，这会导致模型 memorize 训练集，从而降低泛化能力。在本文中，我们提出了一种简单且计算有效的减少梯度方差的方法：在训练过程中计算微批量梯度之间的余弦距离，并在平均之前过滤掉冲突的更新。我们通过显著减小微批量的大小提高了验证精度，并且展示了这种方法可以降低对噪声标签的 memorization。我们还在标准图像分类基准数据集，包括 CIFAR-100 和 CIFAR-100N-Fine 上验证了该技术的有效性。我们证明了这种方法在某些情况下相较于传统训练方法可以提高多达 18.2% 的验证准确性，同时因为我们现在可以使用更小的微批量而降低所需的计算量近一个数量级，从而不会导致训练不稳定。 
...
**Abstract (ZH)**: 在二语习得领域，预测性建模帮助教育者实施多种教学策略，吸引了大量研究关注。然而，尽管模型准确性得到了广泛研究，模型公平性却仍被忽视。模型公平性确保对不同群体的公正待遇，防止基于性别、种族或经济背景等因素的无意偏见。一个公正的模型应当产生公正的结果，不会系统性地不利任何群体。

本研究使用了Duolingo数据集的en_es（英语学习者使用西班牙语）、es_en（西班牙语学习者使用英语）和fr_en（法语学习者使用英语）轨道，评估了两个预测模型的公平性。我们分析了以下内容：1. 不同平台（iOS、Android、Web）的算法公平性。2. 发达和发展中地区的算法公平性。

主要发现包括：1. 深度学习在二语知识追踪中优于机器学习，因为其准确性和公平性均有所提升。2. 两种模型均偏好移动用户而非非移动用户。3. 机器学习在针对发展中地区的偏见方面比深度学习更为严重。4. 深度学习在en_es和es_en轨道上在公平性和准确性之间取得了更好的平衡，而机器学习在fr_en轨道上更为适用。

本研究强调了在预测模型中解决公平性问题的重要性，以确保不同平台和地区教育策略的公平性。 

---
# Uncertainty-Aware Critic Augmentation for Hierarchical Multi-Agent EV Charging Control 

**Title (ZH)**: 具有不确定性意识的评论者增强机制在分层多代理电动汽车充电控制中的应用 
...
**Abstract (ZH)**: 先进的双向电动汽车（EV）充电和放电技术旨在支持电网稳定和应急操作，正逐渐引起办公场所应用的兴趣。这项技术不仅有效地降低了电费开支，还增强了处理高峰功率限制、能源价格波动和EV突然离开等实用问题的能力。然而，现有的EV充电策略尚未全面考虑这些因素，以同时惠及办公楼和EV用户。为解决这些问题，我们提出了一种新颖的实时充电控制方法——HUCA（Hierarchical Unified Charging and Discharging Algorithm），用于调节建筑和电动汽车的能源需求。HUCA采用分层 actor-critic 网络，动态调整建筑中的电费开支，同时考虑到动态定价场景中EV充电的需求。为了应对EV不确定离开的问题，我们引入了一种新的批评者增强方法，以评估充电决策时考虑到离开的不确定性，同时保持充电控制的稳健性。在实际电力数据集上进行的实验表明，在模拟确定性和不确定离开场景下，HUCA在总电费开支方面优于基线方法，同时在满足EV充电需求方面也表现出竞争性的性能。案例研究还表明，HUCA能够根据实时信息有效地平衡建筑与EVs之间的能源供应。 

---
# Emoji Retrieval from Gibberish or Garbled Social Media Text: A Novel Methodology and A Case Study 

**Title (ZH)**: 从 gibberish 或加密社交媒体文本中检索表情符号：一种新颖的方法及其案例研究 
...
**Abstract (ZH)**: 情绪符号在社交媒体平台上广泛使用，但在嘈杂或扭曲的文字中往往会被丢失，这对数据分析和机器学习构成了挑战。传统的预处理方法建议去除这些文字，这可能导致情绪符号及其上下文意义的丢失。本文提出了一种三步逆向工程方法，旨在从社交媒体帖子中的扭曲文字中恢复情绪符号。该方法还识别了在社交媒体数据挖掘过程中生成此类文字的原因。为了评估其有效性，该方法被应用于509,248条关于Mpox疫情的推文，这一数据集被大约30篇先前的研究引用，这些研究未能从扭曲文字中检索到情绪符号。我们的方法从76,914条推文中检索出了157,748个情绪符号。通过Flesch可读性指数、Flesch-Kincaid 年级水平、Coleman-Liau指数、自动可读性指数、Dale-Chall可读性评分、文本标准和阅读时间等指标展示了文本可读性和连贯性的改善。此外，还分析了这些推文中个别情绪符号的频率及其使用模式，结果也进行了呈现。 

---
# Aligning AI Research with the Needs of Clinical Coding Workflows: Eight Recommendations Based on US Data Analysis and Critical Review 

**Title (ZH)**: 将AI研究与临床编码工作流程需求对齐：基于美国数据的分析与批判性审查提出的八点建议 
...
**Abstract (ZH)**: 临床编码对于医疗收费和数据分析至关重要。手工临床编码劳动密集且易出错，因此推动了全自动化编码过程的研究。然而，基于对美国英语电子健康记录的分析，以及使用这些记录的自动化编码研究显示，广泛使用的评估方法并未与实际临床情境相契合。例如，重点关注最常见的前50个代码的评估太过简化，因为实际中使用的代码多达数千种。本文旨在使人工智能编码研究更贴近临床编码的实际挑战。基于我们的分析，我们提供了八项具体建议，建议改进当前的评估方法。此外，我们提出了超越自动化编码的新的人工智能方法，为临床编码员的工作流程提供替代辅助手段。 

---
# Theoretical Constraints on the Expressive Power of $\mathsf{RoPE}$-based Tensor Attention Transformers 

**Title (ZH)**: 基于RoPE的张量注意力Transformer的表达能力的理论约束 
...
**Abstract (ZH)**: 张量注意力通过捕捉多模态下的高阶相关性，扩展了传统的注意力机制，解决了经典矩阵基注意力的局限性。同时，旋转位置嵌入（$\mathsf{RoPE}$）在长语境场景中展现出优越的性能，显著提升了变换器模型的表现力。尽管这些方法在实验中取得了成功，但它们的理论局限性仍待进一步探究。在本研究中，我们分析了张量注意力和基于$\mathsf{RoPE}$的张量注意力的电路复杂度，表明在多项式精度、常数深度层、线性或次线性隐藏维度的假设下，它们不能解决固定的成员资格问题或$(A_{F,r})^*$封闭性问题，前提是$\mathsf{TC}^0 \neq \mathsf{NC}^1$。这些发现突显了张量注意力和基于$\mathsf{RoPE}$的变换器模型在实验表现与理论约束之间的差距，这为指导基于变换器模型设计与扩展的更多理论导向方法的发展提供了见解。 

---
# AA-SGAN: Adversarially Augmented Social GAN with Synthetic Data 

**Title (ZH)**: AA-SGAN：带有合成数据的对抗增强社交生成对抗网络 
...
**Abstract (ZH)**: 准确预测行人轨迹在自动驾驶或服务机器人等应用中至关重要。深度生成模型在这种任务中能取得最佳性能，前提是可用足够多的标注轨迹进行训练。为此，大量合成生成的、标注过的轨迹存在（例如由视频游戏生成）。然而，这些轨迹并不能真实地表示行人的运动，因此对于训练预测模型效果不佳。为此，我们提出了在训练时增强合成轨迹的方法和架构，并采用了对抗性方法。我们展示了在评估最先进的生成模型时，通过在训练时增强轨迹可以带来显著的性能提升，尤其是在处理真实世界轨迹时。 
...
**Abstract (ZH)**: 神经网络通常被视为黑盒模型，这在理解其内部工作机制方面带来了重大挑战，尤其是在自然语言处理（NLP）应用中。为了应对这种透明度不足的问题，模型解释技术，如局部可解释的模型无关解释（LIME），已成为提供这些复杂系统行为见解的重要工具。本研究利用LIME来解释一个用于文本分类任务的多层感知器（MLP）神经网络。通过分析个体特征对模型预测的贡献，LIME方法增强了可解释性并支持了基于信息的决策。尽管LIME在提供局部解释方面非常有效，但它在捕捉全局模式和特征交互方面存在局限性。本研究突出了LIME的优点和缺点，并提出了未来工作的方向以实现神经NLP模型更全面的可解释性。 

---
# More than Chit-Chat: Developing Robots for Small-Talk Interactions 

**Title (ZH)**: 不仅仅闲聊：开发用于轻松对话的机器人 
...
**Abstract (ZH)**: 超越形式主义，闲聊在社交动态中扮演着至关重要的角色，它作为一种口头握手，有助于建立关系和相互理解。对于对话式人工智能和社交机器人而言，能够进行闲聊能够增强它们的拟人化感知，从而实现更加舒适和自然的用户交互。在本研究中，我们评估了当前大型语言模型（LLMs）驱动社交机器人进行闲聊的能力，并确定了关键改进领域。我们提出了一种新的方法，能够自主生成反馈并确保LLM生成的响应符合闲聊惯例。通过几次评估——包括聊天机器人交互和人机交互——我们展示了该系统引导LLM生成的响应朝向真实、拟人化和自然的闲聊交流的有效性。 
...
**Abstract (ZH)**: 在以大规模语言模型（LLM）为核心迅速发展的AI时代，提高LLM的信任度和效率，尤其是在输出生成（推理）方面，受到了广泛的关注。这旨在减少可能但错误的LLM输出（即幻觉），以满足不断增长的推理需求。本教程探讨了这些努力，并使其对数据库界透明。理解这些努力对于在数据库任务中充分利用LLM以及将数据库技术适应到LLM至关重要。此外，本教程还探讨了LLM与数据库之间的协同效应，强调了它们交汇处的新机遇与挑战。本教程旨在向数据库研究人员和实践者分享有关LLM的关键概念和策略，减少对LLM的陌生感，并激发更多人参与LLM与数据库的交汇领域。 

---
# Integrated Learning and Optimization for Congestion Management and Profit Maximization in Real-Time Electricity Market 

**Title (ZH)**: 实时电力市场中拥堵管理与利润最大化集成学习与优化研究 
...
**Abstract (ZH)**: 我们开发了新颖的集成学习与优化（Integrated Learning and Optimization, ILO）方法来解决经济调度（Economic Dispatch, ED）和直流最优潮流（DC Optimal Power Flow, DCOPF）问题，以实现更有效的经济运行。对于ED的优化问题，将负荷视为未知参数；而DCOPF则将负荷和功率传输分布因子（Power Transfer Distribution Factor, PTDF）矩阵作为未知参数。PTDF 表示由于两个区域之间的有功功率传输而在线路上产生的增量变化，这些值提供了一条线路上有功功率流动的线性化近似。我们开发了新颖的ILO形式来解决电能量市场中的事后惩罚问题以及线路拥塞问题，使用了ED和DCOPF优化框架。所提出的方法能够捕捉实时电能量市场和线路拥塞行为，从而训练后悔函数，最终训练不同母线的未知负荷以及线路上的PTDF矩阵，以实现上述的后验目标。我们提出的该方法与顺序学习与优化（Sequential Learning and Optimization, SLO）方法进行了比较，SLO方法侧重于训练负荷和PTDF的预测以提高准确度，而不是经济运行效率。实验结果证实，在电能量市场中通过ILO方法最小化事后惩罚，在降低线路拥塞方面也显得更有效，从而显著提高了经济运行效率。 

---
# WavePulse: Real-time Content Analytics of Radio Livestreams 

**Title (ZH)**: WavePulse：实时分析无线直播间内容 
...
**Abstract (ZH)**: 广播仍然是广泛传播信息的一种普及媒介，AM/FM广播电台比基于智能手机的社交网络平台或现场电视更为普及，触及到更多的美国人。越来越多地，广播节目也在网上广播，并通过互联网访问。我们提出WavePulse框架，用于实时记录、记录和分析广播内容。尽管我们的框架具有广泛适用性，但我们与一组政治科学家团队合作，重点关注2024年总统选举，以展示WavePulse的有效性。我们使用WavePulse监控了三个月内396家新闻广播电台的现场广播流，处理了近50万小时的音频流。这些流被转换为带时间戳的、分段转录，并进行分析，以跟踪关键的政治科学问题，涵盖了国家级和州级层面。我们的分析揭示了局部问题与全国趋势的相互作用，提供了信息流的洞察。我们的结果证明了WavePulse在从网上获取的广播直播流内容捕获与分析方面的有效性。代码和数据集可在 \url{this https URL} 获取。 

---
# Multi-Agent Path Finding in Continuous Spaces with Projected Diffusion Models 

**Title (ZH)**: 连续空间中基于投影扩散模型的多agent路径规划 
...
**Abstract (ZH)**: 多智能体路径规划（Multi-Agent Path Finding, MAPF）是机器人领域中的一个基本问题，需要为多个从各自起点到终点移动的智能体计算无碰撞路径。在共享环境中协调多个智能体构成了重大挑战，尤其是在连续空间中，传统优化算法难以实现可扩展性。此外，这些算法通常依赖于环境的离散表示，这在基于图像或高维设置中往往是不切实际的。最近，扩散模型在单智能体路径规划中显示出潜力，能够捕捉复杂的轨迹分布，并生成能够导航连续高维空间的平滑路径。然而，将扩散模型直接扩展到MAPF引入了新的挑战，因为这些模型难以确保满足约束条件，例如智能体间的碰撞避免。为克服这一限制，本工作提出了一种新颖的方法，将约束优化与扩散模型相结合，以解决连续空间中的MAPF问题。这种独特的结合直接生成了遵循碰撞避免和动力学约束的可行多智能体轨迹。我们的方法在不同维度的多种挑战性仿真场景中得到了有效性的验证。 

---
# ICPR 2024 Competition on Domain Adaptation and GEneralization for Character Classification (DAGECC) 

**Title (ZH)**: ICPR 2024 领域适应与通用化字符分类挑战赛 (DAGECC) 
...
**Abstract (ZH)**: 在本次ICPR 2024大会框架下组织的DAGECC（领域适应与字符分类泛化挑战赛）的配套论文中，我们介绍了我们为社区提出的任务的整体背景，介绍了为比赛准备的数据，并提供了结果总结，同时描述了前三名获奖作品。该竞赛集中在领域适应与泛化方面，我们的核心目标是通过提供高质量、轻量级且实际应用的数据集来促进这些主题的兴趣和研究进展，该数据集能够支持快速原型开发和新想法的验证。 

---
# TNNGen: Automated Design of Neuromorphic Sensory Processing Units for Time-Series Clustering 

**Title (ZH)**: TNNGen: 自动设计的时间序列聚类神经形态感觉处理单元 
...
**Abstract (ZH)**: 时间神经网络（TNNs），一类特殊的脉冲神经网络，借鉴了新皮层利用脉冲时间进行信息处理的方法。近期的研究提出了一个微架构框架和定制的宏观设计套件，以设计高度能效的应用特定TNNs。这些近期的研究依赖于手动硬件设计，这是一个劳动密集型且耗时的过程。此外，目前没有开源的功能仿真框架来支持TNNs的设计。本文介绍了TNNGen，这是一个旨在从PyTorch软件模型自动设计TNNs到后布局网表的开创性努力。TNNGen 包含一个新型的PyTorch功能仿真器（用于TNN建模和应用探索）和一个基于Python的硬件生成器（用于PyTorch到RTL和RTL到布局的转换）。本文仿真了七个代表性的TNN设计，这些设计覆盖了不同感觉通道的时间序列信号聚类，并评估了它们的后布局硬件复杂性和设计运行时间，以证明TNNGen的有效性。此外，我们还强调了TNNGen在无需运行硬件流程的情况下准确预测硅片性能的能力。 

---
# Improving Sickle Cell Disease Classification: A Fusion of Conventional Classifiers, Segmented Images, and Convolutional Neural Networks 

**Title (ZH)**: 改善镰状细胞病分类：传统分类器、分割图像与卷积神经网络的融合 
...
**Abstract (ZH)**: 镰状细胞贫血是一种由于红细胞形态异常引起的一种疾病，可以通过显微镜图像进行检测。医学中的计算技术能够提高诊断和治疗效率。然而，许多计算技术，尤其是基于卷积神经网络（CNN）的方法，需要大量的资源和时间进行训练，这凸显了低计算开销方法的研究机会。本文提出了一种结合传统分类器、分割图像和CNN的新方法，用于镰状细胞疾病自动分类。我们评估了分割图像对分类的影响，提供了深度学习集成的见解。实验结果表明，使用分割图像和CNN特征与支持向量机（SVM）结合可以达到96.80%的准确率。这一发现适用于计算效率高的场景，并为未来医学图像分析的研究和进步开辟了道路。 

---
# Towards Cognitive Service Delivery on B5G through AIaaS Architecture 

**Title (ZH)**: 面向B5G认知服务交付的AIaaS架构研究 
...
**Abstract (ZH)**: 人工智能（AI）在推动移动网络系统的发展中发挥着关键作用，通过提供智能能力和自动化功能。从4G到5G的转变对AI在构建以垂直行业为主的网络方面具有重大影响。在此背景下，3GPP已在网络的核心层引入了网络数据与分析功能（NWDAF）实体，以通过基于AI算法提供洞察，从而优化网络编排。本文提出了一种演进NWDAF的框架，旨在进一步增强核心网络的AI能力，并适用于B5G和6G技术。此外，我们还确定了一组实现分布式e-NWDAF的研究方向。 

---
# tuGEMM: Area-Power-Efficient Temporal Unary GEMM Architecture for Low-Precision Edge AI 

**Title (ZH)**: tuGEMM：低精度边缘AI的面积和功耗高效的时间一元GEMM架构 
...
**Abstract (ZH)**: 通用矩阵乘法（GEMM）是数据处理中广泛使用的计算内核/算法，尤其是在人工智能（AI）和深度学习（DL）等领域。近年来，向边缘计算的转变激发了基于一元计算的GEMM架构，这些架构主要是随机性和速率编码系统。本文提出了一种基于时间编码的新型GEMM架构，称为tuGEMM，能够进行精确计算。我们介绍了两种tuGEMM的变体，分别是串行和并行版本，具有不同的面积/功率/延迟折中方案。在45 nm CMOS工艺下，我们报告了2比特、4比特和8比特计算的综合后功耗-性能-面积（PPA）结果。该设计在低精度情况下（例如，2比特的面积仅为0.01 mm²，功耗仅为4 mW；4比特的面积为0.03 mm²，功耗为9 mW）显示出在面积-功耗效率方面显著优于最先进的随机一元系统的优势。这使得tuGEMM成为具有功耗限制的移动设备和边缘设备的理想选择，适用于始终在线的实时传感处理。 
...
解释：
- "LMV" 代表 "Large Model Voting"，意味着使用大型模型进行投票。
- "RPA" 代表 "Robotic Process Automation"，即机器人流程自动化。

这样的翻译既保留了原文的技术含义，又符合学术论文的规范。 
...
**Abstract (ZH)**: 自动化处理大量非结构化数据对于提高运营效率至关重要。光学字符识别（OCR）在这一过程中至关重要，但由于复杂布局和模糊文本，往往难以达到高准确性和效率。对于需要同时兼顾速度和精度的大规模任务而言，这些挑战尤为明显。本文介绍了一种名为LMV-RPA（Large Model Voting-based Robotic Process Automation）的系统，以增强OCR工作流。LMV-RPA将来自Paddle OCR、Tesseract OCR、Easy OCR和DocTR等OCR引擎的输出，以及来自LLM（如LLaMA 3和Gemini-1.5-pro）的大型语言模型进行整合。通过多数投票机制，将OCR输出转化为结构化的JSON格式，从而提高准确率，特别是在复杂布局方面。多阶段管道将OCR引擎提取的文本通过LLM处理，并结合结果以保证最高准确性。LMV-RPA在OCR任务中的准确率达到了99%，超过了基础模型的94%准确率，并且将处理时间减少了80%。基准评估证明了其可扩展性，并表明LMV-RPA能够提供一种更快、更可靠、更高效的大型文档处理任务自动化解决方案。 

---
# Analysis of Transferred Pre-Trained Deep Convolution Neural Networks in Breast Masses Recognition 

**Title (ZH)**: 在乳腺肿块识别中迁移预训练深度卷积神经网络的分析 
...
**Abstract (ZH)**: 基于预训练卷积神经网络（CNN）的乳腺癌检测引起了其他传统计算机系统中的广泛关注。在过去几年中，卷积神经网络技术已成为在乳腺X线摄影扫描中发现癌症最有希望的方法之一。在本文中，研究了预训练CNN中层冻结对乳腺癌检测的影响，通过将乳腺X线摄影图像分类为良性或恶性来实现。基于冻结的卷积层块数量，研究了不同的VGG19场景。本研究中共有六种不同的场景。此研究的主要益处在于两方面：提高模型检测乳腺癌病例的能力，同时通过冻结某些层从而缩短VGG19的训练时间。为了评估这些场景的性能，使用了1693张良性与恶性乳腺癌的微生物学图像。根据报告的结果，冻结VGG19的第一个卷积层块获得了最高的识别率，敏感性为95.64%，而整个VGG19的训练结果为94.48%。 

---
# ArchComplete: Autoregressive 3D Architectural Design Generation with Hierarchical Diffusion-Based Upsampling 

**Title (ZH)**: ArchComplete：基于分层扩散增强上采样的自回归三维建筑设计生成 
...
**Abstract (ZH)**: $\textit{ArchComplete}$ 是一种两阶段的密集体素基三维生成管道，旨在应对建筑几何形状和拓扑结构的高复杂性，在早期设计过程中辅助概念生成和几何细节化。在第一阶段，开发了一种 $\textit{3D 薄暮 VQGAN}$ 模型，其组成然后通过自回归变压器来生成粗略模型。随后，在第二阶段，定义了一组三维条件去噪扩散概率模型组成的层次体素上采样网络，用于增加粗略形状的精细几何细节。第一阶段在包含完整建模外部和内部的房屋模型数据集上进行训练，使用了一种新颖的2.5D 空间感知损失，以捕捉不同抽象层次下的输入复杂性，而第二阶段在随机裁剪的局部体素块上进行训练，所需计算和内存资源显著减少。对于推理，在生成过程中首先自回归生成分辨率为 $64^3$ 的房屋模型，然后逐步优化至分辨率为 $256^3$，体素大小小至 $18\text{cm}$。$\textit{ArchComplete}$ 支持多种交互模式，解决各种任务，包括插值、变异生成、无条件合成以及两种条件合成任务：形状补全和计划绘制补全，还包括几何细节化。实验结果表明，在现有指标上取得了显著改进。 

---
# Adaptive Signal Analysis for Automated Subsurface Defect Detection Using Impact Echo in Concrete Slabs 

**Title (ZH)**: 使用混凝土板中冲击回波的自适应信号分析进行自动地下缺陷检测 
...
**Abstract (ZH)**: 本初步研究表明，提出了一种全新的、自动化的、可扩展的方法，用于通过冲击回波（IE）信号分析检测和评估混凝土板中潜在缺陷的地下区域。该方法结合了先进的信号处理、聚类和可视化分析技术，以识别地下异常。采用了一种独特的自适应阈值方法，将基于频率的缺陷识别个性化到每块板的独特材料特性。该方法生成了频率图、二值掩码和k均值聚类图，以自动分类缺陷和非缺陷区域。通过关键可视化，包括三维表面图、聚类图和轮廓图，分析空间频率分布并突出显示结构异常。研究使用了美国联邦公路管理局（FHWA）高级传感技术非破坏性评估实验室构建的带标签数据集。评估涉及地面真实值掩码，通过与FHWA提供的信息生成的顶视二值掩码进行比较，以验证生成的缺陷图。性能指标，特别是F1分数和AUC-ROC，分别达到了0.95和0.83。研究结果表明，该方法具有稳健性，能够准确且一致地识别潜在缺陷区域，同时具有很少的假阳性结果和遗漏的缺陷。自适应频率阈值方法确保能够适应板之间频率变化，从而提供一种可扩展的框架，用于检测结构异常。此外，由于其可推广的阈值机制，该方法可以适应其他基于频率的信号，并具有潜在的多模态传感器融合应用空间。此自动化和可扩展的工作流程减少了手动干预，确保了准确和高效的缺陷检测，进一步推进了非破坏性评估（NDE）技术的发展。 

---
# BenCzechMark : A Czech-centric Multitask and Multimetric Benchmark for Large Language Models with Duel Scoring Mechanism 

**Title (ZH)**: BenCzechMark：一种基于捷克语的多任务多指标基准测试，配备双重评分机制的大语言模型评估标准 
...
我们提出了BenCzechMark (BCM)，这是首个专为大规模语言模型设计的全面捷克语基准测试。该基准测试提供了多种任务、多种任务格式以及多种评估指标。其评分系统基于统计显著性理论，并采用了受社会偏好理论启发的跨任务聚合方法。基准测试包含50项具有挑战性的任务，其中包含相应的测试数据集，主要为捷克本地语言，还有11个新收集的任务。这些任务涵盖了8个类别，并覆盖了多个领域，包括历史上的捷克新闻、学生活作文或语言学习者的文章，以及口语作品。

此外，我们收集并清理了BUT-Large捷克语集合，这是目前可获取的最大规模的纯净捷克语语料库，并利用该语料库进行 (i) 污染分析；(ii) 首个以捷克语为中心的7B语言模型的连续预训练，其中包含了专门针对捷克语的分词。我们使用该模型作为基准，以便与现有的多语言模型进行比较。最后，我们发布了并维护了一个排行榜，已有44个模型提交，新的模型提交可以通过<https://your-website-url> 进行提交。

请注意，其中的网址 `<https://your-website-url>` 需替换为实际的提交链接地址。 

---
# A Novel Approach to Balance Convenience and Nutrition in Meals With Long-Term Group Recommendations and Reasoning on Multimodal Recipes and its Implementation in BEACON 

**Title (ZH)**: 一种平衡便利性和营养性的新方法：基于长期组推荐和多模态食谱的综合推理及其在BEACON中的实现 
...
**Abstract (ZH)**: 人们无论是健康状态还是存在健康状况，都会做出选择一日三餐（早餐、午餐和晚餐）的过程中包括各种食物的组合来满足开胃菜、主菜、配菜、甜点和饮料的需求。这一选择通常会涉及到营养选择（比如食盐和糖的含量、营养成分）和便捷性（比如成本、可及性、菜肴类型和食品来源类型）之间的权衡。我们提出了一种基于数据驱动的餐食推荐解决方案，该方案考虑了可定制的餐食配置和时间范围。该解决方案在平衡用户偏好时也考虑了食物成分和烹饪过程。我们的贡献包括引入了良好的度量标准，将文本中的食谱转换为最近引入的多模态丰富食谱表示（R3）格式的方法，以及使用上下文多臂老虎机进行学习的方法，初步结果显示出有希望的结果。此外，我们还提出了一个原型系统——BEACON系统，该系统受到了用户使用情况的启发。 
...
**Abstract (ZH)**: 大规模语言模型（LLMs）在广泛的语言相关任务上展示了卓越的能力，包括解决复杂的推理问题。通过上下文学习的有效技术可以提高LLM的性能，该技术通过包含解释性的例子来引导模型的响应过程，从而鼓励逐步的推理过程。然而，选择适用于模型的合适例子是一个挑战，因为每个数据集都需要一套独特的例子来使LLM能够有效学习并在测试集上表现出色。当前的研究往往依赖于不确定性或多样性导向的选择策略来为标注选择例子并改进模型学习。然而，这些研究通常采用非适应性方法，在一次选择一组例子。我们认为，这种非适应性策略可能导致选择的一组例子在覆盖的知识方面具有高度冗余，最终降低了它们的整体信息量。为了解决这一不足，我们提出了一种名为\textsc{Adaptive-Prompt}的新方法，该方法通过利用之前选择的例子的模型反馈来适应性地选择例子。实验结果表明，\textsc{Adaptive-Prompt}显著提高了LLM在各种推理任务上的性能。 

---
# Stability Bounds for the Unfolded Forward-Backward Algorithm 

**Title (ZH)**: 展开前向-后向算法的稳定性边界 
...
**Abstract (ZH)**: 我们考虑了一种用于解决降级操作符为线性和已知的逆问题的神经网络架构。该架构通过展开一个源自目标函数最小化的方法而构建，该目标函数结合了数据保真项、Tikhonov型正则化项和可能非光滑的凸惩罚项。本文理论分析了此逆方法对输入扰动的稳健性。确保稳健性符合逆问题理论的原则，它保证了逆方法的连续性和对小噪声的抵抗能力——这在已知深度神经网络对对抗性扰动的脆弱性背景下尤为重要。我们工作的关键 novelty 之处在于研究了所提出的网络对其偏差扰动的稳健性，这里的偏差代表逆问题中的观测数据。此外，我们还提供了分析中推导出的可微分Lipschitz边界值的数值示例。 

---
# In Defence of Post-hoc Explainability 

**Title (ZH)**: 论事后的可解释性的重要性 
...
**Abstract (ZH)**: 机器学习在科学研究中的广泛应用在根本上加剧了模型不透明性和科学理解之间的矛盾。虽然有些人提倡使用固有的可解释性模型，我们提出了计算解释主义（Computational Interpretabilism, CI）作为一种后验可解释性在科学人工智能中的哲学框架。借鉴人类专业知识的逻辑，在人类专业知识中后验合理化与可靠性能并存，CI 认为通过适当的实证验证来限定的模型解释结构化地促成了科学知识的产生。借助中介理解和有界真知，我们展示了后验方法如何在不需要完全机械透明性的前提下获得知识论上正当的见解，从而解决模型复杂性和科学理解之间的矛盾。 
...
**Abstract (ZH)**: 在本文中，我们介绍了并应用了运筹优化问答（ORQA），这是一个新的基准，旨在评估大语言模型（LLMs）在运筹学（OR）这一专业技术领域中的泛化能力。该基准评估LLMs在面对各种复杂优化问题时，是否能够模仿运筹学专家的知识和推理能力。运筹学专家开发的数据集包含了需要多步推理来构建其数学模型的真实世界优化问题。我们对诸如LLaMA 3.1、DeepSeek和Mixtral等开源LLMs的评估显示，这些模型的表现较为有限，凸显了它们在向专业技术领域泛化的不足。本文为LLMs泛化能力的探讨作出了贡献，并提供了未来研究的宝贵见解。数据集和评估代码已公开。 
...
**Abstract (ZH)**: 大型语言模型中存储的知识需要及时更新，以反映现实世界信息的动态性质。为了更新知识，大多数知识编辑方法集中在较低层，因为最近对知识召回过程的探查表明，答案信息丰富于较低层。然而，这些探查仅揭示了原始答案的关键召回阶段，而编辑的目标是纠正模型对目标答案的预测。这种不一致性表明，探查方法和相关的编辑方法都存在不足。为了缓解这种不一致性并识别关键的编辑区域，我们提出了一种对比探查方法，并识别了两种关键阶段：低层信息增强和高层概率促进，模型行为在原始答案与目标答案之间在此两个阶段出现分歧。基于这些洞见，我们开发了联合知识编辑对信息增强和概率促进（JEEP）方法，该方法同时编辑低层和高层以修改这两个关键的召回阶段。考虑到双重修改带来的相互干扰和不断增加的遗忘，JEEP旨在确保对不同区域的更新具有相同的目标，并且是互补的。我们通过在各种模型（如GPT-J（6B）和LLaMA（7B））上编辑多达数千条事实，并解决各种编辑目标（如添加事实性和反事实知识）来严格评估JEEP。在所有测试的场景中，JEEP均表现出最佳性能，验证了我们探查方法的揭示和我们编辑方法设计的有效性。我们的代码和数据可在以下链接获取：https://your-link-here.com 

---
# Evaluating and Enhancing LLMs for Multi-turn Text-to-SQL with Multiple Question Types 

**Title (ZH)**: 评估和提升针对多种问题类型多层次文本到SQL的LLM性能 
...
**Abstract (ZH)**: 近年来，大规模语言模型（LLMs）在文本到SQL（text-to-SQL）系统的进展显著。然而，大多数基于LLM的方法往往只关注SQL生成，忽略了实际对话查询的复杂性。这种忽略可能导致对模糊问题的不可靠回答，这些问题不能直接通过SQL解决。为弥补这一差距，我们提出了MMSQL，这是一个全面的测试套件，通过模拟多样化的提问类型和多轮问答交互来测试LLMs的问题分类和SQL生成能力。使用MMSQL，我们评估了多种流行的LLM，包括开源和闭源模型，并确定了影响它们在这种场景中表现的关键因素。此外，我们还介绍了一个基于LLM的多智能体框架，该框架利用专门的智能体来识别问题类型并确定适当的回答策略。我们的实验表明，这种方法显著增强了模型在处理对话动态复杂性的能力，有效应对了用户查询的多样性和复杂性。 
...
**Abstract (ZH)**: 租户驱逐会威胁到住房稳定性，许多城市对此非常关注。一个未解决的问题是，数据驱动的方法是否能够提高针对高风险租户的外展项目，从而降低其被驱逐的风险。我们提出了一种新颖的主动空间搜索（AGS）建模框架来解决这一问题。AGS将物业层级信息融入搜索策略中，该策略可以识别一系列需要进行调查的租赁单元，以确定其驱逐风险，并在必要时提供支持。我们提出了一种分层强化学习方法来学习适用于AGS的搜索策略，该方法适用于包含数千个地块的大都市，并且能够平衡探索和利用，同时考虑旅行成本和预算约束。至关重要的是，搜索策略能够根据新发现的驱逐信息进行在线调整。使用大都市区域的驱逐数据进行评估表明，所提出的框架及其算法方法在顺序识别驱逐案件方面比基线方法更为有效。 

---
# LaMI-GO: Latent Mixture Integration for Goal-Oriented Communications Achieving High Spectrum Efficiency 

**Title (ZH)**: LaMI-GO: 潜在混合集成用于目标导向通信以实现高频谱效率 
...
**Abstract (ZH)**: 近年来，语义式通信的兴起显著推动了目标导向通信（GOCOM）的发展，这是一种高效多模态信息传输方式。GOCOM的概念利用先进的人工智能（AI）工具，以应对边缘计算和物联网（IoT）等应用中对带宽效率日益增长的需求。与传统通信系统侧重于源数据准确性不同，GOCOM提供了针对接收端完成下游任务所需特别需求的智能消息传递。在本文中，我们提出了一种新颖的GOCOM框架——LaMI-GO，该框架利用新兴的生成AI技术，以实现超高的通信效率和更好的服务质量（QoS）。具体而言，我们基于潜在扩散模型和矢量量化生成对抗网络（VQGAN）设计了LaMI-GO系统的主干，以实现高效的潜在嵌入和信息表示。该系统在接收端训练了一个公共特征代码本。我们的实验结果表明，LaMI-GO在感知质量、下游任务准确性和带宽消耗方面显著优于现有的GOCOM系统，并确立了我们提出的LaMI-GO通信框架的强大能力。 

---
# Coordinated Power Smoothing Control for Wind Storage Integrated System with Physics-informed Deep Reinforcement Learning 

**Title (ZH)**: 基于物理信息深度强化学习的风电储能集成系统协调功率平滑控制 
...
**Abstract (ZH)**: 具有功率平滑控制（PSC）的风储联合系统已经成为了确保风能高效可靠生成的一种有前景的解决方案。然而，现有的PSC策略忽视了电池与风力涡轮机之间复杂的交互作用及其不同的控制频率，并未考虑到尾流效应和电池退化成本。本文提出了一个基于分层结构的新型协调控制框架，以有效解决上述挑战，该框架整合了尾流模型和电池退化模型。此外，在将问题重新表述为马尔可夫决策过程后，引入了多代理强化学习方法以克服该问题的两层特性。同时，提出了物理信息神经网络辅助多代理深度确定性策略梯度（PAMA-DDPG）算法，以整合功率波动微分方程并加速学习过程。通过使用WindFarmSimulator（WFSim）进行四组不同场景的仿真，评估了所提出方法的有效性。结果表明，所提出的算法可使总收益提高约11%，功率波动减少约19%，从而同时达到经济效率和并网能源可靠性双重目标。 

---
# Transfer Learning with Active Sampling for Rapid Training and Calibration in BCI-P300 Across Health States and Multi-centre Data 

**Title (ZH)**: 基于主动采样的迁移学习在健康状态和多中心数据下实现BCI-P300快速训练与校准 
...
**Abstract (ZH)**: 机器学习和深度学习的进步提升了脑机接口（BCI）的表现，但由于个体健康状况、硬件差异和文化差异等因素的影响，其在广泛的应用中的适用性仍然受到限制。现有的研究往往集中在单一且统一的实验中，导致在现实多样性中难以迁移所获得的高精度。深度学习模型旨在提高BCI分类准确率，并且有研究表明，通过在基于他人数据训练的基础模型上使用迁移学习，可以适应个体的神经模式。这种方法有望提高泛化能力和减少过拟合，但处理不同设备、不同受试者、不同国家多个中心以及健康和病患人群之间的多样化和不平衡数据集时仍存在挑战。

在最大程度的异质性环境下，我们提出了使用基于毒药采样盘（PDS）的自适应迁移学习方法（称为主动采样法，AS）来检测BCI中的P300波形。这种方法能够灵活调整源数据到目标域的过渡。我们使用两个国际上复制的数据集进行的实验表明，通过40%的自适应微调，结合自适应迁移学习方法，平均分类准确率提高了5.36%，标准差降低了12.22%。这些结果在分类准确率、计算时间和训练效率方面均优于其他方法，主要得益于所提出的主动采样（AS）方法在迁移学习中的应用。 

---
# MANGO: Multimodal Acuity traNsformer for intelliGent ICU Outcomes 

**Title (ZH)**: MANGO: 多模态敏锐度变换器以实现智能ICU预后 
...
**Abstract (ZH)**: 重症监护室（ICU）患者警严重度的评估对于确保及时和适当的干预至关重要。人工智能（AI）技术的进步极大地提高了警严重度预测的准确性。然而，之前使用机器学习进行警严重度预测的研究主要依赖电子健康记录（EHR）数据，往往忽视了ICU留院期间的其他关键方面，如患者活动能力、环境因素以及面部表情等表示疼痛或激动的线索。为解决这一问题，我们提出了MANGO：用于智能ICU预后结果的多模态警严重度转换器，旨在增强对患者警严重度状态、转变及生命维持治疗需求的预测。我们收集了一个多模态数据集ICU-Multimodal，包含了四种关键模态：EHR数据、穿戴传感器数据、患者面部表情的视频数据以及环境传感器数据，并将这些数据用于训练MANGO。MANGO模型采用基于Transformer的掩码自注意力方法的多模态特征融合网络，即使某些模态缺失，也能够捕捉和学习这些异质数据模态之间的复杂交互。实验结果表明，多模态融合显著提高了模型预测警严重度状态、状态转变和生命维持治疗需求的能力。最佳模型在预测警严重度状态转变和生命维持治疗需求方面分别取得了0.76（95%CI：0.72-0.79）和0.82（95%CI：0.69-0.89）的受试者操作特征曲线下面积（AUROC）。 

---
# RUL forecasting for wind turbine predictive maintenance based on deep learning 

**Title (ZH)**: 基于深度学习的风力发电机剩余寿命预测及其在预知维护中的应用 
...
**Abstract (ZH)**: 预测性维护（PdM）正日益受到重视，它通过准确预测剩余使用寿命（RUL）并战略性地安排维护来降低风电场的操作和维护成本。然而，风电场的偏远位置往往使得当前的方法无效，因为这些方法无法为维护计划提供足够可靠的时间提前量窗口，限制了PdM的实际应用。本研究提出了一种新的基于深度学习（DL）的方法，用于未来RUL的预报。通过采用一种多参数注意力机制的DL方法，避免了特征工程，从而最大限度地减少了人为错误的风险，我们提出了两种模型：ForeNet-2d和ForeNet-3d。这些模型成功地在2周的预报窗口内预测了七种复杂风力涡轮机（WT）故障的RUL。最准确的预测仅比实际RUL偏离了10分钟，而最不准确的预测则偏离了1.8天，大多数预测的偏差仅为几个小时。该方法为访问远程风力涡轮机并进行必要维护提供了可观的时间窗口，从而使得PdM的实践实施成为可能。 

---
# The Rosetta Paradox: Domain-Specific Performance Inversions in Large Language Models 

**Title (ZH)**: 罗塞塔悖论：大型语言模型在特定领域中的性能反转 
...
**Abstract (ZH)**: 尽管像GPT和BERT这样的大规模语言模型已经在自然语言处理以及特定领域应用中展现了前所未有的能力，我们却遇到了一个未被探索的现象，我们称之为“罗塞塔悖论”。罗塞塔悖论描述了知识领域之间反直观的性能反转现象。这种悖论揭示了这些语言模型在高度专业化领域表现出色，但在需要一般性日常知识的任务上却表现不佳。本文正式定义了罗塞塔悖论，并引入了一个全景分析框架，该框架包括领域特异性指数（Domain Specificity Index, DSI）和性能反转度量（Performance Inversion Metric, PIM），用于一致地量化大规模语言模型在不同领域的表现。

我们采用了这一悖论，并通过涉及多种不同模型和知识领域的广泛实验进行了系列调查，范围从丰富的技术领域到常识推理。我们的研究结果表明，罗塞塔悖论很可能不仅仅是数据分布的伪像，而是深度神经网络内在的结构和涌现性质。我们对不同模型架构、规模和训练方法进行了比较分析，揭示了这一悖论出现的独特方式，并挑战了标准的评估指标。 
...
**Abstract (ZH)**: 评估大型语言模型（LLMs）在语言推理能力方面的表现是理解其在大规模应用中可能暴露出的能力差距的重要任务。本文通过极端低资源语言的语言谜题，考察了这些模型在进行抽象多语言推理方面的能力。由于这些翻译任务涉及从参考实例中归纳和演绎推理，我们探讨了是否可以从种子示例自动诱导出多样化的辅助示例，通过类比提示进行。我们采用了两阶段的方法，首先使用语言模型生成类比示例，然后将这些示例与提供的目标语言示例结合，在上下文中应用。在modeLing数据集上的结果显示，类比提示在激发模型对语言语法规则相似性的了解方面是有效的，这使得GPT-4o的表现提升了8.1%，Llama-3.1-405B-Instruct的表现提升了5.9%，超过了基于思考链的方法。这些改进归因于类比示范，无论这些示范是自动生成的还是由较弱的多语言模型生成的。此外，我们展示了我们的方法可以推广到其他 Linguistics Olympiad 竞赛中的任务，并在LINGOLY数据集中的所有问题类型和难度级别上，使用GPT-4o实现了显著的改进。我们还报告了一些关于促进语言推理性能的现象发现，这表明这些谜题是用于新推理方法基准测试的有价值的工具。 

---
# GOPT: Generalizable Online 3D Bin Packing via Transformer-based Deep Reinforcement Learning 

**Title (ZH)**: GOPT：基于Transformer的深度强化学习应用于通用在线三维集装箱装载问题 
...
**Abstract (ZH)**: 机器人对象包装在物流和自动化行业中具有广泛的实际应用，研究人员通常将其形式化为在线三维箱包问题（3D-BPP）。然而，现有的基于深度强化学习（DRL）的方法主要集中在提升在有限包装环境中的表现，而忽略了在不同尺寸的箱体中进行泛化的能力。为了解决这一问题，我们提出了一种GOPT方法，即通过基于Transformer的深度强化学习实现可泛化的在线三维箱包策略。首先，我们设计了一个放置生成模块，生成有限的子空间作为放置候选，并表示箱体。其次，我们提出了一种包装Transformer，将物品和箱体的特征融合，以识别待包装物品与箱体内部可用子空间之间的空间相关性。将这两个组件结合，使GOPT能够在不同尺寸的箱体上执行推理任务。我们进行了大量的实验，并证明GOPT不仅在基准方法上取得了更好的性能，还具有出色的泛化能力。此外，机器人部署的实验展示了我们方法在现实世界中的实际应用价值。源代码将在以下网址公开：这个 https URL。 
...
**Abstract (ZH)**: 我们推导出一套因果深度神经网络，其架构源于张量（多重线性）因子分析框架，该框架支持前向和反向因果推理。前向因果问题通过由因果胶囊和张量变换组成的神经网络架构来解决。因果胶囊计算一组不变的因果因子表示，其相互作用由张量变换所规范。反向因果问题则通过实现多重线性投影算法的神经网络来解决。该架构将前向神经网络的操作顺序逆向，并估计导致结果的原因。作为激进的瓶颈维度降维或正则化回归可能掩盖的本质上欠定反向问题的替代方案，我们建议使用分段张量模型来建模数据形成机制的不同方面。这些模型的多重线性投影产生多个候选解决方案。我们的前向和反向问题可以使用浅层架构解决，但对于计算上可扩展的解决方案，我们通过利用块代数推导出一套深层神经网络。交错内核层次结构产生双重非线性张量因子模型。由此产生的因果神经网络在数据方面是无偏的，但通过面部图像进行了示例说明。描述了顺序、并行和异步并行计算策略。 
